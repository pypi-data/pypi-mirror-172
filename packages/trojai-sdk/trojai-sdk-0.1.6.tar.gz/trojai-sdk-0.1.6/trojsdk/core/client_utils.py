...
def config_from_dict(config:dict, docker_metadata:dict=None) -> Tuple[BaseTrojConfig, dict]:
    ...
    :type config: dict
    :param docker_metadata: The dict containing the current docker data, if any. Defaults to None
    :type docker_metadata: dict, optional

    :return: The BaseTrojConfig type object, with "docker_metadata" popped off, if it existed. Tuples with the docker data if it exists in the config. Otherwise, return the input docker_metadata.
...
class TrojJobHandler:
    ...
    def check_job_status(self, response: dict = None, format_pretty: bool = True):
        ...
        :param format_pretty: Prettifies the JSON output, defaults to True
        :type format_pretty: bool, optional
        """
        try:
            if response is None:
