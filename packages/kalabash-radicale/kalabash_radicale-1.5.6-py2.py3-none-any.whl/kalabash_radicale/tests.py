...
from kalabash.lib.tests import KalmonakTestCase, KalmonakAPITestCase
...
class ViewsTestCase(TestDataMixin, KalmonakTestCase):
    ...
    # def test_index(self):
    #     """Test index view."""
    #     url = reverse("kalabash_radicale:calendar_detail_view")
...
class AccessRuleTestCase(KalmonakTestCase):
    ...
    def setUp(self):
        ...
        self.set_global_parameter(
            "rights_file_path", self.rights_file_path, app="kalabash_radicale")
    ...
    def test_rights_file_generation_with_admin(self):
        self.set_global_parameter(
            "allow_calendars_administration", True, app="kalabash_radicale")
        management.call_command("generate_rights", verbosity=False)
        cfg = ConfigParser()
        with open(self.rights_file_path) as fpo:
            cfg.read_file(fpo)
...
class UserCalendarViewSetTestCase(TestDataMixin, KalmonakAPITestCase):
...
class SharedCalendarViewSetTestCase(TestDataMixin, KalmonakAPITestCase):
...
class AccessRuleViewSetTestCase(TestDataMixin, KalmonakAPITestCase):
...
class EventViewSetTestCase(TestDataMixin, KalmonakAPITestCase):
    """Event viewset tests."""
...
class AttendeeViewSetTestCase(KalmonakAPITestCase):
...
class MailboxViewSetTestCase(KalmonakAPITestCase):
