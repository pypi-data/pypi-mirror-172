...
from ..models import Auth, DetailInfo
...
class UserService:
    ...
    def user_basic_message(user_id):
        # user_basic_message = BaseInfo.objects.filter(id=user_id).first()
        res_data = {}
        user_basic_message = BaseInfo.objects.filter(id=user_id).first()
        if not user_id:
            return None, "user_id不能为空"
        if user_basic_message:
            res_data = model_to_dict(user_basic_message)
            detailinfo = DetailInfo.objects.filter(user_id=user_id).first()
            avatar = None
            if detailinfo:
                detail_info = model_to_dict(detailinfo)
                avatar = detail_info['avatar']
