  ...
  def create_model_minimal_abductive_BT(self, implicant, BTs, prediction, n_classes, implicant_id_features, from_reason):
    ...
    if n_classes > 2: #multi-classes case
      satisfy(
        ...
        [Sum(m[t] for t in range(nTrees) if data_classes[t] == prediction) > Sum(m[t] for t in range(nTrees) if data_classes[t] == other_class) for other_class in set(data_classes) if other_class != prediction]
      )
    else: #2-classes case
      if prediction == 1:
        satisfy(
