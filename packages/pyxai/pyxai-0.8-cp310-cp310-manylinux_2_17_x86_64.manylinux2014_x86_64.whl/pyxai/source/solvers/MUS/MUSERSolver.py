...
class MUSERSolver():
  ...
  def solve(self, time_limit=None):
    time_used = -time.time()
    try:
      p = subprocess.run([MUSER_EXEC, '-comp', '-grp', self.filename_gcnf], timeout=time_limit, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
      time_used+=time.time()
    except subprocess.TimeoutExpired:
      time_used = Explainer.TIMEOUT
    model = [line for line in p.stdout.split('\n') if line.startswith("v")][0]
    model = [int(l) for l in model.split(' ')[1:-1]]
