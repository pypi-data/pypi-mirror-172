...
class DecisionTree(BinaryMapping):
  ...
  def __init__(self, n_features, root, target_class=0, id_solver_results=0, ML_solver_information=None, force_features_equal_to_binaries = False):
    """
    ...
    Args:
        ...
        ML_solver_information (_type_, optional): _description_. Defaults to None.
        force_features_equal_to_binaries (bool, optional): By default, the binaries id in the representation of implicants is not the same that the features id. This option allows to force that these two kinds of ids are the same. This is used to allows that variables directly represent the features. Defaults to False.
    """
    ...
    super().__init__(self.map_id_binaries_to_features, self.map_features_to_id_binaries, ML_solver_information)
  ...
  def compute_id_binaries(self, force_features_equal_to_binaries = False):
    ...
    """
    if force_features_equal_to_binaries == False:
  ...
  def take_decisions_implicant(self, implicant, map_id_binaries_to_features = None):
    """
    Return the prediction (the classification) of an implicant according to this tree
    """
    if map_id_binaries_to_features == None: map_id_binaries_to_features = self.map_id_binaries_to_features
    return self.root.take_decisions_implicant(implicant, map_id_binaries_to_features)
  ...
  def to_CNF(self, instance, target_prediction=None, *, tree_encoding=Encoding.COMPLEMENTARY, format=True, inverse_coding=False):
    ...
    """
    code_prediction = True if tree_encoding == Encoding.TSEITIN else False
    ...
    if target_prediction is None: target_prediction = self.predict_instance(instance)
