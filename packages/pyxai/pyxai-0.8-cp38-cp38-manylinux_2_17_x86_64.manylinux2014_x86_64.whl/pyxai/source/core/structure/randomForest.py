...
class RandomForest(TreeEnsembles):
  ...
  def __init__(self, forest, n_classes=2, ML_solver_information=None):
    super().__init__(forest, ML_solver_information)
  ...
  def is_implicant(self, implicant, prediction):
    ...
    #print(n_trues, n_trees, n_trues > int(n_trees / 2), self.predict_instance(implicant), prediction)
    return n_trues > int(n_trees / 2)


  def to_CNF(self, instance, implicant, target_prediction=None, *, tree_encoding=Encoding.COMPLEMENTARY, cardinality_encoding=Encoding.SEQUENTIAL_COUNTER):
    ...
    """
    n_original_variables = len(implicant)
    if target_prediction is None: target_prediction = self.take_decisions_instance(instance)  
