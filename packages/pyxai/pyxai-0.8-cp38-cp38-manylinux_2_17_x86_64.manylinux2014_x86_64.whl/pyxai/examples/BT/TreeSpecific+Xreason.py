...
for id, model in enumerate(models):
  ...
  for instance, prediction in instances:
    ...
    tree_specific = explainer.tree_specific_reason(
      n_iterations=int(Tools.Options.n_iterations), 
      time_limit=0, 
      reason_expressivity=Explainer.FEATURES)
    tree_specific_time = float(local_stopwatch.elapsed_time(reset=True))
    tree_specific_stats = explainer.reason_statistics(tree_specific, reason_expressivity=Explainer.FEATURES)

    start_from_filename = Tools.Options.dataset.split("/")[-1].split(".")[0]+"_"+str(id)+"_"+str(id_instance) + ".start"
    start_from_str = os.getcwd() + "/" + start_from_filename
    start_from_data = {"features": tree_specific}
    start_from_json = json.dumps(start_from_data)
    with open(start_from_filename, 'w') as outfile:
      json.dump(start_from_json, outfile)

    print("tree_specific_time:", str(tree_specific_time), "seconds")
    print("tree_specific:", tree_specific)
    print("tree_specific stats:", tree_specific_stats)
    ...
    print("sufficient_stats:", explainer.reason_statistics(sufficient, reason_expressivity=Explainer.FEATURES))
