...
class LeafNode:
  ...
  def take_decisions_implicant(self, implicant=None, map_features_to_id_binaries=None):
    return self.value
...
class DecisionNode:
  ...
  def is_implicant(self, reason, target_prediction, map_features_to_id_binaries):
    ...
    if id_variable in reason:
      return self.right.is_implicant(reason, target_prediction, map_features_to_id_binaries)
    elif -id_variable in reason:
      return self.left.is_implicant(reason, target_prediction, map_features_to_id_binaries) 
    else:
      right = self.right.is_implicant(reason, target_prediction, map_features_to_id_binaries)
      left = self.left.is_implicant(reason, target_prediction, map_features_to_id_binaries)
      return right and left

  def take_decisions_implicant(self, implicant, map_features_to_id_binaries):
    """
    Return the prediction (the classification) of a implicant according to this node.
    Warning: The implicant (propositional formula calculated from a instance) have to be complete (i.e only a direct reason, not a reason)
    Warning: right nodes are considered as the 'yes' responses of conditions, left nodes as 'no'.  
    """
    id_variable = map_features_to_id_binaries[(self.id_feature, self.operator, self.threshold)][0]
    assert (id_variable in implicant) or (-id_variable in implicant), "The implicant have to be complete !"
    if id_variable in implicant:
      return self.right.take_decisions_implicant(implicant, map_features_to_id_binaries)
    else:
      return self.left.take_decisions_implicant(implicant, map_features_to_id_binaries) 
