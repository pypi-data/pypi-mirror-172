...
class Xgboost(MLSolver):
    ...
    def to_BT(self, ML_solver_information=None):   
      ...
      if self.n_features is None:
        self.n_features = ML_solver_information[0].raw_model.n_features_in_
      if self.n_labels is None:
        self.n_labels = len(ML_solver_information[0].raw_model.classes_)
      
      self.id_features = {"f{}".format(i):i for i in range(self.n_features)}
      BTs = [BoostedTrees(self.results_to_trees(id_solver_results), n_classes=self.n_labels, ML_solver_information=ML_solver_information) for id_solver_results, ML_solver_information in enumerate(self.ML_solver_information)]
      return BTs

    def save_model(self, ML_solver_information, filename):
      ML_solver_information.raw_model.save_model(filename + ".model")

    def results_to_trees(self, id_solver_results):
      xgb_BT = self.ML_solver_information[id_solver_results].raw_model.get_booster()
