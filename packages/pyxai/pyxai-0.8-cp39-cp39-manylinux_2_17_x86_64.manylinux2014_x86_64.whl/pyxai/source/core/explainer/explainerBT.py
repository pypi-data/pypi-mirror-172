...
class ExplainerBT(Explainer):
  ...
  def to_implicant(self, instance):
    return self.boosted_trees.instance_to_binaries(instance)
  ...
  def predict_instance(self, instance):
    return self.boosted_trees.predict_instance(instance)
  ...
  def to_features(self, implicant, *, eliminate_redundant_features=True, details=False):
    return self.boosted_trees.to_features(implicant, details=details)
  ...
  def sufficient_reason(self, *, n=1, seed=0, time_limit=None):
    ...
    if self.boosted_trees.n_classes > 2: raise NotImplementedError
  ...
  def minimal_tree_specific_reason(self, *, time_limit=None, from_reason=None):
    ...
    cp_solver.create_model_minimal_abductive_BT(self.implicant, self.boosted_trees, self.target_prediction, self.boosted_trees.n_classes,
    ...
    time_used += time.time()
    self.elapsed_time = time_used if result == "OPTIMUM" else Explainer.TIMEOUT

    if result == UNSAT or result == UNKNOWN: return None
    return Explainer.format([l for i, l in enumerate(self.implicant) if solution[i] == 1])
  ...
  def tree_specific_reason(self, *, n_iterations=50, time_limit=None, seed=0):
    ...
    reason = c_explainer.compute_reason(self.c_BT, self.implicant, self.implicant_id_features, self.target_prediction, n_iterations, time_limit,
  ...
  def is_tree_specific_reason(self, reason):
...
#     prediction = self.boosted_trees.is_implicant(complete)
