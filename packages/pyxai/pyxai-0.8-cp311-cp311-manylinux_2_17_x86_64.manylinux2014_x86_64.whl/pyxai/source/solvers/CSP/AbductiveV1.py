...
class AbductiveModelV1():
  ...
  def get_leaves(self, BTs, implicant, is_removed):
    leaves = []
    trees = BTs.forest
    for tree in trees:
  ...
  def create_model_is_abductive(self, implicant, is_removed, BTs, prediction):
    ...
    leaves = self.get_leaves(BTs, implicant, is_removed)
    weights = [[self.weight_float_to_int(leave.value) for leave in tmp_leaves] for tmp_leaves in leaves]
