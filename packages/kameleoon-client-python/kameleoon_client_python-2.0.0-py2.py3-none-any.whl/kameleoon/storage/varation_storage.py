...
class VariationStorage:
    ...
    def get_variation_id(self,
            visitor_code: str,
            experiment_id: int) -> Optional[int]:
        """
    ...
    def is_variation_id_valid(self,
            visitor_code: str,
            experiment_id: int,
            respool_time: Optional[int]=None) -> Optional[int]:
        """
        ...
        """            
        if self.storage.get(visitor_code) is not None and \
           self.storage[visitor_code].get(experiment_id) is not None:
            variation = self.storage[visitor_code][experiment_id]
            return variation.id if variation.is_valid(respool_time) else None
        return None
    ...
    def get_saved_variation_id(self,
            visitor_code: str) -> Dict[int, int]:
        """
        ...
        if storage_visitor_code:
            dict_saved_varation_id = {key: value.id for key, value in storage_visitor_code.items()}
