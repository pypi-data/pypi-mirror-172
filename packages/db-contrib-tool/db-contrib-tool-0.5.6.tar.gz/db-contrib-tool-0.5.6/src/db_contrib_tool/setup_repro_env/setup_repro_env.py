...
class DownloadRequest(NamedTuple):
    ...
    def __eq__(self, other: object) -> bool:
        """Download objects are identical if the URLs and name suffix are identical."""
        if isinstance(other, self.__class__):
            return (other.urls_info.urls == self.urls_info.urls) and (
                other.bin_suffix == self.bin_suffix
            )
        return False
        ...
        for discovery_request in discovery_request_list:
            ...
            try:
                ...
                bin_suffix = self._get_bin_suffix(
                    discovery_request.identifier, urls_info.project_identifier
                )
                download_request = DownloadRequest(bin_suffix, discovery_request, urls_info)
        ...
        for download_request in download_request_set:
            try:
                linked_dir = self.artifact_download_service.download_and_extract(
                    download_request.urls_info.urls,
                    download_request.bin_suffix,
                    download_request.urls_info.evg_version_id,
                    setup_repro_params.download_options,
                )
                if linked_dir:
                    link_directories.append(linked_dir)
                downloaded_versions.append(download_request.urls_info.evg_version_id)
                LOGGER.info("Setup request completed", request=download_request)
            except (
