...
class Timeout(Exception):
    ...
    def __str__(self):
        exception_message = "Timed out after %s seconds: %s" % (
            self.timeout,
            self.message,
        )
        LOG.error(exception_message)
...
class Failed(Exception):
    ...
    def __str__(self):
        exception_message = "Failure detected for %s/%s: %s" % (
...
class BadHttpAuthArg(ClientError):
    def __init__(self):
        super().__init__(
            message="You need to provide a login password or an oidc_token to connect to a Atlas sever"
        )
