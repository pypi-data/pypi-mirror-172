...
def ValidateFunctionArguments(func: ast_function_definition.FunctionDefinition, scope: mscope.Scope, filename: str) -> None:
    for arg in func.Arguments.Left:
        ...
        if var is None:
            ...
            raise exceptions.RuntimeError(f'Function ({func.Name}) argument {arg.Token.Lexeme} not found.', arg.Token.Location, filename)
        if arg.ExprType == types.ExprTypes.FunctionArgument:
            t = var.GetType().lower()
...
class Interpreter:
    ...
    def InterpretVariableExpression(self, expr: ast.AST, scope: mscope.Scope) -> value_item.ValueItem:
        ...
        if expr.Left is not None and expr.Left.ExprType == types.ExprTypes.VariableIndex: return self.InterpretIndexedVariableExpression(var, expr.Left, scope)
        return var.Value
    ...
    def InterpretIndexedVariableExpression(self, var: variable.Variable, idx: ast_expr.Expr, scope: mscope.Scope)-> value_item.ValueItem:
        index, result, expr, flat_index = self.GetFromIndexed(var, idx, scope)
        if index is None: raise exceptions.RuntimeError(f'Invalid index. ({var.Name}{self.FlatIndexToString(flat_index)}).', idx.Token.Location, self.filename)
    ...
    def InterpretReturn(self, instruction: ast_return.Return, scope: mscope.Scope) -> None:
        if not scope.isFunction: raise exceptions.RuntimeError("Invalid return instruction outside function.",instruction.Token.Location, self.filename)
    ...
    def GetEmptyRecord(self, fields: typing.List[ast_select_field.SelectField]) -> value_item.ValueItem:
        return_data = value_item.ValueItem().SetFromMacal(f, types.VariableTypes.Record, {})
        for fld in fields:
    ...
    def InterpretSelectMerge(self, into: value_item.ValueItem, data: value_item.ValueItem)-> value_item.ValueItem:
        ...
        # Into can be nil here, so we must check.
            ...
            returnData.Value.append(into)
        #elif into.Type != types.VariableTypes.Nil:
        #    raise exceptions.RuntimeError(f"Merge error, invalid into variable type ({into.Type}).", into.Token.Location, self.filename)
        if data.Type == types.VariableTypes.Array:
            for rec in data.Value:
                returnData.Value.append(rec)
        elif data.Type == types.VariableTypes.Record:
            returnData.Value.append(data)
        #elif data.Type != types.VariableTypes.Nil:
        #    raise exceptions.RuntimeError(f"Merge error, invalid data type ({data.Type}).", data.Token.Location, self.filename)
        return returnData
    ...
    def EvaluateDatafieldExpression(self, expr: ast_expr.Expr, rec: value_item.ValueItem, scope: mscope.Scope) -> value_item.ValueItem:
        ...
        if expr.ExprType == types.ExprTypes.Grouping: return self.EvaluateDatafieldGrouping(expr, rec, scope)
    ...
    def EvaluateDatafieldIndexedVariable(self, var: variable.Variable, idx: ast_expr.Expr, rec: dict, scope: mscope.Scope)-> value_item.ValueItem:
        index, result, expr, flat_index = self.GetFromIndexed(var, idx, scope)
        if index is None:
            raise exceptions.RuntimeError(f'Invalid index. ({var.Name}{self.FlatIndexToString(flat_index)}).', idx.Token.Location, self.filename)
        return result.Value[index.Value]
    ...
    def FlattenIndex(self, index: ast_expr.Expr, scope: mscope.Scope) -> typing.List[value_item.ValueItem]:
        vind = []
        while index.ExprType != types.ExprTypes.VariableIndexStart:
            vind.insert(0, self.InterpretExpression(index.Right, scope))
            index = index.Left
        return vind, index


    @staticmethod
    def FlatIndexToString(flat_index: typing.List[value_item.ValueItem], expr: ast_expr.Expr) -> str:
        s = ""
    ...
    def WalkIndex(self, value: value_item.ValueItem, index: typing.List[value_item.ValueItem], length: int, name: str) -> typing.Tuple[value_item.ValueItem, value_item.ValueItem]:
        i = 0
        idx = index[i]
        while i < length - 1:
            if value.Type == types.VariableTypes.Record:
                if i < length - 2:
                    ...
                    sys.exit(1)
                if idx.Value not in value.Value:
                    break
            if value.Type not in [types.VariableTypes.String, types.VariableTypes.Record, types.VariableTypes.Array]:
                raise exceptions.RuntimeError(f'Non indexable value. ({name}{self.FlatIndexToString(index)}', value.Token.Location, self.filename)
            if (((value.Type == types.VariableTypes.String or value.Type == types.VariableTypes.Array) and idx.Type != types.VariableTypes.Int) 
                ...
                raise exceptions.RuntimeError(f'Invalid index type ({idx.Type}). ({name}{self.FlatIndexToString(index)}', value.Token.Location, self.filename)
    ...
    def GetFromIndexed(self, var: variable.Variable, idx: ast_expr.Expr, scope: mscope.Scope)-> typing.Tuple[value_item.ValueItem, value_item.ValueItem,ast_expr.Expr,typing.List[value_item.ValueItem]]:
        flat_index, expr = self.FlattenIndex(idx, scope)
        l = len(flat_index)
        if l > 0:
            index, result = self.WalkIndex(var.Value, flat_index, l, var.Name)
        else:
            index = None
            result = var.Value
        return index, result, expr, flat_index



    def AssignToIndexed(self, var: variable.Variable, idx: ast_expr.Expr, value: value_item.ValueItem, can_append: bool, scope: mscope.Scope):
        index, result, expr, flat_index = self.GetFromIndexed(var, idx, scope)
        if expr.Right is not None and expr.Right.ExprType == types.ExprTypes.NewArrayIndex:
            if result.Type != types.VariableTypes.Array or can_append == False:
                raise exceptions.RuntimeError(f'Append not supported. ({var.Name}{self.FlatIndexToString(flat_index)}', value.Token.Location, self.filename)
            if index is None:
                result.Value.append(value)
            else:
                result.Value[index.Value].append(value)
        else:
            if index is None:
                raise exceptions.RuntimeError(f'Invalid None index. ({var.Name}{self.FlatIndexToString(flat_index)}', value.Token.Location, self.filename)
            if index.Type == types.VariableTypes.NewArrayIndex and result.Type == types.VariableTypes.Array and can_append:
                result.Value.append(value)
            else:
