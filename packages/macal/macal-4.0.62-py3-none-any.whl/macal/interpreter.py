...
class Interpreter:
    ...
    def InterpretAssignToIndexedVar(self, var: variable.Variable, idx: ast_expr.Expr, value: value_item.ValueItem, scope: mscope.Scope):
        self.AssignToIndexed(var, idx, value, True, scope)
    ...
    def SetIndexedIntoVariableValue(self, var: variable.Variable, idx: ast_expr.Expr, data: value_item.ValueItem, scope: mscope.Scope)-> None:
        self.AssignToIndexed(var, idx, data, False, scope)
    ...
    def GetFromIndexed(self, var: variable.Variable, idx: ast_expr.Expr, scope: mscope.Scope)-> typing.Tuple[value_item.ValueItem, value_item.ValueItem,ast_expr.Expr,typing.List[value_item.ValueItem]]:
        flat_index, expr = self.FlattenIndex(self, idx, scope)
        l = len(flat_index)
        if l > 0:
    ...
    def AssignToIndexed(self, var: variable.Variable, idx: ast_expr.Expr, value: value_item.ValueItem, can_append: bool, scope: mscope.Scope):
        index, result, expr, flat_index = self.GetFromIndexed(self, var, idx, scope)
        if expr.Right is not None and expr.Right.ExprType == types.ExprTypes.NewArrayIndex:
