...
class Interpreter:
    ...
    def InterpretInstruction(self, instruction: ast.AST, scope) -> None:
    ...
    def InterpretFunctionDefinition(self, instruction: ast_function_definition.FunctionDefinition, scope: mscope.Scope) -> None:
    ...
    def BinaryExpressionOperandsTypeCheck(self, left:value_item.ValueItem, right:value_item.ValueItem, operator:str) -> None:
        ...
        if left.Type != right.Type:
            if (operator == '==' or operator == '!=') and (right.Type == types.VariableTypes.Bool or right.Type == types.VariableTypes.Nil or left.Type == types.VariableTypes.Nil or left.Type == types.VariableTypes.Bool):
                return
    ...
    def InterpretWhile(self, instruction: ast_while.While, scope: mscope.Scope) -> None:
        ...
        while condition.Value is True and infiniteLoopProtection > 0:
            ...
            if whs.Continue: whs.Continue = False
    ...
    def GetEmptyRecord(self, fields: typing.List[ast_select_field.SelectField]) -> value_item.ValueItem:
        ...
        for fld in fields:
            return_data.Value[fld.AsName] = value_item.ValueItem().SetFromMacal(token.LexToken.NullToken(), types.VariableTypes.Nil, types.VariableTypes.Nil)
        return return_data
    ...
    def ApplyFieldFilters(self, data: value_item.ValueItem, fields: typing.List[ast_select_field.SelectField]) -> value_item.ValueItem:
        ...
        if data is None:
            ...
            for rec in data.Value:
                r = value_item.ValueItem().SetFromMacal(rec.Token, types.VariableTypes.Record, {})
                for fld in fields:
