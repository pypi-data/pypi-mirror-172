...
class artificialneuralnetwork_classifier :
  ...
  def predict(self,x):
    zi = np.matrix([[ self.THR.predict(x), self.LGR.predict(x), THd.predict(x), THM.predict(x) ]])
    return self.PF.predict(zi)
