...
class ish_report(object):
    ...
    MISSING = "9999"
    MAP = {
        "AA1": ["LIQUID-PRECIP", 8, PrecipitationComponent],
    ...
    def formatted(self):
        ...
        return f"""
Weather Station: {self.weather_station} ({self.latitude}, {self.longitude})
Elevation: {self.elevation}` m
Time: {self.datetime} UTC
Air Temperature: {self.air_temperature} C ({self.air_temperature.get_fahrenheit()} F)
Wind Speed: {self.wind_speed} m/s ({self.wind_speed.get_MilesPerHour()} mph)
Wind Direction: {self.wind_direction}
Present Weather Obs: {self.present_weather}
Precipitation: {self.precipitation}
Cloud Coverage: {self.sky_cover} oktas
Cloud Summation: {self.sky_cover_summation}
Solar Irradiance: {self.solar_irradiance}
    """
    ...
    def loads(self, noaa_string):
        ...
        if actual_length != expected_length:
            msg = (
                f"Non matching lengths. Expected {expected_length}, got {actual_length}"
            )
        ...
        self.wind_direction = Direction(
            noaa_string[60:63], Direction.RADIANS, noaa_string[63:64]
        )
        ...
        self.sky_ceiling = Distance(
            int(noaa_string[70:75]), Distance.METERS, noaa_string[75:76]
        )
        self.sky_ceiling_determination = noaa_string[76:77]
        self.visibility_distance = Distance(
            int(noaa_string[78:84]), Distance.METERS, noaa_string[84:85]
        )
        ...
        self.air_temperature = Temperature(
            int(noaa_string[87:92]) / self.TEMPERATURE_SCALE,
            Units.CELSIUS,
            noaa_string[92:93],
        )
        self.dew_point = Temperature(
            int(noaa_string[93:98]) / self.TEMPERATURE_SCALE,
            Units.CELSIUS,
            noaa_string[98:99],
        )
    ...
    def _get_remarks_component(self, string, initial_pos):
        ...
        if remarks_code != "REM":
            raise ish_reportException(
                f"Parsing remarks. Expected REM but got {remarks_code}."
            )
        ...
        while position < expected_length:
            ...
            if key == "EQD":
                break
            chars_to_read = string[
                position
                + self.ADDR_CODE_LENGTH : position
    ...
    def _get_component(self, string, initial_pos):
        ...
        try:
            ...
            raise BaseException(
                f"Cannot find code {add_code} in string {string} ({initial_pos})."
            )
        ...
        if useable_map[1] is False:
            chars_to_read = string[
                initial_pos
                + self.ADDR_CODE_LENGTH : initial_pos
                + (self.ADDR_CODE_LENGTH * 2)
            ]
            chars_to_read = int(chars_to_read)
