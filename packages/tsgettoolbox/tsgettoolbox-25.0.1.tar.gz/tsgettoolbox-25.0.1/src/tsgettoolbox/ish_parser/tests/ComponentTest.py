...
from ish_parser import CloudCoverage, Irradiance, Minutes
...
class SkyCoverSummationComponentTest(unittest.TestCase):
    def test_string(self):
        ...
        sky_cover_sum = sd.sky_cover_summation

        self.assertDictEqual(
            sd.sky_cover_summation,
            {
                "coverage": "OVERCAST - 8/8 coverage",
                "coverage_simple": "Overcast",
...
class SnowDepthComponentTest(unittest.TestCase):
    def test_string(self):
        ...
        sd.loads(simple_string)
        self.assertDictEqual(
            sd.snow_depth, {"depth": 8, "quality": "5", "condition": "9"}
        )
...
class SkyCoverComponentTest(unittest.TestCase):
    def test_string(self):
        ...
        self.assertEqual(
            sky.sky_cover["coverage"], CloudCoverage("00", CloudCoverage.OKTA, "5")
        )
...
class SolarIrradianceComponentTest(unittest.TestCase):
    def test_string(self):
        ...
        self.assertEqual(
            sol.solar_irradiance["irradiance_data_flag"],
            "Value estimated; passes all pertinent SERI_QC tests",
        )
        self.assertEqual(
            sol.solar_irradiance["direct_beam_irradiance"], Irradiance("0989")
        )
        self.assertEqual(
            sol.solar_irradiance["uvb_global_irradiance"], Irradiance("9999")
        )
...
class SkyConditionObservationComponentTest(unittest.TestCase):
    def test_string(self):
        ...
        sky.loads(sample_string)
        self.assertEqual(
            sky.sky_condition_observation["total_coverage"],
            CloudCoverage("08", CloudCoverage.OKTA, "1"),
        )
        self.assertEqual(
            sky.sky_condition_observation["total_lowest_coverage"],
