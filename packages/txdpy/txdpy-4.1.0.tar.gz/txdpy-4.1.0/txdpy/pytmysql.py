# coding:utf-8
import re,pymysql,hashlib,sys,random,requests
from pymysql.converters import escape_string
from loguru import logger

class PyMySQL:
    
    def __init__(self,host:str='localhost',port=3306, user:str='root', password:str='', database:str='',auto_add_key=False):
        """
        :param host: 默认本地地址
        :param user: 默认root
        :param password: 数据库密码
        :param database: 数据库
        :param auto_add_key: 自动添加字段默认False
        """
        self.db = pymysql.connect(host=host,port=port, user=user, password=password, database=database)
        logger.info(f'已连接数据库：host={host}, user={user}, password={password}, database={database}')
        self.cursor = self.db.cursor()
        self.hk=[]
        self.auto_add_key=auto_add_key

    #插入数据库
    def insert_into(self,database=None,fields=[],values=[],update_fields={},SQL=False,auto_key=False):
        """
        :param database: 数据表名
        :param fields: 字段名列表
        :param values: 值列表
        :param update_fields: 更新字段字典
        :param SQL: 输出mysql插入语句
        :return: 插入数据库
        """
        self.db.ping()
        if self.auto_add_key:
            if not self.hk:
                self.hk=self.getdatah(database)
            for field in fields:
                if field not in self.hk:
                    self.cursor.execute(f"""ALTER TABLE `{database}` ADD COLUMN `{field}` varchar(255) NULL;""")
                    self.hk.append(field)
        fields_format=','.join([f'`{field}`' for field in fields])
        values_format='","'.join([escape_string(str(value)) for value in values])
        fields_update_format=','.join([f'{key}="{escape_string(value)}"' for key, value in update_fields.items()])
        sql=f"""
            INSERT INTO {database}
            ({fields_format})
            VALUES ("{values_format}")
            """
        if update_fields:
            sql = f"""
                INSERT INTO {database}
                ({fields_format})
                VALUES ("{values_format}")
                ON DUPLICATE KEY UPDATE
                {fields_update_format}
                """
        if SQL:
            logger.info(sql)
        VALUES=re.search('VALUES \((.*)\)',sql).group(1)[:250]
        try:
            self.cursor.execute(sql)
            self.db.commit()
            logger.info(VALUES+'\t成功加入数据库！')
        except Exception as e:
            self.except_handl(e,sql)
            self.db.rollback()

    #自定义sql语句
    def custom_sql(self,sql):
        """
        :param sql: 自定义sql语句
        :return: 执行自定义sql语句
        """
        self.db.ping()
        try:
            self.cursor.execute(sql)
            self.db.commit()
            logger.info(sql)
        except Exception as e:
            logger.error(e,sql)
            self.db.rollback()

    #查询数据
    def select(self,sql,fetch='all'):
        """
        :param sql: 查询数据sql
        :return: 查询结果
        """
        self.db.ping()
        self.cursor.execute(sql)
        if fetch=='one':
            return self.cursor.fetchone()
        return self.cursor.fetchall()

    #转义字符格式化
    def escape_string(self,character_string):
        """
        :param character_string: 部分文本中字符影响SQL语句，需要做字符串格式化处理
        :return: 处理好的字符串
        """
        self.db.ping()
        return escape_string(character_string)

    #获取表头字段
    def getdatah(self,database:str=''):
        """
        :return: 获取表头字段
        """
        self.db.ping()
        if database!="":
            return [k[0] for k in self.select(
                f"""SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_NAME = '{database}';""")]
        return [a[0] for a in self.cursor.description]

    #关闭数据库
    def close(self):
        """
        :return: 关闭数据库的所有操作
        """
        self.db.ping()
        self.cursor.close()
        self.db.close()
        logger.info('数据库连接已断开！')

    def except_handl(self,e,sql):
        if str(e).startswith('(1062, "Duplicate entry'):
            logger.info(VALUES + '\t数据重复!')
        else:
            appid = '20220712001270949'
            q = str(e)[1:-1]
            secret_key = 'ODwtPobgXFes3sBML_NM'
            salt = str(random.randint(1000000000, 9999999999))
            m = hashlib.md5()
            m.update((appid + q + salt + secret_key).encode("utf8"))
            s = f'http://api.fanyi.baidu.com/api/trans/vip/translate?q={q}&from=en&to=zh&appid={appid}&salt={salt}&sign={m.hexdigest()}'
            result = requests.get(s).json()
            logger.error(q)
            logger.error(result["trans_result"][0]['dst'])
            logger.error(sql)
            sys.exit()
