...
class CfdiPago():
    ...
    def generar_cfdi(self, configuracion, timbrado_prueba=None, pac=None):
        ...
        for rp in self.pagos:
            ...
            for documento in rp["documentos"]:
                ...
                if documento['moneda'] != rp["MonedaP"]:
                    ...
                    equivalencia_dr =  round(to_decimal(documento['importe_abono']) / to_decimal(rp["Monto"]), 2)
        ...
        for imp_p in rp.get("impuestos_p", []):
            if str(imp_p['tasa0_cuota_p']) == "0.160000":
                iva_p = "0.160000"
                base_iva += imp_p['base_p']
                impuesto_iva += imp_p['importe_p']
                
            if str(imp_p['tasa0_cuota_p']) == "0.080000":
                iva_p = "0.080000"
                base_iva += imp_p['base_p']
                impuesto_iva += imp_p['importe_p']
    
            if str(imp_p['tasa0_cuota_p']) == "0.00":
                iva_p = "0.00"
                base_iva += imp_p['base_p']
                impuesto_iva += imp_p['importe_p']

            if str(imp_p["impuesto_p"]) == "003":
                base_ieps += imp_p['base_p']
                impuesto_ieps += imp_p['importe_p']

        
        for imp_p in rp.get("impuestos_p", []):
            if not imp_p["tasa0_cuota_p"] in IMPUESTOS_LIST:
                IMPUESTOS_LIST.append(imp_p['tasa0_cuota_p'])

                if str(imp_p["impuesto_p"]) == "002":
                    base = base_iva
                ...
                rp["ImpuestoP_set"].append({
                    ...
                    'TasaOCuotaP':imp_p['tasa0_cuota_p'],
                    'ImporteP':impuesto,
                })
        ...
        if cfdi.Version == "4.0":
            ...
            cp = ComplementoPago()
        if str(iva_p) == "0.160000":
            cp.TotalTrasladosBaseIVA16 = base_iva
            cp.TotalTrasladosImpuestoIVA16 = impuesto_iva

        if str(iva_p) == "0.080000":
            cp.TotalTrasladosBaseIVA8 = base_iva
            cp.TotalTrasladosImpuestoIVA8 = impuesto_iva
        if str(iva_p) == "0.00":
            cp.TotalTrasladosBaseIVA0 = base_iva
            cp.TotalTrasladosImpuestoIVA0 = impuesto_iva
