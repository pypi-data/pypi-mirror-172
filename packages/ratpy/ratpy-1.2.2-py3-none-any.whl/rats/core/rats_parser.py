...
import pandas as pd
import numpy as np
from rats.core.errors import RatsError, SamplesMissingError
...
class RatsParser:
        ...
        def __operation(df):
            ...
            for field in self.packet_format:
                if field == 'DATA':
                    ...
                    if (((packet_dictionary['PAYLOAD_SIZE']-8)*2)/len(flaglist))%1: # if the payload size reports missing samples
                        self.metadata['log'] += f'The number of samples in packet {df.name} is not divisible by the number of active rats inputs\n'

                        while (((len(bytes)))/len(flaglist))%1 > 0: # while the data stream is too short, pad with 0
                            bytes.pop()
    ...
    def _find_outliers(self):
        ...
        """
        def __operate(df):
            sip_df = df[df['SIP'] == 0]  # make sure to only select for the scan data
    ...
    def parse(self, test=False):
        try:
            ...
            except SamplesMissingError:
                self.metadata['log'] += 'There was an error while parsing this file, ' \
                                        'so a generic, empty dataframe was created \n'
                self.metadata['log'] += 'While parsing, there was at least one instance where the number of samples' \
                                        'in an LLC was not divisible by the number of RATS inputs. As it stands, ' \
                                        'the parser cannot handle that error state, so processing stopped \n'
                self.metadata['parse_completed_without_error'] = False
                if not test:
                    self.save_dataframe()
