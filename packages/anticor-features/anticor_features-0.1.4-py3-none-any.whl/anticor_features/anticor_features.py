...
from anticor_features.anticor_stats import no_p_spear, no_p_pear, dense_rank, get_shuffled
...
def ensg_list_to_orig(annotations, ensg_idx, rm_features_ensg):
    ...
    for i in range(1,len(annotations)):
        ...
        if temp_ensg is None:
            ...
            temp_ensg = annotations[i][1]
        ensg_to_orig_lookup[temp_ensg]=annotations[i][1]
    out_converted = []
...
def pathways_to_genes(rm_paths, species, all_features):
    ...
    results = gprof_pathways_to_genes(rm_paths, species)
    print(pd.DataFrame(results).head())
    remove_dict=process_dict(results,3)
...
def get_n_expressed(exprs, cell_axis, all_features):
    ...
    for i in range(len(all_features)):
        ...
        if exprs_idxs.shape[0]>0:
            mean_nz[i] = np.mean(temp_exprs_vect[exprs_idxs])
...
def rewrite_full_dset(exprs, feature_ids, pre_remove_features, scratch_dir, cell_axis, method="spear"):
    ...
    exprs_out_mat = exprs_f.create_dataset("infile", (len(feature_ids),exprs.shape[cell_axis]), dtype=np.float16)
...
def get_empiric_FPR_cutoff_pos(in_vect, FPR, cap = 0.9):
    target_index = floor(in_vect.shape[0]*FPR)
