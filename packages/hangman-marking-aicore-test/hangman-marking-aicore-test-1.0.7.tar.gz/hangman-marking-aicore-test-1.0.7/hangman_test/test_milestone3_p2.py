...
class HangmanTestCase(unittest.TestCase):
    ...
    def test_check_letter(self):
        self.assertIn('if guess in word', self.code, 'You should use an if statement to check if the letter entered by the user is in the word. Remember that the input should be stored in a variable called "guess" and the randomly picked word should be stored in a variable called "word". You can use the "in" operator to check if a character is in a string')
        # Check the line number of the if statement and the else statement
        if_found = False
        n_line_else = 0
        for n, line in enumerate(self.lines):
            if 'if guess in word' in line:
                n_line_if = n
                if_found = True
