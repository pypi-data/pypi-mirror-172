...
if 'milestone_5_p2.txt' in os.listdir('.'):
    ...
    if len(errors) == 0:
        ...
        for n_task, (task_name, task_id) in enumerate(task_name_list):
            if task_name not in errors:
                mark_complete(task_id)
            else:
                mark_incomplete(task_id, errors[task_name])
                break
        if n_task == len(task_name_list):
            print('All tasks in milestone 5 completed!')
        else:
            for task_name, task_id in task_name_list[n_task + 1:]:
