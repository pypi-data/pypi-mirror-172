...
import numpy as np
from ..utils.transformations import dh_matrix, translation_matrix, euler_matrix
...
class SixDoFCollabRobot(object):
    ...
    Attributes:
        ...
        l01: link from world to joint 1
        l12: link from joint 1 to joint 2
        l23: link from joint 2 to joint 3
        l34: link from joint 3 to joint 4
        l45: link from joint 4 to joint 5
        l56: link from joint 5 to joint 6
    ...
    def cartesian_space_interpolated_motion(self, x, y, z, roll, pitch, yaw, sol_id = 0, 
        ...
        Args:
            ...
            x_speed: speed alone X dimetion(m/s)
            y_speed: speed alone Y dimetion(m/s)
            z_speed: speed alone Z dimetion(m/s)
    ...
    def forward_kinematics(self, axes):
        ...
        Returns:
            dictionary of robot end transform
        ...
        T12 = dh_matrix(0    , -self.l12, 0 , axes[1])
        T23 = dh_matrix(0    , -self.l23, 0 , axes[2])
        T34 = dh_matrix(pi/2 , 0 , self.l34, axes[3])
        T45 = dh_matrix(-pi/2, 0 , self.l45, axes[4])
        T56 = dh_matrix(0    , 0 , self.l56, axes[5])
        T06 = T01 @ T12 @ T23 @ T34 @ T45 @ T56
        return T06

    def inverse_kinematics(self, x, y, z, roll, pitch, yaw, sol_id):
        ...
        for i in range(8):
            ...
            sol[5, i] = atan2(-T61[1, 2] / sin(sol[4, i]), T61[0, 2] / sin(sol[4, i]))
