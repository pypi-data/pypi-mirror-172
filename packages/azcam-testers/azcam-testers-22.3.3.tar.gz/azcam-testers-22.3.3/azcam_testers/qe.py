...
class Qe(Tester):
    ...
    def __init__(self):
        ...
        self.scan_times = 0  # include scan of exposure times
        self.exposure_scan_times = {400: [1]}
    ...
    def acquire(self):
        ...
        for wave in waves:
            ...
            if not self.scan_times:
                ...
                etimes = self.exposure_scan_times[wave]
                for etime in etimes:
                    title = f"{wave} nm QE flat for {etime} secs"
                    instrument.set_wavelength(wave)
    ...
    def plot(self):
        ...
        ax.grid(1)
        if self.scan_times:
            azcam.plot.plt.xlabel("Exposure times [sec]", fontsize=bigfont)
        else:
        ...
        if self.scan_times:
            qevals = []
            etimes = self.exposure_scan_times[self.wavelengths[0]]
            for etime in etimes:
                qevals.append(self.qe[etime])
            azcam.plot.plt.errorbar(
                etimes, [x * 100.0 for x in qevals], yerr=3.0, marker="o", ls=""
            )
        else:
            waves = self.wavelengths
            qevals = []
