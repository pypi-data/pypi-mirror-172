...
def files_create( fp , trimpath = "" ) :
    ...
    # FIXME TODO WARNING only for HTML files and given macro name
    print(s_util.path_ext( fp ))

    if( s_util.path_ext( fp ) in [ ".html" , ".php" , ".js" , ".css" ] ) :
        fc = fc.decode( "utf8" ).replace( "_SAPI_DB_FILES_MACRO_HTML_CACHEBUST_" , s_util.time_hexepoch_str( ) ).encode( "utf8" )
        print("YAY")
...
def files_createall( pat , trim_path = True ) :
