    ...
    if installer == "conda":
        ...
        if os.name == "nt":
            potential_egg_link_name = (
                prefix / "site-packages" / Path(dist.name).with_suffix(".egg-link")
            )
        else:
...
async def scan_pip(
    ...
    if os.name == "nt":
        site_packages = Path(prefix) / "site-packages"
    else:
        python = (Path(prefix) / "bin" / "python").resolve().name
        site_packages = Path(prefix) / sys.platlibdir / python / "site-packages"
