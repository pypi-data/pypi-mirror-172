...
class AWSOptions(BackendOptions, total=False):
    ...
    send_prometheus_metrics: Optional[bool]
...
class CloudBeta(OldCloud, Generic[IsAsynchronous]):
    ...
    async def _create_senv_package(
        self, package_file: BinaryIO, account: Optional[str] = None
    ) -> int:
        logger.info(f"Starting upload for {package_file}")
        package_data = package_file.read()
        package_md5 = md5(package_data).hexdigest()
        account = account or self.default_account
        response = await self._do_request(
            "POST",
            self.server
            + f"/api/v2/software-environment/account/{account}/package-upload",
            json={"name": Path(package_file.name).name, "md5": package_md5},
        )
        data = await response.json()
        # cant use the default session as it has coiled auth
        # headers
        if data["created"]:
            try:
                await self._put_package(
                    url=data["upload_url"], package_data=package_data
                )
            except ClientResponseError as e:
                raise ServerError(f"Error uploading package : {e.status}") from e
        else:
    ...
    async def _put_package(self, url: str, package_data: bytes):
        async with ClientSession() as session:
            async with session.put(url=url, data=package_data) as resp:
    ...
    async def _create_software_environment_v2(
        ...
        for pkg in file_packages:
            file_id = await self._create_senv_package(pkg["sdist"])
            env.append(
                {
                    "name": pkg["name"],
                    "source": pkg["source"],
