...
class ClusterBeta(Cluster, Generic[IsAsynchronous]):
    ...
    def __init__(
        ...
        *,
        ...
        if allow_ingress_from is not None or allow_ssh is not None:
            if backend_options is not None and "ingress" in backend_options:
                raise ArgumentCombinationError(
                    "You cannot use `allow_ingress_from` or `allow_ssh` when "
    ...
    async def _start(self):
        ...
        try:
            ...
            if self.package_sync:
                ...
                self.auto_env_issues: Dict[str, Tuple[magic.PackageInfo, int]] = {
                    pkg["name"]: (pkg, lookup.get(pkg["name"], 50))
                    for pkg in env
                    if pkg["issue"]
                }
                halting_failures: List[magic.PackageInfo] = []
