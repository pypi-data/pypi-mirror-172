...
AWS_INSTANCES_FAMILY_FILTER = ["t2", "t3", "m5", "c2", "c5"]
...
def get_instance_type_from_cpu_memory(
    ...
    if backend == "gcp":
        ...
        if gpus:
            aws_family_filter = re.compile("^g4dn")
        else:
            # match by exact family, so t3 won't match t3a family
            aws_family_filter = re.compile(
                f"^({'|'.join(AWS_INSTANCES_FAMILY_FILTER)})\\b"
            )

        instances = [
            instance_name
            for instance_name in instance_types
            if aws_family_filter.match(instance_name)
        ]
...
def cluster_firewall(
    ...
    https: bool
        Open port used for https (443), closed by default.
    """
    if target == "everyone":
        cidr = "0.0.0.0/0"
    elif target == "me":
        ...
        my_public_ip = requests.get("https://api.ipify.org").content.decode("utf-8")
...
async def md5_wheel(wheel: Path) -> str:
    hash = md5()
