...
def print_rich_package_warning_table(pkg_warnings: Dict[str, Tuple[PackageInfo, int]]):
    table = rich.table.Table(expand=True, box=rich.box.MINIMAL)
    table.add_column("Package")
    table.add_column("Issue")
    table.add_column("Risk Level")
    for (pkg_info, level) in sorted(
        pkg_warnings.values(), key=lambda p: (p[1], p[0]["name"]), reverse=True
    ):
        table.add_row(pkg_info["name"], pkg_info["issue"], level_to_str(level))
    panel = rich.panel.Panel(table, title="[bold green]Package Issues[/bold green]")
