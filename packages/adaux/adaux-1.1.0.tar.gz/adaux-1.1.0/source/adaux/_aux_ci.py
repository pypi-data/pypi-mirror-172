...
class Job:
    ...
    def is_up_to_date(  # pylint: disable=no-self-use
        self,
        env: tp.Mapping[str, tp.Any],  # pylint: disable=unused-argument
    ) -> bool:
        return True

    def is_included(  # pylint: disable=no-self-use
        self,
        env: tp.Mapping[str, tp.Any],  # pylint: disable=unused-argument
    ) -> tp.Tuple[bool, str]:
...
class DockerBuildMixin(DockerShared, _DockerBuildMixinState, Job):
    ...
    def registry_api_token(self) -> str:
        ...
        req = requests.get(
            url, auth=(os.environ["CI_REGISTRY_USER"], os.environ["CI_JOB_TOKEN"])
        )
...
class SpezRun(ConfigAccess, CommonJob):
    ...
    def release_tag(self) -> None:
        ...
        if len(self.parents) != 1:
            raise RuntimeError(
                f"release-tag should only have 1 parent, not {len(self.parents)}!"
            )
        par = self.parents[0]
        ...
        for release_version in [version, "latest"]:
