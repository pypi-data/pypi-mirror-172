...
def get_table_settings_from_engine(engine: str = None):
    """
    >>> get_table_settings_from_engine()
    ''
    >>> get_table_settings_from_engine('MergeTree() PARTITION BY (toDate(timestamp)) ORDER BY (local_date, cod_store)')
    ''
    >>> get_table_settings_from_engine('MergeTree() PARTITION BY (toDate(timestamp)) ORDER BY (local_date, cod_store), SETTINGS index_granularity=8129')
    'index_granularity=8129'
    >>> get_table_settings_from_engine('MergeTree() PARTITION BY (toDate(timestamp)) ORDER BY (local_date, cod_store), settings index_granularity=8129')
    'index_granularity=8129'
    >>> get_table_settings_from_engine('MergeTree() PARTITION BY (toDate(timestamp)) ORDER BY (local_date, cod_store), SETTINGS index_granularity=8129, storage_policy=other')
...
def add_engine_storage_policy(engine: str = '', engine_storage_policy: str = None):
    ...
    if engine and engine_storage_policy and 'SETTINGS '.lower() in engine.lower():
        settings = get_table_settings_from_engine(engine)
        if 'storage_policy'.lower() in settings.lower():
            return engine
        return engine.replace(settings, settings + f", storage_policy='{engine_storage_policy}'")

    elif engine and engine_storage_policy:
        return f"{engine}, SETTINGS storage_policy='{engine_storage_policy}'"
