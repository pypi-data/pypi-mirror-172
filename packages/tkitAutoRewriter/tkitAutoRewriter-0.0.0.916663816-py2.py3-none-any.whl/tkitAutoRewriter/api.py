...
class AutoReWrite(object):
    ...
    def __init__(self, api='http://192.168.1.18:3008', api_sbert_host=None, **kwargs):
        self.api_host = api
    ...
    def auto_summary(self, text,auto_sample=False,temperature=1.15,config=None, **kwargs):
        """
        自动生成摘要，设置了默认参数
    ...
    def auto_optimize_content(self, text,auto_sample=False,temperature=1.15,config=None, **kwargs):
        """
        自动优化内容，设置了默认参数
    ...
    def simplified(self, text, simlimit=0.8, num_beams=5, num_return_sequences=5, max_length=128, simlimitmax=1.0,
                   auto_sample=False,temperature=1.25, **kwargs):
        """
        对文本进行简化重写,
                    ...
                    if len(sent) > 5 and len(images) == 0:
                        ...
                        try:
                            ...
                            if auto_sample == False:
                                newdata.append(
                                    out['out']['results']['best_pair'].replace("888-349-8884", ""))
                                # print(newdata[-1])
                            else:
                                # 筛选出匹配的index
                                pair_index = []
    ...
    def rewriter(self, text, simlimit=0.8, num_beams=5, num_return_sequences=5, max_length=128, simlimitmax=1.0,
                 auto_sample=False,temperature=1.1125, pre_text="", **kwargs):
        """
        对文本进行重写,
        自动将文章拆分成句子，之后逐句生成同义句，
        使用逐句重写方案，后对句子的相关性进行判别
    ...
    def rewriter_by_summary(self, text, simlimit=0.8, num_beams=5, num_return_sequences=5, max_length=128, simlimitmax=1.0,
                 auto_sample=False,temperature=1.1125, pre_text="", **kwargs):
        """
        对文本进行重写,
        自动将文章拆分成句子，之后逐句生成同义句，
        ...
        for item in tqdm(auto_cut_markdown(text)):
            if len(item['text'])>0:
                try:
                    out=self.auto_summary(item['text'],auto_sample=True,temperature=temperature)
                except Exception as e:
                    # 失败后返回原文
                ...
                # print(out)
                try:
                    if out.get("results") and type(out)==dict and out['results'][0][0]['summary_text'] is not None:

                        item["results"]=out['results'][0][0]['summary_text']
                except Exception as e:
                    if type(out)==dict and out['results'][0][0][0]['summary_text'] is not None:

                        item["results"]=out['results'][0][0][0]['summary_text']
...
def rewriter_by_summary(text,auto_sample=False,simlimit=0.6,temperature=1.2,api='http://192.168.1.18:3011',api_sbert_host="http://192.168.1.18:3016",**kwargs):
    """
    自动处理重写任务
    ...
    for item in rewriter.rewriter_by_summary(text, simlimit=0.6, simlimitmax=0.999999999, auto_sample=False,temperature=temperature):
        yield item
