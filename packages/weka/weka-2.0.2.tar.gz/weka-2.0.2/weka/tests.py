...
from weka.classifiers import IBk, SMO # pylint: disable=no-name-in-module
...
"""
        ...
        self.assertEqual(predictions[0], PredictionResult(actual=None, predicted=7, probability=None))
...
"""
        ...
        rows = [
            dict(Sex=Nom('M'), Length=Num(0.35), Diameter=Num(0.286), Timestamp=Date('2017-12-01'), Class_Rings=Int(15, cls=True)),
            dict(Sex=Nom('F'), Diameter=Num(0.86), Timestamp=Date('2017-12-27'), Whole_weight=Num(0.98), Class_Rings=Int(7, cls=True)),
        ]
        rows_extra = [
            dict(Sex=Nom('N'), Length=Num(0.35), Diameter=Num(0.286), Class_Rings=Int(15, cls=True)),
            dict(Sex=Nom('B'), Diameter=Num(0.86), Whole_weight=Num(0.98), Class_Rings=Int(7, cls=True)),
        ]
...
"""
    ...
    def test_smo(self):
        ...
        # Train without K sub-args.
        c = SMO(C=100, c=1, K='weka.classifiers.functions.supportVector.RBFKernel') # pylint: disable=undefined-variable
        training_fn = os.path.join(BP, 'fixtures/abalone-train.arff')
        c.train(training_fn, verbose=1)
        self.assertTrue(c._model_data)

        # Train with K sub-args.
        c = SMO(C=100, c=1, K='weka.classifiers.functions.supportVector.RBFKernel -C 25007 -G 0.1') # pylint: disable=undefined-variable
        training_fn = os.path.join(BP, 'fixtures/abalone-train.arff')
        c.train(training_fn, verbose=1)
        self.assertTrue(c._model_data)
