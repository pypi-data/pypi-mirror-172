...
from cachetools import cached, cachedmethod, LRUCache, RRCache, TTLCache, keys
...
@decorator
def pickle_cache(func, location='cachedir', ttl_cache=TTLCache(maxsize=128, ttl=8), *args, **kwargs):
    k = md5(f"cache_{func.__name__}_{args}_{kwargs}")
    output = Path(location) / Path(k) / '__output.pkl'

    if k in ttl_cache and output.is_file():
...
@decorator
def disk_cache(func, location='cachedir', ignore=None, compress=False, verbose=1, *args, **kwargs):
    """硬盘缓存
