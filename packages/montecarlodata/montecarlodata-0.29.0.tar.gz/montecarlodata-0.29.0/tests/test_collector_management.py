...
class CollectorManagementTest(TestCase):
    ...
    def test_upgrade_to_janus(self):
        self._aws_wrapper_mock.get_stack_parameters.return_value = [
            ...
            {'ParameterKey': 'workerLambdaConcurrentExecutions', 'ParameterValue': '12'},
        ]
        self._aws_wrapper_mock.upgrade_stack.return_value = True

        self._request_wrapper_mock.make_request_v2.return_value = _SAMPLE_MONOLITH_RESPONSE
        self._service.upgrade_template(update_infra=True, new_params={
            'EnableRemoteUpdates': 'True',
            'CreateEventInfra': 'False'
        })
        ...
        self._aws_wrapper_mock.upgrade_stack.assert_called_once_with(
            ...
            parameters=[
                # updated parameter
                {'ParameterKey': 'CreateEventInfra', 'ParameterValue': 'False', 'UsePreviousValue': False},
