...
class DatabricksMetastoreOnboardingTest(TestCase):
    ...
    def test_databricks_gql_api_calls(self, onboard_mock):
        ...
        self._service.onboard_databricks_metastore(connection_type=DATABRICKS_METASTORE_CONNECTION_TYPE, **options, **_SAMPLE_BASE_OPTIONS)
        ...
        expected_notebook_job_call.create_databricks_notebook_job(databricks_config=databricks_config, connection_options={'dc_id': 'data collector id'})
        ...
        for i, expected_call in enumerate(expected_calls):
            ...
            self.assertEqual(str(self._mc_client_mock.call_args_list[i]).strip(), str(expected_call).strip())
