...
from ..data import collate, preprocess, utils
from ..excepts import CorruptedMetadata, CorruptedONNXModel, SelectModelRequired
from ..runner.artifact import RunArtifact
from ..runner.model import RunnerModel
from . import builders
from .load import load_artifact
from .utils import convert_backbone_to_embedding_model

logger = logging.getLogger(__name__)


class InferenceEngine(abc.ABC):
    _artifact: Union[str, RunnerModel]
    _is_onnx: bool
    _token: Optional[str]
    _batch_size: int
    _select_model: str
    ...
    def __init__(
        self,
        artifact: Union[str, RunnerModel, RunArtifact],
        is_onnx: bool = False,
        ...
        **kwargs,
        ...
        """
        The finetuner inference engine can load any model trained with Finetuner and use it to
        ...
        :param artifact: Specify a finetuner run artifact. Can be a path to a local
            ...
            :class:`RunnerModel` or a :class:`RunArtifact`, can be passed as well.
    ...
    def _load_artifact(
        self,
        artifact: Union[str, RunnerModel, RunArtifact],
    ) -> Union[str, RunnerModel]:
        """If artifact is a string, then load the artifact either from a directory,
        a zip file or directly from Hubble.
        If artifact is a :class:`RunnerModel` or :class:`RunArtifact`, then load the artifact directly from
        memory.
        ...
        :return: The artifact to be used by the engine going forward. In the case that a
            :class:`RunArtifact` is passed, then a :class:`RunnerModel` will be returned, otherwise the input
            artifact will be returned"""
        ...
        if isinstance(artifact, RunnerModel):
            logger.debug(f'Artifact as RunnerModel set to [{artifact._stub.name}]')
