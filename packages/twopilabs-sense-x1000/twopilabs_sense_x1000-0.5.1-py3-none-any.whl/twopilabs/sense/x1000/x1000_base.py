...
class SenseX1000Base(object):
    ...
    class AcqSubheaderAcqTiming(NamedTuple):
        ...
        @classmethod
        def from_tuple(cls, attr_tuple: Iterable) -> 'SenseX1000Base.AcqSubheaderAcqTiming':
            # Get correct data types and convert iterable elements
            subheader_types = [t._evaluate(globals(), locals()) if type(t) == ForwardRef else t
                               for t in cls.__annotations__.values()]
            subheader_list = [t(v) for t, v in zip(subheader_types, attr_tuple)]

            return cls(*subheader_list)
    ...
    class AcqSubheaderTimeAxis(NamedTuple):
        ...
        @classmethod
        def from_tuple(cls, attr_tuple: Iterable) -> 'SenseX1000Base.AcqSubheaderTimeAxis':
            # Get correct data types and convert iterable elements
            subheader_types = [t._evaluate(globals(), locals()) if type(t) == ForwardRef else t
                               for t in cls.__annotations__.values()]
            subheader_list = [t(v) for t, v in zip(subheader_types, attr_tuple)]
    ...
    class AcqSubheaderFreqAxis(NamedTuple):
        ...
        @classmethod
        def from_tuple(cls, attr_tuple: Iterable) -> 'SenseX1000Base.AcqSubheaderFreqAxis':
            # Get correct data types and convert iterable elements
            subheader_types = [t._evaluate(globals(), locals()) if type(t) == ForwardRef else t
                               for t in cls.__annotations__.values()]
            subheader_list = [t(v) for t, v in zip(subheader_types, attr_tuple)]
    ...
    class AcqSubheaderSupplementalData(NamedTuple):
        ...
        class AcqSubHeaderSupplementalDataItem(NamedTuple):
            ...
            @classmethod
            def from_tuple(cls, attr_tuple: Iterable) -> 'SenseX1000Base.AcqSubheaderSupplementalData.AcqSubHeaderSupplementalDataItem':
                # Get correct data types and convert iterable elements
                subheader_types = [t._evaluate(globals(), locals()) if type(t) == ForwardRef else t
                                   for t in cls.__annotations__.values()]
                subheader_list = [t(v) for t, v in zip(subheader_types, attr_tuple)]
        ...
        def from_stream(cls, stream: BinaryIO) -> 'SenseX1000Base.AcqSubheaderSupplementalData':
            ...
            items = [cls.AcqSubHeaderSupplementalDataItem.from_stream(bytesio) for i in range(n_items)]

            return cls(items)

        @classmethod
        def from_tuple(cls, attr_tuple: Iterable) -> 'SenseX1000Base.AcqSubheaderSupplementalData':
            # Construct subheader items from iterating down into the tuple
            item_tuples = attr_tuple[0]

            items = [cls.AcqSubHeaderSupplementalDataItem.from_tuple(t) for t in item_tuples]
    ...
    class AcqBaseHeader(NamedTuple):
        ...
        @classmethod
        def from_dict(cls, attr_dict: dict) -> 'SenseX1000Base.AcqBaseHeader':
            # Filter dictionary for keys that we require for instantiation
            header_dict = {k: attr_dict[k] for k in attr_dict.keys() if k in SenseX1000Base.AcqBaseHeader._fields}

            # Get required types required for instantiation
            header_types = {k: cls.__annotations__[k] for k in header_dict}
            header_types = {k: t._evaluate(globals(), locals()) if type(t) == ForwardRef else t
                                for k, t in header_types.items()}
    ...
    class AcqHeader(AcqBaseHeader):
        ...
        @classmethod
        def from_dict(cls, attr_dict: dict) -> 'SenseX1000Base.AcqHeader':
            baseheader = SenseX1000Base.AcqBaseHeader.from_dict(attr_dict)
            subheaders_dict = {}
            ...
            if subheaders_length > 0:
                # Build a list of subheader objects from the subheader table, when they
                # are indicated in the baseheader.subheader_flags attribute
                subheaders_dict = {a: c.from_tuple(attr_dict[a]) for a, c in cls.subheaders().items()
                                   if baseheader.subheader_flags & c.flag()}
    ...
    class AcqData(NamedTuple):
        ...
        @classmethod
        def from_stream(cls, stream: BinaryIO, header: 'SenseX1000Base.AcqHeader', seq_num: int, n_sweeps: int) -> 'SenseX1000Base.AcqData':
            ...
            return SenseX1000Base.AcqData.create(header, data[0], data[1:], seq_nums)

        @classmethod
        def create(cls, header: 'SenseX1000Base.AcqHeader', array: np.ndarray, supplemental_data: List[Optional[np.ndarray]], seq_nums: range) -> 'SenseX1000Base.AcqData':
        ...
        def to_hdf5(self, parent: Union['h5py.Group', str], name: Optional[str] = None, **kwargs) -> 'h5py.Group':
            ...
            if self.header.subhdr_freq_axis is not None:
                ...
                freqaxes_scale = group.create_dataset(name='freq_axes', data=h5py.Empty)
            ...
            if self.header.subhdr_time_axis is not None:
                ...
                timeaxes_scale = group.create_dataset(name='time_axes', data=h5py.Empty)

            freqaxes_scale.attrs.create('COMMENT', 'References to the used time axis for each sweep')
            ...
            acq_dataset.attrs.create(name='freq_axes', data=freqaxes_scale.ref)
            acq_dataset.attrs.create(name='time_axes', data=timeaxes_scale.ref)
        ...
        def from_hdf5(cls, parent: Union['h5py.Group', str], name: str, **kwargs) -> 'SenseX1000Base.AcqData':
            ...
            header = SenseX1000Base.AcqHeader.from_dict(header_dict)
            ...
            # Create data object
            acqData = SenseX1000Base.AcqData.create(header, data[0], data[1:], range(seq_nums[0], seq_nums[0] + len(seq_nums)))
