	...
	if args.cmd == 'scaffold':
		...
		for f, tbls in targets.items():
			...
			if type_tpls:
				...
				for x in tbls:
					out = template.render(table(Table(x, this=x, fileKind=schema.get('fileKind'), schema=schema_data, schemas=schema_map, args=args)))
					if out.strip():
...
def merge_schemas(schemas, kinds):
	...
	for x in ret:
		...
		for y in x['data']:
			...
			if updated:
				y['fields'] = new_fields
				y['fieldMap'] = {yy['key']: yy for yy in y['fields']}
	return ret
