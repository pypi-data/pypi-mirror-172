...
def inference(model_dir, text, batch_size=None):
    ...
    prediction = []
    for i, o in enumerate([sorted(x.items(), key=lambda kv: kv[1], reverse=True) for x in pd.DataFrame(all_logits, columns=labels).to_dict("record")]):
        r = dict(o)
        r["text"] = text[i]
        prediction.append(r)
    return prediction
