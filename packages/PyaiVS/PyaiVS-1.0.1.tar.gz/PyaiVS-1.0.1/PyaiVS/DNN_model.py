...
def get_split_index(X, Y,split_type='random',random =42):
...
def best_model_runing(seed, opt_res, X, Y, split_type='random', FP_type='ECFP4',task_type='cla',model_dir=False,difftasks=['activity'],my_df = pd.DataFrame()):
    ...
    if task_type=='cla':
        while True:
            tr_indx, val_indx, te_indx = get_split_index(X, Y, split_type='random', random=seed)
    ...
    best_model = MyDNN(inputs=inputs, hideen_units=opt_hidden_units, outputs=len(difftasks),
    ...
    if task_type=='cla':
        ...
        val_scores = run_an_eval_epoch(best_model, validation_loader, args)
        va_results = [seed,FP_type, split_type, 'val',val_scores['se'], val_scores['sp'],
...
def tvt_dnn(X,Y,split_type='random',FP_type='ECFP4',task_type='cla',model_dir=False,file_name='',difftasks=['activity']):
    ...
    while True:
        tr_indx, val_indx, te_indx = get_split_index(X, Y, split_type=split_type, random=random_state)
        data_tr_x, data_tr_y = my_df.loc[tr_indx,'Smiles'],my_df.loc[tr_indx,difftasks]
        data_va_x, data_va_y = my_df.loc[tr_indx,'Smiles'],my_df.loc[val_indx,difftasks]
        data_te_x, data_te_y = my_df.loc[tr_indx,'Smiles'],my_df.loc[te_indx,difftasks]
    ...
    best_file_name = '%s/%s_%s_%s_%s_%.4f_%d_%d_%d_%.4f_early_stop.pth' % (model_dir,task_type,FP_type,split_type,'dataset', opt_res['dropout'],
                                                                   hidden_unit1_ls[opt_res['hidden_unit1']],
                                                                   hidden_unit1_ls[opt_res['hidden_unit2']],
                                                                   hidden_unit1_ls[opt_res['hidden_unit3']],
                                                                   opt_res['l2'])
    ...
    pd_res = []
    if not reg :
        tr_results = [FP_type, split_type, 'tr', opt_hidden_units,opt_res['l2'],opt_res['dropout'], tr_scores['se'], tr_scores['sp'],
                      tr_scores['acc'], tr_scores['mcc'], tr_scores['pre'],
                      tr_scores['prc_auc'], tr_scores['roc_auc']]
        va_results = [FP_type, split_type, 'va', opt_hidden_units,opt_res['l2'],opt_res['dropout'], val_scores['se'], val_scores['sp'],
                      val_scores['acc'], val_scores['mcc'], val_scores['pre'],
                      val_scores['prc_auc'], val_scores['roc_auc']]
        te_results = [FP_type, split_type, 'te', opt_hidden_units,opt_res['l2'],opt_res['dropout'], te_scores['se'], te_scores['sp'],
                      te_scores['acc'], te_scores['mcc'], te_scores['pre'],
                      te_scores['prc_auc'], te_scores['roc_auc']]
        pd_res.append(tr_results)
        pd_res.append(va_results)
        pd_res.append(te_results)
        para_res = pd.DataFrame(pd_res, columns=['FP_type', 'split_type', 'type',
                    'opt_hidden_units', 'l2','dropout','se', 'sp','acc', 'mcc', 'precision','auc_prc', 'auc_roc'])
    else :
        tr_results = [FP_type, split_type, 'tr',
                      opt_hidden_units, opt_res['l2'], opt_res['dropout'],  tr_scores['rmse'],tr_scores['mae'], tr_scores['r2']]
        va_results = [FP_type, split_type, 'va',
                      opt_hidden_units, opt_res['l2'], opt_res['dropout'],  val_scores['rmse'],val_scores['mae'], val_scores['r2']]
        te_results = [FP_type, split_type, 'te',
                      opt_hidden_units, opt_res['l2'], opt_res['dropout'],  te_scores['rmse'],te_scores['mae'], te_scores['r2']]
        pd_res.append(tr_results)
        pd_res.append(va_results)
        pd_res.append(te_results)
        para_res = pd.DataFrame(pd_res, columns=['FP_type', 'split_type', 'type',
                                                 'opt_hidden_units','l2','dropout',
                                                 'rmse','mae','r2'])


    pd_res = []
    for i in range(10):
        item = best_model_runing((i + 1) * 500,opt_res, X, Y, split_type=split_type, FP_type=FP_type,task_type=task_type,model_dir=model_dir,difftasks=difftasks,my_df=my_df)
        pd_res.extend(item)
    if  not reg :
