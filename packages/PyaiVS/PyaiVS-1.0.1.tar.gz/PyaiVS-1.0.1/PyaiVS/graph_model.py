...
from script.gnn_utils import AttentiveFPBondFeaturizer, AttentiveFPAtomFeaturizer, collate_molgraphs, \
...
def get_split_index(X, Y,split_type='random'):
...
def tvt_dl(X,Y,split_type='random',model_name='gcn',task_type='cla',file_name=None,model_dir=None,device ='cpu',difftasks='activity'):
    ...
    tr_indx, val_indx, te_indx = get_split_index(X,Y,split_type=split_type)
    ...
    def hyper_opt(hyper_paras):
        ...
        if model_name == 'gcn':
            ...
            model_file_name = '%s/%s_%s_%s_%s_%.6f_%s_%s.pth' % (model_dir, args['model'], split_type, args['task'],
            ...
            model_file_name = '%s/%s_%s_%s_%s_%.6f_%s_%s_%s.pth' % (model_dir, args['model'], split_type, args['task'],
                                                                    hyper_paras['l2'], hyper_paras['lr'],
            ...
            model_dir, args['model'], split_type, args['task'],
            hyper_paras['l2'], hyper_paras['lr'],
   ...
   # construct the model based on the optimal hyper-parameters
    ...
    if model_name == 'gcn':
        ...
        best_model_file = '%s/%s_%s_%s_%s_%.6f_%s_%s.pth' % (model_dir,args['model'], split_type,args['task'],
                                                                         l2_ls[opt_res['l2']], lr_ls[opt_res['lr']],
                                                                         hidden_feats_ls[opt_res['gcn_hidden_feats']],
                                                                         classifier_hidden_feats_ls[opt_res['classifier_hidden_feats']])

        param = {'l2':l2_ls[opt_res['l2']], 'lr':lr_ls[opt_res['lr']],
             'gcn_hidden_feat':hidden_feats_ls[opt_res['gcn_hidden_feats']],
             'classifier_hidden_feats':classifier_hidden_feats_ls[opt_res['classifier_hidden_feats']]}
    elif model_name == 'mpnn':
        ...
        best_model_file = '%s/%s_%s_%s_%s_%.6f_%s_%s_%s.pth' % (model_dir,args['model'], split_type, args['task'],
                                                                        l2_ls[opt_res['l2']], lr_ls[opt_res['lr']],
        ...
        best_model_file = '%s/%s_%s_%s_%s_%.6f_%s_%s_%s_%s.pth' % (model_dir,args['model'], split_type, args['task'],
                                                                           l2_ls[opt_res['l2']], lr_ls[opt_res['lr']],
    ...
    record = {'roc_auc':[tr_scores['roc_auc'],val_scores['roc_auc'],te_scores['roc_auc']],
              'prc_auc':[tr_scores['prc_auc'],val_scores['prc_auc'],te_scores['prc_auc']],
    ...
    best_res = pd.DataFrame(record,index=['tr','val','te'])
    ...
    for seed in range(1, repetitions + 1):
        ...
        if args['metric'] == 'roc_auc':
            ...
            while True:
                data_re, data_te = train_test_split(my_df, test_size=0.1, random_state=42)

                data_tr, data_va = train_test_split(data_re, test_size=(8 / 9), random_state=seed)
            ...
            data_tr, data_re = train_test_split(my_df, test_size=0.2, random_state=seed)

            data_va, data_te = train_test_split(data_re, test_size=0.5, random_state=seed)
    ...
    tr_pd['seed'] = [seed*500 for seed in range(1, repetitions + 1)]
    tr_pd['type'] = 'tr'
    val_pd = pd.DataFrame(val, columns=cols)
    val_pd['seed'] = [seed*500 for seed in range(1, repetitions + 1)]
    val_pd['type'] = 'va'
    te_pd = pd.DataFrame(te, columns=cols)
    te_pd['seed'] = [seed*500 for seed in range(1, repetitions + 1)]
    te_pd['type'] = 'te'
