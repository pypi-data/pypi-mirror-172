...
def best_model_runing(seed, opt_res, data, split_type='random', FP_type='ECFP4',task_type='cla',model_dir=False,difftasks=['activity'],my_df = pd.DataFrame()):
...
def tvt_dnn(X,Y,split_type='random',FP_type='ECFP4',task_type='cla',model_dir=False,file_name='',difftasks=['activity']):
    ...
    pd_res = []
    for i in range(9):
        item = best_model_runing((i + 1),opt_res, data, split_type=split_type, FP_type=FP_type,task_type=task_type,model_dir=model_dir,difftasks=difftasks,my_df=my_df)
        pd_res.extend(item)
    if  not reg :
