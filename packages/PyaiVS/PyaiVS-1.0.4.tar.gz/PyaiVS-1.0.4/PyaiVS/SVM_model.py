...
def best_model_runing(seed,best_results,X,Y,split_type='random',FP_type='ECFP4',task_type = 'cla',model_dir=False):
    pd_res = []
    if task_type == 'cla':
        while True:
    ...
    if task_type == 'cla':
        ...
        pd_res.append(te_results)
    return pd_res
def tvt_svm(X,Y,split_type='random',FP_type='ECFP4',task_type = 'cla',model_dir=False):
    # data_tr_x, data_va_x, data_te_x, data_tr_y, data_va_y, data_te_y = split_dataset(X, Y, split_type=split_type,
    #                                                                                      valid_need=True,random_state=40)
    data_x, data_te_x, data_y, data_te_y = split_dataset(X, Y, split_type='random', valid_need=False, random_state=42,
                                                         train_size=0.90)
