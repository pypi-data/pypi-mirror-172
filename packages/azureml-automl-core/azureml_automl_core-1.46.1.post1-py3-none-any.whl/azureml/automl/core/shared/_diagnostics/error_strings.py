...
class AutoMLErrorStrings:
    ...
    FORECAST_MISALIGNED_TIMES = \
        "Input prediction data contains times which are not aligned with timeseries frequency " \
        "determined during training. Please check the prediction data frequency and whether it forms " \
        "a regularly spaced grid with the training data times. " \
        "If the misalignment is expected, please use the `ignore_data_errors=True` option in forecast() " \
        "or rolling_forecast(). In this case, the misaligned rows will be not be forecasted. " \
        "The forecaster will impute values at the missing grid point in this case as well, so treat results " \
        "with caution."
    ...
    NLP_ARGUMENT_PARSING_ERROR = "Encountered exception while parsing command-line arguments: {details}"
    ...
    NLP_INVALID_SWEEP_ARGUMENT = "Invalid sweeping argument detected. {log_safe_description:log_safe}."
    ...
    NLP_MALFORMED_LABEL_COLUMN = "Failed while parsing input label column in {split_type:log_safe} dataset. " \
                                 ...
                                 "{info_link:log_safe}."
    NLP_MALFORMED_NER_INFERENCE_INPUT = "If a list is provided to the NER model wrapper, it must contain a single " \
                                        "element corresponding to the CoNLL-formatted input string. The provided " \
                                        "list had {list_length:log_safe} elements."
    ...
    NLP_NER_INFERENCE_TYPE_MISMATCH = "A CoNLL-formatted input string or a single numpy array containing a " \
                                      "CoNLL-formatted input string are currently the only accepted input types for " \
                                      "NER inferencing. An input of type {bad_type} was encountered instead. Please " \
    ...
    NLP_VNET_SUSPECTED = "Encountered connection-related error that prevented the downloading of " \
                         "critical training resources (such as pretrained model embeddings). This is most commonly " \
                         "due to an improperly configured virtual network. If you're running under a vnet, " \
                         "please update your firewall settings per: {info_link:log_safe}. Otherwise, this is likely " \
                         "due to a transient network issue or service outage that's impacting either your " \
                         "network or our dedicated resource CDN. Please try again later."
    ...
    ROLLING_FORECAST_MISSING_TARGET_COLUMN = "The provided test data is missing target column which is required " \
                                             "when 'forecast_mode' is set to 'rolling'. Please either set " \
                                             "'forecast_mode' to 'recursive' or provide 'target_column_name'."
    ROLLING_FORECAST_MISSING_TARGET_VALUES = 'The provided test data is missing target values for some times within ' \
                                             'the testing period. These values are either NaN or the time point ' \
                                             'they belong to is absent from the data entirely. Please check that ' \
                                             'the test data has no missing or null values and that the time column ' \
                                             'forms a regularly spaced grid with respect to the timeseries ' \
                                             'frequency. ' \
                                             'If the missing values are expected, please run rolling_forecast() ' \
                                             "with the `ignore_data_errors=True` option. " \
                                             "In this case, missing values will be imputed."
    ...
    TCN_EXTRA_COLUMN_IN_TEST_SET = "The data set contains extra column {column}. " \
                                   "Please remove extra column and run prediction again."
    ...
    TIMESERIES_DF_DUPLICATED_INDEX_TM_COL_NAME = \
        ...
        "to define the time series identifiers. If the dataset needs to be aggregated, " \
    ...
    TIMESERIES_NO_DATA_CONTEXT = \
        ("Missing prediction context. There is a gap between the end of {data_period} "
         "period and the beginning of the provided test set; the testing period is "
         "expected to directly follow the {data_period} period since the forecaster "
         "may depend on past values. If this gap is expected, please run {method} with "
         "the `ignore_data_errors=True` option. "
         "In this case, the target and feature values in the gap will be imputed.")
