...
class NiraapadClientHelper:
    ...
    def initialize(self, id, backend_type, args_pickled, kwargs_pickled,
                   stacktrace_pickled):
        print("In NiraapadClient Servcier")
    ...
    def uninitialize(self, id, backend_type, backend_instance_id):
        resp = self.stub.Uninitialize(
            niraapad_pb2.UninitializeReq(
                id=id,
                backend_type=backend_type,
    ...
    def uninitialize_trace(self, id, backend_type, backend_instance_id,
                           exec_time_sec):
        trace_msg = niraapad_pb2.UninitializeTraceMsg(
            req=niraapad_pb2.UninitializeReq(
                id=id,
                backend_type=backend_type,
                backend_instance_id=backend_instance_id,
                time_profiles=NiraapadClient.get_time_profiles()),
    ...
    def config_variables(self, args_pickled, kwargs_pickled):
        send_msg = self.stub.ConfigSetter(niraapad_pb2.ConfigSetterReq(args=args_pickled, kwargs=kwargs_pickled))
    
    def config_getter(self, args_pickled, kwargs_pickled):
        resp = self.stub.ConfigGetter(niraapad_pb2.ConfigGetterReq(args=args_pickled, kwargs=kwargs_pickled))
        return resp
...
class NiraapadClient:
    ...
    @staticmethod
    def update_arrival_time(ignore_time=False):
    ...
    @staticmethod
    def update_departure_time(id, arrival_time, ignore_time=False):
        NiraapadClient.niraapad_lock.acquire()
        departure_time = 0
        if ignore_time == False:
    ...
    def uninitialize(self, *args, **kwargs):
        ...
        initialization.
        """
        [id, arrival_time] = NiraapadClient.update_arrival_time(ignore_time=True)
        ...
        if NiraapadClient.niraapad_mos[
            ...
            try:
                NiraapadClient.niraapad_client_helper.uninitialize_trace(
                    id, self.niraapad_backend_type,
                    self.niraapad_backend_instance_id, (end - start))
            except Exception as e:
                self.handle_exception("uninitialize_trace" % e)

        NiraapadClient.update_departure_time(id, arrival_time, ignore_time=True)
    ...
    def config_variables(self, *args, **kwargs):
        ...
        if NiraapadClient.niraapad_mos[
            ...
            resp = None
                ...
                self.niraapad_backend_type] == utils.MO.VIA_MIDDLEBOX:
            try:
                resp = NiraapadClient.niraapad_client_helper.config_variables(
                    pickle.dumps(args), pickle.dumps(kwargs))
            except Exception as e:
                self.handle_exception(method_name, e, True)
    ...
    def config_getter(self, *args, **kwargs):
        ...
        if NiraapadClient.niraapad_mos[
            ...
            resp = None
                ...
                self.niraapad_backend_type] == utils.MO.VIA_MIDDLEBOX:
            try:
                resp = NiraapadClient.niraapad_client_helper.config_getter(
                    pickle.dumps(args), pickle.dumps(kwargs))
            except Exception as e:
                self.handle_exception(method_name, e, True)
