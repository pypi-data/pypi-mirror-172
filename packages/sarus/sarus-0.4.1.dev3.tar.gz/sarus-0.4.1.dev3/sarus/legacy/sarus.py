...
import sarus.legacy.pandas as spd
import sarus.legacy.tensorflow as sarus_tensorflow
...
class Dataset:
    ...
    def as_tensorflow(
        ...
        original: bool = False,
    ) -> sarus_tensorflow.Dataset:
        """Return the corresponding `sarus.legacy.tensorflow.Dataset` object.
        ...
        Returns:
            A sarus.legacy.tensorflow.Dataset.
        """
        return sarus_tensorflow.Dataset(
            self,
    ...
    def _adapt_for_tf(
        ...
        for feature in features:
            ...
            if dtype == "datetime":
                ...
                cleaned_batch = [
                    x if x is not None else "" for x in batch[feature["name"]]
                ]
                adapted_batch[feature["name"]] = tf.constant(
                    cleaned_batch, dtype=tf.string
                )
