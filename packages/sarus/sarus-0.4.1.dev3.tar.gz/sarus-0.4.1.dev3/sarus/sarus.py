...
from sarus.manager.typing import LocalSDKManager
from sarus.typing import DataSpecVariant, DataSpecWrapper, SyncPolicy
...
class Dataset:
    ...
    def __init__(
        ...
        if dataspec is None:
            ...
            _ = (
                client.context()
                .factory()
                .create(dict_deserialize(dataspec["schema"]))
            )
            _ = (
                client.context()
                .factory()
                .create(dict_deserialize(dataspec["synthetic_schema"]))
            )
            _ = (
                client.context()
                .factory()
    ...
    def table(self, table_name: t.List[str]) -> Table:
        ...
        return Table(selected_ds, table_name)
    ...
    def _fetch_synthetic(
        ...
        if not has_enough_synthetic or force_refresh:
            ...
            resp = self.client.session.get(
                f"{self.client.base_url}/synthetic_data/{self.id}/{table_name}",
                stream=True,
                params={
    ...
    def tables(self):
        return _get_tables(self._dataspec)
...
class Client:
    ...
    def __init__(
        ...
        self._context.set_sync_policy(SyncPolicy.SEND_ON_VALUE)
        push_global_context(self._context)

    def context(self) -> LocalSDKContext:
        return self._context

    def save(self, wrapper: DataSpecWrapper, path: str) -> None:
        """Save the DataSpecWrapper for loading in another notebook."""
        manager: LocalSDKManager = self.context().manager()

        user_ds = wrapper.dataspec(DataSpecVariant.USER_DEFINED)
        alternative_ds = wrapper.dataspec(DataSpecVariant.ALTERNATIVE)
        mock_ds = wrapper.dataspec(DataSpecVariant.MOCK)
        ...
        alternative_refs = {alternative_ds}
        ...
        refs = user_refs | alternative_refs | mock_refs
    ...
    def load(self, path: str) -> DataSpecWrapper:
        """Load a DataSpecWrapper from a file."""
...
class Table:
    """Class representing a Sarus Table, part of a Sarus Dataset."""

    def __init__(self, filtered_dataspec, table_name: t.List[str]):
        self._dataspec = filtered_dataspec
        self.name = table_name

    def __repr__(self) -> str:
        return f"<Sarus Table name={self.name}>"
...
def _get_tables(dataspec) -> t.List[t.List[str]]:
    """For given parameters of a Sarus Dataset return a list of Sarus Tables."""

    dataspec_tables = [
        el.to_strings_list()[0] for el in dataspec.schema().tables()
    ]
    # remove protection name and add schema_name
    dataspec_tables = [
        [dataspec.schema().name(), *(val for val in el[1:])]
        for el in dataspec_tables
    ]
