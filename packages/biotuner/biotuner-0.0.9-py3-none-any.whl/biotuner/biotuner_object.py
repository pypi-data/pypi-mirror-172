...
class compute_biotuner(object):
    ...
    def peaks_extraction(
        ...
        self.peaks_ratios_cons, b = consonant_ratios(
            self.peaks, limit=scale_cons_limit
        )
        if ratios_extension is True:
            ...
            if b is not None:
                ...
                bound_ = [rebound(x, low=1,
                                  high=octave,
                                  octave=octave) for x in b]
    ...
    def peaks_extension(
        ...
        if method == "harmonic_fit":
            ...
            self.extended_peaks = np.sort(list(peaks) +
        ...
        if method == "multi_consonant_harmonic_fit":
            ...
            extended_temp = multi_consonance(cons_pairs, n_freqs=10)
            self.extended_peaks = np.sort(np.round(extended_temp, 3))
        self.extended_peaks = [i for i in self.extended_peaks
                               if i < self.sf / 2]
    ...
    def ratios_extension(
        self,
        ratios,
        ratio_fit_bounds=0.001,
        ratios_n_harms=None
    ):
        """
    ...
    def compute_peaks_metrics(
        ...
        peaks_ratios = compute_peak_ratios(
            peaks, rebound=True,
            octave=self.octave, sub=self.compute_sub_ratios
        )
        metrics = {"cons": 0,
                   "euler": 0,
                   "tenney": 0,
    ...
    def compute_harmonic_entropy(
        ...
        if input_type == "extended_peaks":
            ratios = compute_peak_ratios(self.extended_peaks,
                                         rebound=rebound,
                                         sub=sub)
        ...
        if peaks_function == "EEMD_FOOOF":
            ...
            for imf in IMFs:
                ...
                try:
                    ...
                    peaks_temp.append(
                        [x for _, x in sorted(zip(amps_temp_EMD,
