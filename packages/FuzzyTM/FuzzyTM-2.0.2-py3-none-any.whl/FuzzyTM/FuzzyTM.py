...
class FuzzyTM():
    ...
    def _check_format_input_data(self):
        ...
        for i, doc in enumerate(self._input_file):
            ...
            if not len(doc) > 0:
                raise ValueError(
                    "The input_file has an empty list at index ",
                    str(i),
    ...
    def _create_vocabulary(input_file):
        """
        Create the vocabulary from 'input_file'.

        Parameters
        ----------
             input_file : list of lists of str
    ...
    @staticmethod
    def _create_sum_words (input_file):
        """
        Creates a Counter object that stores the count of each word in the corpus (input_file).
    ...
    def _create_sparse_local_term_weights(
            input_file, vocabulary_size, word_to_index,
            ):
        """
    ...
    def _create_projected_data(
        ...
        For SVD on a sparse matrix, the sparsesvd package is used (https://pypi.org/project/sparsesvd/)
    ...
    def show_topics(
        ...
        """
        Show the top-n words associated with each topic.
        ...
        ----------
            ...
            index_to_word : dict {int : str}
                Maps each unique index number to a unique vocabulary word.
            representation : str
                Indicates whether only words are returned or a combination
                  of words and probabilities per word (either 'both' or 'words').
    ...
    def get_topic_embedding(
        ...
        ----------
            ...
            topn : int
                The top-n words to include (needs only to be used when 'method=topn'.
            perc: float
                The benchmark percentile until which words need to be added (between 0 and 1).

        Returns
        -------
            numpy.array : float
                Array in which each row gives the topic embedding for the associated document.
        """
        self._check_format_input_data()
        if prob_word_given_topic is None:
            prob_word_given_topic = self._prob_word_given_topic
    ...
    def _create_dictlist_topn(
        ...
        -------
             list of dicts {int : float}
                Keys: all the indices of words from prob_word_given_topic who's weight's
                    are amongst the top percentage.
    ...
    def _create_dictlist_percentile(
        ...
        ----------
             perc : float
                The top percentile words to include (needs only to be used when 'method=percentile'.
             prob_word_given_topic : numpy.array : float
                Matrix that gives the probability of a word given a topic.
             index_to_word : dict {int : str}
                Maps each unique index number to a unique vocabulary word.
    ...
    def get_coherence_score(
        ...
        ----------
             ...
             topics : list of lists of str
                 The words per topics, equivalent to self.show_topics(representation='words').
             coherence : str
...
class FLSA_V(FuzzyTM):
    ...
    Parameters
        ----------
            ...
            map_file: pd.DataFrame
                 The output file from Vosviewer.
                 The Dataframe needs to contain the following columns: 'id','x','y').
            num_topics : int
                The number of topics that the model should train.
            num_words : int
                 Indicates how many words per topic should be shown.
    ...
    def __init__(
        ...
        self.coordinates = self._get_coordinates_from_map_file(
            self.map_file, vocab,
            )

    @staticmethod
    def _get_filtered_input(
            input_file, map_file,
            ):
        '''
    ...
    @staticmethod
    def _filter_map_file(
            map_file, filtered_input,
            ):
        """
        ...
        filtered_input : list of lists of strings
            Output from '_get_filtered_input()'

        Returns:
        -------
        pd.DataFrame
            The filtered map file.
        ...
        return map_file[map_file.id.isin(intersect)]

    @staticmethod
    def _get_coordinates_from_map_file(
            map_file, vocab,
            ):
        """
        ...
        -------
             numpy.array : float
                The coordinates from the map_file

        """
        for word in map_file['id']:
            if word not in vocab:
                map_file = map_file[map_file.id != word]
        x_axis = np.array(map_file['x'])
        y_axis = np.array(map_file['y'])
        return np.array([x_axis,y_axis]).T

    def get_matrices(self):
        """
        Method to run after the FlsaV class has been initialized to obtain the output matrices.
