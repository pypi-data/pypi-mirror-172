...
class UserInstanceRegistry():
    ...
    Args: clusters (List[MCLICluster]): A list of clusters to search. If not provided, all of the user's clusters
    will be checked
    ...
    def __init__(self, clusters: Optional[List[MCLICluster]] = None):
        self.registry = self.build_registry(clusters=clusters)

    @staticmethod
    def _get_cluster_instances(cluster: MCLICluster) -> GPUConfig:
        # pylint: disable-next=import-outside-toplevel
        from mcli.serverside.clusters import GenericK8sCluster

        k8s_cluster = GenericK8sCluster.from_mcli_cluster(cluster)
        return k8s_cluster.allowed_instances.available_instances

    def build_registry(self, clusters: Optional[List[MCLICluster]] = None) -> Dict[str, GPUConfig]:
        ...
        registry = {pl.name: self._get_cluster_instances(pl) for pl in clusters}
        return registry
...
class ClusterInstanceGPUConfiguration(NamedTuple):
...
class LocalClusterInstances(ClusterInstances):
    ...
    def __init__(
        self,
        instance_types: Optional[List[InstanceType]] = None,
        gpu_configurations: Optional[List[ClusterInstanceGPUConfiguration]] = None,
    ) -> None:
    ...
    def add_cluster_instance_gpu_configuration(
        self,
        gpu_configuration: ClusterInstanceGPUConfiguration,
    ):
