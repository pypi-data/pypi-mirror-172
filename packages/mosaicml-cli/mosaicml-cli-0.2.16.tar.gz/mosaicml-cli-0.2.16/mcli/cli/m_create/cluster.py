...
class ClusterCreator(ClusterValidator, ClusterFiller):
    ...
    @staticmethod
    def get_all_clusters() -> List[MCLICluster]:
        conf: MCLIConfig = MCLIConfig.load_config()
        return conf.clusters

    @staticmethod
    def get_available_contexts(clusters: List[MCLICluster]):
    ...
    def create_all(self,) -> List[MCLICluster]:
        created_clusters = []
    ...
    def create(
        ...
        return MCLICluster(name=name, kubernetes_context=new_context.name, namespace=namespace)
...
def _setup_k8s_cluster(cluster: MCLICluster):
...
def _sync_cluster(cluster: MCLICluster):
