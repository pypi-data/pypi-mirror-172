...
from mcli.api.runs import get_runs
...
def cli_get_runs(clusters: Optional[List[str]] = None,
    ...
    try:
        ...
        with console.status('Retrieving requested runs...'):
            if conf.feature_enabled(FeatureFlag.USE_MCLOUD):
                runs = get_runs(clusters=clusters, gpu_types=gpu_type, gpu_nums=gpu_num, statuses=status, timeout=None)
            else:
                runs = kube_get_runs(clusters=clusters,
                                     gpu_types=gpu_type,
                                     gpu_nums=gpu_num,
                                     statuses=status,
                                     timeout=None)
