...
def _get_runs_kube(
    ...
    if clusters is not None:
        chosen_clusters: List[MCLICluster] = []
...
def get_runs(
    ...
    clusters: Optional[Union[List[str], List[MCLICluster]]] = None,
...
def get_runs(
    ...
    List runs that have been launched in the MosaicML Cloud. The returned list will
    ...
    Arguments:
        ...
        clusters (``Optional[List[str] | List[``:class:`~mcli.models.mcli_cluster.MCLICluster` ``]]``):
            List of clusters to filter runs. This can be a list of str or
            :class:`~mcli.models.mcli_cluster.MCLICluster`
    ...
    if clusters is not None:
        clusters = [pl.name if isinstance(pl, MCLICluster) else pl for pl in clusters]
    elif runs:
        ...
        clusters = list(known_clusters) or None
    ...
    runs_future = _get_runs_kube(run_names, clusters, gpu_types, gpu_nums, statuses)
