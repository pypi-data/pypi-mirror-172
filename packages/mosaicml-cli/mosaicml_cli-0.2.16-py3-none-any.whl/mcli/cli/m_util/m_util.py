...
def get_util(cluster: str, **kwargs) -> int:
    del kwargs

    load_config = config.MCLIConfig.load_config()
    clusters = [x for x in load_config.clusters if cluster == x.name]

    for plat in clusters:
...
def _get_cluster_args(clusters: list) -> dict:
    args = {
...
def configure_parser(parser: argparse.ArgumentParser):
    ...
    args = _get_cluster_args(available_clusters)
    parser.add_argument('cluster', **args)
