...
if MCLIConfig.load_config(safe=True).feature_enabled(FeatureFlag.USE_MCLOUD):
    from mcli.api.runs import delete_runs, follow_run_logs, get_runs

    def get_run_logs(run: Run, rank: Optional[int], timeout: int = 0) -> str:
        raise NotImplementedError('get_run_logs is not yet implemented serverside: HEK-1245')
else:
    from mcli.sdk import delete_runs, follow_run_logs, get_run_logs, get_runs  # type: ignore [reportGeneralTypeIssues]
    ...
    try:
        ...
        with err_console.status('Getting run details...') as spinner:
            if run_name is None:
                ...
                run = get_latest_run()
                logger.info(f'{INFO} No run name provided. Displaying log of latest run: [blue]{run.name}[/]')
            else:
        ...
        if follow and run.status.after(RunStatus.QUEUED) and run.status.before(RunStatus.RUNNING):
            timeout = 300
            with MCLICluster.use(run.config.cluster) as cluster:
        ...
        if follow:
            ...
            for line in get_run_logs(run, rank=rank, timeout=300).split('\n'):
            ...
            if last_line and last_line.endswith('\x1B[A'):
                print('')
