...
from lfmaptools.netcdf_utils import nc2xr, nc_max, nc_min, nc_min_max
...
class LaharFlowData(object):
    ...
    def raster(
        ...
        raster = self._get_data(filename)

        extra_cols = ["x", "y", "latitude", "longitude"]

        if var is not None:
            raster = raster[var + extra_cols]
