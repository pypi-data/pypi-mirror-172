...
def Coefficient(v1, v2):
    return dot(v2, v1) / numpy.dot(v1, v1)
...
def Lanczos(A, m):
    ...
    V = numpy.zeros((n, m))
    v = [numpy.zeros(n), Normalize(numpy.ones(n))]

    for i in range(0, m):
        w = numpy.dot(A, v[1]).squeeze()

        alpha[i] = numpy.dot(w, v[1])
...
def EigenSolve(A, m):
    ...
    sortedIndex = numpy.argsort(numpy.absolute(d))[::-1]
    d           = d[sortedIndex]
    v           = v[:, sortedIndex]
