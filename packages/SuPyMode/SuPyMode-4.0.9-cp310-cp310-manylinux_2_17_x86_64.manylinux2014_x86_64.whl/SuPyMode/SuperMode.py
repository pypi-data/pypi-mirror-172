...
class SuperMode(object):
    ...
    def __render__index__(self, Figure: Plots.Scene, Ax: Plots.Axis) -> None:
        """
        Args:
            Figure: MPSPlots Scene instance.
            Ax: MPSPLots Axis instance.
        """
    ...
    def __render__beta__(self, Figure: Plots.Scene, Ax: Plots.Axis) -> None:
        """
        Args:
            Figure: MPSPlots Scene instance.
            Ax: MPSPLots Axis instance.
    ...
    def __render__adiabatic__(self, Figure: Plots.Scene, Ax: Plots.Axis, Other=None) -> None:
        """
        Args:
            Figure: MPSPlots Scene instance.
            Ax: MPSPLots Axis instance.
            Other: Other SuperMode instance to evaluate the adiabatic criterion with.
        """
        Figure.UnitSize = (10,4)
    ...
    def __render__coupling__(self, Figure: Plots.Scene, Ax: Plots.Axis, Other=None) -> None:
        """
        Args:
            Figure: MPSPlots Scene instance.
            Ax: MPSPLots Axis instance.
            Other: Other SuperMode instance to evaluate the coupling with.
        """
        Figure.UnitSize = (10,4)
    ...
    def __render_field__(self, Figure: Plots.Scene, 
                               Ax: Plots.Axis, 
                               ITR: float=None, 
                               Slice: int=None) -> None:
        ...
        """
        Args:
            ITR: Value of ITR value to evaluate the mode field.
            Slice: Value reprenting the slice where the mode field is evaluated.
        """
        Figure.UnitSize = (3,3)
        ...
        if ITR is not None:
            Slice = self.ParentSet.ITR2Slice(ITR) 

        if Slice is not None:
            ITR = self.ParentSet.Slice2ITR(Slice)

        Ax.Title = f'{self.Name}\n[Slice: {Slice}  ITR: {ITR:.2f}]'
        field = self.FullFields[Slice]

        artist = Plots.Mesh(X=xAxis, Y=yAxis, Scalar=field, ColorMap=Plots.CMAP.BKR)
    ...
    def __getitem__(self, N: int) -> _SuperMode:
        return self.Slice[N]
