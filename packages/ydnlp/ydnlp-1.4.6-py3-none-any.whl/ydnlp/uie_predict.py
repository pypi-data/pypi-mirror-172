...
from ydnlp.utils.downloader import download_model_name_or_path,MODEL_HOME
...
class InferBackend(object):
    ...
    def __init__(self,
                 ...
                 use_fp16=False):
        # print(">>> [InferBackend] Creating Engine ...")
        onnx_model_file = get_model_data_from_path(model_path_prefix)
        
        providers = ['CPUExecutionProvider']
        

        sess_options = ort.SessionOptions()
        self.predictor = ort.InferenceSession(onnx_model_file,
                                              sess_options=sess_options,
                                              providers=providers)
...
class UIEPredictor(object):
    ...
    def __init__(self,  max_seq_len, batch_size, schema, model_name_or_path='model',position_prob=0.5):
        download_model_name_or_path(model_name_or_path)
        vocab_file = os.path.join(MODEL_HOME,'vocab.txt')
        model_path = os.path.join(MODEL_HOME,model_name_or_path+'.onnx')
        self._tokenizer = MyTokenizer(vocab_file)
        ...
        self.inference_backend = InferBackend(model_path, device='cpu', use_fp16=False)
...
def main():
    ...
    predictor = UIEPredictor(512, 4, schema1, model_name_or_path='model',position_prob=0.5)
    print("-----------------------------")
