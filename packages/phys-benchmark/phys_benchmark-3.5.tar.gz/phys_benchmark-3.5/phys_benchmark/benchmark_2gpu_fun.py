...
def Benchmark_2gpu():
    ...
    for index, row in df.iterrows():
        with open("job_script_2_" + (str(row["run"])), "w") as f:
            f.write(
                    "#!/bin/bash -l" +  "\n"
                    ...
                    "srun gmx_mpi mdrun -v -s *.tpr -ntomp $OMP_NUM_THREADS -pme gpu -pmefft gpu -npme 1 -bonded gpu -nb gpu -maxh 0.25"+  "\n"
        )
        f.close()
