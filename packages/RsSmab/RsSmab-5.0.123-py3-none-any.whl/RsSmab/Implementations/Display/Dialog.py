...
class DialogCls:
	...
	def close(self, dialog_id: str) -> None:
		...
		Closes the specified dialog. \n
			:param dialog_id: string To find out the dialog identifier, use the query method RsSmab.Display.Dialog.id. The DialogName part of the query result is sufficient.
		"""
	...
	def close_all_with_opc(self, opc_timeout_ms: int = -1) -> None:
		...
		Closes all open dialogs. \n
		Same as close_all, but waits for the operation to complete before continuing further. Use the RsSmab.utilities.opc_timeout_set() to set the timeout value. \n
	...
	def open(self, dialog_id: str) -> None:
		...
		Opens the specified dialog. \n
			:param dialog_id: string To find out the dialog identifier, use the query method RsSmab.Display.Dialog.id. The DialogName part of the query result is mandatory.
		"""
		param = Conversions.value_to_quoted_str(dialog_id)
