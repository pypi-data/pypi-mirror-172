...
class MyDrawWidget(QtWidgets.QWidget):
    ...
    def curved_connector(self,qp,x0,y0,xf,yf,xi,yi,xd,yd,dashed=False,is_flow=True):
        """
        ...
        # define some points
        end_is_start = False
        if abs(x0-xf) < 0.02 and abs(y0-yf) < 0.02:
            end_is_start = True
    ...
    def paintEvent(self, event):
        ...
        if self.parent().checkBox_7.isChecked(): # draw boundary rect with white background?
            br_white = QtGui.QBrush(QtGui.QColor(255, 255, 255, 255),1)
            qp.fillRect(QtCore.QRect(QtCore.QPoint(0,0),QtCore.QPoint(self.W-2,self.H-2)),br_white) # background 
        ...
        for conn in self.connectors:
            ...
            if conn.c == self.highlighted:
                ...
                pen = QtGui.QPen(self.parent().theme_manager.get_color(2),1.0)

                if conn in active_connectors:
                
                    # is actually a flow?
                    is_flow = False
                    for i,data in enumerate(self.parent().entry_data): # TODO make this more efficient 
                    ...
                    if is_flow:
                        ...
                        pen = QtGui.QPen(self.parent().theme_manager.get_color(7),1)
            ...
            if conn in active_connectors:
                ...
                if not self.parent().checkBox_8.isChecked():
                ...
                if self.parent().checkBox_5.isChecked(): # draw at handles
                    if self.parent().checkBox_2.isChecked():
                        ...
                        if self.parent().checkBox_4.isChecked():
                            fm = qp.fontMetrics()
                            text_length = fm.boundingRect(conn_label_name).width()
                        ...
                        qp.drawText(a,b,conn_label_name)
        ...
        for box in self.boxes:
            ...
            if box.ishelper:
                ...
                if self.parent().checkBox_4.isChecked():
                    br_label = QtGui.QBrush(self.parent().theme_manager.get_color(3),1)
                    fm = qp.fontMetrics()
                    text_length = fm.boundingRect(box.name.replace("_"," ")).width()
                    
                    qp.fillRect(QtCore.QRect(QtCore.QPoint(int(a-.1),b-10),QtCore.QPoint(int(a+text_length),b+7)),br_label) # background 
                
                qp.drawText(a ,b,box.name) # text 
