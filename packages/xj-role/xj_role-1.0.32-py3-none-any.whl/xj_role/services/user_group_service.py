...
class UserGroupService(object):
    ...
    @staticmethod
    def get_user_group_list(params):
        """获取用户组列表"""
        ...
        # 代码不要偷懒，**params，这种算什么参数，API层和Model层直接传参耦合，代码根本看不明白在干嘛？更严重的是直接暴露了数据安全给前端。 20221007 by Sieyoo
    ...
    def get_user_group_tree(group_id=None, is_family_tree=False):
        ...
        if group_id:
            group_tree = JRecur.filter_forest(source_forest=group_tree, find_key='id', find_value=group_id,
    ...
    def group_user_detail(user_id):
        ...
        #     return None, err
        # if not user_serv:
        #     user_serv_dict, err = UserService.user_basic_message(user_id)
        #     if err:
        #         return None, err
        #     user_serv = user_serv_dict[0]
        user_serv_dict, err = UserService.user_basic_message(user_id)
        if err:
            return None, err
        user_serv = user_serv_dict
        # sso = UserSsoToUser.objects.filter(user_id=current_user.get('id', None)).first()
        # if sso:
        #     sso = model_to_dict(sso)
        #     user_serv['openid'] = sso['sso_unicode']
    ...
    def group_user_add(params):
        ...
        if err:
            return None, err
        if user_group_list:
            UserGroupService.bind_user_group(user_serv['user_id'], user_group_list)
    ...
    def group_user_edit(params):
        ...
        user_role_list = params.get('user_role_list', None)
        user_group_list = params.get('user_group_list', None)
        user_serv, err = UserService.user_role_edit(params)
        if err:
            return None, err
        UserGroupService.bind_user_group(user_serv['user_id'], user_group_list)
        RoleService.bind_user_role(user_serv['user_id'], user_role_list)
