...
def tasksubmit(**kwargs):
    ...
    response = requests.post(
        'https://appeears.earthdatacloud.nasa.gov/api/task', headers={"Authorization": f"Bearer {token}"}, json=payload)
    if response.status_code in error_codes.keys():
        print(error_codes.get(response.status_code))
        sys.exit(response.json()['message'])
    else:
...
def tasksubmit_from_parser(args):
    tasksubmit(name=args.name, product=args.product, start=args.start,
               end=args.end, recurring=args.recurring, input=args.geometry)
...
def main(args=None):
    ...
    required_named = parser_tasksubmit.add_argument_group(
        "Required named arguments.")
    required_named.add_argument(
        "--name", help="Task name", required=True
    )
    required_named.add_argument(
        "--product", help="Product ID returned from product tool", required=True
    )
    required_named.add_argument(
        "--geometry", help="Full path to geometry.geojson file point or single polygon", required=True
    )
    required_named.add_argument(
