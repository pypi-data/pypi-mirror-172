...
class ModelInfo:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if fall_back is not None:
            LOG.warning('WARNING: fall_back will be deprecated in a future version. Use missing_value_encodings instead. ')
        if missing_value_encodings is None:
            ...
            if fall_back is not None and fall_back != missing_value_encodings:
                LOG.warning('WARNING: Both missing_value_encodings and fall_back are specified, accept '
                            'missing_value_encodings and ignore fall_back. ')
        if self.missing_value_encodings is not None:
            ModelInfo.validate_missing_value_encodings(self.missing_value_encodings, self.get_all_cols())
        self.fall_back = self.missing_value_encodings  # backward compatability

        available_cols = self.get_input_names() + self.get_target_names() + self.get_metadata_names()
    ...
    @staticmethod
    def validate_missing_value_encodings(missing_value_encodings: dict, all_columns: List[Column]) -> None:
        unmatched_col = []
        for k, v in missing_value_encodings.items():
            ...
            if matched_column is None:
                LOG.warning(f'WARNING: Missing_value_encodings(fall_back): Column {k} Not found in all column names.'
                            'Dropping it.')
                unmatched_col.append(k)
            else:
                # check if the column is specified as is_nullable
                if matched_column.is_nullable is not None and matched_column.is_nullable is False:
                    raise ValueError(
                        f'Missing_value_encodings(fall_back) specified for Column({k}) should not be nullable. '
                        f'Please change the definition of Column({k})'
                    )
    ...
    def from_dict(cls, deserialized_json: dict):
        ...
        if model_task == ModelTask.RANKING:
            ...
            if 'is_binary_ranking_model' in deserialized_json:
                is_binary_ranking_model = deserialized_json.pop('is_binary_ranking_model')
    ...
    def ranking_cls_inference(
        ...
        return output_names, target_class_order, ranking_top_k, group_by, is_binary_ranking_model
