...
def get_client_price(client: dict, plan_price: int, corpo_discount: float, sum_access=True,
    ...
    if corpo_discount:
        prices["corpo_discount"] = round(- plan_price * corpo_discount / 100, 2)
        ...
        if client["discounts"]:
            try:
                for discount in [i for i in client["discounts"] if i["aplicaciones_restantes"] > 0]:
                    ...
                    prices[concepto] = round(- discount_price, 2)
            except:
...
def process_payment(payment_data: dict,
    ...
    :rtype: dict
    """
    if not sdk_cadena or 'seller_fee' not in payment_data:
        sdk_cadena = sdk
    ...
    if "seller_fee" in payment_data.keys():
        if 100 > payment_data["seller_fee"] > 0:
            ...
            application_fee = (1 - payment_data["seller_fee"] / 100) * payment_data["charges_detail"]["final_price"]
            mp_payment_data["application_fee"] = round(application_fee, 2)
...
def update_payment_data(payment_result: dict, payment_data: dict, card_data: dict):
    ...
    if "processor" in payment_result:
        intento["processor"] = payment_result["processor"]
