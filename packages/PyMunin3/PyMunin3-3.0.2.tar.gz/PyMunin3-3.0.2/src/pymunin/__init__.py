...
class MuninAttrFilter:
    ...
    def __init__(
        self, list_include=None, list_exclude=None, attr_regex=None, default=True
    ):
...
class MuninPlugin:
        ...
        if self.isMultigraph:
            ...
            if self.isMultiInstance:
                ...
                if self._instanceName is not None:
                    label_type = self.envGet("instance_label_format", "suffix").lower()
            ...
            if subgraphs is not None:
                ...
                if fail_noexist and subgraph is None:
                    raise AttributeError(
                        "Invalid subgraph name %s"
                        "for graph %s." % (graph_name, parent_name)
                    )
                else:
                    return subgraph
            else:
                raise AttributeError(
                    "Parent graph %s has no subgraphs." % (parent_name,)
                )
        else:
            raise AttributeError(
                "Invalid parent graph name %s "
                "for subgraph %s." % (parent_name, graph_name)
            )
    ...
    def envCheckFlag(self, name, default=False):
        ...
        if name in self._flags:
            ...
            if val is None:
                ...
                raise AttributeError(
                    "Value for flag %s, must be yes, no, on or off" % name
                )
    ...
    def appendSubgraph(self, parent_name, graph_name, graph):
        ...
        if parent_name in self._graphDict:
            ...
            raise AttributeError(
                "Invalid parent graph name %s used for subgraph %s."
    ...
    def setGraphVal(self, graph_name, field_name, val):
        ...
        if graph.hasField(field_name):
            ...
            raise AttributeError(
                "Invalid field name %s for graph %s." % (field_name, graph_name)
            )
    ...
    def setSubgraphVal(self, parent_name, graph_name, field_name, val):
        ...
        if subgraph.hasField(field_name):
            ...
            raise AttributeError(
                "Invalid field name %s for subgraph %s "
                "of parent graph %s." % (field_name, graph_name, parent_name)
            )
    ...
    def getSubgraphList(self, parent_name):
        ...
        if parent_name in self._graphDict:
            ...
            raise AttributeError("Invalid parent graph name %s." % (parent_name,))
    ...
    def config(self):
        ...
        if (
            self.isMultigraph
            and self._nestedGraphs
            and self._subgraphDict
            and self._subgraphNames
        ):
...
class MuninGraph:
    ...
    def __init__(
        ...
        """
        self._graphAttrDict = dict(
            (k, v)
            for (k, v) in locals().items()
            if (v is not None and k not in ("self", "autoFixNames"))
        )
        self._fieldNameList = []
        self._fieldAttrDict = {}
        self._fieldValDict = {}
    ...
    def getConfig(self):
        ...
        """
        return {
            "graph": self._graphAttrDict,
            "fields": [
                (field_name, self._fieldAttrDict.get(field_name))
    ...
    def _fixName(self, name):
        ...
        """
        return re.sub("[^A-Za-z0-9_]", "_", re.sub("^[^A-Za-z_]", "_", name))
