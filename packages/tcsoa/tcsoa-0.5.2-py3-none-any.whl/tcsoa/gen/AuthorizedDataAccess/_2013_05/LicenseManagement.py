...
class GetLicenseDetailsResponse(TcBaseObj):
    """
    LicenseDetailsResponse structure represents all the details of an ADA_License business object in licDetails and
    operation status in serviceData.
    
    :var licDetails: Structure containing the properties of the given ADA_License business objects.
...
class LicenseDetails(TcBaseObj):
    ...
    :var licenseType: The type of ADA License object. The type can be ITAR_License, Exclude_License, or IP_License. The
    license type of existing licenses cannot be modified. An error will be thrown if the value of licenseType does not
    match the type of the license being modified.
    ...
    string ("").
    :var userCitizenships: The value for user citizenships property of an ADA license. This is an optional parameter.
    The value specified for userCitizenships parameter is a two-letter country code. If an invalid country code is
    entered, partial error code 10219 will be returned.
