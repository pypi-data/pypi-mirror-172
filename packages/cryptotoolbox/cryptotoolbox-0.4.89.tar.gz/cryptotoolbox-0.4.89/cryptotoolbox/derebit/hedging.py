...
def hedge_global_lp_position(maturity_df, get_lp_value_gain=None, lp_lower_bound=None, lp_upper_bound=None, plot_html=True):
    try:
        ...
        def to_optimize_l1_distance(x, longs_val=None, shorts_val=None,lp_lower_bound=None, lp_upper_bound=None):
            l1_norm = 0
            for final_price in [lp_lower_bound + (i / 100.) * (lp_upper_bound - lp_lower_bound) for i in range(0, 100, 10)]:
                lp_value = get_lp_value_gain(final_price)
                print(f'final price {final_price}')
        ...
        gogo = lambda x: to_optimize_l1_distance(x, longs_val=longs_val, shorts_val=shorts_val, lp_lower_bound=lp_lower_bound, lp_upper_bound=lp_upper_bound)
        low_bound = 0.
        up_bound = +np.infty
        ...
        if plot_html:
            ...
            value_df = compute_optional_profile(longs_i, shorts_i, longs_val=longs_val, shorts_val=shorts_val)
