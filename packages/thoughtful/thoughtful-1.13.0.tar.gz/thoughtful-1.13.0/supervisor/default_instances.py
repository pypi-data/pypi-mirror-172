...
from supervisor.environment_variables import EnvironmentVariables
...
if env_vars.callback_url and env_vars.job_id:
    ...
    streaming_callback = StreamingCallback.from_env_defaults(
        auth=aws_auth, job_id=env_vars.job_id, callback_url=env_vars.callback_url
    )
else:
    logger.warning(f"Missing ENV variable for callback url or job id")
