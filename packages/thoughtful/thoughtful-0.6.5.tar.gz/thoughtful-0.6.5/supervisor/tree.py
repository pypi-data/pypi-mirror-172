...
from supervisor.nodes import Node, NodeBuilder


class NaryTree(DeprecatedBecauseNonDynamic):
    ...
    def preorder_step_ids(self, ans: list = None) -> t.List[int]:
        """
    ...
    def search_narytree(self, node_uuid: StepId) -> NaryTree:
        """
        Method returns a NaryTree given a StepId
        """
        current = self.root
        if current.uuid == node_uuid:
            return self
        ...
        for n in current.children:
            NaryTree_node = n.search_narytree(node_uuid)
            if NaryTree_node:
                return NaryTree_node
