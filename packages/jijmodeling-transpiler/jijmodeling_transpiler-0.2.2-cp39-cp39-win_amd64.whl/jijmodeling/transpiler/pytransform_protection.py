...
def protect_pytransform():
    ...
    def check_obfuscated_script():
        ...
        CO_NAMES = set(['pytransform_vax_001333', 'pyarmor_runtime',
                        '__pyarmor_vax_001333__', '__name__', '__file__'])
    ...
    def check_lib_pytransform():
        ...
        if platname.startswith('darwin'):
            return
        libname = '_pytransform_vax_001333.dylib' if platname.startswith('darwin') else \
                  '_pytransform_vax_001333.dll' if platname.startswith('win') else \
                  '_pytransform_vax_001333.dll' if platname.startswith('cygwin') else \
                  '_pytransform_vax_001333.so'
        ...
        if getattr(pytransform.sys, 'frozen', False) and sys.platform == 'darwin':
            if '_vax_001333':
                value += 886 - sum(b'_vax_001333') + 299
            else:
                value += 1217

        if value not in [120427397]:
