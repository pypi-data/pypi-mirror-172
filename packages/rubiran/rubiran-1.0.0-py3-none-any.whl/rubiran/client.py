...
class rubiran:
	...
	def registerDevice(self):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
	        except:continue
	...
	def getChatAds(self):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get('chat_ads')
	        except:continue
	def getMessagesInterval(self):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("messages")
	        except:continue
	...
	def mrgohst(self):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran.socket(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
	        except:continue
	...
	def getGroupOnlineMember(self, chat_id):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get('data').get('online_count')
	        except:continue
	def getCommonGroups(self, chat_id):
	    inData = {"method":"getCommonGroups","input":{"user_guid": chat_id},"client":SetClines.android}
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get('data').get('abs_groups')
	        except:continue
	...
	def updateProfile_rubino(self, name=None, bio=None, email=None):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._rubino(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
	        except:continue
	...
	def getContacts(self, user_guid):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("users")
	        except:continue
	...
	def getInfoByUsername(self, username):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("user")
				break
			except: continue
	...
	def getGroupLink(self, chat_id):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("join_link")
				break
			except: continue
	...
	def getLinkFromAppUrl(self, app_url):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("link").get("open_chat_data")
	            break
	        except:continue
	...
	def serch(self,object_guid, search_text):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("message_ids")[:5]
	            break
	        except:continue
	...
	def channelPreviewByJoinLink(self, link):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get('data').get('channel')
	        except:continue
	...
	def getChatsUpdate(self):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("chats")
				break
			except: continue
	...
	def see_GH_whith_Linkes(self,link_gh):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data")
				break
			except: continue
	...
	def _requestSendFile(self, file):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._SendFile(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data")
				break
			except: continue
	...
	def uploadAvatar(self,myguid,main,thumbnail=None):
		mainID = str(rubiran._uploadFile(self, main)[0]["id"])
		thumbnailID = str(rubiran._uploadFile(self, thumbnail or main)[0]["id"])
		inData = {
			"method":"uploadAvatar",
			"input":{
				"object_guid":myguid,
	...
	def getAvatar(self, myguid):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("avatars")
	        except:continue
	...
	def sendDocument(self, chat_id, file, caption=None, message_id=None):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(rubiran._SendFile(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
				break
			except: continue
	...
	def sendPhoto(self, chat_id, file, size=[], thumbnail=None, caption=None, message_id=None):
		...
		elif "." in thumbnail:thumbnail = str(rubiran._getThumbInline(open(file,"rb").read() if not "http" in file else get(file).content))

		if size == []: size = rubiran._getImageSize(open(file,"rb").read() if not "http" in file else get(file).content)
