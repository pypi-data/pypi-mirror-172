...
def do_ml_gpr(i_cube, noise_cube, ml_gpr_config, rnd_seed=1):
    from ps_eor import ml_gpr

    # Saving PS stacker results for the residual, the noise
...
def run_ml_gpr(file_i, file_v, ml_gpr_config_file, flag_config, eor_bins_list, ps_conf,
    ...
    if file_noise is not None:
        ...
        if ml_gpr_config.kern.noise.estimate_baseline_noise_from_stokes == 'V':
            noise_cube_df = v_cube.make_diff_cube()
        elif ml_gpr_config.kern.noise.estimate_baseline_noise_from_stokes == 'I':
            noise_cube_df = i_cube.make_diff_cube()
        else:
            print('Error: estimate_baseline_noise_from_stokes must be I or V')
            return

        w = i_cube.weights.copy()
        w.scale_with_noise_cube(noise_cube_df, ps_conf.empirical_weighting_polyfit_deg)
        sefd = v_cube.make_diff_cube().estimate_sefd()
        print(f'Using simulated noise with SEFD={sefd:.1f} Jy')
        noise_cube = w.simulate_noise(sefd, i_cube.meta.total_time, fake_apply_win_fct=True)
    ...
    for eor_name in ps_builder.eor_bin_list.get_all_names():
        ...
        ml_gpr_res = do_ml_gpr(i_cube_slice, n_cube_slice, ml_gpr_config, rnd_seed=rnd_seed)
        ...
        ps_eor.get_ps2d().save_to_txt(os.path.join(out_dir, 'ps2d_eor.mc.h5'))
        ps_fg.get_ps2d().save_to_txt(os.path.join(out_dir, 'ps2d_fg.mc.h5'))

        ps3d_noise = ps_gen.get_ps3d(kbins, scaled_noise_cube)
        ps3d_res = ps_residual.get_ps3d()
        ps3d_rec = (ps3d_res - ps3d_noise)

        ps3d_res.save_to_txt(os.path.join(out_dir, 'ps3d_I_residual.mc.h5'))
        ps3d_rec.save(os.path.join(out_dir, 'ps3d_I_minus_noise.mc.h5'))
        ps3d_noise.save_to_txt(os.path.join(out_dir, 'ps3d_I_noise.mc.h5'))
...
def validate_dec_deg(ctx, param, value):
    if value is None:
        return 90
    if isfloat(value):
        return float(value)
        ...
        return value
    click.BadParameter("dec_deg must be a float or 'drift'")


@main.command('simu_uv')
@click.argument('instru', type=click.Choice(['ska_low', 'nenufar', 'nenufar_80', 'nenufar_52', 'lofar_hba', 'a12_hba', 'a12_lba', 'mwa1'], case_sensitive=False))
...
@click.option('--dec_deg', help='Target declination, in degrees (or "drift" for drift scan mode)', default=None, 
              callback=validate_dec_deg, show_default=True)
...
def simu_uv(instru, z, out_filename, umin, umax, dec_deg, fov, bandwidth, df, total_time, int_time, 
    ...
    uu_meter, vv_meter, _ = telescop_simu.simu_uv()

    if res is not None:
        osamp_factor = int(np.round((1 / telescop_simu.umax) / np.radians(6 / 60.)))
    else:
        osamp_factor = 4

    uv_simu = telescop_simu.simu_gridded_uv(uu_meter, vv_meter, fov, osamp_factor, min_weight=0)
...
def simu_noise_img(uv_simu_h5, total_time_hour, out_filename, min_weight):
    ...
    uv_simu = obssimu.TelescopGriddedUV.load(uv_simu_h5)
    freqs = uv_simu.weights.freqs

    print(uv_simu.weights.ru.min(), uv_simu.weights.ru.max(), np.diff(uv_simu.weights.uu)[0])
...
def simu_noise_ps_zrange(uv_simu_h5, total_time_hour, k, out_filename, bandwidth, delta_z, dk_over_k,
                         filter_kpar_min, filter_wedge_theta, min_weight, smooth):
    ...
    uv_simu = obssimu.TelescopGriddedUV.load(uv_simu_h5)
    z_max = np.round(psutil.freq_to_z(uv_simu.weights.freqs.min() + bandwidth / 2 * 1e6), 1)
    z_min = np.round(psutil.freq_to_z(uv_simu.weights.freqs.max() - bandwidth / 2 * 1e6), 1)
    ...
    for z in zs:
        ...
        uv_simu_z = uv_simu.get_slice(mfreq - bandwidth / 2 * 1e6, mfreq + bandwidth / 2 * 1e6)
    ...
    header = f'Spherically averaged power-spectra {uv_simu.name} {total_time_hour}h\n'
    ...
    name = f'{out_filename}_ps_{uv_simu.name}_z{z_min:.1f}-{z_max:.1f}_{total_time_hour}h_k{k}'
