...
class CovarianceGenerator(object):
    ...
    @lru_cache(maxsize=10)
    def __init__(self, freqs, uv_bins, log_scale_interpolatation=True):
        ...
        k_par_max = np.pi  / d_delta_r
        self.k_par = np.arange(0, k_par_max, k_par_max / len(r))
        ...
        def ps_fct(k):
            if self.log_scale_interpolatation:
                return 1 / k ** 3 * 10 ** scipy.interpolate.interp1d(np.log10(k_mean), np.log10(ps3d), bounds_error=False,
                                                                     kind='quadratic', fill_value='extrapolate')(np.log10(k))
            else:
                return 1 / k ** 3 * scipy.interpolate.interp1d(k_mean, ps3d, bounds_error=False, kind='quadratic',
                                                               fill_value='extrapolate')(k)

        return np.clip(self.c_nu_from_ps21_fct(ps_fct, normalize=normalize), 1e-20, 1e20)
...
class MultiProd(MultiKern, GPy.kern.Prod):
    ...
    def __init__(self, sub_kerns, name='prod'):
        for kern in sub_kerns:
            assert isinstance(kern, MultiKern)
        GPy.kern.Prod.__init__(self, sub_kerns)
        MultiKern.__init__(self)
        self.set_uv_bins(sub_kerns[0].uv_bins)
        self.set_mean_fmhz(sub_kerns[0].mean_fmhz)
...
class MultiStationaryKern(MultiKern):
    ...
    def __init__(self, kern_class, variance=1, lengthscale=1, ls_alpha=0, var_alpha=0, theta_rad=0.1, name='mkern',
                 wedge_parametrization=False, ls_is_period=False):
        MultiKern.__init__(self)
    ...
    def load_from_dict(cls, name, d):
        ...
        if 'wedge_parametrization' in d:
            wedge_parametrization = d['wedge_parametrization']
        kern = cls(name=name, wedge_parametrization=wedge_parametrization)
        return MultiKern._parse_set_params_from_dict(kern, d)
    ...
    def parameters_changed(self):
        ...
        for i, u_mean in enumerate(u_means):
            if self.wedge_parametrization:
                delays = np.sin(self.theta_rad) * u_mean / (self.mean_fmhz)
            else:
                delays = (1 + self.ls_alpha * 1e-3 * l_m * (u_mean - u_min)) / l_m
            ll = 1 / delays
...
class MCMCSampler(AbstractSampler):
    ...
    def __init__(self, gp, n_walkers, emcee_moves='stretch', debug=False):
        ...
        sampler = emcee.EnsembleSampler(self.n_walkers, self.ndim, self.lnprob, moves=moves)
        AbstractSampler.__init__(self, gp, sampler)
...
class NestedSampler(AbstractSampler):
    ...
    def __init__(self, gp, nlive=500, bound='multi', sample='auto', **kargs_dynesty):
        sampler = dynesty.NestedSampler(self.log_marginal_likelihood, self.prior_transform, self.ndim, nlive=nlive,
...
class UltraNestNestedSampler(AbstractSampler):
    ...
    def __init__(self, gp, nlive=500, bound='multi', **kargs_ultranest):
        ...
        sampler = ultranest.ReactiveNestedSampler(self.get_parameter_names(), self.log_marginal_likelihood,
    ...
    def run(self, live_update=False, **kargs_ultranest):
        ...
        return SamplerResult(self.gp, NestedSamples(d))
...
class MLGPRConfigFile(settings.BaseSettings):
    ...
    def _load_kerns_from_dict(self, k_names, label_prefix):
        ...
        kern_operations = []
        for name in k_names:
            # every odd element in the list should be the operation + or x, if omitted, we assume '+'
            if len(kern_operations) < len(kerns):
                if name.strip() in ['+', 'x']:
                    kern_operations.append(name.strip())
                    continue
                else:
                    kern_operations.append('+')
            if not name in self:
        ...
        final_kern = kerns[0]
        for kern, operator in zip(kerns[1:], kern_operations):
            if operator == '+':
                final_kern = final_kern.add(kern)
            elif operator == 'x':
                final_kern = final_kern.prod(kern)
            else:
                print(f'Error: operator {operator} unknown')
...
class MLGPRResult(object):
    ...
    def get_ps(self, ps_gen, kbins, kern_name, n_pick=50, subtract_from=None):
        return self.sampler_result.get_ps_stack(ps_gen, kbins, self.config.mcmc.n_burn, kern_name=kern_name,
