...
from nornir_maze.configuration_management.cli import write_commands_to_file
...
def pyats_create_testbed(nr_obj, pyats_testbed_file):
    """
    ...
    try:
        ...
        for host in nr_obj.inventory.hosts:
            ...
            host_obj = nr_obj.inventory.hosts[host]
            ...
            pyats_testbed["devices"][str(host_obj)].update(
                {
                    ...
                    "credentials": {
                        "default": {
                            "username": nr_obj.inventory.defaults.username,
                            "password": nr_obj.inventory.defaults.password,
                        }
                    },
                    "connections": {
                        "cli": {
...
def pyats_update_golden_config(pyats_testbed_file):
    """
...
def pyats_artifacts_cleanup(artifacts_list):
    """
...
def pyats_update_golden_cleanup(nr_obj, config_status, task_text, pyats, verbose=False):
    """
    This function is used to verify if a new golden config can be created based on the config_status boolian.
    ...
    if config_status:
        ...
        if pyats["generate_testbed"]:
            ...
            pyats_create_testbed(nr_obj=nr_obj, pyats_testbed_file=pyats["testbed"])
        ...
        rc_cisco_rpc_is_syncing(nr_obj=nr_obj, verbose=verbose)

        rc_cisco_rpc_copy_file(
            nr_obj=nr_obj,
            task_name="RESTCONF save golden config",
        ...
        error_msg = (
            "\n\033[1m\u001b[31m"
            "-> Analyse the Nornir output for failed config tasks\n"
            "-> May apply Nornir inventory changes and run the script again\n\n"
            "The golden config has not been updated yet!"
            "\033[0m\n"
        )
