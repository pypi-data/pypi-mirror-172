...
def rc_cisco_operation_rpc_task(task, rpc, payload):
    """
...
def rc_cisco_rpc_is_syncing_task(task):
    """
...
def rc_software_install_one_shot_task(task, host_dict):
    """
    This custom Nornir task takes the result of the function host_dict as an argument and executes
    the Cisco specific operations RESTCONF RPC Cisco-IOS-XE-install-rpc:install to install a software file in
    a one-shot approach which will install, commit and reload the switch. The custom Nornir result from the
    task rc_cisco_operation_rpc_task is returned for futher processing.
    """

    # Get the host destination file from host_dict
    dest_file = host_dict[str(task.host)].result["dest_file"]

    # Run the Nornir Task rc_cisco_operation_rpc_wrapper_task Cisco-IOS-XE-rpc:copy
    result = task.run(
...
def rc_install_remove_inactive_task(task):
    """
    ...
    # Run the Nornir Task rc_cisco_operation_rpc_wrapper_task Cisco-IOS-XE-rpc:remove
    result = task.run(
        task=rc_cisco_operation_rpc_task,
...
def print_rc_cisco_rpc_result(task_text, task_result, add_result=False, verbose=False):
    ...
    for host in task_result:
        msg_changed = task_result[host].changed
        ...
        if (
            ...
            print(task_info(text=task_text, changed=msg_changed))
        ...
        # This happen when commands change between software releases
            ...
            print(task_error(text=task_text, changed=msg_changed))
            print(f"'{task_text}' -> RestconfResponse {response} in {elapsed}s")
            # If add_result is True -> Print every list item
            if add_result:
                for item in add_result:
...
def rc_software_install_one_shot(nr_obj, host_dict, verbose=False):
    """
    ...
    task = nr_obj.run(task=rc_software_install_one_shot_task, host_dict=host_dict, on_failed=True)
