...
class Tweet(umbraSnscrpe.base.Item):
	url: str
	...
	username = umbraSnscrpe.base._DeprecatedProperty('username', lambda self: self.user.username, 'user.username')
	outlinksss = umbraSnscrpe.base._DeprecatedProperty('outlinksss', lambda self: ' '.join(self.outlinks) if self.outlinks else '', 'outlinks')
	tcooutlinksss = umbraSnscrpe.base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(self.tcooutlinks) if self.tcooutlinks else '', 'tcooutlinks')
...
@dataclasses.dataclass
class User(umbraSnscrpe.base.Entity):
...
@dataclasses.dataclass
class Trend(umbraSnscrpe.base.Item):
	name: str
	domainContext: str
...
class _CLIGuestTokenManager(GuestTokenManager):
	def __init__(self):
		...
		dir = os.path.join(cacheHome, 'umbraSnscrpe')
...
class _TwitterAPIScraper(umbraSnscrpe.base.Scraper):
		...
		if self._guestTokenManager.token is None:
			...
			if not self._guestTokenManager.token:
				...
				if not o.get('guest_token'):
					raise umbraSnscrpe.base.ScraperError('Unable to retrieve guest token')
	...
	def _get_api_data(self, endpoint, params):
		...
		try:
			...
			raise umbraSnscrpe.base.ScraperException('Received invalid JSON from Twitter') from e
		return obj
	...
	def _instructions_to_users(self, obj, includeConversationThreads = False):
		# No data format test, just a hard and loud crash if anything's wrong :-)
		for instruction in obj['globalObjects']['users']:
			yield self._user_to_user(obj['globalObjects']['users'][instruction])
	...
	def _instruction_tweet_entry_to_tweet(self, entryId, entry, obj):
		if 'tweet' in entry['item']['content']:
			...
			raise umbraSnscrpe.base.ScraperException(f'Unable to handle entry {entryId!r}')
...
class TwitterSearchScraper(_TwitterAPIScraper):
	...
	def cli_setup_parser(cls, subparser):
		...
		subparser.add_argument('query', type = umbraSnscrpe.base.nonempty_string('query'), help = 'A Twitter search string')
...
class TwitterSearchScraperPeopple(_TwitterAPIScraper):
	name = 'twitter-search-people'

	def __init__(self, query, cursor = None, top = False, **kwargs):
		if not query.strip():
			raise ValueError('empty query')
		super().__init__(baseUrl = 'https://twitter.com/search?' + urllib.parse.urlencode({'f': 'user', 'lang': 'en', 'q': query, 'src': 'spelling_expansion_revert_click'}), **kwargs)
		self._query = query  # Note: may get replaced by subclasses when using user ID resolution
		self._cursor = cursor
		self._top = top
	...
	def get_items(self):
		...
		for obj in self._iter_api_data('https://api.twitter.com/2/search/adaptive.json', params, paginationParams, cursor = self._cursor):
			yield from self._instructions_to_users(obj)
...
class TwitterHashtagScraper(TwitterSearchScraper):
	...
	@classmethod
	def cli_setup_parser(cls, subparser):
		subparser.add_argument('hashtag', type = umbraSnscrpe.base.nonempty_string('hashtag'), help = 'A Twitter hashtag (without #)')
...
class TwitterListPostsScraper(TwitterSearchScraper):
	...
	@classmethod
	def cli_setup_parser(cls, subparser):
		subparser.add_argument('list', type = umbraSnscrpe.base.nonempty_string('list'), help = 'A Twitter list ID or a string of the form "username/listname" (replace spaces with dashes)')
