...
def is_compatible_type(type: Type) -> bool:
    """Returns `True` if the type is wefaas-compatible."""
    try:
...
def get_input_type(func: Callable) -> Type:
    ...
    if "input" not in type_hints:
        raise ValueError(
            ...
            "For example: `def my_wefaas(input: InputModel) -> OutputModel:`."
        )
...
def get_callable(import_string: str) -> Callable:
    ...
    if callable_seperator not in import_string:
        raise ValueError("The callable path MUST specify the function. Import string: {}".format(import_string))
