...
def cache_symbol_id(username , password , td_obj):
    ...
    segments = ['EQ', 'FO', 'MCX', 'CDS', 'IN']
    cache_dir = '/'.join(__file__.split('\\')[:-1]) + '/cache'
    if not os.path.exists(cache_dir) or not os.path.exists(f'{cache_dir}/sym_cache_{dt.now().strftime("%d%m%y")}.pkl'):
        ...
        get_symbol_id(segments , url , username , password , cache_dir )
    with open( f"{cache_dir}/sym_cache_{dt.now().strftime('%d%m%y')}.pkl" , 'rb') as pkl:
