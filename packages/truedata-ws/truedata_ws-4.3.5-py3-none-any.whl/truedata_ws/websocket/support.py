...
class full_feed_bidask:
    ...
    def __post_init__(self):
        ...
        self.timestamp = datetime.strptime(self.raw_tick[1], '%Y-%m-%dT%H:%M:%S')
        #scientific notation hanlde remove int converstion 
        self.best_bid_price = float(self.raw_tick[2])
        self.best_bid_qty   =  int(float(self.raw_tick[3]))
        self.best_ask_price = float(self.raw_tick[4])
        self.best_ask_qty   = int(float(self.raw_tick[5]))
