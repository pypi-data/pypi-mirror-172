...
from .support import full_feed , full_feed_bidask , bar_feed
...
class LiveClient(WebSocketApp):
    ...
    def __init__(self, parent_app, url, *args):
        ...
        self.full_feed_bidask_callback = None
    ...
    def on_msg_func(self, *args):
        ...
        if 'trade' in msg_keys and self.parent_app.full_feed :
            feed_tick = full_feed( msg['trade'] , self.parent_app.symbol_id_map_dict[msg['trade'][0]] )
            self.full_feed_trade_callback( feed_tick ) if self.full_feed_trade_callback else 0   
        if 'bidask' in msg_keys and self.parent_app.full_feed :
            bid_ask = full_feed_bidask( msg['bidask'] , self.parent_app.symbol_id_map_dict[msg['bidask'][0]])
            self.full_feed_bidask_callback( bid_ask ) if self.full_feed_bidask_callback else 0  
        if 'interval' in msg_keys and self.parent_app.full_feed :
            ...
            for bar in list(bar_data):
                self.full_feed_bar_callback ( bar ) if self.full_feed_bar_callback else 0

        if not self.parent_app.full_feed:
            if 'trade' in msg_keys:
                self.handle_trade_data( msg['trade'] )
            elif 'bidask' in msg_keys:
                self.handle_bid_ask_data( msg['bidask'] )
        ...
        try:
            ...
            for ticker_id in self.parent_app.symbol_mkt_id_map[symbol]:
                ...
                if self.trade_callback:
                    try:
                        self.trade_callback(ticker_id, self.parent_app.live_data[ticker_id])
                    except Exception as e:
    ...
    def handle_bid_ask_data(self, bidask_tick):
        try:
            ...
            for ticker_id in self.parent_app.symbol_mkt_id_map[symbol]:
                ...
                if self.bidask_callback:
                    try:
                        self.bidask_callback(ticker_id, self.parent_app.live_data[ticker_id])
                    except Exception as e:
                        self.logger.error(f'{Style.BRIGHT}{Fore.RED}Encountered error with bidask_callback - {type(e)} - {e}{Style.RESET_ALL}')
        except KeyError:
            pass
        except Exception as e:
            self.logger.error(f'{Style.BRIGHT}{Fore.RED}Bid-ask feed error encountered ({type(e)})- {e}{Style.RESET_ALL}')
