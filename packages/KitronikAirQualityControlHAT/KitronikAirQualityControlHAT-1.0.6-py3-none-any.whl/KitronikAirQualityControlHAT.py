...
class KitronikSerial:
    ...
    def __init__(self):
        air_board_path = "/dev/serial0"
        self.ser = serial.Serial(air_board_path, baudrate=115200,
...
class KitronikADC:
    ...
    def read(self):
        req = [self.adcNumber, ord('\n')]
        request = bytearray(req)
        self.ser.write(request)
        response = str(self.ser.readline())
        if len(response) < 4: return 0
        return int(response[2:len(response) - 3])
...
class KitronikRTC:
    ...
    def set(self, year, month, day, dayOfWeek, hour, minute, second):
        req = [6, (year >> 8) & 0xff, year & 0xff, month, day, dayOfWeek, hour, minute, second, ord('\n')]
        request = bytearray(req)
        self.ser.write(request)
...
class KitronikZIPLEDs:
    def __init__(self, brightness = 25, autoShow = True):
        s = KitronikSerial()
        self.ser = s.getSerial()
