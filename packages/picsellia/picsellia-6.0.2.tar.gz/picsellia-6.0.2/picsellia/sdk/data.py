...
class Data(Dao, Taggable):
    ...
    @exception_handler
    @beartype
    def get_datasource(self) -> Union[DataSource, None]:
        r = self.sync()
        if "data_source" not in r or r["data_source"] is None:
            raise exceptions.PicselliaError("This data has no datasource")
...
class MultiData(MultiObject[Data], Taggable):
    ...
    def __str__(self):
        return f"{Colors.GREEN}MultiData{Colors.ENDC} object, size: {len(self)}"
