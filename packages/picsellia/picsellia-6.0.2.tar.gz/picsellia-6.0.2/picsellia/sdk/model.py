...
class Model(Dao, Taggable):
    ...
    def __str__(self):
        return f"{Colors.BLUE}Model '{self.name}' with type {self.type.name} and framework {self.framework.name} {Colors.ENDC} (id: {self.id})"
    ...
    def update(
        ...
        assert (
            type is None
            or type == InferenceType.CLASSIFICATION
            or type == InferenceType.OBJECT_DETECTION
            or type == InferenceType.SEGMENTATION
        ), "Model can only be a Classification, Detection or Segmentation"
        payload = {
            "name": name,
            "type": type,
            "framework": framework,
