...
from picsellia.exceptions import NoConnectorFound, NoDataError, PicselliaError
...
from picsellia.utils import convert_tag_list_to_query_language, filter_payload
...
class Datalake(Dao):
    ...
    def upload_data(
        ...
        def upload_filepath(path: str):
            try:
                ...
                logger.error(f"\nCould not upload {path} because {str(e)}")
                return None
        ...
        if len(data_list) != len(filepaths):
            logger.error(
                f"❌ {len(filepaths) - len(data_list)} data not uploaded. Check readability / check paths"
            )

        if len(data_list) == 0:
            raise PicselliaError("Nothing has been uploaded.")
        elif len(data_list) == 1:
            logger.info(f"✅ {data_list[0].filename} data uploaded in {self}")
            return data_list[0]
        else:
            logger.info(f"✅ {len(data_list)} data uploaded in {self}")
            return MultiData(self.connexion, self.id, data_list)

    @exception_handler
    @beartype
    def find_data(self, filename: str = None, object_name: str = None) -> Data:
        ...
        """
        assert not (
            filename is None and object_name is None
    ...
    def list_data(
        ...
        tags: Optional[Union[str, Tag, List[Union[str, Tag]]]] = None,
        ...
        """
        q = convert_tag_list_to_query_language(tags)

        datas = mlt.do_paginate(
            limit,
            offset,
            page_size,
            partial(self._do_list_data, q, order_by, filenames),
        )

        if len(datas) == 0:
            raise NoDataError("No data found in this datalake")
    ...
    def _do_list_data(
        ...
        r = self.connexion.get(f"/sdk/datalake/{self.id}/datas", params=params).json()
    ...
    def find_all_datas(self, object_names: List[str]) -> MultiData:
        ...
        r = self.connexion.xget(
            f"/sdk/datalake/{self.id}/datas", data=orjson.dumps(payload)
        ).json()
        datas = list(map(partial(Data, self.connexion, self.id), r["items"]))
