...
class Connexion(TokenServiceConnexion):
    ...
    @beartype
    def generate_data_object_name(self, filename: str, connector_id: UUID = None):
        return self._generate_object_name(filename, ObjectDataType.DATA, connector_id)
    ...
    @exception_handler
    @beartype
    def _init_upload(self, path: str) -> Tuple[bool, str]:
        if not os.path.isfile(path):
            raise exceptions.FileNotFoundException(f"{path} not found")

        is_large = Path(path).stat().st_size > 5 * 1024 * 1024
    ...
    @retry((KeyError, ValueError, exceptions.PicselliaError), total_tries=3)
    @exception_handler
    @beartype
    def _do_upload_file(
        self, object_name: str, path: str, presigned_url_data: dict
    ) -> requests.Response:
        if not os.path.isfile(path):
            raise exceptions.FileNotFoundException(f"{path} not found")
        ...
        with open(path, "rb") as file:
            try:
                ...
                raise exceptions.NetworkError("Impossible to upload file")

        if response.status_code != 204:
            logging.error(
                f"Could not upload {path} into S3. Status code: {response.status_code}"
            )
            raise exceptions.PicselliaError(f"Could not upload {path} into S3")
    ...
    @beartype
    def upload_file(
        self, object_name: str, path: str, connector_id: UUID = None
    ...
    @beartype
    def _upload_parts(
        self, object_name: str, upload_id: str, path: str, connector_id: UUID
    ) -> list:
    ...
    def _do_upload_one_part(
        ...
        try:
            ...
            raise exceptions.PicselliaError("Impossible to get an url for part")
    ...
    def _upload_large_file(
        self, object_name: str, path: str, connector_id: UUID
    ) -> requests.Response:
    ...
    def do_download_file(
        self, path: str, presigned_url: str, is_large: bool, force_replace: bool
    ) -> bool:
        """Retrieve a presigned url of this object name in order to download it"""
        ...
        if response.status_code != 200:
            logger.debug(
                f"A get on {presigned_url} returned {response.status_code} while a 200 was expected."
            )
            return False

        total_length = response.headers.get("content-length")
        if total_length is None:
            logger.debug(f"A get on {presigned_url} returned no content.")
            return False
    ...
    def download_file(
        ...
        return self.do_download_file(path, presigned_url, is_large, force_replace)
