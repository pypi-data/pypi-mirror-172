...
def compile_if_needed_and_load(
    ...
    if not os.path.exists(gsl_root) or not os.listdir(gsl_root):
        download_folder = os.path.join(cache_folder, "gsl")
    ...
    libf2c_path = compile_libf2c(cache_folder)
...
def compile_libf2c(cache_folder):
    folder = os.path.join(cache_folder, "libf2c", get_platform())
...
class LoadedModule:
    ...
    def _setup_functions(self):
        for name in self._module._api:
            _fun = setup_function(name, self._module)
            setattr(self._module, name, _fun)

        for name, f in self._module.__dict__.items():
            if name.startswith("solve_fast_"):
                setattr(self._module, name, functools.partial(f, self))
...
def setup_function(name, module):
    ...
    arg_decl = ", ".join(args)
    numba_decorator = (
        f"@numba.njit([{numba_signature}])"
        if not vector_flags or any(vector_flags)
        else f"@numba.vectorize([{numba_signature}])"
    )
    code = align(
        f"""
    |{numba_decorator}
