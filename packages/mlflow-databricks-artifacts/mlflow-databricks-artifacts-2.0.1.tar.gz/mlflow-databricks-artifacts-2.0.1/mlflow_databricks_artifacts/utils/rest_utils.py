...
from mlflow_databricks_artifacts.exceptions import get_error_code
from mlflow.exceptions import MlflowException, RestException

from mlflow_databricks_artifacts.environment_variables import (
...
def http_request(
    ...
    **kwargs,
    ...
    from mlflow_databricks_artifacts.tracking.request_header.registry import resolve_request_headers
...
def verify_rest_response(response, endpoint):
    ...
    # response
    if endpoint.startswith(_REST_API_PATH_PREFIX) and not _can_parse_as_json_object(
        response.text
    ):
        base_msg = (
...
def extract_all_api_info_for_service(service, path_prefix):
    ...
    for service_method in service_methods:
        endpoints = service_method.GetOptions().Extensions[databricks_pb2.rpc].endpoints
        res[service().GetRequestClass(service_method)] = [
            (_get_path(path_prefix, endpoint.path), endpoint.method)
    ...
    for i, (endpoint, method) in enumerate(endpoints):
        try:
            return call_endpoint(
                host_creds, endpoint, method, json_body, response_proto
            )
        except RestException as e:
            if (
                e.error_code != ErrorCode.Name(ENDPOINT_NOT_FOUND)
                or i == len(endpoints) - 1
            ):
                raise e
