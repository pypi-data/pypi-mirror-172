...
def parse_tf_serving_input(inp_dict, schema=None):
            ...
            if schema.has_input_names():
                ...
                if not isinstance(input_data, list):
                    raise MlflowException(
                        ...
                        " however, an input of type {0} was found.".format(
                            type(input_data)
                        )
                    )
    ...
    if not (
        list(inp_dict.keys()) == ["instances"] or list(inp_dict.keys()) == ["inputs"]
    ):
        raise MlflowException(
...
class _DateTimeEncoder(json.JSONEncoder):
    def default(self, o):
        ...
        if isinstance(
            o, (datetime.datetime, datetime.date, datetime.time, pd.Timestamp)
        ):
            return o.isoformat()
