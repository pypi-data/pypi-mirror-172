...
def get_server_cert(response):
    ...
    if isinstance(raw_response, HTTPResponse):
        ...
        try:
            ...
            logger.info("unable to get server certificate")
        else:
...
def get_certificate_hash_bytes(certificate_der):
    ...
    logger.info("peer/server cert hash: %s", binascii.hexlify(certificate_hash_bytes))
...
def get_cbt_data(response):
    ...
    if not cert_hash_bytes:
        logger.info("server cert not found, channel binding tokens (CBT) wont be used")
        return None
    ...
    logger.info("cbt data: %s", cbt_data.get_data())
    return cbt_data
