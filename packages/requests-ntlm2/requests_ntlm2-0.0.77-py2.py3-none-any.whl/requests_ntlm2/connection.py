...
class VerifiedHTTPSConnection(_VerifiedHTTPSConnection):
    ...
    def _tunnel(self, retry=False):
        username, password, domain = self._ntlm_credentials
        logger.info("* attempting to open tunnel using HTTP CONNECT; retry=%s", retry)
        ...
        if code == PROXY_AUTHENTICATION_REQUIRED:
            ...
            if authenticate_hdr is None and not retry:
                logger.warning(
                    '* the NTLM challenge was not found! retrying to establish the tunnel'
                )
                return self._tunnel(retry=True)
