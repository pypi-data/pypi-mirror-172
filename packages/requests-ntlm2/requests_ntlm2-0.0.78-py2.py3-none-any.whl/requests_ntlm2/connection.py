...
class VerifiedHTTPSConnection(_VerifiedHTTPSConnection):
    ...
    def _flush_response_buffer(response):
        ...
        if ready:
            ...
            logger.info('* successfully flushed the response buffer: data=%r', data)
    ...
    def _tunnel(self):
        ...
        logger.info('* library version=6.4.4')
        ...
        logger.info('* sending the NEGOTIATE message')
        ...
        if code == PROXY_AUTHENTICATION_REQUIRED:
            ...
            while True:
                line_count += 1
                logger.info('* reading line number %s', line_count)
                if last_line_is_blank:
                    line = self._read_response_line_if_ready(response)
                else:
                    line = response.fp.readline()
                logger.info('* read line: %r', line)
                ...
                if last_line_is_blank and this_line_is_blank:
                    ...
                    logger.info('* breaking from the loop because both lines are blank')
                    break
            ...
            if not authenticate_hdr:
                logger.warning('* the challenge message was not found!')
            header_bytes = self._get_header_bytes(proxy_auth_header=authenticate_hdr)
            logger.info('* sending the authenticate message')
            self.send(header_bytes)
            version, code, message, response = self._get_response()

        if code != 200:
            logger.info('* HTTP %s: failed to establish proxy tunnel', code)
            self.close()
            raise socket.error(
                "Tunnel connection failed: %d %s" % (code, message.strip())
            )
        while self._continue_reading_headers:
            logger.info('draining socket')
            line = response.fp.readline()
            logger.info('< %r', line)
            if len(line) > _MAXLINE:
                raise LineTooLong("header line")
            if self._is_line_blank(line):
                break
        logger.info('* successfully established proxy tunnel!')
