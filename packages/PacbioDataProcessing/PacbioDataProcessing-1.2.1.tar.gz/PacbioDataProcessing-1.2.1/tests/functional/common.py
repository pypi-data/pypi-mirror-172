...
class SmAnalysisMixIn:
    ...
    def check_sm_analysis_with_bam_and_expected_results(
        ...
        expected_pbmm2_calls = 4
        if self.pbmm2_ccs.exists():
            expected_pbmm2_calls -= 1
        ...
        if self.unaligned_input:
            ...
            if self.aligned_bam:
                assert (
                    ...
                    f"found: 'pbmm2.{bam}'. It will be used.") in output
                expected_pbmm2_calls -= 1
            else:
                assert (
                    "...since no aligned version of the input BAM was found, "
                    f"one has been produced and it will be used: 'pbmm2.{bam}'"
                ) in output
            if self.pi_shifted_aligned_bam:
                assert (
                    "...but a possible pi-shifted aligned version of the input"
                    f" BAM was found: 'pi-shifted.pbmm2.{bam}'. It will be "
                    "used."
                ) in output
                expected_pbmm2_calls -= 1
            else:
                assert (
                    ...
                    f"'pi-shifted.pbmm2.{bam}'"
                ) in output
        else:
            expected_pbmm2_calls -= 2
        ...
        aligned_ccs_generated_msg = (
            f"[pbmm2] Aligned file 'pbmm2.ccs.{bam}' generated"
        )
