...
class Pbmm2(AlignerMixIn, ExternalProgram):
    ...
    def __call__(self,
                 ...
                 fasta: PathOrStr,
                 out_bamfile: PathOrStr) -> Optional[ReturnCode]:
        return super().__call__(in_bamfile, out_bamfile, fasta)

    def _run(self,
             in_bamfile: PathOrStr,
             out_bamfile: PathOrStr,
             fasta: PathOrStr) -> tuple[ReturnCode, str]:
        self.cmd_issued = (
            str(self.program), "align", str(fasta), str(in_bamfile),
            str(out_bamfile)
        )
