...
class Alexandre:
        ...
        if isinstance(var, list): # Se é uma lista de variáveis
            if self.lon and self.lat != None:
                ...
                for n in range(1):
                    ...
                    #file = var_ar[:, 0:len(self.dataset.longitude):-1]
                    #
                    file = var_ar
                    print(file)
                    print(var)
    ...
    def to_netcdf(self, path):
        ...
        print(f'O arquivo netcdf foi salvo em {path}')
    def prcptot(self,with_x=False):
        import Indice
        if self.var != 'pr':
            raise "é necessário usar dados de precipitação (pr)"
    ...
    def tx(self, with_x=False):
        ...
        if self.var.lower in ['t_maxmin' ,'tmax']:
            raise "é necessário usar pelo menos os dados de temperatura máxima (Tmax)"
        self.to_csv('temp.nc', var=self.var)
        a = indice.indice(pd.read_csv('temp.nc'))
    ...
    def tn(self, with_x=False):
        ...
        if self.var.lower in ['t_maxmin', 'tmin']:
            raise "é necessário usar pelo menos os dados de temperatura mínima (Tmin)"
        self.to_csv('temp.nc', var=self.var)
        a = indice.indice(pd.read_csv('temp.nc'))
