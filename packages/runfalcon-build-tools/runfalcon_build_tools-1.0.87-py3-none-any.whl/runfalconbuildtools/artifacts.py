...
class ArtifactsManager:
    ...
    def get_artifact_from_code_artifact(self, domain:str, repository:str, format:str, namespace:str, package:str, package_version:str = None, out_dir:str = '.', asset:str = None):
        ...
        self.__logger.info('Downloading {package_version}:{package}:{format} from {domain}:{repository} to {out_dir} ...'.format( \
        ...
        if not file_exists(out_dir + '/' + asset):
            raise Exception('{package_version}:{package}:{format}:{asset} not downloaded.'.format( \
                            package_version=package_version, \
                            package=package, \
                            format=format, \
                            asset=asset \
