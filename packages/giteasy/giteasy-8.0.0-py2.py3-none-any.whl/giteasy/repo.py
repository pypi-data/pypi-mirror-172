...
class Repo :
    ...
    def __init__(self ,
        ...
        self.tok_json_fp = tok_json_fp

        self.cln_src_url = clean_github_url(src_url)

        self.user_reponame = self.cln_src_url.split(cte.gi)[1]

        self.user = self.user_reponame.split('/')[0]
        self.repo_name = self.user_reponame.split('/')[1]

        self._init_local_path(local_path = local_path)
    ...
    def _init_local_path(self , local_path = None) :
        self.local_path = local_path

    def _stage_all_changes(self) :
        idx = self.rp.open_index()
    ...
    def overwriting_clone(self , overwrite = True , depth = 1) :
        ...
        try :
            ...
            url = self._clone_url
            self.rp = porcelain.clone(url , trgdir , depth = depth)
    ...
    def commit_and_push(self , message , branch = 'main') :
        ...
        self.rp.do_commit(message.encode())
