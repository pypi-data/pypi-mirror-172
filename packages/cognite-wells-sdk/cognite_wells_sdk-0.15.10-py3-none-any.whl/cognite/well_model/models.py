...
class MnemonicSearch(WellsBaseModel):
    ...
    mnemonics: List[str] = Field(
        ..., description="List of mnemonics you want to search for.", example=["DEPT", "GR", "TENS", "CVEL", "INCL"]
    )
...
class RigOperation(WellsBaseModel):
    ...
    wellbore_asset_external_id: str = Field(
        ...
        description="The CDF asset external id to the connected wellbore.",
        max_length=255,
    )
...
class RigOperationIngestion(WellsBaseModel):
    ...
    wellbore_asset_external_id: str = Field(
        ...,
        alias="wellboreAssetExternalId",
        description="The CDF asset external id of the wellbore where the operation taken place. The asset must have been ingested as a wellbore with the same source name as this Rig Operation.",
    )
    source: SourceExternalId = Field(..., description="The source external ID in CDF.")
    rig_name: str = Field(..., alias="rigName", max_length=255)
    start_time: Optional[int] = Field(
        None,
        alias="startTime",
        description="Epoch timestamp in milliseconds for when the operation started.",
...
class WellIngestion(WellsBaseModel):
    ...
    unique_well_identifier: Optional[str] = Field(
        None, alias="uniqueWellIdentifier", description="Also called UWI.", max_length=255
    )
    country: Optional[str] = Field(None, description="Country of the well.", example="Norway")
    ...
    spud_date: Optional[date] = Field(
        None,
        alias="spudDate",
        description="The date a new well was spudded or the date of first actual penetration of the earth with a drilling bit.",
        example="1993-02-17",
    )
    block: Optional[str] = Field(
        None,
        description="The block of the well. This is the second part of the unique well identifer used on the norwegian continental shelf. The wellbore `15/9-19-RS` in the VOLVE fild is in block `15/9`.",
        example="15/9",
    )
    field: Optional[str] = Field(None, description="Field of the well.", example="VOLVE")
    operator: Optional[str] = Field(
        None, description="Operator that owns the well.", example="Den norske stats oljeselskap a.s"
    )
    well_type: Optional[str] = Field(
        None, alias="wellType", description="Exploration or development.", example="exploration"
    )
    license: Optional[str] = Field(None, description="Well licence.")
    water_depth: Optional[Distance] = Field(
        None,
        alias="waterDepth",
        description="Water depth of the well. Vertical distance from the mean sea level (MSL) to the sea bed.",
    )
    wellhead: Optional[Wellhead] = None
    source: AssetSource = Field(..., description="Connection between this well and the well asset with a given source.")
...
class Npt(WellsBaseModel):
    ...
    start_time: Optional[int] = Field(
        None,
        alias="startTime",
        description="Epoch timestamp in milliseconds for when the NPT event started.",
        example=1,
    )
    end_time: Optional[int] = Field(
        None, alias="endTime", description="Epoch timestamp in milliseconds for when the NPT event ended.", example=1
    )
...
class NptIngestion(WellsBaseModel):
    ...
    wellbore_asset_external_id: str = Field(
        ...,
        alias="wellboreAssetExternalId",
        description="The CDF asset external id of the wellbore where the event happened. The asset must have been ingested as a wellbore with the same source as this NPT event.",
    )
    npt_code: Optional[str] = Field(
        None, alias="nptCode", description="The NPT code to identify event.", example="ijiefe3:3463:npt"
    )
    ...
    start_time: Optional[int] = Field(
        None, alias="startTime", description="Epoch timestamp in milliseconds for when the NPT event started."
    )
    end_time: Optional[int] = Field(
        None, alias="endTime", description="Epoch timestamp in milliseconds for when the NPT event ended."
    )
    measured_depth: Optional[Distance] = Field(None, alias="measuredDepth")
    root_cause: Optional[str] = Field(
        None,
...
class Nds(WellsBaseModel):
    ...
    hole_top: Optional[Distance] = Field(
        None, alias="holeTop", description="The top measured depth that this event is associated with."
    )
    hole_base: Optional[Distance] = Field(
        None, alias="holeBase", description="The base measured depth that this event is associated with."
    )
...
class NdsIngestion(WellsBaseModel):
    ...
    wellbore_asset_external_id: str = Field(
        ...,
        alias="wellboreAssetExternalId",
        description="The CDF asset external id of the wellbore where the event happened. The asset must have been ingested as a wellbore with the same source as this NDS event.",
    )
    source: EventSource
    description: Optional[str] = Field(
        None, description="Textual description of the event.", example="REAM OUT TIGHT HOLE"
...
class Wellbore(WellsBaseModel):
    ...
    unique_wellbore_identifier: Optional[str] = Field(
        None, alias="uniqueWellboreIdentifier", description="Also called UBI.", max_length=255
    )
...
class WellTopsIngestion(WellsBaseModel):
    ...
    measured_depth_unit: DistanceUnitEnum = Field(..., alias="measuredDepthUnit")
    phase: Optional[PhaseEnum] = None
    tops: List[WellTopSurfaceIngestion] = Field(..., description="A list of well tops to ingest")
...
class HoleSectionGroupIngestion(WellsBaseModel):
    ...
    is_definitive: Optional[bool] = Field(
        False,
        alias="isDefinitive",
        description="If true, this hole section group will become a new definitive hole section group for the matched wellbore.",
    )
...
class WellFilterRequest(CursorAndLimit):
    ...
    aggregates: Optional[List[WellFilterAggregatesEnum]] = []
...
class Well(WellsBaseModel):
    ...
    matching_id: str = Field(
        ..., alias="matchingId", description="Unique identifier used to match wells from different sources."
    )
    name: str = Field(..., description="Name of the well.", example="15/9-19 SR")
    description: Optional[str] = Field(None, description="Description of the well.")
    unique_well_identifier: Optional[str] = Field(
...
class CasingComponent(WellsBaseModel):
    ...
    joints: Optional[int] = Field(None, description="Number of component joints", ge=1.0)
...
class CasingAssembly(WellsBaseModel):
    ...
    original_measured_depth_top: Distance = Field(
        ...,
        alias="originalMeasuredDepthTop",
        description="measured depth top of the casing assembly. Must be less than or equal to originalMeasuredDepthBase.",
    )
    original_measured_depth_base: Distance = Field(
        ...,
        alias="originalMeasuredDepthBase",
        description="measured depth baes of the casing assembly. Must be larger than or equal to originalMeasuredDepthTop.",
    )
