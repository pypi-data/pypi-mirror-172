...
class ColumnListBlock(Block):
    ...
    def get_children(self) -> List[Optional[Block]]:
        children = []
        for child in self.children:
            if isinstance(child, ColumnBlock):
...
class BasicBlock(Block):
    ...
    title: Optional[str] = property_map("title")
    title_plaintext: Optional[str] = property_map(
        "title",
        python_to_api=plaintext_to_notion,  # type: ignore
...
class CollectionViewBlock(MediaBlock):
    ...
    def collection(self):
        ...
        if not collection_id:
            collection_id = self.get("format", {}).get("collection_pointer").get("id")
        if not collection_id:
            return None
    ...
    @property
    def title_plaintext(self) -> Optional[str]:
        return self.collection.name_plain_text

    @title.setter
    def title_plaintext(self, val):
        self.collection.name_plain_text = val

    @property
    def icon(self) -> str:
        return self.collection.icon

    @property
    def children(self) -> List[Optional[Block]]:
        if self.collection is not None:
            return self.collection.get_rows()
        return []
