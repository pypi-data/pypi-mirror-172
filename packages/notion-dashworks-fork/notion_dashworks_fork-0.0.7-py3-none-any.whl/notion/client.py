...
class NotionClient(object):
    ...
    def _update_user_info(self):
        ...
        self.spaces = list(
            filter(
                None, [self.get_space(space_id) for space_id in records["space"].keys()]
            )
        )
        return records
    ...
    def get_top_level_pages(self):
        ...
        return [
            self.get_block(bid)
            for bid, v in records["block"].items()
            if v["value"]["parent_table"] == "space"
        ]

    def get_top_level_page_for_space(
        self, space_id: str
    ) -> List[Optional[Union[PageBlock, CollectionViewPageBlock]]]:
        records = self._update_user_info()
        return [
            self.get_block(bid)
            for bid, value in records["block"].items()
            if value["value"]["space_id"] == space_id
            and value["value"]["parent_table"] == "space"
        ]
    ...
    def get_user(self, user_id: str, force_refresh=False) -> Optional[User]:
        """
    ...
    def get_pages_with_ancestor(self, ancestor_id: str, space_id: str, min_time: str):
        data = {
            "type": "BlocksInSpace",
            "query": "",
            "spaceId": space_id,
            ...
            "filters": {
                ...
                "editedBy": [],
                "lastEditedTime": {
                    "starting": {"type": "date", "start_date": min_time}
                },
                "createdTime": {},
            },
            "sort": "LastEditedNewest",
            "source": "quick_find",
        }
        response = self.post("search", data).json()
        self._store.store_recordmap(response["recordMap"])
