...
def node(node_name = "", filename = "", lineno = "", node_description="", path=None):
  ...
  os.environ['prev_node'] = str(response_dict['node_id'])
...
def tracefunc(frame, event, arg, indent=[0]):
    if "Python" in frame.f_code.co_filename:
        ...
        if event == "call":
            os.environ["count"] = int(os.environ["count"]) + 1
            print(os.environ["count"])
            print(frame.f_code.co_name, " ", len(frame.f_code.co_name))

            print(len(frame.f_code.co_filename))
            ...
            #print("-" * indent[0] + "> call function", frame.f_code.co_name)
                ...
                os.environ["prev_file"] = frame.f_code.co_filename
