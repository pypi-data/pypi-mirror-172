...
def decls(
    ...
    for id_ in hub.idem.resolve.init.iter(state):
        if not isinstance(state[id_], Dict):
            if isinstance(state[id_], str):
                ...
                if "." in state[id_]:
                    ...
                    state[id_] = {
                        "__sls__": __get_sls_param(sls_ref),
                        state_ref: [comps[-1]],
                    }
                    continue
        ...
        if "__sls__" not in state[id_]:
            state[id_]["__sls__"] = __get_sls_param(sls_ref)


def __get_sls_param(sls_ref: str) -> str:
    """
    Returns the value for "__sls__" from sls_ref
    """
    return sls_ref.rsplit(".", 1)[-1]
