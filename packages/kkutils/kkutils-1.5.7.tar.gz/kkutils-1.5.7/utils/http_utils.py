    ...
    def _retry(self, url, kwargs, retry):
        ...
        for i in range(retry + 1):
            try:
                ...
                reason = str(e)
                self.logger.error(f'url: {url}, retry: {i}, exception: {e}')
...
class Aiohttp(AsyncRequest):
    ...
    def __init__(self, **kwargs):
        ...
        self.session = aiohttp.ClientSession(cookie_jar=cookiejar)
        self.load_cookie()
...
class Chrome(webdriver.Chrome):
    ...
    def __init__(self, path=None, headless=False, proxy=None, mobile=None):
