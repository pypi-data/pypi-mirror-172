...
PROJECT_ID = "emart-datafabric"
TEMP_DATASET = "temp_1d"
CREDENTIALS_SECRET_MOUNT_POINT = "datafabric"
CREDENTIALS_SECRET_PATH = "gcp/emart-datafabric/datafabric"
PRINT_QUERY_JOB_RESULTS = False


def print_query_job_results(print_query=False):
    global PRINT_QUERY_JOB_RESULTS

    PRINT_QUERY_JOB_RESULTS = print_query
...
def get_credentials():
    ...
    key = get_secrets(CREDENTIALS_SECRET_MOUNT_POINT, CREDENTIALS_SECRET_PATH)["config"]
...
def set_default_credentials():
    ...
    key_json = get_secrets(CREDENTIALS_SECRET_MOUNT_POINT, CREDENTIALS_SECRET_PATH)[
        "config"
    ]
...
def _get_result_schema(sql, bq_client=None, project=PROJECT_ID):
    from google.cloud.bigquery.job import QueryJobConfig

    if bq_client is None:
        bq_client = get_bigquery_client(project=project)
...
def _print_query_job_results(query_job):
    try:
        ...
        if PRINT_QUERY_JOB_RESULTS:
            print(f"query: {query_job.query}\n")
...
def bq_insert_overwrite_table(sql, destination, project=PROJECT_ID):
        ...
        if partition_type == "DATE":
            ...
            qjc = QueryJobConfig(
                ...
                range_partitioning=RangePartitioning(
                    PartitionRange(start=200001, end=209912, interval=1),
                    field=partition_by,
                ),
                clustering_fields=clustering_fields,
            )
            ...
            raise Exception(
                f"Partition column[{partition_by}] is neither DATE or INTEGER type."
            )
    else:
        qjc = QueryJobConfig(
            destination=destination,
...
def _bq_query_to_new_table(sql, destination=None, project=PROJECT_ID):
    return bq_ctas(sql, destination, project=project)


def _bq_query_to_existing_table(sql, destination, project=PROJECT_ID):
    from google.cloud.bigquery.job import QueryJobConfig
...
def _bq_table_to_pandas(table, project=PROJECT_ID, progress_bar_type="tqdm"):
    credentials = get_credentials()
    bq = get_bigquery_client(credentials=credentials, project=project)
    ...
    if args.params is not None:
        try:
            ...
            params = _helpers.to_query_parameters(
                parameters=ast.literal_eval("".join(args.params)),
...
def get_bigquery_client(credentials=None, project=PROJECT_ID):
...
def bq_table_exists(table, project=PROJECT_ID):
    try:
        get_bigquery_client(project=project).get_table(table)
    except NotFound:
        return False
    return True


def get_unhidden_partitions(table_name, project=PROJECT_ID):
    parts = filter(
        lambda x: x not in ["__NULL__", "__UNPARTITIONED__"],
        get_bigquery_client(project=project).list_partitions(table_name),
    )
    return parts


def _get_partition_filter(dataset, table_name, partition, project=PROJECT_ID):
    table = get_bigquery_client(project=project).get_table(f"{dataset}.{table_name}")
...
def bq_to_pandas(sql, large=False, verbose=True):
    destination = None
    if large:
    ...
    return _bq_table_to_pandas(destination, progress_bar_type=progress_bar_type)
...
def _bq_table_to_df(
    ...
    df = (
        ...
        .option("table", f"{project}:{dataset}.{table_name}")
        .option("credentials", base64.b64encode(key.encode()).decode())
    )
    if partition:
        table = get_bigquery_client(project=project).get_table(
...
def bq_table_to_df(
    ...
    return _bq_table_to_df(
        dataset, table_name, col_list, project, partition, where, spark_session
    )
...
def bq_table_to_parquet(
    ...
    ).write.mode(mode).parquet(output_dir)
...
def bq_table_to_csv(
    ...
    bq_table_to_df(
        ...
        where=where,
        spark_session=spark_session,
    ).write.mode(mode).csv(output_dir)


def bq_table_to_json(
    dataset,
    table_name,
    output_dir,
    col_list="*",
    ...
    bq_table_to_df(
        ...
        where=where,
        spark_session=spark_session,
    ).write.mode(mode).json(output_dir)


def bq_table_to_pandas(
    dataset, table_name, col_list="*", project=PROJECT_ID, partition=None, where=None
):
    col_list_str = ", ".join(col_list) if isinstance(col_list, list) else col_list
    query = f"SELECT {col_list_str} FROM {project}.{dataset}.{table_name} "
    if partition or where:
        query += "WHERE "
        conditions = []
        if partition:
            conditions.append(
                f" {_get_partition_filter(dataset, table_name, partition)} "
            )
        if where:
...
def _df_to_bq_table(
    ...
    df = (
        ...
        .option("credentials", base64.b64encode(key.encode()).decode())
        .option("table", table)
        .option("temporaryGcsBucket", "emart-datafabric-temp-7d")
    )
    if partition_field:
...
def parquet_to_bq_table(
    ...
    try:
        ...
        df = spark_session.read.format("parquet").load(parquet_dir)
        _df_to_bq_table(
            df,
            dataset,
            table_name,
...
def pandas_to_bq(
    ...
    if time_partitioning or range_partitioning:
        if time_partitioning:
            input_partitions = [
                (p.strftime("%Y%m%d"), p) for p in set(pd_df[partition])
            ]
...
def bq_to_df(query, project=PROJECT_ID, spark_session=None):
...
def load_query_result_to_table(
    dest_table, query, project=PROJECT_ID, part_col_name=None, clustering_fields=None
):
    bq_client = get_bigquery_client(project=project)
    print(query)
    if bq_table_exists(dest_table):
        ...
        temp_table_name = get_temp_table()
        bq_client.query(
            f"CREATE OR REPLACE TABLE {temp_table_name} AS {query}"
        ).result()
        if part_col_name:
...
def get_temp_table(project=PROJECT_ID):
    import uuid

    table_id = str(uuid.uuid4()).replace("-", "_")
    full_table_id = f"{project}.{TEMP_DATASET}.{table_id}"
...
def load_query_result_to_partitions(query, dest_table, project=PROJECT_ID):
    ...
    for p in partitions:
        ...
        if p == "__NULL__":
            if table.range_partitioning:
                columns = [
                    "NULL" if f.name.lower() == part_name.lower() else f.name
                    for f in table.schema
                ]
            elif table.time_partitioning:
                columns = [
                    "DATE(NULL)" if f.name.lower() == part_name.lower() else f.name
                    for f in table.schema
                ]
            job = bq.query(
                f"""
            ...
            project, dataset_id, table_id = dest_table.split(".")
            ref = TableReference(
                DatasetReference(project, dataset_id), f"{table_id}${p}"
            )
            qjc = QueryJobConfig(
                destination=ref,
                write_disposition="WRITE_TRUNCATE",
...
def get_max_part(table_name, project=PROJECT_ID):
...
def bq_insert_overwrite(
    ...
    if suffixes:
        bq_insert_overwrite_with_suffixes(
            sql, destination, suffixes, project, partition, clustering_fields
        )
    else:
        bq_insert_overwrite_without_suffixes(
            sql, destination, project, partition, clustering_fields
        )
...
def bq_insert_overwrite_with_suffixes(
    ...
    bq_ctas(
        sql, temp_table, partition_by=partition, clustering_fields=clustering_fields
    )
    bq = get_bigquery_client(project=project)
...
def bq_insert_overwrite_without_suffixes(
    sql, destination, project=PROJECT_ID, partition=None, clustering_fields=None
):
    if bq_table_exists(destination):
        bq_insert_overwrite_table(sql, destination, project=project)
    else:
        bq_ctas(
            sql,
            destination,
