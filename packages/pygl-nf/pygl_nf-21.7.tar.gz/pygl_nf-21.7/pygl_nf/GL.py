    ...
    class GL():
        ...
        class Rect:
            def __init__(self, COLOR=(), POSITION=[], SIZE=[], THICKNESS=0, SURF=None, FUNCTION='none', R=0):
                ...
                if R > self.cant_size[0]/2 or R > self.cant_size[1]/2:
                    self.r = self.size[0] / \
                        2 if self.size[0] < self.size[1] else self.size[1]/2
            ...
            def FILL(self):
                ...
                Display_init_.GL.Circle(
                    self.col, self.cant_pos, self.r, 0, self.surf, 'D')
                Display_init_.GL.Circle(self.col, [
                                        self.cant_pos[0]+self.cant_size[0], self.cant_pos[1]], self.r, 0, self.surf, 'D')
                Display_init_.GL.Circle(self.col, [
                                        self.cant_pos[0]+self.cant_size[0], self.cant_pos[1]+self.cant_size[1]], self.r, 0, self.surf, 'D')
                Display_init_.GL.Circle(self.col, [
                                        self.cant_pos[0], self.cant_pos[1]+self.cant_size[1]], self.r, 0, self.surf, 'D')

                Display_init_.GL.Rect(self.col, [self.pos[0], self.pos[1]-self.r], [
                                      self.size[0], self.size[1]+2*self.r], 0, self.surf, 'D')
                Display_init_.GL.Rect(self.col, [self.pos[0]-self.r, self.pos[1]], [
                                      self.size[0]+2*self.r, self.size[1]], 0, self.surf, 'D')
            ...
            def POUTLINE(self, COLOR=None, THICKNESS=None, PLS=10):
                ...
                if THICKNESS != None:
                    self.sh2 = THICKNESS
                Display_init_.GL.Dashed_line(self.obv_color, self.pos, [
                                             self.pos[0]+self.size[0], self.pos[1]], self.sh2, PLS, self.surf, 'D')
                Display_init_.GL.Dashed_line(self.obv_color, self.pos, [
                                             self.pos[0], self.pos[1]+self.size[1]], self.sh2, PLS, self.surf, 'D')
                Display_init_.GL.Dashed_line(self.obv_color, [self.pos[0], self.pos[1]+self.size[1]], [
                                             self.pos[0]+self.size[0], self.pos[1]+self.size[1]], self.sh2, PLS, self.surf, 'D')
                Display_init_.GL.Dashed_line(self.obv_color, [self.pos[0]+self.size[0], self.pos[1]], [
                                             self.pos[0]+self.size[0], self.pos[1]+self.size[1]], self.sh2, PLS, self.surf, 'D')
        ...
        class Poligon:
            def __init__(self, COLOR=(), POINTS=(), THICKNESS=0, SURF=None, FUNCTION='none'):
                ...
                self.obv_col = 'black'
                if FUNCTION == 'D':
                    pygame.draw.polygon(self.surf, self.col,
                                        self.points, self.sh)
                elif FUNCTION[1] == 'D':
                    ...
                    if THICKNESS != None:
                        self.sh2 = THICKNESS
                    pygame.draw.polygon(
                        self.surf, self.obv_col, self.points, self.sh2)
                elif FUNCTION[0] == 'O':
            ...
            def OUTLINE(self, COLOR=None, THICKNESS=None):
                ...
                if THICKNESS != None:
                    self.sh2 = THICKNESS
                pygame.draw.polygon(self.surf, self.obv_col,
                                    self.points, self.sh2)
...
class Animations_():
    def __init__(self, surf, frames=[], pos=[], delta=5, looping=False) -> None:
...
class Sprite_sheat_loader():
    def __init__(self, sheat_w, sheat_h, sprite_wh, count_w, count_h, sprite_sheat) -> None:
        self.sheat_w = sheat_w
        self.sheat_h = sheat_h
        self.sprite_wh = sprite_wh
        self.count_w = count_w
        self.count_h = count_h

        self.sprite_sheat = sprite_sheat
        self.ss = Sprites_(self.sprite_sheat)
