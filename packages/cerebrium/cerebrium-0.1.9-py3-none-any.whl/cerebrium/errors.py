class CerebriumRequestError(Exception):
    ...
    def __str__(self):
        if self.status_code == 401:
            msg = f"{self.status_code}\n`{self.endpoint} API Call Failed. External API Key missing. Please contact the Cerebrium team."
            ...
            msg = f"{self.status_code}\n`{self.endpoint}` API Call Failed.{specific_message} Please ensure your Cerebrium API key is correct and retry."
