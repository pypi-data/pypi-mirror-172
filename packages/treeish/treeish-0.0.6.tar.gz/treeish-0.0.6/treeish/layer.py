...
class ListType(Enum):
    DIGITS: list = [i for i in DIGIT_RANGE]
    UPPER_ALPHA: list = list(set_alphabet(string.ascii_uppercase, 3))
    LOWER_ALPHA: list = list(set_alphabet(string.ascii_lowercase, 3))
    UPPER_ROMAN: list = [romanize(i) for i in DIGIT_RANGE]
    LOWER_ROMAN: list = [romanize(i).lower() for i in DIGIT_RANGE]
