...
from tethysts.utils import read_json_zstd, create_public_s3_url, make_run_date_key, download_results
...
import shutil
from s3tethys import get_object_s3, s3_client, put_object_s3, list_objects_s3, put_file_s3
...
class Titan(object):
    ...
    def status_checks(self, connection_config: tdm.base.ConnectionConfig, bucket: str, public_url: str = None, system_version: int = 4, load_diagnostics: bool = False):
        ...
        # Write permissions
        test1 = io.BytesIO(b'write test')
        test_key = 'tethys/testing/test_write.txt'

        try:
            _ = put_object_s3(client, bucket, test_key, test1, retries=1)
        ...
        if isinstance(public_url, str):
            try:
                ...
                _ = get_object_s3(test_key, public_url=public_url, bucket=bucket)
            except:
        ...
        # Version 3+ check
        objs1 = list_objects_s3(client, bucket, 'tethys/', delimiter='/')
        if not objs1.empty:
            exist_version = int(objs1.iloc[0].Key.split('/')[1].split('.')[0][1:])
        else:
        # Version 2 check
            objs2 = list_objects_s3(client, bucket, 'tethys/v2/', delimiter='/')
            if objs2.empty:
                exist_version = None
            else:
                exist_version = 2
        ...
        if load_diagnostics:
            ...
            try:
                resp = get_object_s3(dgn_key, bucket, client, public_url=public_url)
                f = Fernet(self.key)
                dgn = read_json_zstd(f.decrypt(resp.read()))
                self.diagnostics = dgn
                _ = [setattr(self, k, v) for k, v in dgn['attributes'].items()]
    ...
    def save_diagnostics(self):
        ...
        f = Fernet(self.key)
        dgn_obj = io.BytesIO(f.encrypt(misc.write_json_zstd(copy.deepcopy(self.diagnostics))))
        ...
        obj_resp = put_object_s3(client, self.bucket, dgn_key, dgn_obj, {'run_date': run_date1}, 'application/json')
    ...
    def _update_datasets(self):
        ...
        dss_obj = get_object_s3(dss_key, bucket=self.bucket, connection_config=self.connection_config, public_url=self.public_url)

        if dss_obj is None:
            old_datasets = None
        else:
            old_datasets = read_json_zstd(dss_obj.read())

        # Process and save the new file
        _ = processing.update_dataset_agg(ds_paths, self.preprocessed_path, old_datasets)
    ...
    def _upload_final_objects(self, threads=60):
        ...
        if len(rc_paths) > 0:
            ...
            results_paths = glob.glob(os.path.join(self.preprocessed_path, '*results.h5'))
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
                ...
                for s3_dict in s3_list:
                    f = executor.submit(put_file_s3, **s3_dict)
                    futures.append(f)
    ...
    def _reset_auxillary_objects(self, dataset_id, threads=80, max_workers=4):
        ...
        obj_list1 = list_objects_s3(client, self.bucket, key_prefix)
