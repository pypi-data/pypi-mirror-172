...
ds_stn_file_str = '{ds_id}_{stn_id}_{date}.h5'
results_file_str = '{ds_id}_{version_date}_{stn_id}_{chunk_id}_{chunk_hash}_results.h5'
...
def load_dataset(path):
    ...
    """
    if str(path).endswith('.h5') or str(path).endswith('.nc'):
        data = xr.load_dataset(path, engine='h5netcdf')
...
def open_dataset(path):
    ...
    if str(path).endswith('.h5') or str(path).endswith('.nc'):
        ...
        data = xr.open_dataset(utils.read_pkl_zstd(path))
...
def add_metadata_results(results, metadata, version_date):
    ...
    param_attrs = orjson.loads(tdm.dataset.ParameterAttrs(**md).json(exclude_none=True))
...
def update_compare_results(previous_result, metadata, version_date, new_path):
    ...
    old_xr = load_dataset(previous_result)
    ...
    if not new_results:
        ...
        new_xr = load_dataset(new_result_path)
    ...
    hdf5tools.xr_to_hdf5(up1, new_result_path)
...
def update_versions(version_data, new_path, old_versions=None):
    ...
    version_dict1 = orjson.loads(tdm.dataset.ResultVersion(**version_data).json(exclude_none=True))
...
def prepare_file_for_s3(path, s3, bucket, system_version=4):
    ...
    if file_name.endswith('results.h5'):
    ...
    dict1 = {'s3': s3, 'bucket': bucket, 'file_path': path, 'obj_key': key_name, 'metadata': metadata, 'content_type': content_type}
...
def preprocess_data_structure(nc_path, time_name, x_name, y_name, variables, time_index_bool=None, projected_coordinates=True):
    ...
    for v in variables:
        ...
        if projected_coordinates:
            ...
            new_file_name_str = '{var}_proj_{date}.h5'
        else:
            xr2 = xr2.rename({x_name: 'lon', y_name: 'lat'})
            new_file_name_str = '{var}_wgs84_{date}.h5'
        ...
        hdf5tools.xr_to_hdf5(xr2.to_dataset(), new_path)
...
def resample_to_wgs84_grid(nc_path, proj4_crs, grid_res, order=2, min_val=None, max_val=None, bbox=None, time_name='time', x_name='x', y_name='y', save_data=True):
    ...
    if save_data:
        ...
        hdf5tools.xr_to_hdf5(new_grid4, new_path)
        os.remove(nc_path)
...
def calc_new_variable(path_dict, dataset, version_date, variables, func, out_path):
    ...
    data_list = [open_dataset(p) for p in paths]
    ...
    new_file_name = '{ds_id}_{date}.h5'.format(ds_id=dataset['dataset_id'], date=date)
    new_path = out_path.parent.joinpath(new_file_name)
    # results.to_netcdf(new_path, unlimited_dims=['time'])
    hdf5tools.xr_to_hdf5(results, new_path)
...
def save_dataset_stations(file_path, block_length, remove_station_data=True):
    """
    ...
    for c in chunks_list:
        ...
        hdf5tools.xr_to_hdf5(b, new_file_path)
...
def save_new_results(nc_path, metadata, version_date, system_version=4, overwrite=False, base_path=None):
    ...
    for b in chunks_list:
        ...
        version_date_key = make_run_date_key(version_date)
        file_name = results_file_str.format(ds_id=ds_id, stn_id=stn_id, chunk_id=chunk_id, chunk_hash=hash1, version_date=version_date_key)
        file_path = base_path.joinpath(file_name)

        if file_path.exists() and (not overwrite):
            print(str(file_path) + ' already exists!')
        else:
            # file_path.parent.mkdir(parents=True, exist_ok=True)
            hdf5tools.xr_to_hdf5(f, str(file_path))
            results_new_paths.append(str(file_path))
