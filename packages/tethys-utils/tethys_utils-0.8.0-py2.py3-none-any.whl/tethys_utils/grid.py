...
import xarray as xr
import pandas as pd
from tethys_utils import misc, processing, titan, data_io, s3
...
import dill
from s3tethys import s3_client, list_objects_s3, list_object_versions_s3
...
# interim_file_str = '{ds_id}_{stn_id}_{date}.h5'
...
def multi_save_dataset_stations(nc_paths, block_length, block_length_factor: int = 10, remove_station_data=True, max_workers=3):
    """
    ...
    if max_workers <= 1:
        ...
        for nc_path in nc_paths1:
            new_paths0 = processing.save_dataset_stations(nc_path, block_length * block_length_factor, remove_station_data=remove_station_data)
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
            futures = []
            for nc_path in nc_paths1:
                f = executor.submit(processing.save_dataset_stations, nc_path, block_length * block_length_factor, remove_station_data=remove_station_data)
                futures.append(f)
...
def get_obj_list(glob_path, date_format=None, freq=None, from_date=None, to_date=None, connection_config=None, bucket=None):
    ...
    if isinstance(connection_config, dict) and isinstance(bucket, str):
        ...
        obj_list = list_objects_s3(client, bucket, glob_path3, delimiter='/', date_format=date_format)
...
def copy_source_objs(source_paths, dest_path, bucket=None, connection_config=None, public_url=None, threads=None, max_workers=None):
    """
    ...
    if isinstance(connection_config, (dict, str)) and isinstance(bucket, str):
        ...
        if isinstance(max_workers, int):
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
                futures = []
                for obj in objs:
                    f = executor.submit(data_io.copy_s3_file, source_path=obj, dest_path=dest_path, connection_config=connection_config, bucket=bucket, public_url=public_url)
                    futures.append(f)
                runs = concurrent.futures.wait(futures)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
                futures = []
                for obj in objs:
                    f = executor.submit(data_io.copy_s3_file, source_path=obj, dest_path=dest_path, s3=client, connection_config=connection_config, bucket=bucket, public_url=public_url)
                    futures.append(f)
                runs = concurrent.futures.wait(futures)
        ...
        new_paths = []
        for obj in objs:
            path1 = data_io.copy_file(source_path=obj, dest_path=dest_path)
...
def copy_interim_objs(source_paths, dest_path, bucket, connection_config=None, public_url=None, threads=20):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        for source_path in source_paths:
            ...
            file_name = processing.ds_stn_file_str.format(ds_id=ds_id, stn_id=stn_id, date=date)
...
def resample_to_wgs84_grids(nc_paths, proj4_crs, order=1, min_val=None, max_val=None, bbox=None, time_name='time', x_name='x', y_name='y', max_workers=4):
    ...
    xr1 = xr.open_dataset(nc_paths1[0], engine='h5netcdf', cache=False)
...
def multi_mergetime_hdf5_remove(source_paths, by, max_workers=3):
    """
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
        ...
        for ds_id, paths in source_paths_dict.items():
            f = executor.submit(data_io.mergetime_hdf5_files, paths, by)
            futures.append(f)
        runs = concurrent.futures.wait(futures)
...
class Grid(titan.Titan):
    ...
    def copy_source_objs(self, source_paths, threads=None, max_workers=None):
        """

        """
        misc.diagnostic_check(self.diagnostics, 'get_obj_list')

        new_paths = copy_source_objs(source_paths, self.preprocessed_path, threads=threads, connection_config=self.source_connection_config, bucket=self.source_bucket, public_url=self.source_public_url, max_workers=max_workers)
    ...
    def merge_time_hdf5_files(self, source_paths, by, max_workers=4):
        """
        ...
        new_paths = multi_mergetime_hdf5_remove(source_paths, by=by, max_workers=max_workers)
    ...
    def save_dataset_stations(self, source_paths, block_length, block_length_factor=10, max_workers=4):
        """

        """
        new_paths = multi_save_dataset_stations(source_paths, block_length, block_length_factor=block_length_factor, max_workers=max_workers)
    ...
    def get_interim_results_list(self, max_size_gb=0.1):
        ...
        obj_list1 = list_objects_s3(client, self.bucket, key)
    ...
    def clear_interim_results(self):
        ...
        obj_list = list_object_versions_s3(client, self.bucket, prefix)
