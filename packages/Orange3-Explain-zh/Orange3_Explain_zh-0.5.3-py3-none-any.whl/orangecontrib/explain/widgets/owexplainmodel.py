...
class Legend(QGraphicsWidget):
    ...
    def _add_high_label(self):
        self.__high_label = item = QGraphicsSimpleTextItem("高")
        item.setX(self.BAR_WIDTH + self.__offset)
        item.setY(0)
        self.__group.addToGroup(item)

    def _add_low_label(self):
        self.__low_label = item = QGraphicsSimpleTextItem("低")
        item.setX(self.BAR_WIDTH + self.__offset)
        self.__group.addToGroup(item)

    def _add_feature_label(self):
        self.__feature_label = item = QGraphicsSimpleTextItem("特征值")
        item.setRotation(-90)
...
class OWExplainModel(OWExplainFeatureBase):
    name = "解释模型(Explain Model)"
    description = "使用SHAP summary plots 解释模型"
    keywords = ["explain", "explain prediction", "explain model", "shap", "jieshimoxing"]
    ...
    class Outputs(OWExplainFeatureBase.Outputs):
        impact = Output("影响(Impact)", Table, replaces=["Impact"])
    ...
    def _add_controls(self):
        ...
        super()._add_controls()
        gui.checkBox(self.display_box, self, "show_legend", "显示图例",
