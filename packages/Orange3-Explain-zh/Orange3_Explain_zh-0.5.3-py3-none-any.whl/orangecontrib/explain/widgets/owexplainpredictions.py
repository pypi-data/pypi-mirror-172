...
class OWExplainPredictions(OWWidget, ConcurrentWidgetMixin):
    name = "解释多个预测(Explain Predictions)"
    description = "使用SHAP值解释多个预测."
    keywords = ["explain", "explain prediction", "explain model", "shap", "jieshiduogemoxing"]
    ...
    class Outputs:
        selected_data = Output("选中的数据(Selected Data)", Table, default=True)
    ...
    def _add_controls(self):
        ...
        box = gui.vBox(self.controlArea, "实例顺序")
        ...
        box = gui.vBox(self.controlArea, "标注")
        ...
        gui.checkBox(box, self, "show_tooltip", "显示提示",
