...
from orangecontrib.explain.explainer import RGB_LOW, RGB_HIGH, \
...
class OWExplainPrediction(OWWidget, ConcurrentWidgetMixin):
    name = "解释预测(Explain Prediction)"
    description = "使用SHAP值解释单个预测."
    keywords = ["explain", "explain prediction", "explain model", "shap", "jieshiyuce"]
    ...
    class Inputs:
        ...
        background_data = Input("背景数据(Background Data)", Table, replaces=["Background Data"])
    ...
    def _add_controls(self):
        ...
        box = gui.hBox(self.controlArea, "缩放")
        ...
        box = gui.vBox(self.buttonsArea, "预测信息")
    ...
    def update_scene(self):
        ...
        if self.__results is not None:
            ...
            self.mo_info = f"模型预测: {_str(plot_data.model_output)}"
            self.bv_info = f"基准值(Base value): {_str(plot_data.base_value)}"
