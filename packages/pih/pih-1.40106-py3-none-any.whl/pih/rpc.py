...
from pih.collection import ServiceRoleInformation, ServiceRoleValue, Subscriber, Subscribtion
from pih.tools import DataTools, EnumTools, ParameterList
from pih.const import ServiceCommands, ServiceRoles, SubscribtionType
...
class RPC:
    ...
    class UnaryService(pb2_grpc.UnaryServicer):
            ...
            def unsubscribe_all_internal() -> None:
                for service_command in self.subscriber_map:
                    for role_item in self.subscriber_map[service_command]:
                        ...
                        DataTools.rpc_unrepresent(RPC.internal_call(
        ...
        def subscribe(self, parameter_list: ParameterList) -> bool:
            role: ServiceRoles = EnumTools.get(
                ServiceRoles, parameter_list.next())
            service_command: ServiceCommands = EnumTools.get(
                ...
                for role_item in self.subscriber_map[service_command]:
                    ...
                    if (subscriber.type & SubscribtionType.BEFORE) != 0 and subscriber.enabled:
                        ...
                        if subscriber.available:
                            before_result = ParameterList(DataTools.rpc_unrepresent(RPC.internal_call(
        ...
        def rpcCallCommand(self, command, context):
            ...
            return pb2.rpcCommandResult(data=DataTools.rpc_represent(self.internal_handler(command.name, parameters, context)))
    ...
    class Service:
                    ...
                    if RPC.check_availability(role):
                        for service_command in subscribtion_map[role]:
                            ...
                            if not subscribtion.actiated:
                                if DataTools.rpc_unrepresent(RPC.internal_call(role, ServiceCommands.subscribe, (RPC.Service.role, subscribtion.service_command, subscribtion.type, subscribtion.name))):
        ...
        def subscribe_on(service_command: ServiceCommands, type: int = SubscribtionType.BEFORE, name: str = None) -> bool:
            ...
            if service_role is not None:
                if not service_role_value.debug:
    ...
    def ping(role: ServiceRoles) -> ServiceRoleInformation:
        try:
            return DataTools.fill_data_from_rpc_str(ServiceRoleInformation(), RPC.internal_call(role, ServiceCommands.ping))
        except Error:
            return None
