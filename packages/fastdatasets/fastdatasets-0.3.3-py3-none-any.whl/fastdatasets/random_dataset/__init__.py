...
__all__ = ["SingleRandomRecordDataset","tfrecords","logging"]
    ...
    def gen_indexes(self):
        if self.is_file:
            ...
            if is_need_update_idx:
                logging.info('update index for {}...'.format(self.data_path_or_data_list))
                if os.path.exists(self.data_path_or_data_list):
                    if filestat is None:
                        filestat = os.stat(self.data_path_or_data_list)
                    cur_file_size = filestat.st_size
                ...
                logging.info('update index for {} finish'.format(self.data_path_or_data_list))
        else:
...
class MultiRandomRecordDataset(RandomDatasetBase):
    ...
    def __reopen__(self):
        for it_obj in self.iterators_:
            it_obj['inst'] = SingleRandomRecordDataset(it_obj["file"],index_path=self.index_path,
