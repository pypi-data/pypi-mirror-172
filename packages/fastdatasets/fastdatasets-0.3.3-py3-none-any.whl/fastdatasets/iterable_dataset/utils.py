...
class IterableDatasetBase:
    ...
    def save(self,filename,options=TFRecordOptions()):
        fw = TFRecordWriter(filename, options=options)
        for item in self:
            fw.write(item)
        fw.close()
...
class IntervalIterableDataset(IterableDatasetBase):
    ...
    select one from N
    '''
    def __init__(self,dataset,interval: int,id : int = 0):
      assert id >=0
      assert interval >=0
      assert id < interval
      self.dataset = dataset
