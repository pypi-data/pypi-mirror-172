...
def hs300option_price(option, \
    ...
    if graph:
        ...
        collabel_so='期权价格'
        collabel_si='沪深300指数'
        titletxt_so_si='沪深300股指期权：期权价格 vs 标的物价格'
        colname='Close'; ylabeltxt=''    
        
        footnote1="新浪期权代码："+symbol+"，到期日："+maturity+"，行权点位："+exercisestr+"，方向："+direction
        import datetime; today = datetime.date.today()
...
def hs300option_compare(option1,option2,loc1='best',loc2='best'):
    """
    ...
    colname1='SO Close_x'; label1="收盘价"
    
    ticker2="期权2："+option_description(option2)
    colname2='SO Close_y'; label2="收盘价"
    
    ylabeltxt="期权价格"
    titletxt='沪深300股指期权：期权价格的运动趋势与影响因素'
...
def fin_option_risk_sse(option,maturity,exercise,trade_date,printout=True):
    ...
    print("标的物代码:",underlying,"\b，到期时间:",maturity,"\b，行权点位:",exercise)
...
def fin_option_risk_sse2(option,maturity,exercise,trade_date,printout=True):
    """
    功能：显示指定上交所金融期权的风险指标
    option: 期权名称
    maturity: 到期年月YYMM
    ...
    #绘制看涨看跌期权风险对比柱状图
    #排序，保证看涨期权排在前面
    df9.sort_values(by='direction',ascending=True,inplace=True)
    df9b=df9[['delta','theta','gamma','vega','rho','implied volatility']]
    dfg=df9b.T
    dfg.columns=['Call','Put'] 
    ...
    footnote0="风险指标\n"
    footnote1="注："+"到期日"+maturity+"，行权点位"+str(exercise)+"，交易日期"+trade_date
    import datetime; today = datetime.date.today()
...
if __name__=='__main__':     
    df=fin_option_risk_sse2(option='50ETF',maturity='2209',exercise=3000,trade_date='2022-9-26')
...
def fin_option_maturity_risk_sse(option,exercise,trade_date, \
    ...
    #绘图：看涨+看跌
    ticker1=option+"看涨期权"; colname1=measure
    #label1=measure
    ticker2=option+"看跌期权"; colname2=measure
    #label2=measure
    #ylabeltxt="风险指标"
    ylabeltxt=ectranslate(measure)
    titletxt='上交所金融期权分析：到期时间风险'
    
    footnote0="到期时间（针对不同期权）\n"
    footnote1="期权："+option+"，行权点位："+str(exercise)+"，交易日期："+trade_date
    import datetime; today = datetime.date.today()
    footnote2="数据来源: 新浪财经/东方财富, 制图："+str(today)
    footnote=footnote0+footnote1+"\n"+footnote2
...
def fin_option_time_risk_sse(option,maturity,exercise,start,end, \
    ...
    #ylabeltxt="风险指标"
    ylabeltxt=ectranslate(measure)
    titletxt='上交所金融期权分析：时间推移风险'
    
    footnote0="日期（针对同一期权）\n"
    footnote1="期权："+option+"，行权点位："+str(exercise)+"，到期时间："+maturity
    import datetime; today = datetime.date.today()
    footnote2="数据来源: 新浪财经/东方财富, 制图："+str(today)
    footnote=footnote1+"\n"+footnote2
