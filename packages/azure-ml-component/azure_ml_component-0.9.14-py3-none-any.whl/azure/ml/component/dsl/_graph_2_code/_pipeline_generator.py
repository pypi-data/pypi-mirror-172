...
from azure.ml.component._restclients.designer.models import (
    PipelineGraph, ModuleDto, ModuleScope
)
...
class PipelineCodeGenerator(CodeBaseGenerator):
    ...
    def _get_components(self):
        for module_id in self.util.module_id_2_module.keys():
            ...
            if component_type == ModuleScope.ANONYMOUS and _is_pipeline_component(component_def):
    ...
    def get_pipeline_runsettings(self):
        ...
        for runsetting_key, runsetting_param in custom_runsettings.params.items():
            if pipeline_runsettings_map.get(runsetting_param.definition.name, None) is None:
                continue
            pipeline_runsettings_ret[runsetting_key] = runsetting_param.definition.parameter_type_in_py(
                pipeline_runsettings_map[runsetting_param.definition.name])
        # string value need to add '' for filling into the jinjia2 template
        pipeline_runsettings_ret = self.transform_pipeline_runsetting(pipeline_runsettings_ret)

        return pipeline_runsettings_ret

    def transform_pipeline_runsetting(self, input_val):
        """
        transform pipeline runsetting val for filling into the jinjia2 template.

        bool: True/False -> True/False
        int: 8 -> 8
        float: 8.6 -> 8.6
        str: 'cpu-cluster' -> "'cpu-cluster'"
        list: [True, 8, 'cpu-cluster'] -> [True, 8, "'cpu-cluster'"]
        dict: {'default_compute_target': 'cpu-cluster', 'timeout_seconds': 500} ->
              {'default_compute_target': "'cpu-cluster'", 'timeout_seconds': 500}
        """
        ...
        if isinstance(input_val, list):
            for i in range(len(input_val)):
                input_val[i] = self.transform_pipeline_runsetting(input_val[i])
        if isinstance(input_val, dict):
            for key, val in input_val.items():
                input_val[key] = self.transform_pipeline_runsetting(val)
