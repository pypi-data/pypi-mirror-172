...
class Subscriptions(BaseSubscriptions):
    ...
    def create_subscription_using_training_data(self, subscription_name, datamart_id, service_provider_id, model_info, sample_csv, spark_credentials, data_warehouse_connection, payload_table, feedback_table, scored_training_data_table, managed_by):
        ...
        if (managed_by == "self"):
            ...
            self._validate_provider_for_batch(provider)
    ...
    def create_subscription(self, subscription_name: str, datamart_id, service_provider_id, configuration_archive, spark_credentials, data_warehouse_connection, payload_table: dict = {}, feedback_table: dict = {} , model_info: dict = {}, **kwargs):
        """
        ...
        try:
            ...
            asset = Asset(
                ...
                asset_type=AssetTypes.MODEL,
                input_data_type=InputDataType.STRUCTURED,
                problem_type=get(common_configuration,"model_type")
            )
            ...
            if get(common_configuration,"model_type")!=ProblemType.REGRESSION:
    ...
    def _validate_provider_for_batch(self, provider):
        if provider.entity.service_type==ServiceTypes.WATSON_MACHINE_LEARNING:
            secret_id = provider.entity.credentials.secret_id
            token = self._ai_client.authenticator.token_manager.get_token()
            url = '{}/v2/secrets/{}'.format(self._ai_client.service_url,secret_id)
            headers = {
                'Content-type': 'application/json',
                'Authorization': "Bearer " + token
            }
            secret_info = requests.get(url, headers=headers, verify=False).json()
            if secret_info['entity']['credentials']['wml_location'] == 'cpd_local':
                raise Exception("WML local provider is not supported for batch subscription. Please create WML provider by selecting `Cloud Pak for Data` and provide username + apikey")
