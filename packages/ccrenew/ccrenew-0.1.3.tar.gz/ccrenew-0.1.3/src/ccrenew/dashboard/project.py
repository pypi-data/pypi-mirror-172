...
import ccrenew.dashboard.data_processing.Bluesky_weather_fucntions_v01 as blu
import ccrenew.dashboard.data_processing.BV_pp_deg as batt
import ccrenew.dashboard.data_processing.CCR as ccr
...
class Project(object):
    ...
    def __init__(self, proj_init_dict):
        ...
        self.pickle_jar = os.path.join(self.dashboard_dir,
                                       self.project_directory,
                                       self.project_name,
                                       'Pickle_Jar')
        """Directory to store pickle files"""

        if not os.path.isdir(self.pickle_jar):
            os.makedirs(self.pickle_jar)
    ...
    def export_summary(self, table='test'):
        try:
            engine
        except:
            engine = ccr.get_sql_engine()
        ...
        if table.lower() == 'prod':
            table = 'am_summary_data'
        else:
            table = 'am_summary_data_test'
        engine.execute("""
                       DELETE
                         FROM 
                              dev_analytic.%(table)s
                        WHERE site_name = %(site_name)s
                          AND year = %(year)s;
                        """, table=table, site_name=self.project_name, year=self.year
        )

        df_toSQL.to_sql(name=table,
                        con=engine,
                        schema='dev_analytic',
                        if_exists='append',
                        index=False,
    ...
    def save_pickle(self, year=None):
        """Saves project to a pickle

        Args:
            year (int, optional): Year to save the pickle. Defaults to current year.
        """
        if not year:
