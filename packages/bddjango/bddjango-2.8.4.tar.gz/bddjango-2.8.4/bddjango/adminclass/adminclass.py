    ...
    def import_csv(self, request):
        ...
        try:
            if request.method == "POST":
                ...
                new_verbose_names = list(field_dc.keys())
                valid_columns = [column_i in field_names or column_i in new_verbose_names for column_i in df.columns]
                ...
                for index, row in df1.iterrows():
                    ...
                    if not (self.use_original_id and set(df1.columns).intersection(set(['id', 'ID', 'pk', 'PK']))):
                        # 如果[不使用原始id 并且 表格中有`id`列],  则启用自定义id
                        dc.update({'pk': curr_id})
                        curr_id += 1

                    if self.use_bulk_create:
                        md = self.model(**dc)
                        md_ls.append(md)
                    else:
                        md_dc_ls.append(dc)
                    ...
                    # if self.use_bulk_create is False:
