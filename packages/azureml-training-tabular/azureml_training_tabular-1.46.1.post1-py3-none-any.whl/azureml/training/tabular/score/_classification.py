...
from . import _scoring_utilities, constants
from ._metric_base import ClassMetric, Metric, NonScalarMetric, ScalarMetric
...
class ClassificationMetric(Metric):
    ...
    def __init__(
        ...
        if multilabel and y_transformer:
            self._test_labels = np.array(y_transformer.inverse_transform(y_test))
            test_labels = list()
...
class Accuracy(ClassificationMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        return sklearn.metrics.accuracy_score(
            y_true=self._y_test, y_pred=self._y_pred, sample_weight=self._sample_weight
        )
...
class WeightedAccuracy(ClassificationMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        return sklearn.metrics.accuracy_score(y_true=self._y_test, y_pred=self._y_pred, sample_weight=updated_weights)
...
class BalancedAccuracy(ClassificationMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        return sklearn.metrics.recall_score(
            y_true=self._y_test, y_pred=self._y_pred, average=average_type, sample_weight=self._sample_weight
        )
...
class NormMacroRecall(ClassificationMetric, ScalarMetric):
    ...
    def _norm_macro_recall(self, y_test_bin, y_pred_proba, n_classes, sample_weight=None, **kwargs):
        ...
        cmat = sklearn.metrics.confusion_matrix(y_true=y_test_bin, y_pred=y_pred, sample_weight=sample_weight)
    ...
    def compute(self):
        ...
        if not self._multilabel:
            return sklearn.metrics.log_loss(
                y_true=self._y_test, y_pred=y_pred_proba, labels=self._class_labels, sample_weight=self._sample_weight
            )
        else:
            return sklearn.metrics.log_loss(
                y_true=self._y_test, y_pred=y_pred_proba, sample_weight=self._sample_weight
            )
...
class F1(ClassificationMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        return sklearn.metrics.f1_score(
            y_true=self._y_test, y_pred=self._y_pred, average=self._average_type, sample_weight=self._sample_weight
        )
...
class F1Binary(F1):
    ...
    def compute(self):
        ...
        if self._positive_label is None:
            ...
            return sklearn.metrics.f1_score(
                y_true=y_true, y_pred=y_pred, average=self._average_type, sample_weight=self._sample_weight
            )
...
class F1Classwise(ClassificationMetric, ClassMetric):
    ...
    def compute(self):
        """Compute the score for the metric."""
        classwise_f1_score = sklearn.metrics.f1_score(
            y_true=self._y_test,
            y_pred=self._y_pred,
...
class PrecisionClasswise(ClassificationMetric, ClassMetric):
    ...
    def compute(self):
        """Compute the score for the metric."""
        classwise_precision_score = sklearn.metrics.precision_score(
            y_true=self._y_test,
            y_pred=self._y_pred,
            average=ClassificationMetric.NO_AVERAGE,
            sample_weight=self._sample_weight,
...
class RecallClasswise(ClassificationMetric, ClassMetric):
    ...
    def compute(self):
        """Compute the score for the metric."""
        classwise_recall_score = sklearn.metrics.recall_score(
            y_true=self._y_test,
            y_pred=self._y_pred,
            average=ClassificationMetric.NO_AVERAGE,
            sample_weight=self._sample_weight,
...
class AveragePrecisionBinary(AveragePrecision):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(ClassificationMetric.NO_AVERAGE, constants.AVERAGE_PRECISION_BINARY, *args, **kwargs)
...
class AveragePrecisionMacro(AveragePrecision):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(ClassificationMetric.MACRO_AVERAGE, constants.AVERAGE_PRECISION_MACRO, *args, **kwargs)
...
class AveragePrecisionMicro(AveragePrecision):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(ClassificationMetric.MICRO_AVERAGE, constants.AVERAGE_PRECISION_MICRO, *args, **kwargs)
...
class AveragePrecisionWeighted(AveragePrecision):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(ClassificationMetric.WEIGHTED_AVERAGE, constants.AVERAGE_PRECISION_WEIGHTED, *args, **kwargs)
...
class AveragePrecisionClasswise(ClassificationMetric, ClassMetric):
    ...
    def compute(self):
        ...
        for label, average_precision_score in zip(string_labels, cast(Iterable[float], classwise_score)):
...
class AUC(ClassificationMetric, ScalarMetric):
    ...
    def _validate_one_class(self):
        ...
        is_class_averaged = self._average_type in [
...
class AUCBinary(AUC):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(ClassificationMetric.NO_AVERAGE, constants.AUC_BINARY, *args, **kwargs)
...
class AUCMacro(AUC):
    ...
    def __init__(self, *args, **kwargs):
        """Initialize AUCMacro."""
        super().__init__(ClassificationMetric.MACRO_AVERAGE, constants.AUC_MACRO, *args, **kwargs)
...
class AUCMicro(AUC):
    ...
    def __init__(self, *args, **kwargs):
        """Initialize AUCMicro."""
        super().__init__(ClassificationMetric.MICRO_AVERAGE, constants.AUC_MICRO, *args, **kwargs)
...
class AUCWeighted(AUC):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(ClassificationMetric.WEIGHTED_AVERAGE, constants.AUC_WEIGHTED, *args, **kwargs)
...
class AUCClasswise(ClassificationMetric, ClassMetric):
    ...
    def compute(self):
        ...
        if np.isscalar(classwise_score):
            classwise_score = [cast(float, classwise_score)]
        auc_roc_dict = dict()
        for label, auc_roc_score in zip(string_labels, cast(Iterable[float], classwise_score)):
...
class IOUClasswise(ClassificationMetric, ClassMetric):
    ...
    def compute(self):
        ...
        if sklearn.__version__ >= '0.21.0':
            ...
            raise ClientException(safe_message).with_generic_msg(safe_message)
...
class AccuracyTable(ClassificationMetric, NonScalarMetric):
    ...
    * The confusion values (TP, FP, TN, FN) are computed with the
    ...
    def aggregate(scores: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        data_agg = {
            AccuracyTable.PROB_TABLES: (np.sum(prob_tables, axis=0)),
            AccuracyTable.PERC_TABLES: (np.sum(perc_tables, axis=0)),
            AccuracyTable.PROB_THOLDS: (score_data[0][AccuracyTable.PROB_THOLDS]),
            AccuracyTable.PERC_THOLDS: (score_data[0][AccuracyTable.PERC_THOLDS]),
            AccuracyTable.CLASS_LABELS: (score_data[0][AccuracyTable.CLASS_LABELS]),
        }
...
class ConfusionMatrix(ClassificationMetric, NonScalarMetric):
    ...
    def _compute_matrix(self, class_labels, sample_weight=None):
        ...
        try:
            matrix = sklearn.metrics.confusion_matrix(
                y_true=y_test, y_pred=y_pred_labels, sample_weight=sample_weight, labels=class_labels
            )
        except Exception:
            debug_stats = _scoring_utilities._get_debug_stats(
                y_test, y_pred_labels, class_labels, self._y_pred_proba, sample_weight
            )
    ...
    def compute(self):
        ...
        matrix = self._compute_matrix(self._class_labels, sample_weight=self._sample_weight)
    ...
    def aggregate(scores: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        data_agg = {ConfusionMatrix.CLASS_LABELS: agg_class_labels, ConfusionMatrix.MATRIX: matrix_sum}
...
class ClassificationReport(ClassificationMetric, NonScalarMetric):
    ...
    def _compute_matrix(self):
        ...
        try:
            ...
            debug_stats = _scoring_utilities._get_debug_stats(
                self._y_test, self._y_pred, self._class_labels, self._y_pred_proba, self._sample_weight
            )
