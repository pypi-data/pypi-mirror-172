...
list_patterns = [
    ...
    re_path(r'^members/options/(?P<email>.+)$',
            list_views.list_member_options,
            name='list_member_options'),
    re_path(r'^members/(?P<role>\w+)/$',
            list_views.ListMembersViews.as_view(),
            name='list_members'),
    re_path(r'^$', list_views.ListSummaryView.as_view(),
            name='list_summary'),
    re_path(r'^subscribe$', list_views.ListSubscribeView.as_view(),
            name='list_subscribe'),
    ...
    re_path(r'^unsubscribe/$', list_views.ListUnsubscribeView.as_view(),
            name='list_unsubscribe'),
    re_path(r'^subscription_requests$',
...
urlpatterns = [
    ...
    re_path(r'^accounts/subscriptions/$',
            user_views.user_subscriptions,
            name='ps_user_profile'),
    re_path(r'^accounts/per-address-preferences/$',
    ...
    re_path(r'^accounts/per-subscription-preferences/$',
            user_views.UserSubscriptionPreferencesView.as_view(),
            name='user_subscription_preferences'),
    re_path(r'^accounts/mailmansettings/$',
            user_views.UserMailmanSettingsView.as_view(),
            name='user_mailmansettings'),
    re_path(r'^accounts/list-options/(?P<member_id>[^/]+)/$',
            user_views.UserListOptionsView.as_view(),
            name='user_list_options'),
    # /domains/
    re_path(r'^domains/$', domain_views.domain_index,
            name='domain_index'),
    re_path(r'^domains/new/$', domain_views.domain_new,
            name='domain_new'),
    re_path(r'^domains/(?P<domain>[^/]+)/$', domain_views.domain_edit,
            name='domain_edit'),
    re_path(r'^domains/(?P<domain>[^/]+)/delete$',
            domain_views.domain_delete,
            name='domain_delete'),
    re_path(r'^domains/(?P<domain>[^/]+)/owners$',
            domain_views.domain_owners,
            name='domain_owners'),
    re_path(r'^domains/(?P<domain>[^/]+)/owners/(?P<user_id>.+)/remove$',
    ...
    # /system/
    re_path(r'^system/$', system_views.system_information,
    ...
    # /api/
    re_path(r'^api/list/(?P<list_id>[^/]+)/held_message/(?P<held_id>\d+)/$',  # noqa: E501
            rest_views.get_held_message, name='rest_held_message'),
    re_path(r'^api/list/(?P<list_id>[^/]+)/held_message/(?P<held_id>\d+)/'
