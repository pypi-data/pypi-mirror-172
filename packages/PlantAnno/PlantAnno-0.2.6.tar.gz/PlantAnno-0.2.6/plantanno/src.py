...
def annocleaner_job(genome_fasta, gene_gff_file, repeat_gff_file, work_dir, mode='orthoDB', num_threads=8, nr_diamond_db=""):
    mkdir(work_dir, True)
...
def clean_dir(top_dir, keep_file_list):
    for i in os.listdir(top_dir):
        if not i in keep_file_list:
            rmdir(os.path.join(top_dir, i))