...
def plantanno_main(args):
    ...
    if args.mode == 'orthoDB':
        ...
        mkdir(ac_dir, True)
        annoclear_tsv, annoclear_pt_file, ip_tsv = annocleaner_job(genome_fasta, evm_output_gff, edta_gff_file, ac_dir, args.mode, num_threads=args.num_threads, nr_diamond_db=args.nr_diamond_db)
        ...
        # clean work_dir
        rmdir(os.path.join(args.work_dir, '2.BRAKER2', 'orthoDB.all.fasta'))
        clean_dir(os.path.join(args.work_dir, '2.BRAKER2', 'braker'), ['braker.evm.gff3', 'braker.cds.fasta', 'braker.pt.fasta'])
        clean_dir(os.path.join(args.work_dir, '3.HugeP2G'), ['hugep2g.evm.gff'])
        clean_dir(os.path.join(args.work_dir, '4.EVM'), ['EVM.all.gff3', 'EVM.pt.fasta', 'EVM.cds.fasta', 'busco'])
        clean_dir(os.path.join(args.work_dir, '5.Filter', 'orthofinder'), ['Orthogroups.tsv', 'filter_gene.pt.fasta'])
        clean_dir(os.path.join(args.work_dir, '5.Filter', 'annocleaner'), ['annoclear.output.tsv', 'annoclear.output.pt.fasta', 'raw.pt.interproscan.tsv'])
