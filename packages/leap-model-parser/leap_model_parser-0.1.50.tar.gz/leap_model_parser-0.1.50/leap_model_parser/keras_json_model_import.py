...
class KerasJsonModelImport:
    def __init__(self, custom_layers: Optional[Dict[str, Type[tf.keras.layers.Layer]]] = None, dataset_version: Optional[Dict] = None, dataset_name: Optional[str] = None) -> None:
        self.id = 1
    ...
    def _get_layer_metadata(self, layer_json: Dict) -> Optional[Dict]:
        ...
        if layer_metadata is None:
            if layer_json['class_name'] in self.custom_layers:
                layer_metadata = {
                    'type': 'UserCustomLayer',
                    'name': layer_json['class_name']
                }
    ...
    def get_connection_key(self, node_input_layer, node: NodeResponse, direction: str, index: int) -> Optional[str]:
        ...
        if _is_represntaion_block_node(node) or layer_metadata is None:
            ...
            has_dataset_version_but_no_input_is_match = self.dataset_version is not None and not any(input_data['name'] == node_input_layer['name'] for input_data in inputs)
            if has_dataset_version_but_no_input_is_match:
                return None
    ...
    def get_and_add_dataset_node_if_not_existed(self) -> NodeResponse : 
        ...
        if datasetNode is None:
            ...
            if self.dataset_version is not None:
                datasetNode.data['datasetVersion'] = self.dataset_version
                datasetNode.data['selected_dataset'] = self.dataset_name
