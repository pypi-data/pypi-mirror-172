...
from embedops_cli.embedops_authorization import get_registry_token
...
def docker_run(run_context: LocalRunContext):
    ...
    aws_token_data = get_registry_token()
    ...
    dockercmd.extend([f"-v {os.path.expanduser('~')}/.ssh:/home/ci/.ssh"])

    quoted_script = quote_str_for_platform(script)
    _docker_run_cmd_arg = f"/bin/bash -l -c -e {quoted_script}"
