...
# @generated by pegen from hgl.gram

import ast
import sys
from . import hgl_tokenize as tokenize                              # modified

from typing import Any, Optional

from .parser import memoize, memoize_left_rec, logger, Parser       # modified
...
import os
import sys
from . import hgl_token as token                                    # modified
from typing import (
    Any, Callable, Iterator, List, Literal, Tuple, TypeVar, Union, NoReturn
)

from .tokenizer import Tokenizer                                    # modified
...
def parse_string(
    ...
    parser = HGLParser(tokenizer, verbose=verbose, py_version=py_version)
...
class HGLParser(Parser):
    ...
    def function_def(self) -> Optional[Union [ast . FunctionDef , ast . AsyncFunctionDef]]:
        # function_def: '@' dec_maybe_call NAME ['(' params? ')'] &&':' block | decorators function_def_raw | function_def_raw
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    def when_stmt(self) -> Optional[ast . With]:
        # when_stmt: "when" named_expression ':' block
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    def elsewhen_stmt(self) -> Optional[ast . With]:
        # elsewhen_stmt: "elsewhen" named_expression ':' block
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    def otherwise_stmt(self) -> Optional[ast . With]:
        # otherwise_stmt: "otherwise" ':' block
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    def switch_stmt(self) -> Optional[ast . With]:
        # switch_stmt: "switch" named_expression ':' NEWLINE INDENT switch_block+ DEDENT
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    def switch_block(self) -> Optional[ast . With]:
        # switch_block: "once" star_named_expressions ':' block
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    def expression(self) -> Optional[Any]:
        # expression: invalid_legacy_expression | disjunction 'if' disjunction 'else' expression | disjunction | lambdef
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    def inversion(self) -> Optional[Any]:
        # inversion: 'not' inversion | hgl_imply
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    def factor(self) -> Optional[Any]:
        # factor: '!' factor | '>>>' factor | '+' factor | '-' factor | '~' factor | power
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    def primary(self) -> Optional[Any]:
        # primary: primary '.' NAME | primary genexp | primary '(' arguments? ')' | primary '[' slices ']' | primary hgl_unit | atom
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    def _gather_59(self) -> Optional[Any]:
        # _gather_59: with_item _loop0_60
        mark = self._mark()
        if (
            (elem := self.with_item())
            is not None
    ...
    def _gather_117(self) -> Optional[Any]:
        # _gather_117: (starred_expression | (assignment_expression | expression !':=') !'=') _loop0_118
        mark = self._mark()
        if (
            (elem := self._tmp_221())
            is not None
    ...
    def _tmp_221(self) -> Optional[Any]:
        # _tmp_221: starred_expression | (assignment_expression | expression !':=') !'='
        mark = self._mark()
        if (
    ...
    def _tmp_230(self) -> Optional[Any]:
        # _tmp_230: expression ['as' star_target]
        mark = self._mark()
        if (
            (expression := self.expression())
            and
    ...
    def _tmp_231(self) -> Optional[Any]:
        # _tmp_231: expressions ['as' star_target]
        mark = self._mark()
        if (
            (expressions := self.expressions())
            and
    ...
    def _tmp_232(self) -> Optional[Any]:
        # _tmp_232: assignment_expression | expression !':='
        mark = self._mark()
        if (
            (assignment_expression := self.assignment_expression())
        ):
    ...
    KEYWORDS = ('def', 'except', 'yield', 'return', 'global', 'None', 'del', 'elif', 'async', 'for', 'import', 'assert', 'and', 'False', 'nonlocal', 'or', 'class', 'if', 'True', 'finally', 'is', 'from', 'in', 'continue', 'as', 'while', 'raise', 'await', 'break', 'else', 'with', 'not', 'lambda', 'try', 'pass')
    SOFT_KEYWORDS = ('elsewhen', 'switch', 'once', 'when', 'otherwise', 'case', 'match', '_')


if __name__ == '__main__':
    from pegen.parser import simple_parser_main
    simple_parser_main(HGLParser)
