...
class ClearSiteData(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if self.Routes.__len__() == 0:
            raise SyntaxError(
                'Cannot Set Clear-Site-Data header if the routes are empty')
        ...
        for url in self.Routes:
            if url == request.url.components.path:
                ...
                if 'cache' in option and option['cache'] is True:
                    self.policyString += '"cache"' if list(
                        option.keys()).__len__() == 1 else '"cache", '
                    option.pop('cache')
                if 'cookies' in option and option['cookies'] is True:
                    self.policyString += '"cookies"' if list(
                        option.keys()).__len__() == 1 else '"cookies", '
                    option.pop('cookies')
                ...
                if list(option.keys()).__len__() != 0:
                    raise SyntaxError(
                        'Clear-Site-Data has 4 options 1> "cache" 2> "cookies" 3> "storage" 4> "*"')
