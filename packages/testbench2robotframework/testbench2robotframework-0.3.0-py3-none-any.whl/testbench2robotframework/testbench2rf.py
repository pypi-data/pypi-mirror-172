...
class RfTestCase:
    ...
    def _append_atomic_ia(
        ...
        if len(ia_path_parts) == 1:
            root_subdivision = "UNKNOWN"
            import_prefix = ia_path_parts[0]
        else:
            root_subdivision, import_prefix = ia_path_parts[:2]
        if root_subdivision not in self.used_imports:
            self.used_imports[root_subdivision] = {import_prefix}
        else:
            self.used_imports[root_subdivision].add(import_prefix)
...
class RobotSuiteFileBuilder:
    ...
    def _create_rf_resource_imports(self, import_dict: Dict[str, Set[str]]) -> List[ResourceImport]:
        resources = {
            ...
            if self._is_resource(resource_root)
        }
    ...
    def _is_library(self, root_subdivision: str) -> bool:
        if root_subdivision in self.config.rfLibraryRoots:
            return True
        return False

    def _is_resource(self, root_subdivision: str) -> bool:
    ...
    def _create_rf_unknown_imports(self, import_dict: Dict[str, Set[str]]) -> List[Comment]:
        unknown_imports = {
            root_subdivision: next(iter(unknown_import))
            for root_subdivision, unknown_import in import_dict.items()
