...
@login_required
@needs_mailbox()
def index(request, tplname="kalabash_sievefilters/index.html"):
    try:
...
def getfs(request, name):
    ...
    if editormode == "raw":
        htmlcontent = render_to_string(
            "kalabash_sievefilters/rawfilter.html", {
                "name": name, "scriptcontent": content
            })
    else:
        htmlcontent = render_to_string(
            "kalabash_sievefilters/guieditor.html", {"fs": content})
...
@login_required
@needs_mailbox()
def newfilter(request, setname, tplname="kalabash_sievefilters/filter.html"):
    ctx = {
        "title": _("New filter"),
        "formid": "filterform",
        "action": reverse("kalabash_sievefilters:filter_add", args=[setname]),
...
@login_required
@needs_mailbox()
def editfilter(request, setname, fname, tplname="kalabash_sievefilters/filter.html"):
    ctx = {
        "title": _("Edit filter"),
        "formid": "filterform",
        "action": reverse("kalabash_sievefilters:filter_change",
                          args=[setname, fname]),
...
def new_filters_set(request, tplname="common/generic_modal_form.html"):
    ...
    ctx = {"title": _("Create a new filters set"),
           ...
           "action_label": _("Create"),
           "action_classes": "submit",
           "action": reverse("kalabash_sievefilters:fs_add"),
           "withmenu": False,
           "withunseen": False,
           "form": FiltersSetForm()}
    return render(request, tplname, ctx)
...
def move_filter(request, setname, fname, direction):
    ...
    return ajax_response(
        request, template="kalabash_sievefilters/guieditor.html", fs=fset
    )
