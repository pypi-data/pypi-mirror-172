...
class SignatureElement(aa.BaseSerialization):
  ...
  def init_default_value(self):
    config = configurator.Configurator(
        file_priority=True, section=SECTION_SIGN)
    parent_dir = Path(__file__).parent.resolve()
    self.key = config.get_file(
        key='key', fallback=parent_dir.joinpath(DEFAULT_KEY))
    self.key_pass = None
    self.cert = config.get_file(
        key='cert', fallback=parent_dir.joinpath(DEFAULT_PEM))
    self.signjar = config.get_file(
        key='signjar', fallback=parent_dir.joinpath(DEFAULT_SIGN_JAR))
...
def run_cmd(cmd) -> bool:
  return subprocess.call(cmd, shell=True, encoding='utf-8')
...
def run_apk_signature(options: SignatureElement):
  ...
  if options.print:
    ...
    cmd = 'java {} -jar {} verify --print-certs "{}"'.format(
        DEFAULT_JAVA_OPT, options.signjar, options.input)
    run_cmd(cmd)
    return
  ...
  if not options.output:
    if options.replace:
      ...
      options.output = options.input.with_suffix(
          '.sign' + options.input.suffix)
  ...
  cmd = 'java {} -jar {} sign --key {} --cert {} {} --in "{}" --out "{}"'.format(
      DEFAULT_JAVA_OPT, options.signjar, options.key, options.cert, password, options.input, options.output)
...
def run_menu_cmd(options):
  if options.delete:
    ...
    if cmd is None:
      simple_log.fatal(
          _('不支持的 shell,无法自动调用 shell 命令: %s'), options.shell)
    else:
      try:
        subprocess.call(cmd.format(
            cmd='echo', param='supported', input=''))
      except Exception as e:
        simple_log.fatal(
            _('不存在指定 shell (\'%s\'), 如果确定存在请将它添加到环境变量中\n错误如下:\n%s'), options.shell, e)
    ...
    set_option_subcommand(main_menu, 2, _('签名文件(&P)'), cmd.format(
        cmd=DEFAULT_MENU_CMD, param='sign', input='%1'))
    set_option_subcommand(main_menu, 3, _('查看签名(&S)'), cmd_noexit.format(
        cmd=DEFAULT_MENU_CMD, param='sign --print', input='%1'))
    if options.jadx:
      set_option_subcommand(main_menu, 4, _('打开至jadx(&J)'), cmd.format(
          cmd=options.jadx, param='', input='%1'))
...
def init_arguments():
  ...
  parser.add_argument('-v', '--verbose',
                      help=_('输出更详细的日志'), action='store_true')

  sub_parser: aa.SubArgumentsAction = parser.add_argument(
      'options', action=aa.TopSubArgumentsAction, help=_('可选的子功能'))
  sub_parser.set_program_name(parser)

  cmd_sign = sub_parser.add_parser('sign', help=_(
      '签名apk,jar,aab等文件 (默认动作)'), formatter_class=aa.DefaultsHelpFormatter)
  aa.add_object_to_argument(cmd_sign, SignatureElement)
  cmd_sign.set_defaults(func=run_apk_signature)
  configurator.add_global_config_argument(
      parser, sub_parser, help=_('修改windows上下文菜单配置'), section=SECTION_SIGN)

  hash_cmd = sub_parser.add_parser('hash', help=_(
      '计算文件 hash 值'), formatter_class=aa.DefaultsHelpFormatter)
  hash_cmd.add_argument('input', nargs='+',
                        help=_('输入文件'), action=aa.FileAction)
  hash_cmd.set_defaults(func=run_file_hash_cmd)

  menu_cmd = sub_parser.add_parser('menu', help=_(
      '注册/删除右键菜单,需要管理员权限运行'), formatter_class=aa.DefaultsHelpFormatter)
  menu_cmd.add_argument('-n', '--name', help=_('右键菜单名称'), default=_('开发者工具'))
  menu_cmd.add_argument(
      '-d', '--delete', action='store_true', help=_('删除注册菜单'))
  menu_cmd.add_argument('-s', '--shell', help=_('选择使用的 shell, 根据自身系统的安装情况选择'),
                        choices=['cmd', 'pwsh', 'powershell'], default='pwsh')
  menu_cmd.add_argument(
      '--jadx', help=_('设置快速 jadx 打开 apk'), action=aa.FileAction)
