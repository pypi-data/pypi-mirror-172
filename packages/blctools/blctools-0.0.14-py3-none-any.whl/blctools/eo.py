...
def crear_bins_viento_vel(lower_val=0,upper_val=40,step=0.5,return_dict=False):
    bins = __np.arange(lower_val,upper_val,step)
    
    rangos = [tuple(bins[i:i+2]) for i in range(len(bins)-1)]
    labels = [__np.mean(x) for x in rangos]
