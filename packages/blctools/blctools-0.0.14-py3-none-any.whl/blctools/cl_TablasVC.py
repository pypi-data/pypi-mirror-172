...
class TablasVC(SQLConnector):
    ...
    def __init__(
        ...
        self.consultar_datos_basicos(mensajes = mensajes, desconectar = not cargar_incidencias)
        if cargar_incidencias:
            self.consultar_incidencias(mensajes = mensajes)
    ...
    def idsApiCammesa(self):
        if self._idsApiCammesa == {}:
            if self.checkear_conexion():
                self.consultar_datos_basicos(mensajes=True,desconectar=True)
    ...
    def __consultar(self,tablas,mensajes=False,desconectar=True):
    ...
    def consultar_datos_basicos(self,mensajes=False,desconectar=True):  
        tablas = [
            'central',
            'origen',
            'empresas',
    ...
    def consultar_incidencias(self,mensajes=False,desconectar=True):
        if mensajes:
