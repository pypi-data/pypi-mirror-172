    ...
    def scan_mouse(self):
        if self.is_disabled is False:
            ...
            if self.mouseOn and click is True and self.focus is False:
                ...
                c = 0
                while self.texte.font_object.size(self.texte.value[0:c])[0] < x - self.parent.__print_x - self.x and c <= len(
                        self):
                    c += 1
...
class Dialog(Boite):
    ...
    def __init__(self, parent, rect, **kwargs):
        ...
        if self.cadre:
            self.addObjet(
                Boite(self, (2, 2, self.width - 19, 16), couleurBord=Dialog.BORD, largeurBord=1,
                      couleurFond=Dialog.FOND),
                "title_box")
    ...
    def center(self, objet_name: [str, list[str]]):
        if isinstance(objet_name, list):
            ...
            maxi = 0
            for o in objet_name:
                if self.objet[o].__print_x < mini:
                    mini = self.objet[o].__print_x
                if self.objet[o].__print_x + self.objet[o].width > maxi:
                    maxi = self.objet[o].__print_x + self.objet[o].width
            w = maxi - mini
            decalage = (self.width - w) // 2 - mini
            for o in objet_name:
                self.objet[o].setX(decalage + self.objet[o].__print_x)
        else:
        ...
        if self.visible and self.focus:
            ...
            for o in self.objet.values():
                if type(o) == Bouton:
                    ...
                    # if o.is_disabled == False and o.collidepoint(x, y) and click is True:
                    #     o.focus = True
                    o.scan_mouse()
    ...
    def ajuste(self, margin=10):
        ...
        for o in self.objet.values():
            if o.__print_x + o.width > widthmax:
                widthmax = o.__print_x + o.width
            if o.__print_y + o.height > heightmax:
                heightmax = o.__print_y + o.height
        self.setWidth(widthmax + margin)
        self.setHeight(heightmax + margin)
        self.positionne()

    def objet_by_name(self, name: str) -> Boite:
