...
class GeneticAlgorithm:
    def __init__(
        ...
        * hyperparameter_ranges : list of dictionaries -
        Each element corresponds with a given model class.
        Any hyperparameter_ranges not included will not be tuned.
    ...
    def selection(self, X, y, test_size, seed, population, loss, top_k, higher_is_better=True):
        ...
        * higher_is_better - bool.  Default is true.  For measures like
        ...
        return sorted_score_model = sorted(
            score_model,
            key=lambda t: t[0],
            reverse=higher_is_better
        )[:top_k]
        
    def crossover_and_mutate(self, population, breeding_rate, mutation_rate, stacking_model_hyperparameters):
        '''
        ...
        '''
        p = int((breeding_rate) * 100)
        q = int((1 - breeding_rate) * 100)
        should_breed = [True for _ in range(p)]
        shouldnt_breed = [False for _ in range(q)]
        should_breed += shouldnt_breed
        model_pairs = []
        for param in stacking_model_hyperparameters:
            # to do - add parameter fuzzing
