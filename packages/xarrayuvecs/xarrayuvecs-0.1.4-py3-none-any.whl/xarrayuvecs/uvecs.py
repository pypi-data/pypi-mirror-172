...
#-----------------------------------colormap function-------------------------------------
    def calc_colormap(self,**kwargs):
        ...
        return xr.DataArray(img,dims=ll)
#--------------------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------------------
    def plotODF(self,nbr=10000,bw=0.2,projz=1,plot_cm=True,plotOT=True,angle=np.array([30.,60.]),cline=10,ax=None,xoz_plane=False,**kwargs):
        ...
        ax.tricontour(xx, yy, np.exp(weights)/np.mean(np.exp(weights)), cline, linewidths=0.5, colors='k')
        ...
        return ax,sp
#-------------------------------------------------------------------------------------------
    def plotly_ODF(self,nbr=10000,bw=0.2,plot_cm=True,plotOT=True,angle=np.array([30.,60.]),cline=10,xoz_plane=False,res=400,cf=True):
            '''
            Plot the Orienation Density Function
            ...
            w_plt=kde.score_samples(np.transpose(np.array([pef-np.pi/2.,tef-np.pi])))
            w0=np.zeros(res**2)
            w0[:]=np.nan
            w0[idd]=w_plt
            ...
            if plotOT:
                ...
                for i in list(range(3)): # Loop on the 3 eigenvalue
                    ...
                    fig.add_trace(go.Scatter(
                        x=np.array([res/2+xxv/(2**0.5)*res/2]),
                        y=np.array([res/2+yyv/(2**0.5)*res/2]),
                        mode="markers+text",
                        name='e'+str(i+1),
                        text=str(round(eigvalue[i],2)),
                        textposition="top center",
                        marker=dict(
                            color='Black',
                            size=10,
