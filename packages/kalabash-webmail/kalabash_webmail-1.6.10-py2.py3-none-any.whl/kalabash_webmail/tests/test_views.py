...
class WebmailTestCase(KalmonakTestCase):
    ...
    def test_listmailbox(self):
        ...
        url = reverse("kalabash_webmail:index")
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "alphamonak@monakmail.com", response.json()["listing"])
    ...
    def test_attachments(self):
        ...
        url = reverse("kalabash_webmail:attachment_list")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        ...
        url = reverse("kalabash_webmail:attachment_delete")
        with self.settings(MEDIA_ROOT=self.workdir):
    ...
    def test_send_mail(self):
        """Check compose form."""
        url = "{}?action=compose".format(reverse("kalabash_webmail:index"))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(
            mail.outbox[0].from_email, '"Alphamonak Solutions" <user@test.com>')

    def test_signature(self):
        """Check signature in different formats."""
        signature = "Alphamonak Solutions"
        self.user.parameters.set_value("signature", signature)
        self.user.save()

        response = self.client.get(reverse("kalabash_webmail:index"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_reply_to_email(self):
        ...
        url = "{}?action=reply&mbox=INBOX&mailid=46931".format(
            reverse("kalabash_webmail:index"))
        session = self.client.session
    ...
    def test_getmailcontent_empty_mail(self):
        ...
        url = "{}?action=reply&mbox=INBOX&mailid=33".format(
            reverse("kalabash_webmail:mailcontent_get"))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_getmailsource(self):
        ...
        url = "{}?mbox=INBOX&mailid=133872".format(
            reverse("kalabash_webmail:mailsource_get"))
        response = self.client.get(url)
