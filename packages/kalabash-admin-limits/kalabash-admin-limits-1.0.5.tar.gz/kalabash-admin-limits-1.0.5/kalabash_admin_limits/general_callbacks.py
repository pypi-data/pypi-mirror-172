...
from kalabash_admin.kalmonak_extension import PERMISSIONS as ADMIN_PERMS
...
PERMISSIONS = {
    "Resellers": (
        ...
        [["kalabash_admin", "domain", "view_domains"],
         ["kalabash_admin", "domain", "add_domain"],
         ["kalabash_admin", "domain", "change_domain"],
         ["kalabash_admin", "domain", "delete_domain"]]
    )
}
...
def display_pool_usage(user, target, currentpage):
    ...
    if len(limits) == 0:
        return []
    return [
        render_to_string("kalabash_admin_limits/poolusage.html",
                         dict(limits=limits))
    ]
