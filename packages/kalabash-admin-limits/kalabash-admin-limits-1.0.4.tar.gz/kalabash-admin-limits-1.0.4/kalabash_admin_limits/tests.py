...
from kalabash_admin.factories import populate_database
from kalabash_admin.models import Alias, Domain

from .models import LimitTemplates


class PermissionsTestCase(KalmonakTestCase):
    ...
    def test_domainadmin_deletes_reseller(self):
        ...
        self.ajax_post(reverse("kalabash_admin:account_add"), values)
        account = User.objects.get(username="reseller@test.com")
        self.clt.logout()
        self.clt.login(username="admin@test.com", password="toto")
        resp = self.ajax_post(
            reverse("kalabash_admin:account_delete", args=[account.id]),
            {}, status=403
        )
        self.assertEqual(resp, "Permission denied")


class ResourceTestCase(KalmonakTestCase):
    ...
    def _create_account(self, username, role='StandardUsers', status=200):
        values = dict(
        ...
        return self.ajax_post(
            reverse("kalabash_admin:account_add"), values, status
        )
    ...
    def _create_alias(self, email, rcpt='user@test.com', status=200):
        ...
        return self.ajax_post(
            reverse("kalabash_admin:alias_add"), values, status
        )
    ...
    def _create_domain(self, name, status=200, withtpl=False):
        ...
        return self.ajax_post(
            reverse("kalabash_admin:domain_add"), values, status
        )
    ...
    def _domain_alias_operation(self, optype, domain, name, status=200):
        ...
        for cpt, alias in enumerate(aliases):
            ...
            values[fname] = alias
        self.ajax_post(
            reverse("kalabash_admin:domain_change", args=[dom.id]),
            values, status
        )
...
class DomainAdminTestCase(ResourceTestCase):
    ...
    def test_mailboxes_limit(self):
        ...
        self._check_limit('mailboxes', 2, 2)
        self.ajax_post(
            reverse('kalabash_admin:account_delete',
    ...
    def test_aliases_limit(self):
        ...
        self._check_limit('mailbox_aliases', 2, 2)
        self.ajax_post(
            reverse('kalabash_admin:alias_delete') + '?selection=%d'
    ...
    def test_aliases_limit_through_account_form(self):
        ...
        self.ajax_post(
            reverse("kalabash_admin:account_change", args=[user.id]),
            values
        )
...
class ResellerTestCase(ResourceTestCase):
    ...
    def test_reseller_deletes_domain(self):
        ...
        self.ajax_post(
            reverse("kalabash_admin:domain_delete", args=[dom.id]),
            {}
        )
    ...
    def test_sadmin_removes_ownership(self):
        ...
        self.ajax_get(
            "{0}?domid={1}&daid={2}".format(
                reverse('kalabash_admin:permission_remove'), dom.id, self.user.id
            ), {}
        )
        self._check_limit('domains', 0, 2)
    ...
    def test_allocate_from_pool(self):
        ...
        # reseller's pool
        self.ajax_post(
            reverse("kalabash_admin:account_delete", args=[user.id]),
            {}
        )
        self._check_limit('mailboxes', 0, 2)
        self._check_limit('mailbox_aliases', 0, 2)
    ...
    def test_change_role(self):
        ...
        # to the reseller.
        values = {
            'username': user.username, 'role': 'StandardUsers', 'quota_act': True,
            'is_active': user.is_active, 'email': user.email,
        }
        self.ajax_post(
            reverse("kalabash_admin:account_change", args=[user.id]),
