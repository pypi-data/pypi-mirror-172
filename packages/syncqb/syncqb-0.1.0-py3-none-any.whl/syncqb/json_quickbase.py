...
class MissingValueException(Exception):
    ...
    def __init__(self, message="Missing value") -> None:
...
class Client(object):
    ...
    def __init__(self, username=None, password=None, base_url='https://www.quickbase.com',
                 timeout=90, database=None, realmhost=None, ticket=None, user_token=None):
        ...
        if user_token is not None:
            ...
            try:
                ...
                print("You must provide some credentials, preferably a user token.")
                quit()
    ...
    def authenticate(self, password, hours):
        ...
        if error_code == 0:
            ...
            return int(xml_dict.get("errcode")), xml_dict.get("error")

    def request(self, action, database, data, required=None):
        ...
        # self.headers.update({'encoding': 'UTF-8'})
        # self.headers.update({'msInUTC': '1'})
        ...
        while True:
            try:
                ...
                response = requests.post(
                    url, json={"to": database,
                               "data": data}, headers=self.headers, timeout=None)
    ...
    def get_schema(self, database=None, include_permissions=False):
        ...
        params = {
            'tableId': database,
            'includeFieldPerms': include_permissions
        }
    ...
    def do_query(self, query=None, qid=None, qname=None, columns=None, sort=None,
                 structured=True, num=None, skip=None, ascending=True,
                 database=None, round_ints=False):
        ...
        if query:
            ...
            try:
                ...
                res_json['data'] = self.fix_nums(res_json['data'])
                if sort:
                    res_json = self.sort_response_by_fid(
                        res_json, sort, not ascending)
        ...
        if qid and columns is None:
            try:
                ...
                response = self.run_report(
                    url, database=database, skip=skip, max=num)
            ...
            try:
                ...
                response['data'] = self.fix_nums(response['data'])

                if sort:
                    response = self.sort_response_by_fid(response, sort)
                ...
                for record in records:
                    ...
                    for field in columns:
                        record_dict[str(field)] = record[str(field)]['value']
    ...
    def run_report(self, url=None, database=None, skip=None, max=None):
    ...
    def sort_response_by_fid(self, response, sort, reverse=False) -> Dict[str, Any]:
        """sorts response (json) by the fields provided (sort should be list of fid's)"""
        for fid in sort:
            response['data'] = sorted(
                response['data'], key=lambda record: record[str(fid)]['value'], reverse=reverse)
    ...
    def add_record(self, fields, database=None, uploads=None, safemode=False):
        """
        ...
        response = self.request(
            action='add', database=database or self.database, data=[record])
    ...
    def purge_records(self, database, query=None, rids: List = None):
        ...
        records = ""
        if rids:
            "OR".join(["{3.EX.%s}" % rid for rid in rids])
        elif query:
            records = query
        else:
        ...
        return res_json.get("numberDeleted")

    def get_file(self, url=None, database=None, record=None, field=None, version=1, user_agent=None):
        """
        ...
        user_agent: optional string, not entirely sure what this is for, but the api call has it
        """
    ...
    def edit_record(self, rid=None, key=None, fields=None, database=None, uploads=None):
        """
    ...
    def change_record_owner(self, rid, database, new_owner):
        ...
        return "Sucessfully changed record owner."
