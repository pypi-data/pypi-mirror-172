...
class Mis:
    ...
    def netsale_tax_cogs(self,Gross,Returns,stores,fofo_tag = 'no'):
        ...
        if fofo_tag=='no':
            ...
            gross_returns = pd.merge(left=gross_sale_summary, right=returns_summary,
                                     how='left', on=['store_id_', 'type1_', 'order_source_'])
        ...
        if fofo_tag == 'yes':
            ...
            gross_returns = pd.merge(left=gross_sale_summary, right=returns_summary,
                                     how='left', on=['store_id_', 'type1_', 'order_source_','fofo_distributor_'])
    ...
    def inventory_ageing(self, Inventory, stores, mis_tag = 'breakup',fofo_tag = 'no'):
        ...
        inventory_data['days'] = (pd.to_datetime('today') - inventory_data['created_at']).dt.days
        conditions = [
    ...
    def near_expiry(self,Inventory,stores,mis_tag = 'breakup',fofo_tag = 'no'):
        ...
        inventory_data['days_to_expiry'] = (pd.to_datetime('today') - inventory_data['expiry_date']).dt.days
    ...
    def total_bills_chronic_acute(self, Sales, Customer_returns, Stores, fofo_tag = 'no'):
        ...
        if fofo_tag=='no':
            ...
            df_a_b = pd.merge(left=df_a1a, right=df_b2,
                              how='left', on=['store_id_', 'order_source_', 'type1_', 'category_'])
            df_a_b.fillna(0, inplace=True)
            ...
            df_a_b = pd.merge(left=df_a1a, right=df_b2,
                              how='left', on=['store_id_', 'order_source_', 'fofo_distributor_', 'type1_', 'category_'])
            df_a_b.fillna(0, inplace=True)
    ...
    def purchase_from_worckell(self, Purchase_from_workcell_data, Stores,mis_tag,fofo_tag = 'no'):
    ...
    def inventory_6to12months(self, Inventory, Stores, mis_tag = 'breakup'):
        ...
        df_dd['days'] = (pd.to_datetime('today') - df_dd['created_at']).dt.days
        conditions = [
