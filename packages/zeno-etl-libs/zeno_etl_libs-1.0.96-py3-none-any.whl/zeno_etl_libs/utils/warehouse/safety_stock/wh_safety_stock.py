...
def lead_time():
    ...
    sql_bhw = '''
           SELECT * FROM (SELECT
            ...
            sp.Qty as "quantity" ,
            DATEDIFF(day, s.PBillDt , s.UpdatedOn) as "lead_time"
            FROM
            SalePurchase2 sp
            left join Item i on
            sp.Itemc = i.code
            ...
            and i.Barcode NOT LIKE '%[^0-9]%' and isnumeric(i.Barcode) = 1
            and a.code NOT IN  (59468,59489)) a 
            Where (a."lead_time">0 and  a."lead_time"<7)
    '''
    ...
    sql_tepl = '''                  
       SELECT * FROM (SELECT
               ...
               and  sp.Vdt <=cast(DATEADD(day, -1, GETDATE()) as date)
               and i.Compname NOT IN ('GOODAID', 'PURE & C')
               and i.Barcode NOT LIKE '%[^0-9]%' and isnumeric(i.Barcode) = 1) b
               Where (b."lead_time">0 and  b."lead_time"<7);
       '''
