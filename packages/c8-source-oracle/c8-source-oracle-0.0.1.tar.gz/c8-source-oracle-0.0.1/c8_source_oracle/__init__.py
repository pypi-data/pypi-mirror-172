...
from c8connector import C8Connector, ConfigProperty
import c8_source_oracle.db as orc_db
import c8_source_oracle.sync_strategies.log_miner as log_miner
import c8_source_oracle.sync_strategies.full_table as full_table
import c8_source_oracle.sync_strategies.incremental as incremental
import c8_source_oracle.sync_strategies.common as common
...
def main_impl():
   ...
   conn_config = {'user': args.config['user'],
                  ...
                  'scn_window_size': args.config.get('scn_window_size'), }
   ...
   if args.discover:
      ...
      filter_schemas = []
      if filter_schemas_prop:
         filter_schemas = filter_schemas_prop.split(',')
...
class OracleSourceConnector(C8Connector):
   ...
   def package_name(self) -> str:
      ...
      return "c8-source-oracle"

   def version(self) -> str:
      """Returns the version of the connector."""
      return pkg_resources.get_distribution('c8_source_oracle').version
   ...
   def description(self) -> str:
      ...
      return "GDN data connector source for OracleDB"
   ...
   def config(self) -> list[ConfigProperty]:
      ...
      return [
         ...
         ConfigProperty('default_replication_method', 'string', True),
         ConfigProperty('filter_schemas', 'string', False),
         ConfigProperty('service_name', 'string', False),
         ConfigProperty('pdb_name', 'string', False),
         ConfigProperty('multitenant', 'boolean', False),
         ConfigProperty('scn_window_size', 'integer', False),
      ]
