...
from Shared.certoraUtils import safe_create_dir, remove_file, normalize_double_paths
from Shared.certoraUtils import is_hex, decimal_str_to_cvt_compatible, hex_str_to_cvt_compatible, mode_has_spec_file
from Shared.certoraUtils import print_completion_message, run_local_spec_check, CertoraUserInputError
from Shared.certoraUtils import read_json_file, replace_file_name, PACKAGE_FILE, run_solc_cmd
from Shared.certoraUtils import get_closest_strings, flatten_set_list, get_certora_sources_dir
...
class InputConfig:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.files = sorted(list(args.file_paths))
        self.solc = args.solc
        ...
        self.prototypes = self.handle_prototypes(args)

    def __str__(self) -> str:
        return str(self.__dict__)

    @staticmethod
    def handle_prototypes(arg: argparse.Namespace) -> Dict[str, List[str]]:
        to_ret: Dict[str, List[str]] = dict()
        if arg.prototype is None:
            return to_ret
        for proto in arg.prototype:
            (sig, nm) = proto.split("=")
            if nm not in to_ret:
                to_ret[nm] = []
...
class SDC:
    ...
    def sources_as_absolute(self) -> Set[Path]:
        return set(map(lambda x: Path(x).resolve(), self.original_srclist.values()))
...
def convert_pathname_to_posix(json_dict: Dict[str, Any], entry: str) -> None:
    """
...
class CertoraBuildGenerator:
    ...
    def get_standard_json_data(self, sdc_name: str, smart_contract_lang: CompilerLang) -> Dict[str, Any]:
        json_file = smart_contract_lang.get_standard_json_filename(sdc_name, self.config_path)
    ...
    def cleanup_standard_json_data(self, sdc_name: str, smart_contract_lang: CompilerLang) -> None:
        json_file = smart_contract_lang.get_standard_json_filename(sdc_name, self.config_path)
    ...
    def collect_for_file(self,
        ...
        if not is_vyper:
            ...
            if self.input_config.packages is not None:
                ...
                paths_for_remappings = list(map(lambda remap: f'"{remap.split("=")[1]}"', remappings))
        ...
        if not LoggingManager().is_debugging:
            self.cleanup_standard_json_data(sdc_name, smart_contract_lang)
        ...
        while contracts_to_add_dependencies_queue:
            contract_file = str(contracts_to_add_dependencies_queue.pop())
                ...
                for contract_name in contract_list:
                    ...
                    for c_file, _, _ in base_contracts:
                        ...
                        if norm_c_file not in seen_link_refs:
                            ...
                            contracts_to_add_dependencies_queue.add(norm_c_file)
                    ...
                    lib_link_refs = contract_object["evm"]["deployedBytecode"]["linkReferences"]
                    for lib_link_ref in lib_link_refs:  # linkReference is a library reference
                        ...
                        if norm_link_ref not in seen_link_refs:
                            ...
                            contracts_to_add_dependencies_queue.add(norm_link_ref)
                    ...
                    for ref in set(internal_refs):
                        ...
                        if len(set(contract_files_resolved)) != 1:
                            ...
                            if internal_link_ref not in seen_link_refs:
                                ...
                                contracts_to_add_dependencies_queue.add(internal_link_ref)
    ...
    def build(self, args: argparse.Namespace) -> None:
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            ...
            if compiler_lang == CompilerLangSol():
                added_finders_to_sdc = self.instrument_auto_finders(build_arg_contract_file, i,
                                                                    sdc_pre_finders)  # type: List[Tuple[Dict, SDC]]
    ...
    def instrument_auto_finders(self, build_arg_contract_file: str, i: int, sdc_pre_finders: List[SDC]) -> \
        ...
        try:
            new_sdcs = self.collect_for_file(new_file, i, get_compiler_lang(build_arg_contract_file), None,
    ...
    def to_autofinder_file(contract_file: str) -> str:
        return replace_file_name(contract_file, f"{AUTO_FINDER_PREFIX}{Path(contract_file).name}")
...
# The sources that are collected for the .certora_sources directory are all file that are provided as input (i.e. they
# are not generated during the certora build process) that are needed for precise rerunning certoraRun.
#
# Including:
#
...
def collect_sources(generator: CertoraBuildGenerator, verifier: CertoraVerifyGenerator, args: argparse.Namespace)\
        -> Set[Path]:
    sources = generator.all_contract_files
    sources |= verifier.get_spec_files()
    if PACKAGE_FILE.exists():
        sources.add(PACKAGE_FILE)
    if args.mode == Mode.BYTECODE:
        for bytecode_resource in args.bytecode_jsons + [args.bytecode_spec]:
...
def build_source_tree(sources: Set[Path]) -> None:
    shutil.rmtree(get_certora_sources_dir(), ignore_errors=True)
    ...
    # By getting the relative paths of all the sources the original directory structure can be copied to a new location
    # In order to be able to rerun the cetoraRun, also the current working directory should be mapped
    # that is why CWD is added to the list of sources

    common_path = Path(os.path.commonpath(list(sources.union({Path(os.getcwd())}))))

    for source_file in sources:
        # copy file to the path of the file from the common root under the sources directory
        target_file = get_certora_sources_dir() / source_file.relative_to(common_path)
        ...
        try:
            ...
            build_logger.debug(f"Couldn't copy {source_file} to {target_file}", exc_info=e)


def build(args: argparse.Namespace, ignore_spec_syntax_check: bool = False) -> None:
    """
    ...
    try:
        ...
        if mode_has_spec_file(args.mode) and not args.build_only and not ignore_spec_syntax_check:
            if args.disableLocalTypeChecking:
                ...
                if spec_check_exit_code != 0:
                    raise RuntimeError("Local specification checking failed")
        ...
        # Build sources tree
        sources = collect_sources(certora_build_generator, certora_verify_generator, args)
        build_source_tree(sources)
        try:
            build_source_tree(sources)
