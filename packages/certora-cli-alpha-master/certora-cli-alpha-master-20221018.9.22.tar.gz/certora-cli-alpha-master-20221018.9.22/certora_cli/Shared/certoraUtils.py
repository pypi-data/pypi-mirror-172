...
DEV_PACKAGE_NAME = "certora-cli-alpha-master"
...
def reset_certora_internal_dir(build_dir_str: Optional[str]) -> None:
    """
    ...
    if build_dir_str is None:
        build_uuid = f"{datetime.now().strftime('%y_%m_%d_%H_%M_%S')}_{uuid.uuid4().hex.upper()[0:3]}"
...
def get_package_and_version() -> Tuple[str, str]:
    ...
    if distribution is None:
        distribution = pkg_resources.working_set.by_key.get(DEV_PACKAGE_NAME, None)  # type: ignore
