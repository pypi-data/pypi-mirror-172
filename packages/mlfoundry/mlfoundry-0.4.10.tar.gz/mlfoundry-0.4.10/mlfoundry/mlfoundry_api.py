...
def _init_session(tracking_uri: str, api_key: Optional[str]) -> UserInfo:
    rest_store = get_rest_store(tracking_uri)
    auth_service = AuthService(auth_server_info=rest_store.get_auth_server_info())
    session = Session(auth_service=auth_service, tracking_uri=tracking_uri)
    return session.init_session(api_key)


def init_rest_tracking(tracking_uri: str):
    """init_rest_tracking.
...
def get_client(
    ...
    if tracking_uri.startswith("file:"):
        ...
        monitoring_uri = append_monitoring_path_to_rest_tracking_uri(tracking_uri)
        tracking_uri = append_path_to_rest_tracking_uri(tracking_uri)
        user_info = _init_session(tracking_uri, api_key)
        init_rest_tracking(tracking_uri=tracking_uri)
    ...
    return MlFoundry(tracking_uri=tracking_uri, monitoring_uri=monitoring_uri)
...
class MlFoundry:
    ...
    def __init__(
        ...
        if monitoring_uri:
            self.monitoring_client = MonitoringClient(monitoring_uri=monitoring_uri)
