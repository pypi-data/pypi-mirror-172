...
# Builds CLI only rpm
CLI_RPM_BASE="chinilla-blockchain-cli-$CHINILLA_INSTALLER_VERSION-1.$REDHAT_PLATFORM"
mkdir -p "dist/$CLI_RPM_BASE/opt/chinilla"
mkdir -p "dist/$CLI_RPM_BASE/usr/bin"
cp -r dist/daemon/* "dist/$CLI_RPM_BASE/opt/chinilla/"
ln -s ../../opt/chinilla/chinilla "dist/$CLI_RPM_BASE/usr/bin/chinilla"
...
fpm -s dir -t rpm \
  ...
  --name chinilla-blockchain-cli \
  --license Apache-2.0 \
  --version "$CHINILLA_INSTALLER_VERSION" \
  --architecture "$REDHAT_PLATFORM" \
  --description "Chinilla is a modern cryptocurrency built from scratch, designed to be efficient, decentralized, and secure." \
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create chinilla-blockchain-$CHINILLA_INSTALLER_VERSION.rpm"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Chinilla/chinilla-blockchain/issues/3846
