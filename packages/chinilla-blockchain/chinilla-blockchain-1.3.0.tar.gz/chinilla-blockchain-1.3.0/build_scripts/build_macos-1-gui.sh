...
PATH=$(npm bin):$PATH
cd ../../ || exit
git submodule update --init chinilla-blockchain-gui
...
# Other than `chinilla-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
echo "Remove unused @chinilla packages to make cache slim"
ls -l packages
rm -rf packages/api
...
# Remove `packages/gui/node_modules/@chinilla` because it causes an error on later `electron-packager` command
rm -rf "@chinilla"
