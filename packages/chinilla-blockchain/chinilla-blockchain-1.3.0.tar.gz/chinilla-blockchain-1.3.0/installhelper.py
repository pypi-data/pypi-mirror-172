...
# Gets the version from the result of "chinilla version"
...
def get_chinilla_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["chinilla", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_chinilla_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/chinilla-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/chinilla-blockchain-gui/packages/gui/package.json")
