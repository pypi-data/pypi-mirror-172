...
from chinilla.simulator.time_out_assert import time_out_assert, time_out_assert_custom_interval
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment):
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "hcx"), encode_puzzle_hash(observer_pool, "hcx")
    )
    ...
    assert config["farmer"]["hcx_target_address"] == encode_puzzle_hash(observer_farmer, "hcx")
    assert config["pool"]["hcx_target_address"] == encode_puzzle_hash(observer_pool, "hcx")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "hcx")
