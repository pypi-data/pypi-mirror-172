...
@pytest.mark.asyncio
async def test_help(chinilla_root: ChinillaRoot) -> None:
    """Just a trivial test to make sure the subprocessing is at least working and the
    data executable does run.
    """
    completed_process = chinilla_root.run(args=["data", "--help"])
    assert "Show this message and exit" in completed_process.stdout


@pytest.mark.xfail(strict=True)
@pytest.mark.asyncio
def test_round_trip(chinilla_root: ChinillaRoot, chinilla_daemon: None, chinilla_data: None) -> None:
    """Create a table, insert a row, get the row by its hash."""

    with chinilla_root.print_log_after():
        create = chinilla_root.run(args=["data", "create_data_store"])
