...
from chinilla.server.start_farmer import create_farmer_service
from chinilla.server.start_full_node import create_full_node_service
from chinilla.server.start_harvester import create_harvester_service
from chinilla.server.start_introducer import create_introducer_service
from chinilla.server.start_timelord import create_timelord_service
from chinilla.server.start_wallet import create_wallet_service
from chinilla.simulator.block_tools import BlockTools
from chinilla.simulator.start_simulator import create_full_node_simulator_service
...
async def setup_wallet_node(
    ...
    with TempKeyring(populate=True) as keychain:
        ...
        service_config["hcx_spam_amount"] = hcx_spam_amount
...
async def setup_farmer(
    ...
    service_config["hcx_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "hcx")
    service_config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    service_config["port"] = port
    service_config["rpc_port"] = uint16(0)
    config_pool["hcx_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "hcx")
