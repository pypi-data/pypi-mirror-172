...
from chinilla.util.ints import uint64, uint32, uint8, uint128
...
from chinilla.wallet.did_wallet.did_info import DIDInfo
...
from chinilla.wallet.did_wallet import did_wallet_puzzles
from chinilla.wallet.derive_keys import master_sk_to_wallet_sk_unhardened
...
class DIDWallet:
    ...
    async def coin_added(self, coin: Coin, _: uint32, peer: WSChinillaConnection):
    ...
    async def load_parent(self, did_info: DIDInfo):
        ...
        peer: WSChinillaConnection = wallet_node.get_full_node_peer()
        if peer is None:
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            chinilla_tx = await self.create_tandem_hcx_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
        ...
        if chinilla_tx is not None and chinilla_tx.spend_bundle is not None:
            ...
            await self.wallet_state_manager.add_pending_transaction(chinilla_tx)
