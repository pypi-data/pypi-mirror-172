...
from chinilla.wallet.trading.offer import OFFER_MOD, NotarizedPayment, Offer
...
from chinilla.wallet.util.debug_spend_bundle import disassemble
...
class NFTWallet:
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            chinilla_tx = await self.create_tandem_hcx_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
    ...
    async def make_nft1_offer(
        ...
        if offering_nft:  # if we are offering an NFT.
            ...
            if requested_asset_id is None:  # If we are just asking for hcx.
            ...
            if offered_asset_id is None:  # hcx offer
