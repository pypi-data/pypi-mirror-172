...
from chinilla.util.bech32m import bech32_decode, bech32_encode, convertbits
...
class Offer:
    ...
    def notarize_payments(
        requested_payments: Dict[Optional[bytes32], List[Payment]],  # `None` means you are requesting HCX
        coins: List[Coin],
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "hcx" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
