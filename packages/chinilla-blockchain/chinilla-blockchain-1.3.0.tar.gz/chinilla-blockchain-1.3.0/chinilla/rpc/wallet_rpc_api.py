...
from chinilla.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from chinilla.rpc.rpc_server import Endpoint, EndpointResult
...
from chinilla.util.errors import KeychainIsLocked
from chinilla.util.ints import uint8, uint32, uint64, uint16
from chinilla.util.keychain import bytes_to_mnemonic, generate_mnemonic
...
from chinilla.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from chinilla.wallet.nft_wallet.nft_wallet import NFTWallet, NFTCoinInfo
...
from chinilla.wallet.util.wallet_types import AmountWithPuzzlehash, WalletType
...
class WalletRpcApi:
    ...
    async def create_new_wallet(self, request: Dict) -> EndpointResult:
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from chinilla.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "hcx")
        ...
        key = None
        if asset_id is not None and asset_id != "hcx":
