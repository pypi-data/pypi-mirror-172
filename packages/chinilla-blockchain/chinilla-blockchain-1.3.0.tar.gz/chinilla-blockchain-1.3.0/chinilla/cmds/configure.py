...
from chinilla.util.config import load_defaults_for_missing_services, lock_and_load_config, save_config, str2bool
    ...
    with lock_and_load_config(root_path, config_yaml, fill_missing_services=True) as config:
        ...
        if testnet:
            if testnet == "true" or testnet == "t":
                ...
                if "seeder" in config:
                    config["seeder"]["port"] = int(vanillanet_port)
                    config["seeder"]["other_peers_port"] = int(vanillanet_port)
