...
from chinilla.util.misc import format_minutes
from chinilla.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total chinilla farmed: {amounts['farmed_amount'] / units['chinilla']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['chinilla']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['chinilla']}")
    ...
    if amounts is None:
        ...
        print("Note: log into your key using 'chinilla wallet show' to see rewards for each key")
