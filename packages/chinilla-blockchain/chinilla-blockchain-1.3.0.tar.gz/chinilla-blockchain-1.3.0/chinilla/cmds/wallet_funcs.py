...
from chinilla.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
from chinilla.util.config import load_config, selected_network_address_prefix
from chinilla.util.default_root import DEFAULT_ROOT_PATH
from chinilla.util.ints import uint32, uint64
from chinilla.wallet.nft_wallet.nft_info import NFTInfo
...
from chinilla.wallet.util.address_type import AddressType, ensure_valid_address
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if typ == WalletType.STANDARD_WALLET:
        ...
        final_min_coin_amount = uint64(int(min_coin_amount * units["chinilla"]))
...
async def take_offer(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    from chinilla.util.bech32m import bech32_decode, convertbits
    from chinilla.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    if nft_coin_id is not None and nft_royalty_percentage > 0:
        ...
        if nft_royalty_asset_id == "hcx":
            nft_royalty_currency = "HCX"
        else:
            result = await cat_name_resolver(bytes32.fromhex(nft_royalty_asset_id))
            if result is not None:
                nft_royalty_currency = result[1]

        nft_royalty_divisor = units["chinilla"] if nft_royalty_asset_id == "hcx" else units["cat"]
...
async def mint_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    royalty_address = (
        ...
        else ensure_valid_address(args["royalty_address"], allowed_types={AddressType.HCX}, config=config)
    )
    target_address = (
        None
        if not args["target_address"]
        else ensure_valid_address(args["target_address"], allowed_types={AddressType.HCX}, config=config)
    )
...
async def transfer_nft(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    try:
        ...
        nft_coin_id = args["nft_coin_id"]
        config = load_config(DEFAULT_ROOT_PATH, "config.yaml")
        target_address = ensure_valid_address(args["target_address"], allowed_types={AddressType.HCX}, config=config)
        fee: int = int(Decimal(args["fee"]) * units["chinilla"])
