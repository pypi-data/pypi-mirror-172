...
# the new passphrase.
DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE = "$ chinilla passphrase set # all the cool kids are doing it!"

MASTER_PASSPHRASE_SERVICE_NAME = "Chinilla Passphrase"
MASTER_PASSPHRASE_USER_NAME = "Chinilla Passphrase"
...
def check_legacy_keyring_keys_present(keyring: LegacyKeyring) -> bool:
    ...
    from chinilla.util.keychain import default_keychain_user, default_keychain_service, get_private_key_user, MAX_KEYS
...
class KeyringWrapper:
    ...
    def __init__(self, keys_root_path: Path = DEFAULT_KEYS_ROOT_PATH, force_legacy: bool = False):
        ...
        """
        from chinilla.util.errors import KeychainNotSet
    ...
    def _get_initial_cached_passphrase(self) -> str:
        ...
        """
        from chinilla.util.keychain import supports_os_passphrase_storage
    ...
    def set_master_passphrase(
        ...
        *,
        ...
        """
        from chinilla.util.errors import KeychainRequiresMigration, KeychainCurrentPassphraseIsInvalid
        from chinilla.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            if response:
                from chinilla.cmds.passphrase_funcs import prompt_for_new_passphrase
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from chinilla.util.keychain import get_private_key_user, Keychain, MAX_KEYS
    ...
    async def migrate_legacy_keyring_interactive(self) -> bool:
        ...
        """
        from chinilla.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running

        # Let the user know about the migration.
        if not self.confirm_migration():
            print("Migration aborted, can't run any chinilla commands.")
            return False
