...
class ViewsTestCase(TestDataMixin, KalmonakTestCase):
    """Test views."""
    ...
    def test_index(self):
        """Test index view."""
        url = reverse("kalabash_amavis:index")
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

        url = reverse("kalabash_amavis:_mail_list")
    ...
    def test_viewmail(self):
        ...
        url = reverse("kalabash_amavis:mail_detail", args=[mail_id])
        url = "{}?rcpt={}".format(url, smart_text(self.msgrcpt.rid.email))
        response = self.ajax_get(url)
        self.assertIn("menu", response)
        content_url = reverse("kalabash_amavis:mailcontent_get", args=[mail_id])
    ...
    def test_viewmail_selfservice(self):
        ...
        url = reverse("kalabash_amavis:mailcontent_get", args=[mail_id])
        url = "{}?secret_id={}".format(
            url, smart_text(self.msgrcpt.mail.secret_id))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_viewheaders(self):
        ...
        mail_id = smart_text(self.msgrcpt.mail.mail_id)
        url = reverse("kalabash_amavis:headers_detail", args=[mail_id])
        response = self.client.get(url)
    ...
    def test_delete_msg(self):
        ...
        mail_id = smart_text(self.msgrcpt.mail.mail_id)
        url = reverse("kalabash_amavis:mail_delete", args=[mail_id])
    ...
    def test_release(self, mock_socket):
        ...
        mail_id = smart_text(self.msgrcpt.mail.mail_id)
        url = reverse("kalabash_amavis:mail_release", args=[mail_id])
        data = {"rcpt": smart_text(self.msgrcpt.rid.email)}
        response = self.ajax_post(url, data)
        self.msgrcpt.refresh_from_db()
    ...
    def test_release_selfservice(self, mock_socket):
        ...
        self.client.logout()
        mail_id = smart_text(self.msgrcpt.mail.mail_id)
        base_url = reverse("kalabash_amavis:mail_release", args=[mail_id])
        self.set_global_parameter("self_service", True)
        # Missing rcpt -> fails
    ...
    def _test_mark_message(self, action, status):
        ...
        mail_id = smart_text(self.msgrcpt.mail.mail_id)
        url = reverse("kalabash_amavis:mail_mark_as_" + action, args=[mail_id])
        data = {"rcpt": smart_text(self.msgrcpt.rid.email)}
        response = self.ajax_post(url, data)
        self.assertEqual(
    ...
    def test_mark_as_ham(self):
        ...
        url = reverse("kalabash_amavis:learning_recipient_set")
    ...
    def test_manual_learning_as_user(self):
        ...
        self.client.force_login(user)
        mail_id = smart_text(self.msgrcpt.mail.mail_id)
        url = reverse("kalabash_amavis:mail_mark_as_ham", args=[mail_id])
        data = {"rcpt": smart_text(self.msgrcpt.rid.email)}
        response = self.ajax_post(url, data)
    ...
    def test_process(self, mock_socket):
        ...
        url = reverse("kalabash_amavis:mail_process")
        selection = [
            "{} {}".format(
                smart_text(self.msgrcpt.rid.email),
                smart_text(self.msgrcpt.mail.mail_id)),
