...
def viewm_menu(user, mail_id, rcpt):
    ...
    entries = [
        ...
        {"name": "release",
         ...
         "class": "btn-success",
         "url": (
             reverse("kalabash_amavis:mail_release", args=[mail_id]) +
        ...
        {"name": "delete",
         ...
         "img": "fa fa-trash",
         "url": (
             reverse("kalabash_amavis:mail_delete", args=[mail_id]) +
             ("?rcpt=%s" % rcpt if rcpt else "")),
         "label": _("Delete")},
        {"name": "headers",
         "class": "btn-default",
        ...
        entries.insert(3, {
            ...
            "menu": [
                {"name": "mark-as-spam",
                 ...
                 "url": reverse(
                     "kalabash_amavis:mail_mark_as_spam", args=[mail_id]
                 ) + ("?rcpt=%s" % rcpt if rcpt else ""),
                 "extra_attributes": {
                ...
                {"name": "mark-as-ham",
                 ...
                 "url": reverse(
                     "kalabash_amavis:mail_mark_as_ham", args=[mail_id]
                 ) + ("?rcpt=%s" % rcpt if rcpt else ""),
                 "extra_attributes": {
                     "data-mail-id": mail_id
                 }}
...
def viewm_menu_simple(user, mail_id, rcpt, secret_id=""):
    release_url = "{0}?rcpt={1}".format(
        reverse("kalabash_amavis:mail_release", args=[mail_id]), rcpt)
    delete_url = "{0}?rcpt={1}".format(
        reverse("kalabash_amavis:mail_delete", args=[mail_id]), rcpt)
    if secret_id:
...
def quar_menu(user):
    ...
    return render_to_string("kalabash_amavis/main_action_bar.html", {
        "extraopts": extraopts,
