...
from kalabash.admin.models import Domain, Mailbox
from kalabash.lib.exceptions import BadRequest
from kalabash.lib.paginator import Paginator
from kalabash.lib.web_utils import getctx, render_to_json_response
...
def empty_quarantine():
    ...
    content = loader.render_to_string(
        "kalabash_amavis/empty_quarantine.html", {
...
def listing_page(request):
    ...
    if context is None:
        ...
        context["rows"] = loader.render_to_string(
            "kalabash_amavis/emails_page.html", {
...
def _listing(request):
    ...
    """
    if not request.user.is_superuser and request.user.role != "StandardUsers":
    ...
    context["listing"] = loader.render_to_string(
        "kalabash_amavis/email_list.html", {
            "email_list": context["rows"],
            "message_types": constants.MESSAGE_TYPES
        }, request
    )
...
def index(request):
    ...
    conf = dict(param_tools.get_global_parameters("kalabash_amavis"))
    ...
    return render(request, "kalabash_amavis/index.html", context)
...
def viewmail_selfservice(request, mail_id,
                         tplname="kalabash_amavis/viewmail_selfservice.html"):
...
def viewmail(request, mail_id):
    ...
    if request.user.email == rcpt:
        ...
        if rcpt == mb.full_address or rcpt in mb.alias_addresses:
            SQLconnector().set_msgrcpt_status(rcpt, mail_id, "V")
    content = loader.get_template("kalabash_amavis/_email_display.html").render(
        {"mail_id": mail_id})
...
def viewheaders(request, mail_id):
    ...
    return render(request, "kalabash_amavis/viewheader.html", context)
...
def release(request, mail_id):
    ...
    if request.user.role == "StandardUsers" and \
...
def mark_messages(request, selection, mtype, recipient_db=None):
    ...
    if recipient_db is None:
        recipient_db = (
            "user" if request.user.role == "StandardUsers" else "global"
        )
...
def learning_recipient(request):
    ...
    return render(request, "common/generic_modal_form.html", {
        ...
        "action": reverse("kalabash_amavis:learning_recipient_set"),
...
def process(request):
    ...
    if not ids or action is None:
        return HttpResponseRedirect(reverse("kalabash_amavis:index"))
