...
'''.join([*([f"{sub_key}={getattr(self, sub_key)}"
        ...
        def _create_config(config: dict, *args, **kwargs):
            error_keys = [key not in cls.__slots__ for key in kwargs]
            if len(error_keys):
                logger.error(f"Keys in your ini_config - {cls.__name__} "
                             f"have no annotation or are not defined!")
                logger.error(f"Please check Keys: {error_keys}")
