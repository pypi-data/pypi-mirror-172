...
from cornflakes.click._rich_group import RichGroup, verbose_option, silent_option
...
def _basic_option_wrapper(click_func, *wrap_args, **wrap_kwargs):
    def wrapper_command(func):
        def wrapper_func(verbose=False, silent=False, *func_args, **func_kwargs):
            logger.setup_logging(default_level=verbose and logging.DEBUG, force=True)
            if silent and func.__module__ != "cornflakes.click":
                subprocess.Popen([sys.executable, "-c",
                                  f"\"import sys; exec(open('"
                                  f"{os.path.abspath(inspect.getfile(func))}').read()); "
                                  f"{func.__name__}(*{func_args},**{func_kwargs});\" "])
                return
...
def group(*args, cls=RichGroup, **kwargs) -> click_group:  # type: ignore
    ...
    """
    return _basic_option_wrapper(click_group, *args, cls=cls, **kwargs)
...
def command(*args, cls=RichCommand, **kwargs) -> click_command:  # type: ignore
    ...
    """
    return _basic_option_wrapper(click_command, *args, cls=cls, **kwargs)
...
def make_cli(
    ...
    if cli.config.BASIC_OPTIONS:
        ...
        cli.params.append(silent_option)
    if context_settings:
