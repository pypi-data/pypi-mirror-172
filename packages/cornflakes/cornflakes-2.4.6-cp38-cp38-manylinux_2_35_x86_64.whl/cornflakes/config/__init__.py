...
def generate_config_group(
    config_module, template_cfg: str = None, target: str = None, class_name: str = None, *args, **kwargs
):
    """Module with function to generate automatically config group module."""
    if isinstance(config_module, str):
        config_module = import_component(config_module)

    if not target:
        target = f'{config_module.__name__.replace(".", "/")}/default.py'

    logger.debug(f"{config_module.__name__} members: {inspect.getmembers(config_module)}")

    ini_config_objects = {}
    imports = []
    for cls_name, cls in inspect.getmembers(config_module):
    ...
    with open(cornflakes.config._config_template.__file__) as file:
        ...
        template = template.replace(
            "# import config",
            f"""from {config_module.__name__} import (
    {''',
    '''.join(imports)}
    )""",
        )
