        ...
        def from_ini(
            ...
            for slot_class in list(cls.__annotations__.values())[len(slot_args):]:
                if hasattr(slot_class, "from_ini"):
                    slot_kwargs.update(slot_class.from_ini(files=files))
...
def ini_config(  # noqa: C901
    ...
    def wrapper(cls):
        ...
        def from_ini(
                files: Union[str, List[str], Dict[str, Union[str, List[str]]]] = None,
                sections: Union[str, List[str], Dict[str, Union[str, List[str]]]] = None,
            ...
            if not files:
                files = cls.__ini_configs__
            logger.debug(f"Load ini from file: {files} - sections: {sections} for config {cls.__name__}")
            if not use_regex:
                if not config_dict:
