...
def ini_config(  # noqa: C901
    ...
    def wrapper(cls):
        ...
        def from_ini(
            ...
            """
            if not sections:
                sections = cls.__ini_config_sections__ or re.sub(r"([a-z])([A-Z])", "\\1_\\2", cls.__name__).lower()
            if not files:
                files = cls.__ini_configs__
            if not use_regex or not isinstance(sections, str):
                ...
                return {sections: _create_config(config_dict or {}, *slot_args, **slot_kwargs)}
            ...
            return {
                ...
                for section, config_dict in (config_dict.items() or isinstance(sections, str) and [(sections, {})] or [(section, {}) for section in sections])
