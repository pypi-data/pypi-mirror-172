...
def csv_to_json(csv_filename: str, csv_key: str = "key") -> dict:
    ...
    # Add CSV file name to the end of the path
    csv_file = current_dir.joinpath(csv_filename)
    ...
    with open(csv_file, "r") as csv_in:
        ...
        for row in DictReader(csv_in):
            ...
            # Replace single quotes with double quotes, as this is the format the Argus datastores use
            csv_data[row[csv_key]] = str(value).replace("'", '"')
...
def validate_data_format(data: str) -> None:
    ...
    :raises ValueError: If the file data points to is not a CSV file.
    """
...
def update(
    ...
    if csv_json:
        try:
            data = csv_to_json(csv_filename=data, csv_key=csv_key)
