...
class Target:
    ...
    def __repr__(self) -> str:
        return f"Target('{self.name}', {self.weight}, {self.ineq}, {self.is_objective}, {self.is_constraint}, {self.is_measurements})"
    ...
    def _update(self) -> None:
        ...
        for target in self.targets:
            ...
            if target.is_objective:
                objective_target_names.append(target.name)
            else:
                additional_target_names.append(target.name)
        ...
        self.ordered_optimization_targets_name = (
            objective_target_names + additional_target_names
        )
...
class Optimization(ABC):
    ...
    def __call__(
        ...
        fval_hist = concatenate(res.algorithm.callback.data["fmodel_hist"])
        aval_hist = res.algorithm.callback.data["additional"]

        if aval_hist != []:
            aval_hist = concatenate(aval_hist)
            fval_hist = concatenate((fval_hist, aval_hist), axis=1)

        x = DataFrame(res.X, columns=self.features.pnames)

        x_hist = DataFrame(x_hist, columns=self.features.pnames)
        fval_hist = DataFrame(
            fval_hist, columns=self.features.ordered_optimization_targets_name
        )[self.features.tnames]
        # reordering according to target list

        results = {"x": x, "x_hist": x_hist, "fval_hist": fval_hist}
...
class Problem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs) -> None:
        ...
        for result, ineq, is_obj, is_consts in zip(
            results,
            self.ineqs.values(),
        ...
        out["F"] = f
        out["G"] = g
        out["A"] = a  # additional results not used for optimization
...
class HistCallback(Callback):
    ...
    def notify(self, algorithm):
        ...
        self.data["additional"].append(algorithm.pop.get("A"))
