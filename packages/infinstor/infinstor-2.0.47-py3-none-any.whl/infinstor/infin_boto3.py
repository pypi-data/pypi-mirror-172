...
class InfinBotoClient():
    ...
    def retrieve_file_with_retry(self, url, fname):
        ...
        while attempts_left > 0:
            ...
            try:
                ...
                print(f'Exception in download, retry {attempts_left} more times')
                if attempts_left > 0:
    ...
    def urlopen_with_retry(self, url):
        ...
        while attempts_left > 0:
            ...
            try:
                ...
                print(f'Exception in urlopen, retry {attempts_left} more times')
                if attempts_left > 0:
                    ##wait before retrying
                    time.sleep(10 * 2**(max_attempts-attempts_left))
                else:
    ...
    def download_file(self, *args, **kwargs):
        print('download_file', args, kwargs)
        if self.timespec_info['type'] == 'infinsnap' or self.timespec_info['type'] == 'infinslice':
            request_bucket = args[0]
            request_file = args[1]
            if request_bucket in self.timespec_info['infinsnap_buckets']:
                self.print_greeting("download_file")
                ...
                if (versionId != None):
                    if self.timespec_info['use_presigned_url_for_infinsnap']:
                        self.retrieve_file_with_retry(presignedUrl, args[2])
                        return
                    else:
                ...
                return self.download_file_presigned(request_bucket, request_file, None, args[2])
            #elif request_bucket in self.timespec_info['non_infinsnap_buckets']:
            else:
                run_id = parse_run_id(request_file)
                if run_id:
                    print("Download file using presigned url")
    ...
    def get_presigned_url(self, bucket, prefix, run_id, method, continuation_param = None):
        if run_id:
            ...
            headers = self.get_auth_header()
            url = os.environ['MLFLOW_CONCURRENT_URI'] + '/api/2.0/mlflow/concurrent/getpresignedurl'
    ...
    def download_file_presigned(self, request_bucket, request_file, run_id, local_path):
        ...
        try:
            self.retrieve_file_with_retry(ps_url, local_path)
        except Exception as ex:
            print('Url retrieve failed: ' + str(ex))
    ...
    def get_object_presigned(self, request_bucket, request_file, run_id):
        ps_url = self.get_presigned_url(request_bucket, request_file, run_id, 'get_object')
        try:
            response = self.urlopen_with_retry(ps_url)
        except HTTPError as http_err:
            print('HTTP error occurred: ' + str(http_err) + ", for url = " + ps_url)
            raise
        except Exception as ex:
            print('Exception in request ' + str(ex) + ", for url: " + ps_url)
            raise ex
    ...
    def get_object(self, **kwargs):
        if self.timespec_info['type'] == 'infinsnap' or self.timespec_info['type'] == 'infinslice':
            ...
            if request_bucket in self.timespec_info['infinsnap_buckets']:
                self.print_greeting("get_object")
                versionId, presignedUrl = self.get_version_id(request_bucket, request_file)
                if (versionId != None):
                    if self.timespec_info['use_presigned_url_for_infinsnap']:
                        print("Using presigned url for get_object", flush=True)
                        res = self.urlopen_with_retry(presignedUrl)
                ...
                return self.get_object_presigned(request_bucket, request_file, None)
            #elif request_bucket in self.timespec_info['non_infinsnap_buckets']:
            else:
                run_id = parse_run_id(request_file)
                if run_id:
...
def fetch_concurrent_user_buckets():
    ...
    if 'MLFLOW_CONCURRENT_URI' in os.environ:
        url = os.environ['MLFLOW_CONCURRENT_URI'] + '/api/2.0/mlflow/concurrent/getuserbuckets'
        token = tokenfile.get_token(builtins.region, False)
        headers = {
            'Content-Type': 'application/x-www-form-urlencoded',
            'Authorization': token
...
def call_requests_get(url, headers=None, retry=False):
    ...
    try:
        ...
        print('HTTP error occurred: ' + str(http_err) + ", for url = " + url)
        raise
    except Exception as ex:
        print('Exception in request ' + str(ex) + ", for url: " + url)
        raise ex
