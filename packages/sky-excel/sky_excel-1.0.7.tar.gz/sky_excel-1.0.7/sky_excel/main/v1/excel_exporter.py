...
class ExcelExport:
    ...
    def __init__(self, input_dict, excel_templet_path=None, excel_templet_title="Sheet1", save_path=None, ):
        ...
        self.wb = load_workbook(filename=excel_templet_path, read_only=True) if excel_templet_path and os.path.exists(excel_templet_path) else None
        # body解析
    ...
    def write_body(self, sheet):
        try:
            ...
            # 写入数据 到数据体里面
            for row_num, row_input_dict in zip(range(start_row, len(self.input_dict) + start_row), self.input_dict):  # 行遍历
                for cell in cell_list:  # 单元格 列遍历
                    # # 数据处理、替换变量表达式
                    ...
                    for k, v in this_cell_value_replaces.items():
                        this_cell_value = this_cell_value.replace(k, str(v))
                    # 数据写入
                    sheet.cell(row=row_num, column=cell.column, value=this_cell_value)
                    if cell.is_merge_cell:
                        this_cell_coordinate = [i + str(row_num) for i in cell.coordinate.translate(None, digits).split(":")]
                        sheet.merge_cells(this_cell_coordinate[0] + ":" + this_cell_coordinate[1])
