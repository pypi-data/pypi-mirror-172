...
def _filter_list_term_types(values: list, term_types: list) -> list:
    term_type_values = [t.value for t in term_types]
    return list(filter(lambda i: i.get('term', {}).get('termType') in term_type_values, values))
