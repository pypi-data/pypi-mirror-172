...
from .product import abg_residue_on_field_nitrogen, blg_residue_nitrogen
...
def get_crop_residue_on_field_N_total(cycle: dict) -> float:
    ...
    """
    products = cycle.get('products', [])
    total = abg_residue_on_field_nitrogen(products) + blg_residue_nitrogen(products)
...
def get_organic_fertiliser_N_total(cycle: dict) -> float:
    ...
    Note: in the event where `dataCompleteness.fertiliser` is set to `True` and there are no organic fertilisers used,
    `0` will be returned.
...
def get_inorganic_fertiliser_N_total(cycle: dict) -> float:
    ...
    Note: in the event where `dataCompleteness.fertiliser` is set to `True` and there are no inorganic fertilisers used,
    `0` will be returned.
...
def _land_occupation_per_ha(model: str, term_id: str, cycle: dict):
    ...
    fallowCorrection = most_relevant_measurement_value(
        (cycle.get('site', {}) or {}).get('measurements', []), 'fallowCorrection', cycle.get('endDate')
    )
...
def impact_lookup_value(
    ...
    factors = list(map(_factor_value(model, term_id, f"{term_type}.csv", lookup_col), blank_nodes))
    values = [value for value in factors if value is not None]
    return list_sum(values) if len(values) > 0 and (allow_missing or len(values) == len(factors)) else None
