...
def test_run_irrigated(*argsm):
    ...
    with open(f"{fixtures_folder}/with-irrigation-practices/result.jsonld", encoding='utf-8') as f:
        expected = json.load(f)
...
@patch(f"{class_path}._new_practice", side_effect=fake_new_practice)
def test_run_low_value(*argsm):
    with open(f"{fixtures_folder}/with-irrigation-inputs-low-value/cycle.jsonld", encoding='utf-8') as f:
        cycle = json.load(f)

    with open(f"{fixtures_folder}/with-irrigation-inputs-low-value/result.jsonld", encoding='utf-8') as f:
        expected = json.load(f)
