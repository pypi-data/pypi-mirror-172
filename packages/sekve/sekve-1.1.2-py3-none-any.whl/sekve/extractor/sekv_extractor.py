...
class Extractor(Params):
    ...
    def __init__(self, vg: ndarray, i: ndarray, vd: float, vs: float = 0.,
                 ...
                 transconductance_cleanning=False
                 ):
        ...
        transconductance_cleanning: bool, optional
        ...
        IndexError
        """
        vg, i = self._user_cleanup(vg, i, transconductance_cleanning)
    ...
    def extract_sigmad_lambdad(self, df: pd.DataFrame,
        ...
        ax1.loglog(ics, gdss, marker="o", ls="", mfc="none", color="k", label="data")
        ax1.loglog(ics, exp_line, "r-", label="Ext. of $\sigma_d$")
        ...
        ax2.loglog(ics, gdss * factor, marker="o", ls="", color="k", label="data", mfc="none")
        ax2.loglog(ics, gds_use_lambdac * factor, ls="-", color="r", label="$\lambda_d$ = $\lambda_c$")
        ax2.loglog(ics, gds_use_lambdad * factor, ls="-", color="b", label="optimzed $\lambda_d$")
        ax2.grid(axis="both", which="major", ls="-", alpha=0.5)
        ax2.grid(axis="both", which="minor", ls="--", alpha=0.5)
        s = [
            "n = %.2f" % self._n,
            "$\sigma_d$ = %.3f" % sigma_d,
            "$\lambda_c$ = %.3f" % self._lambdac,
            "$\lambda_d$ = %.3f" % lambdad,
        ]
