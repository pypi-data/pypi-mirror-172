...
from petljapub.publication_visitor_test import PublicationVisitorTest
...
def test(ctx, lang, sol=0, testcase=-1, example=-1, generated=-1, crafted=-1, recompile=True, timeout=None, verbosity=3):
    """
    ...
    if example >= 1:
        ...
        output = input[:-2] + "out"
        status, time = task.test_on_testcase(solution(sol), lang, input, output, timeout=timeout)
        logger.info(status)
        ...
        task.test_on_all_testcases("ex" + str(sol), lang, force_recompile=recompile, timeout=timeout)

@task
def t(ctx, lang, sol=0, testcase=-1, example=-1, generated=-1, crafted=-1, recompile=True, timeout=None, verbosity=3):
    """
    Test the given solution for the current task (shortcut for 'petljapub test')
    """
    test(ctx, lang, sol, testcase, example, generated, crafted, recompile, timeout, verbosity)
...
def html_preview(ctx, langs=None, css="pandoc.css", header="header.md"):
    """
...
def yaml_html(ctx, yaml, dst, tasks_dir=None, pub_dir=None, standalone=True, join_langs=True, css=None, header=None, lat=False, verbosity=3):
    """
...
def yaml_tex(ctx, yaml, dst, tasks_dir=None, pub_dir=None, header='header.md', tex_template='default.latex', standalone=True, lat=False, tex=True, pdf=False, lang=None):
    """
    ...
    yaml = YAMLSpecification(yaml)
    if lang != None:
        if not lang in yaml.langs():
            logger.error(lang, "is not supported by yaml file")
            return
        langs = [lang]
    else:
        langs = yaml.langs()

    visitor = PublicationVisitorTeX(yaml, task_repo, pub_repo, dst, langs=langs, tex=tex, pdf=pdf, header=header, standalone=standalone, tex_template=tex_template, translit=translit)
    yaml.traverse(visitor)
