...
class LoginFlow(ABC):
    ...
    @classmethod
    def get_login_flow(cls, browser: bool, port: int) -> "LoginFlow":
        if not browser:
...
class PasswordPromptFlow(LoginFlow):
    ...
    def login(self, options: GlobalOptions, org: Organization) -> AuthToken:
        creds = self.prompt_for_user_credentials()
        return self.login_with_user_creds(options, creds, org)

    def prompt_for_user_credentials(self) -> UserCredentials:
        ...
        with self._new_server(options, handler) as httpd:
            ...
            if error_message := getattr(handler, "error_message", None):
                ...
                if "complete sign up" in error_message:
                    click.secho(error_message, fg="cyan")
                    return
                else:
