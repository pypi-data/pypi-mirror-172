...
from scrapy_swarm import connection, defaults
from scrapy_swarm.nsq import NsqHelper
from scrapy_swarm.nsq import NsqWriter
from scrapy_swarm.utils import bytes_to_str, is_dict, TextColor, get_swarm_subclasses
...
class RedisMixin(object):
    ...
    # 这是spider的方法 直接重写掉 start_requests
    ...
    def setup_redis(self, crawler=None):
        """
            ...
            初始化了redis参数, 包括使用的种子url的key, 批量读取url的数量等信
        """
        ...
        NsqHelper.start(crawler.settings)
        # NsqHelper.register(self.name, self)
        NsqHelper.register(SwarmSpider)

        # ok = NsqWriter.push(
        #     topic='scrapy',
        #     data={'spider': self.name, 'bot': settings.get("BOT_NAME", 'bot')}
        # )
        # print(self.name, self.redis_key)
        # print(f"nsq 成功发送到scrapy [{ok}]")
        # print(settings.get('SETTINGS_MODULE'))
        #
        # print(get_swarm_subclasses(SwarmSpider))
...
class SwarmSpider(RedisMixin, Spider):
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        obj = super(SwarmSpider, cls).from_crawler(crawler, *args, **kwargs)
        obj.setup_redis(crawler)
        return obj


class SwarmCrawlSpider(RedisMixin, CrawlSpider):
    """Spider that reads urls from redis queue when idle.
    ...
    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        obj = super(SwarmCrawlSpider, cls).from_crawler(crawler, *args, **kwargs)
        obj.setup_redis(crawler)
        return obj
