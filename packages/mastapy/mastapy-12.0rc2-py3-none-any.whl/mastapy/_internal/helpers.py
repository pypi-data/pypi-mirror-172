...
def _match_versions():
    ...
    message = ('This version of mastapy ({0}) is not supported by the version '
               'of MASTA you are trying to initialise it with.\n\n'
               'You must use either MASTA {0} or newer.\n').format(
                   api_version)
    ...
    if not os.path.exists(full_path):
        if not __api_version__.startswith('10.2.3'):
            message = ('Failed to initialise mastapy. The version of MASTA is '
                       ...
                       'updating MASTA to version {} or installing an older '
                       'version of mastapy.').format(__api_version__)
