...
class CommitServicer(CommitServiceServicer, HGitalyServicer):
    ...
    def GetTreeEntries(self, request: GetTreeEntriesRequest,
        ...
        if changeset is None:
            logger.warn("Revision not found in %r", message.Logging(request))
    ...
    def FindCommits(self, request: FindCommitsRequest,
        ...
        repo, opts = parse_find_commits_request_opts(request, repo)
...
def revset_from_gitlab_revision(repo, revision):
    ...
    No symbolic name like bookmarks, GitLab branches or tags is in the
    resulting revset: there are only operators and revnos or node ids.
    Hence it should be used with the unfiltered repository
    and that is correct.
