...
def _plot(
    ...
    **kwargs,
    ...
    title = title or f"{plot_obj.__class__.__name__}{adjusted}"
    ...
    if bluebelt.config('plotting') == 'matplotlib':
        ...
        data = [
            ...
            go.Histogram(
                ...
                marker=dict(color=bluebelt.style("forecast.hist.facecolor"), opacity=bluebelt.style("forecast.hist.alpha")),
                name='residuals',
                opacity=1,
                showlegend=False,
            )
        ...
        fig.update_layout(
            bargap = 1 - bluebelt.style("forecast.hist.rwidth"),
        )
        ...
        if hasattr(plot_obj, "ci_mean"):
            ...
            fig.add_vrect(
                x0=plot_obj.ci_low,
                x1=plot_obj.ci_high,
                name=f"CI mean ({plot_obj.ci_low:{format_stats}}, {plot_obj.ci_high:{format_stats}})",
                line_width=bluebelt.style("forecast.ci_mean.span.linewidth"),
                fillcolor=bluebelt.style("forecast.ci_mean.span.color"),
                opacity=bluebelt.style("forecast.ci_mean.span.alpha"),
                )
        ...
        if hasattr(plot_obj, "bias"):
            ...
            fig.update_layout(title=f'{title}<br><sup>{t}</sup>', margin_t=50)
