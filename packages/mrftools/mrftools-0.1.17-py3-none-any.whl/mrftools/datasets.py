...
class DatasetManager: 
    ...
    def fetchSingleSeriesFromAzure(self, subject, scanner, set, series, extension=".nii"):
        if(self.connectionString == None or self.container == None):
            ...
            with open(path_base+"_"+series+extension, "wb") as download_file:
                download_file.write(container_client.download_blob(uri_base+"_"+series+extension).readall())
    ...
    def fetchSetFromAzure(self, subject, scanner, set, includeMPRAGE=False, includeTSE=False):
        if(self.connectionString == None or self.container == None):
            ...
            return self.loadDataset(subject, scanner, set, includeMPRAGE=includeMPRAGE, includeTSE=includeTSE)
