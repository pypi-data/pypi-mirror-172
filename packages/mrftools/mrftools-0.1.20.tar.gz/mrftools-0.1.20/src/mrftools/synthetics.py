...
from mrftools import SyntheticSequenceType, DatasetManager, BlochSyntheticGenerator, SimpleITKRunner
import numpy as np
...
class AISyntheticGenerator: 
    def __init__(self,name,description, referenceBlochGenerator:BlochSyntheticGenerator=None, normalizeM0=True, normalizeTargetSeries=True):
        self.name = name   
    ...
    def PrepareTrainingData(self, root, subjects, scanners, sets, targetSeries, dataManager:DatasetManager=None, simpleITKRunner:SimpleITKRunner=None, batch_size=5000, fetchFromAzure=True):
    ...
    def SaveModel(self, modelsDir, compress=True):
        folder = modelsDir + "/" + self.name
    ...
    def LoadModel(self, modelsDir, decompress=True):
        folder = modelsDir + "/" + self.name 
        if(decompress):
            compressedFile = folder+".tar.gz"
