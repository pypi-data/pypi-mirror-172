...
from libixrub.okb import encryption
from libixrub.Servers import SetClines,Server
from re import findall
from pathlib import Path
from random import randint, choice
from json import loads, dumps
from libixrub.How import chup


class xrub_bot:
	def __init__(self, Sh_account):
		self.Auth = Sh_account
		self.prinet = chup.x_coder
	...
	def getMessagesInterval(self):
		inData = {
		    ...
		    "input":{
		       "object_guid":"s0B0e8da28a4fde394257f518e64e800",
		       "middle_message_id":"0"
		    },
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(xrub_bot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("messages")
				break
			except: continue
	...
	def sendCode(self,phone_number,pass_key=None):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(xrub_bot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
				break
			except: continue
	...
	def checkPassword(self, password):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(xrub_bot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("is_vaild")
				break
			except: continue
	...
	def getPostByLink(self, app_url):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(xrub_bot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("link").get("open_chat_data")
				break
			except: continue
	...
	def setMembersAccess(self, chat_id, access_list):
		...
		while 1:
			try:
				return loads(request.urlopen(request.Request(xrub_bot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read())
				break
			except: continue
	...
	def getGroupLink(self, chat_id):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(xrub_bot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("join_link")
				break
			except: continue
	...
	def getChatsUpdate(self):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(xrub_bot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("chats")
				break
			except: continue
	...
	def see_GH_whith_Linkes(self,link_gh):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(xrub_bot._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data")
				break
			except: continue
	...
	def _requestSendFile(self, file):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(xrub_bot._SendFile(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data")
				break
			except: continue

	def _uploadFile(self, file):
		if not "http" in file:
			REQUES = xrub_bot._requestSendFile(self, file)
	...
	def uploadAvatar(self,myguid,main,thumbnail=None):
		mainID = str(xrub_bot._uploadFile(self, main)[0]["id"])
		thumbnailID = str(xrub_bot._uploadFile(self, thumbnail or main)[0]["id"])
		inData = {
			"method":"uploadAvatar",
			"input":{
				"object_guid":myguid,
	...
	def sendDocument(self, chat_id, file, caption=None, message_id=None):
		uresponse = xrub_bot._uploadFile(self, file)
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(xrub_bot._SendFile(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
				break
			except: continue
	...
	def sendPhoto(self, chat_id, file, size=[], thumbnail=None, caption=None, message_id=None):
		...
		elif "." in thumbnail:thumbnail = str(xrub_bot._getThumbInline(open(file,"rb").read() if not "http" in file else get(file).content))

		if size == []: size = xrub_bot._getImageSize(open(file,"rb").read() if not "http" in file else get(file).content)
