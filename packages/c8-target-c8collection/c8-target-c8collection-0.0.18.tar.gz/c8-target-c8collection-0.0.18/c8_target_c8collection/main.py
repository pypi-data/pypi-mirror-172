...
from c8 import C8Client, DocumentInsertError, DocumentDeleteError
...
def try_upsert(collection: StandardCollection, record: dict):
    try:
        ...
        try:
            ...
            logger.warn(f'Failed to insert/update record: {record}')
...
def try_delete(collection: StandardCollection, _key: str):
    try:
        ...
        logger.warn(f'Failed to insert/update record with _key: {_key}')
        ...
        if message_type == 'RECORD':
            ...
            try:
                ...
                try:
                    ...
                    # TODO: Only consider 1pk. Will not work for composite keys.
                    _key = rec[key_properties[stream][0]].strip()
