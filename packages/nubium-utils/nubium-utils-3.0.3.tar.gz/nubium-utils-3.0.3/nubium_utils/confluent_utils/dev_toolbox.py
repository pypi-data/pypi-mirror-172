...
from nubium_utils.confluent_utils.gtfo import GtfoBatchApp
...
class DudeConsumer(GtfoBatchApp):
    ...
    def _app_run(self, *args, **kwargs):
        ...
        if len(messages) < self.transaction._consume_max_count:
...
class KafkaToolbox:
    ...
    def produce_messages(self, topic, message_list, schema=None, cluster_name=None):
        ...
        if not self.producers.get(topic):
            ...
            if not self.schema_registry:
                self.schema_registry = init_schema_registry_configs()
    ...
    @staticmethod
    def run_unit_tests(extra_pytest_args: Optional[List[str]]):
        if _is_nubium_app():
            _sync_virtual_environment()
            run_command_in_virtual_environment("pytest", ["./tests/unit/", "-rA", "-v"] + extra_pytest_args)

    @staticmethod
    def run_integration_tests(extra_pytest_args: Optional[List[str]]):
