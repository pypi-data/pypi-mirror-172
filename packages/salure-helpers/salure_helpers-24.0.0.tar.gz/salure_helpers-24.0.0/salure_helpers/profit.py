...
class GetConnectorAsync:
    def __init__(self, salureconnect_connection: SalureConnect, token_reference: str = None):
        self.profit = GetConnector(salureconnect_connection, token_reference=token_reference)
        self.profit_async = ProfitExtractAsync(salureconnect_connection, token_reference=token_reference)
...
class GetConnector:
    def __init__(self, salureconnect_connection: SalureConnect, token_reference: str):
        self.salureconnect_connection = salureconnect_connection
        if self.salureconnect_connection.test_environment:
            self.base_url = 'resttest.afas.online'
        else:
            self.base_url = 'rest.afas.online'
        credentials = self.salureconnect_connection.get_system_credential(system='profit', reference=token_reference)
...
class UpdateConnector:
    ...
    def update_person(self, data: dict, overload_fields: dict = None, method='PUT') -> requests.Response:
        ...
        allowed_fields = ['employee_id', 'mail_work', 'mail_private', 'mobile_work', 'mobile_private', 'nickname', 'first_name', 'initials', 'prefix', 'last_name', 'prefix_birth_name',
                          ...
                          'birth_name_separate', 'name_use', 'match_employees_on']
                ...
                "Element": {
                    ...
                    "Objects": {
                        "KnPerson": {
                            "Element": {
                                "Fields": {
                                    "MatchPer": "0" if "match_employees_on" not in data else data['match_employees_on'],
                                    "BcCo": data['person_id']
                                }
                            }
                        }
    ...
    def new_employee_with_first_contract(self, data: dict, overload_fields: dict = None) -> requests.Response:
        ...
        allowed_fields_person = ['initials', 'email_work', 'email_home', 'country_of_birth', 'place_of_birth', 'prefix',
                                 'birth_name_separate', 'name_use', 'send_payslip', 'send_annual_statement', 'match_employees_on']
    ...
    def update_applicant(self, data: dict, method: str, overload_fields: dict = None) -> requests.Response:
        ...
        if method == 'DELETE':
            ...
            fields_to_update.update({"EmA": data['work_email']}) if 'work_email' in data else fields_to_update  # private email
