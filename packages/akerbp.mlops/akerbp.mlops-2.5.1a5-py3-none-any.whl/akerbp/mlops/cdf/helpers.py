...
from akerbp.mlops.core import config, logger, exceptions
...
def robust_create(create_function: partial) -> None:
    ...
    for trial in range(max_errors):
        logging.debug(f"Creating function (trial {trial+1}/{max_errors})")
...
def download_file(
    ...
    if external_id is not None:
        logging.debug(f"Download file with {external_id=} to {path}")
...
def copy_file(
    ...
    **kwargs,
    ...
    logging.debug(f"Downloaded file: {source_ext_id}, {f.dump()}")  # type: ignore
    if overwrite_name:
        if name is None:
            raise Exception(
...
def get_arguments_for_redeploying_latest_model_version(
    ...
    if len(stripped_external_id := latest_function.external_id) > 3:  # type: ignore
