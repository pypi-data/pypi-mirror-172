...
def pre_process(runno,detectors='75-95'):
    LoadISISNexus(str(runno),OutputWorkspace=str(runno),LoadMonitors="Exclude")
    ConvertUnits(InputWorkspace=str(runno), OutputWorkspace=str(runno), Target='Wavelength', AlignBins=True)
    try:
        SumSpectra(InputWorkspace=str(runno), OutputWorkspace=str(runno), ListOfWorkspaceIndices=detectors)
    except:
        ExtractSpectra(InputWorkspace=str(runno), OutputWorkspace=str(runno), WorkspaceIndexList=detectors)
    ReplaceSpecialValues(InputWorkspace=str(runno), OutputWorkspace=str(runno), NaNValue=0, InfinityValue=0)
...
pre_process(62097)
Scale("62096",Factor=13,OutputWorkspace="62096") #Quickest way is to do this by hand I think
a=0.7/0.5
Scale("62096",Factor=a,OutputWorkspace="62096") #To make it effectively at 0.7 degrees, same as OFFSPEC beam
SaveAscii("62096",Filename="/mnt/ceph/home/jc15575/Desktop/Enough/enough-is-enough-main/simulation/INTER_Air_DB.dat",WriteSpectrumID=False,ColumnHeader=False)

#POLREF
pre_process(34365,detectors='272-285') #0.5 degrees I think
a=0.7/0.5
Scale("34365",Factor=a,OutputWorkspace="34365") #To make it effectively at 0.7 degrees, same as OFFSPEC beam
SaveAscii("34365",Filename="/mnt/ceph/home/jc15575/Desktop/Enough/enough-is-enough-main/simulation/POLREF_Air_DB.dat",WriteSpectrumID=False,ColumnHeader=False)

#OFFSPEC
pre_process(62802,detectors='380-420') # Both DB's to get correct flux, 0.7 degrees
pre_process(62803,detectors='380-420')
Scale("62802",Factor=3.7,OutputWorkspace="62802") #Quickest way is to do this by hand I think
SaveAscii("62802",Filename="/mnt/ceph/home/jc15575/Desktop/Enough/enough-is-enough-main/simulation/OFFSPEC_Air_DB.dat",WriteSpectrumID=False,ColumnHeader=False)
...
SaveAscii("137344",Filename="/mnt/ceph/home/jc15575/Desktop/Enough/enough-is-enough-main/simulation/SURF_Air_DB.dat",WriteSpectrumID=False,ColumnHeader=False)
