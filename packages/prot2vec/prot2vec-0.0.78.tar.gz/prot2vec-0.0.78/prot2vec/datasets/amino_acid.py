...
def vect_batch(batch, mask_prob, nonmask_weight):
    x = np.concatenate(batch, axis=0)
    y = x.copy()

    x, mask = mask_data(x, mask_prob)
    mask[y == AA_DICT['?']] = False

    if nonmask_weight > 0.0:
        mask = mask.astype('float32') + nonmask_weight
...
def ds_iter(src_file, seq_len, mask_prob, batch_size, nonmask_weight=0.0, to_one_hot=False):
    batch = []
    while True:
        with open(src_file, "r", encoding="utf-8") as fr:
            ...
            for line in fr:
                ...
                if len(batch) >= batch_size:
                    v_batch = vect_batch(batch, mask_prob, nonmask_weight)
                    if to_one_hot:
                        v_batch = tf.one_hot(v_batch[0], 21), v_batch[1], v_batch[2]
                    yield v_batch
                    del batch[:]
...
if __name__ == '__main__':
    ...
    # ds_it = ds_iter(ROOT / 'random' / 'completely_random.tsv', 60, 0.15, 32, to_one_hot=True)
    ds_it = ds_iter(ROOT / 'uniref90_tax-free_shuffled.tsv', 128, 0.15, 1024, to_one_hot=True)
