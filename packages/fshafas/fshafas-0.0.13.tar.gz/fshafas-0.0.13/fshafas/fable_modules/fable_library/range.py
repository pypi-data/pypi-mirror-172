...
from typing import (TypeVar, Callable, Optional, Tuple, Literal)
...
def make_range_step_function(step: _T, stop: _T, zero: _T, add: Callable[[_T, _T], _T]) -> Callable[[_T], Optional[Tuple[_T, _T]]]:
    ...
    def _arrow118(x: Optional[_T]=None, step: _T=step, stop: _T=stop, zero: _T=zero, add: Callable[[_T, _T], _T]=add) -> Optional[Tuple[_T, _T]]:
...
def integral_range_step(start: _T, step: _T, stop: _T, zero: _T, add: Callable[[_T, _T], _T]) -> IEnumerable_1[_T]:
    ...
    def _arrow119(__unit: Literal[None]=None, start: _T=start, step: _T=step, stop: _T=stop, zero: _T=zero, add: Callable[[_T, _T], _T]=add) -> IEnumerable_1[_T]:
...
def range_big_int(start: int, step: int, stop: int) -> IEnumerable_1[int]:
    def _arrow120(x: int, y: int, start: int=start, step: int=step, stop: int=stop) -> int:
        return op_addition(x, y)

    return integral_range_step(start, step, stop, from_zero(), _arrow120)


def range_decimal(start: Decimal, step: Decimal, stop: Decimal) -> IEnumerable_1[Decimal]:
    def _arrow121(x: Decimal, y: Decimal, start: Decimal=start, step: Decimal=step, stop: Decimal=stop) -> Decimal:
        return op_addition_1(x, y)

    return integral_range_step(start, step, stop, from_parts(0, 0, 0, False, uint8(0)), _arrow121)


def range_double(start: float, step: float, stop: float) -> IEnumerable_1[float]:
    def _arrow122(x: float, y: float, start: float=start, step: float=step, stop: float=stop) -> float:
        return x + y

    return integral_range_step(start, step, stop, 0.0, _arrow122)


def range_int64(start: int64, step: int64, stop: int64) -> IEnumerable_1[int64]:
    def _arrow123(x: int64, y: int64, start: int64=start, step: int64=step, stop: int64=stop) -> int64:
        return op_addition_2(x, y)

    return integral_range_step(start, step, stop, int64(0), _arrow123)


def range_uint64(start: uint64, step: uint64, stop: uint64) -> IEnumerable_1[uint64]:
    def _arrow124(x: uint64, y: uint64, start: uint64=start, step: uint64=step, stop: uint64=stop) -> uint64:
        return op_addition_2(x, y)

    return integral_range_step(start, step, stop, uint64(0), _arrow124)
...
def range_char(start: str, stop: str) -> IEnumerable_1[str]:
    ...
    def _arrow125(__unit: Literal[None]=None, start: str=start, stop: str=stop) -> IEnumerable_1[str]:
        return unfold(step_fn, char_code_at(start, 0))

    return delay(_arrow125)
