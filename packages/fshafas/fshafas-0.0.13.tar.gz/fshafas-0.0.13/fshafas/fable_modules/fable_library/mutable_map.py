...
from typing import (Literal, TypeVar, Any, Optional, Tuple, Callable, Generic)
...
class Dictionary(Generic[_KEY, _VALUE]):
    ...
    def System_Collections_Generic_IDictionary_2_get_Keys(self, __unit: Literal[None]=None) -> ICollection[_KEY]:
        this: Dictionary[_KEY, _VALUE] = self
        def _arrow33(__unit: Literal[None]=None) -> IEnumerable_1[_KEY]:
            def _arrow32(pair: Any) -> _KEY:
                return pair[0]

            return map(_arrow32, this)

        return to_array(delay(_arrow33))
    ...
    def System_Collections_Generic_IDictionary_2_get_Values(self, __unit: Literal[None]=None) -> ICollection[_VALUE]:
        this: Dictionary[_KEY, _VALUE] = self
        def _arrow35(__unit: Literal[None]=None) -> IEnumerable_1[_VALUE]:
            def _arrow34(pair: Any) -> _VALUE:
                return pair[1]

            return map(_arrow34, this)

        return to_array(delay(_arrow35))
...
def Dictionary__TryFindIndex_2B595(this: Dictionary[_KEY, Any], k: _KEY) -> Tuple[bool, int, int]:
    ...
    def _arrow37(__unit: Literal[None]=None, this: Dictionary[_KEY, _VALUE]=this, k: _KEY=k) -> Array[Any]:
        return out_arg

    def _arrow38(v: Array[Any], this: Dictionary[_KEY, _VALUE]=this, k: _KEY=k) -> None:
        nonlocal out_arg
        out_arg = v

    match_value = (try_get_value(this.hash_map, h, FSharpRef(_arrow37, _arrow38)), out_arg)
    if match_value[0]:
        def _arrow39(pair: Any, this: Dictionary[_KEY, _VALUE]=this, k: _KEY=k) -> bool:
            return this.comparer.Equals(k, pair[0])

        return (True, h, find_index(_arrow39, match_value[1]))
