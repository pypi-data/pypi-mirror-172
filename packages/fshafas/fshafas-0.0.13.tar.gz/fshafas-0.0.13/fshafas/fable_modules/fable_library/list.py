...
from typing import (TypeVar, Literal, Optional, Callable, Any, Generic, Tuple)
...
class FSharpList(Record, Generic[_T]):
    ...
    def GetHashCode(self, __unit: Literal[None]=None) -> int:
        xs: FSharpList[_T] = self
    ...
    def to_json(self, __unit: Literal[None]=None) -> Any:
...
FSharpList_reflection = _expr69

def _expr70(gen0: TypeInfo) -> TypeInfo:
...
ListEnumerator_1_reflection = _expr70

def ListEnumerator_1__ctor_3002E699(xs: FSharpList[Any]) -> ListEnumerator_1[_T]:
    return ListEnumerator_1(xs)


def FSharpList_get_Empty(__unit: Literal[None]=None) -> FSharpList[Any]:
...
def FSharpList__get_Length(xs: FSharpList[Any]) -> int:
    def loop(i_mut: int, xs_1_mut: FSharpList[_T], xs: FSharpList[_T]=xs) -> int:
        while True:
...
def FSharpList__get_Item_Z524259A4(xs: FSharpList[_T], index: int) -> _T:
    def loop(i_mut: int, xs_1_mut: FSharpList[_T], xs: FSharpList[_T]=xs, index: int=index) -> _T:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            match_value: Optional[FSharpList[_T]] = xs_1.tail
            if match_value is not None:
...
def empty(__unit: Literal[None]=None) -> FSharpList[Any]:
...
def reverse(xs: FSharpList[_T]) -> FSharpList[_T]:
    def _arrow71(acc: FSharpList[_T], x: _T, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        return FSharpList_Cons_305B8EAC(x, acc)

    return fold(_arrow71, FSharpList_get_Empty(), xs)
    ...
    def loop(acc_mut: _STATE, node_mut: FSharpList[_T], gen: Callable[[_STATE], Optional[Tuple[_T, _STATE]]]=gen, state: _STATE=state) -> FSharpList[_T]:
        while True:
            ...
            if match_value is not None:
                ...
                def _arrow72(__unit: Literal[None]=None, acc: _STATE=acc, node: FSharpList[_T]=node) -> FSharpList[_T]:
...
def iterate(action: Callable[[__A], None], xs: FSharpList[__A]) -> None:
    def _arrow73(unit_var: None, x: __A, action: Callable[[__A], None]=action, xs: FSharpList[__A]=xs) -> None:
        action(x)

    fold(_arrow73, None, xs)


def iterate2(action: Callable[[__A, __B], None], xs: FSharpList[__A], ys: FSharpList[__B]) -> None:
    def _arrow74(unit_var: None, x: __A, y: __B, action: Callable[[__A, __B], None]=action, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys) -> None:
        action(x, y)

    fold2(_arrow74, None, xs, ys)


def iterate_indexed(action: Callable[[int, __A], None], xs: FSharpList[__A]) -> None:
    def _arrow75(i: int, x: __A, action: Callable[[int, __A], None]=action, xs: FSharpList[__A]=xs) -> int:
        action(i, x)
        return i + 1

    ignore(fold(_arrow75, 0, xs))


def iterate_indexed2(action: Callable[[int, __A, __B], None], xs: FSharpList[__A], ys: FSharpList[__B]) -> None:
    def _arrow76(i: int, x: __A, y: __B, action: Callable[[int, __A, __B], None]=action, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys) -> int:
        action(i, x, y)
        return i + 1

    ignore(fold2(_arrow76, 0, xs, ys))
    ...
    if is_array_like(xs):
        ...
        with get_enumerator(xs) as enumerator:
            while enumerator.System_Collections_IEnumerator_MoveNext():
                ...
                def _arrow77(__unit: Literal[None]=None, xs: IEnumerable_1[_T]=xs) -> FSharpList[_T]:
...
def concat(lists: IEnumerable_1[FSharpList[_T]]) -> FSharpList[_T]:
    root: FSharpList[_T] = FSharpList_get_Empty()
    node: FSharpList[_T] = root
    def action(xs: FSharpList[_T], lists: IEnumerable_1[FSharpList[_T]]=lists) -> None:
        nonlocal node
        def _arrow78(acc: FSharpList[_T], x: _T, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
            t: FSharpList[_T] = FSharpList(x, None)
            acc.tail = t
            return t

        node = fold(_arrow78, node, xs)
...
def scan(folder: Callable[[_STATE, _T], _STATE], state: _STATE, xs: FSharpList[_T]) -> FSharpList[_STATE]:
    ...
    while not FSharpList__get_IsEmpty(xs_3):
        ...
        def _arrow79(__unit: Literal[None]=None, folder: Callable[[_STATE, _T], _STATE]=folder, state: _STATE=state, xs: FSharpList[_T]=xs) -> FSharpList[_STATE]:
...
def append(xs: FSharpList[_T], ys: FSharpList[_T]) -> FSharpList[_T]:
    def _arrow80(acc: FSharpList[_T], x: _T, xs: FSharpList[_T]=xs, ys: FSharpList[_T]=ys) -> FSharpList[_T]:
        return FSharpList_Cons_305B8EAC(x, acc)

    return fold(_arrow80, ys, reverse(xs))
...
def collect(mapping: Callable[[_T], FSharpList[_U]], xs: FSharpList[_T]) -> FSharpList[_U]:
    ...
    while not FSharpList__get_IsEmpty(ys):
        ...
        while not FSharpList__get_IsEmpty(zs):
            def _arrow81(__unit: Literal[None]=None, mapping: Callable[[_T], FSharpList[_U]]=mapping, xs: FSharpList[_T]=xs) -> FSharpList[_U]:
...
def indexed(xs: FSharpList[__A]) -> FSharpList[Tuple[int, __A]]:
    def _arrow82(i: int, x: __A, xs: FSharpList[__A]=xs) -> Tuple[int, __A]:
        return (i, x)

    return map_indexed(_arrow82, xs)
    ...
    def loop(i_mut: int, acc_mut: FSharpList[_U], xs_1_mut: FSharpList[_T1], ys_1_mut: FSharpList[_T2], mapping: Callable[[int, _T1, _T2], _U]=mapping, xs: FSharpList[_T1]=xs, ys: FSharpList[_T2]=ys) -> FSharpList[_U]:
        while True:
            ...
            if True if FSharpList__get_IsEmpty(xs_1) else FSharpList__get_IsEmpty(ys_1):
                ...
                i_mut = i + 1
                def _arrow83(__unit: Literal[None]=None, i: int=i, acc: FSharpList[_U]=acc, xs_1: FSharpList[_T1]=xs_1, ys_1: FSharpList[_T2]=ys_1) -> FSharpList[_U]:
    ...
    def loop(acc_mut: FSharpList[_U], xs_1_mut: FSharpList[_T1], ys_1_mut: FSharpList[_T2], zs_1_mut: FSharpList[_T3], mapping: Callable[[_T1, _T2, _T3], _U]=mapping, xs: FSharpList[_T1]=xs, ys: FSharpList[_T2]=ys, zs: FSharpList[_T3]=zs) -> FSharpList[_U]:
        while True:
            ...
            if True if (True if FSharpList__get_IsEmpty(xs_1) else FSharpList__get_IsEmpty(ys_1)) else FSharpList__get_IsEmpty(zs_1):
                ...
                def _arrow84(__unit: Literal[None]=None, acc: FSharpList[_U]=acc, xs_1: FSharpList[_T1]=xs_1, ys_1: FSharpList[_T2]=ys_1, zs_1: FSharpList[_T3]=zs_1) -> FSharpList[_U]:
...
def map_fold(mapping: Callable[[_STATE, _T], Tuple[_RESULT, _STATE]], state: _STATE, xs: FSharpList[_T]) -> Tuple[FSharpList[_RESULT], _STATE]:
    def folder(tupled_arg: Tuple[FSharpList[_RESULT], _STATE], x: _T, mapping: Callable[[_STATE, _T], Tuple[_RESULT, _STATE]]=mapping, state: _STATE=state, xs: FSharpList[_T]=xs) -> Tuple[FSharpList[_RESULT], _STATE]:
        ...
        def _arrow85(__unit: Literal[None]=None, tupled_arg: Tuple[FSharpList[_RESULT], _STATE]=tupled_arg, x: _T=x) -> FSharpList[_RESULT]:
            t: FSharpList[_RESULT] = FSharpList(pattern_input[0], None)
            tupled_arg[0].tail = t
            return t

        return (_arrow85(), pattern_input[1])
...
def map_fold_back(mapping: Callable[[_T, _STATE], Tuple[_RESULT, _STATE]], xs: FSharpList[_T], state: _STATE) -> Tuple[FSharpList[_RESULT], _STATE]:
    def _arrow86(acc: _STATE, x: _T, mapping: Callable[[_T, _STATE], Tuple[_RESULT, _STATE]]=mapping, xs: FSharpList[_T]=xs, state: _STATE=state) -> Tuple[_RESULT, _STATE]:
        return mapping(x, acc)

    return map_fold(_arrow86, state, reverse(xs))
...
def try_find(f: Callable[[__A], bool], xs: FSharpList[__A]) -> Optional[__A]:
    def _arrow87(x: Optional[__A]=None, f: Callable[[__A], bool]=f, xs: FSharpList[__A]=xs) -> Optional[__A]:
        return some(x) if f(x) else None

    return try_pick(_arrow87, xs)
...
def partition(f: Callable[[_T], bool], xs: FSharpList[_T]) -> Tuple[FSharpList[_T], FSharpList[_T]]:
    ...
    def folder(tupled_arg: Tuple[FSharpList[_T], FSharpList[_T]], x: _T, f: Callable[[_T], bool]=f, xs: FSharpList[_T]=xs) -> Tuple[FSharpList[_T], FSharpList[_T]]:
        ...
        if f(x):
            def _arrow88(__unit: Literal[None]=None, tupled_arg: Tuple[FSharpList[_T], FSharpList[_T]]=tupled_arg, x: _T=x) -> FSharpList[_T]:
                t: FSharpList[_T] = FSharpList(x, None)
                lacc.tail = t
                return t

            return (_arrow88(), racc)

        else: 
            def _arrow89(__unit: Literal[None]=None, tupled_arg: Tuple[FSharpList[_T], FSharpList[_T]]=tupled_arg, x: _T=x) -> FSharpList[_T]:
                t_2: FSharpList[_T] = FSharpList(x, None)
                racc.tail = t_2
                return t_2

            return (lacc, _arrow89())
...
def contains(value: _T, xs: FSharpList[_T], eq: IEqualityComparer_1[Any]) -> bool:
    def _arrow90(v: Optional[_T]=None, value: _T=value, xs: FSharpList[_T]=xs, eq: IEqualityComparer_1[Any]=eq) -> bool:
        return eq.Equals(value, v)

    return try_find_index(_arrow90, xs) is not None
...
def initialize(n: int, f: Callable[[int], _T]) -> FSharpList[_T]:
    ...
    for i in range(0, (n - 1) + 1, 1):
        def _arrow91(__unit: Literal[None]=None, n: int=n, f: Callable[[int], _T]=f) -> FSharpList[_T]:
...
def replicate(n: int, x: __A) -> FSharpList[__A]:
    def _arrow92(_arg: int, n: int=n, x: __A=x) -> __A:
        return x

    return initialize(n, _arrow92)
...
def for_all(f: Callable[[__A], bool], xs: FSharpList[__A]) -> bool:
    def _arrow93(acc: bool, x: __A, f: Callable[[__A], bool]=f, xs: FSharpList[__A]=xs) -> bool:
        return f(x) if acc else False

    return fold(_arrow93, True, xs)


def for_all2(f: Callable[[__A, __B], bool], xs: FSharpList[__A], ys: FSharpList[__B]) -> bool:
    def _arrow94(acc: bool, x: __A, y: __B, f: Callable[[__A, __B], bool]=f, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys) -> bool:
        return f(x, y) if acc else False

    return fold2(_arrow94, True, xs, ys)
...
def unzip(xs: FSharpList[Tuple[__A, __B]]) -> Tuple[FSharpList[__A], FSharpList[__B]]:
    def _arrow95(tupled_arg: Tuple[__A, __B], tupled_arg_1: Tuple[FSharpList[__A], FSharpList[__B]], xs: FSharpList[Tuple[__A, __B]]=xs) -> Tuple[FSharpList[__A], FSharpList[__B]]:
        return (FSharpList_Cons_305B8EAC(tupled_arg[0], tupled_arg_1[0]), FSharpList_Cons_305B8EAC(tupled_arg[1], tupled_arg_1[1]))

    return fold_back(_arrow95, xs, (FSharpList_get_Empty(), FSharpList_get_Empty()))


def unzip3(xs: FSharpList[Tuple[__A, __B, __C]]) -> Tuple[FSharpList[__A], FSharpList[__B], FSharpList[__C]]:
    def _arrow96(tupled_arg: Tuple[__A, __B, __C], tupled_arg_1: Tuple[FSharpList[__A], FSharpList[__B], FSharpList[__C]], xs: FSharpList[Tuple[__A, __B, __C]]=xs) -> Tuple[FSharpList[__A], FSharpList[__B], FSharpList[__C]]:
    ...
    return fold_back(_arrow96, xs, (FSharpList_get_Empty(), FSharpList_get_Empty(), FSharpList_get_Empty()))


def zip(xs: FSharpList[__A], ys: FSharpList[__B]) -> FSharpList[Tuple[__A, __B]]:
    def _arrow97(x: __A, y: __B, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys) -> Tuple[__A, __B]:
        return (x, y)

    return map2(_arrow97, xs, ys)


def zip3(xs: FSharpList[__A], ys: FSharpList[__B], zs: FSharpList[__C]) -> FSharpList[Tuple[__A, __B, __C]]:
    def _arrow98(x: __A, y: __B, z: __C, xs: FSharpList[__A]=xs, ys: FSharpList[__B]=ys, zs: FSharpList[__C]=zs) -> Tuple[__A, __B, __C]:
        return (x, y, z)

    return map3(_arrow98, xs, ys, zs)
...
def sort(xs: FSharpList[_T], comparer: IComparer_1[_T]) -> FSharpList[_T]:
    def _arrow99(x: _T, y: _T, xs: FSharpList[_T]=xs, comparer: IComparer_1[_T]=comparer) -> int:
...
def sort_by(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer_1[_U]) -> FSharpList[_T]:
    def _arrow100(x: _T, y: _T, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer_1[_U]=comparer) -> int:
...
def sort_descending(xs: FSharpList[_T], comparer: IComparer_1[_T]) -> FSharpList[_T]:
    def _arrow101(x: _T, y: _T, xs: FSharpList[_T]=xs, comparer: IComparer_1[_T]=comparer) -> int:
...
def sort_by_descending(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer_1[_U]) -> FSharpList[_T]:
    def _arrow102(x: _T, y: _T, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer_1[_U]=comparer) -> int:
        return comparer.Compare(projection(x), projection(y)) * -1

    return sort_with(_arrow102, xs)


def sum(xs: FSharpList[_T], adder: IGenericAdder_1[_T]) -> _T:
    def _arrow103(acc: _T, x: _T, xs: FSharpList[_T]=xs, adder: IGenericAdder_1[_T]=adder) -> _T:
...
def sum_by(f: Callable[[_T], _U], xs: FSharpList[_T], adder: IGenericAdder_1[_U]) -> _U:
    def _arrow104(acc: _U, x: _T, f: Callable[[_T], _U]=f, xs: FSharpList[_T]=xs, adder: IGenericAdder_1[_U]=adder) -> _U:
        return adder.Add(acc, f(x))

    return fold(_arrow104, adder.GetZero(), xs)


def max_by(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer_1[_U]) -> _T:
    def _arrow105(x: _T, y: _T, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer_1[_U]=comparer) -> _T:
...
def max(xs: FSharpList[_T], comparer: IComparer_1[_T]) -> _T:
    def _arrow106(x: _T, y: _T, xs: FSharpList[_T]=xs, comparer: IComparer_1[_T]=comparer) -> _T:
...
def min_by(projection: Callable[[_T], _U], xs: FSharpList[_T], comparer: IComparer_1[_U]) -> _T:
    def _arrow107(x: _T, y: _T, projection: Callable[[_T], _U]=projection, xs: FSharpList[_T]=xs, comparer: IComparer_1[_U]=comparer) -> _T:
...
def min(xs: FSharpList[_T], comparer: IComparer_1[_T]) -> _T:
    def _arrow108(x: _T, y: _T, xs: FSharpList[_T]=xs, comparer: IComparer_1[_T]=comparer) -> _T:
        return x if (comparer.Compare(y, x) > 0) else y

    return reduce(_arrow108, xs)
...
def average_by(f: Callable[[_T], _U], xs: FSharpList[_T], averager: IGenericAverager_1[_U]) -> _U:
    count: int = 0
    def _arrow109(acc: _U, x: _T, f: Callable[[_T], _U]=f, xs: FSharpList[_T]=xs, averager: IGenericAverager_1[_U]=averager) -> _U:
        nonlocal count
        count = (count + 1) or 0
        return averager.Add(acc, f(x))

    total: _U = fold(_arrow109, averager.GetZero(), xs)
    return averager.DivideByInt(total, count)
...
def all_pairs(xs: FSharpList[_T1], ys: FSharpList[_T2]) -> FSharpList[Tuple[_T1, _T2]]:
    ...
    def _arrow112(x: Optional[_T1]=None, xs: FSharpList[_T1]=xs, ys: FSharpList[_T2]=ys) -> None:
        def _arrow111(y: Optional[_T2]=None) -> None:
            nonlocal node
            def _arrow110(__unit: Literal[None]=None) -> FSharpList[Tuple[_T1, _T2]]:
        ...
        iterate(_arrow111, ys)

    iterate(_arrow112, xs)
    ...
    def loop(i_mut: int, acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], count: int=count, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if i <= 0:
                ...
                i_mut = i - 1
                def _arrow113(__unit: Literal[None]=None, i: int=i, acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[_T]:
    ...
    def loop(acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], predicate: Callable[[_T], bool]=predicate, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if FSharpList__get_IsEmpty(xs_1):
                ...
                def _arrow114(__unit: Literal[None]=None, acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[_T]:
                    t: FSharpList[_T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail = t
                    return t
    ...
    def loop(i_mut: int, acc_mut: FSharpList[_T], xs_1_mut: FSharpList[_T], count: int=count, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        while True:
            ...
            if i <= 0:
                ...
                i_mut = i - 1
                def _arrow115(__unit: Literal[None]=None, i: int=i, acc: FSharpList[_T]=acc, xs_1: FSharpList[_T]=xs_1) -> FSharpList[_T]:
                    t: FSharpList[_T] = FSharpList(FSharpList__get_Head(xs_1), None)
                    acc.tail = t
                    return t
...
def transpose(lists: IEnumerable_1[FSharpList[_T]]) -> FSharpList[FSharpList[_T]]:
    def mapping_1(xs_1: Array[_T], lists: IEnumerable_1[FSharpList[_T]]=lists) -> FSharpList[_T]:
        return of_array(xs_1)

    def mapping(xs: FSharpList[_T], lists: IEnumerable_1[FSharpList[_T]]=lists) -> Array[_T]:
        return to_array(xs)
...
def insert_at(index: int, y: _T, xs: FSharpList[_T]) -> FSharpList[_T]:
    ...
    def _arrow116(__unit: Literal[None]=None, index: int=index, y: _T=y, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")

    return reverse(result if is_done else (FSharpList_Cons_305B8EAC(y, result) if ((i + 1) == index) else _arrow116()))
...
def insert_many_at(index: int, ys: IEnumerable_1[_T], xs: FSharpList[_T]) -> FSharpList[_T]:
    ...
    def folder(acc: FSharpList[_T], x: _T, index: int=index, ys: IEnumerable_1[_T]=ys, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        nonlocal i, is_done
        i = (i + 1) or 0
        if i == index:
            is_done = True
    ...
    result: FSharpList[_T] = fold(folder, FSharpList_get_Empty(), xs)
    def _arrow117(__unit: Literal[None]=None, index: int=index, ys: IEnumerable_1[_T]=ys, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")

    return reverse(result if is_done else (append(ys_1, result) if ((i + 1) == index) else _arrow117()))
