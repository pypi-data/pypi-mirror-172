...
class Hierarchical(Duplicable):
    ...
    def treeview_panel(self, ar):
        ...
        def append_to_et(child, i=0):
            def get_text(child):
                return (tab * i) + (
                    folder_open if child in openned and child.children.count() == 0
                    else folder_with_child_open if child in openned
                    else folder_closed if child.children.count() == 0
            ...
            if child.children.count() > 0:
                ...
                i -= 1
            if child == self:
                sar = ar.actor.request_from(ar, master_instance=self, is_on_main_actor=False)
