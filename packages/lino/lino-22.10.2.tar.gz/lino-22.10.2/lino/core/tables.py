...
class AbstractTable(actors.Actor):
    ...
    """
    The display mode to select depending on the available width (in characters) on the client.
    """

    @classmethod
    def get_display_mode(cls, available_width=None):
        for dmi in cls.display_mode:
            if dmi[0] is None:  # or available_width is None or available_width >= dmi[0]:
                return dmi[1]
...
method in order to sort the rows of the queryset.
    ...
    @classmethod
    def row_as_page(cls, ar, row):
        """Yield a series HTML strings that represent the given row as a plain page.

        """
        return row.as_page(ar)
