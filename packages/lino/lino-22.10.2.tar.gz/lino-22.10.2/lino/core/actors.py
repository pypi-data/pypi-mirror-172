...
from lino.core.classproperty import classproperty

class ActorMetaClass(type('classproperty', (classproperty,), {})):
...
class Actor(with_metaclass(ActorMetaClass, type('NewBase', (actions.Parametrizable, Permittable), {}))):
        ...
        if h is None:
            ...
            if True:  # 20210610
                ...
                try:
                    ...
                    # traceback.print_exc()
                    logger.warning("%s setup_handle failed with %s (change actors.py to see more)", self, e)
                    delattr(self, hname)
                    # raise
    ...
    def get_data_elem(self, name):
        ...
        if len(s) == 1:
            ...
            if m is None:
                raise Exception("No plugin {} ({}.{})".format(self.app_label, self, name))
                # return None
    ...
    def get_table_as_list(cls, obj, ar):
        ...
        html = ""
        for obj in sar.sliced_data_iterator:
            html += "<p>{}</p>".format(ar.row_as_paragraph(obj))
