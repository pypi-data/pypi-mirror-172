...
class Model(models.Model, fields.TableRow):
    ...
    def fields_to_bleach(self):
        ...
        if bleach:
            for f in self._bleached_fields:
                ...
                try:
                    new = bleach.clean(
                        old, tags=settings.SITE.bleach_allowed_tags,
    ...
    def edge_ngram_field(self):
        values = [str(getattr(self, field.name)) for field in self.quick_search_fields + self.quick_search_fields_digit]
        return ' '.join(values)
    ...
    def as_summary_row(self, ar):
        # used by tickets.Tickets table in the bootstrap frontend
        # also when display_mode = ((None, constants.DISPLAY_MODE_STORY), ) it returns html
        # text for the object that is calling
