...
p.add_argument("-r", "--root", type=str, required=False,
               help="Specify the path to a directory. All files that are part of that directory "
...
p.add_argument("-e", "--extension", type=str, required=False, nargs="+",
               help="Specify file extensions which should be included in the search. "
                    ...
                    "the following files: R, Rmd, RMD, txt, py, ipynb, md, pl, cpp, java")
...
def main():
    ...
    print(logo)
    time.sleep(0.5)
    print(f"In total, there are {len(files_to_search)} files with the indicated file extension.")
    for file in tqdm(files_to_search):
        std_out_lines = []
        try:
    ...
    if n_matches == 0:
        print(
            f"Didnt find a match in any file. The source file was nonetheless moved. Root directory was: {root_directory}")
    else:
