        ...
        def _orderDictToString(orderBy:dict) -> str:
            ...
            for key, value in orderBy.items():
                if value not in ['ASC','DESC']:
                    Utils._error(self, f"Invalid value '{value}' for property '{key}'. Use 'ASC' or 'DESC' instead.")
                    return
                order += f'{{{key}: {value}}}'
            order += ']'
            return order
        ...
        if orderBy != None:
            ...
            if type(orderBy) == list:
                if type(asc) == bool:
                    ...
                    Utils._error(self, f"Invalid type '{asc}' for order direction. Use bool or list of bools.")
            ...
            if type(orderBy) == str:
                if type(asc) == bool:
                    ...
                    Utils._error(self, f"Invalid type '{asc}' for order direction. Use bool instead.")
                order = _orderDictToString(_orderBy)                  
        else:
            order = ''
