...
class FileUtils(UtilsProperty):
                        ...
                        if parentItemMapping != None:
                            if header[j] == uniqueProperty:
                                if parentItemMapping[value] != None:
                                    item.setdefault('sys_parentInventoryItemId', parentItemMapping[value])
                        if referenceMapping != None and header[j] in referenceMapping:
                            parentHeader = header[j].split('.')[0]
                            if parentHeader in isArray:
                                nameList = UtilsProperty._transformList(value, 'String')
                                try:
                                ...
                                try:
                                    item.setdefault(parentHeader, referenceMapping[header[j]][value])
                                except KeyError as key:
                                    logger.warning(f"Line {i}, field '{header[j]}': {key} is not a valid reference.")
                                    return
                        else:
    ...
    def _createParentMapping(client:object, inventoryName:str, content:list, uniqueProperty:str ) -> dict:
        ...
        if not df.empty:
            mapping = {item[1][uniqueProperty]: item[1]['sys_inventoryItemId'] for item in df.iterrows()}
            remainingUniqueValues = list(set(uniqueValues).difference(set(df[uniqueProperty])))
            logger.debug(f"Remaining unqique values: {remainingUniqueValues[:4]}...")
