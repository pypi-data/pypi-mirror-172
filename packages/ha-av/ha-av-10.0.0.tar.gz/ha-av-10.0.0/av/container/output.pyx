...
cdef class OutputContainer(Container):
    ...
    def add_stream(self, codec_name=None, object rate=None, object template=None, options=None, **kwargs):
        ...
        if not (template is None or isinstance(template, (Stream, BitStreamFilterContext))):
            raise ValueError("template must be Stream or BitStreamFilterContext")
        ...
        elif isinstance(template, BitStreamFilterContext):
            codec_obj = Codec(lib.avcodec_get_name((<BitStreamFilterContext>template).ptr.par_out.codec_id))
        else:
            if not template.codec_context:
                raise ValueError("template has no codec context")
            codec_obj = (<Stream>template).codec_context.codec
        codec = codec_obj.ptr
        ...
        if template is not None:
            if isinstance(template, BitStreamFilterContext):
                err_check(lib.avcodec_parameters_to_context(codec_context, (<BitStreamFilterContext>template).ptr.par_out))
                stream.time_base = codec_context.pkt_timebase = (<BitStreamFilterContext>template).ptr.time_base_out
            else:
                err_check(lib.avcodec_parameters_to_context(codec_context, (<Stream>template).ptr.codecpar))
