...
from makeit.core import ExistsChecker
...
def execute_mini_language(commands: list, tmp_path: Path):
    label2task = {}
    backend = tmp_path / "makeit.json"
    ...
    for command in commands:
        match command:
            ...
            case ["execute", kwargs, expected_exception]:
                with pytest.raises(expected_exception):
                    execute_on_change(label2task.values(), dag_name=f"{tmp_path.name}", backend=backend, **kwargs)

            case ["cute", execution, kwargs]:
                execution(label2task.values(), dag_name=f"{tmp_path.name}", backend=backend, **kwargs)

            case ["cute", execution, kwargs, expected_exception]:
                with pytest.raises(expected_exception):
                    execution(label2task.values(), dag_name=f"{tmp_path.name}", backend=backend, **kwargs)
...
@pytest.mark.parametrize("commands", [
    ...
    (
        ["task", ("x", ["a"], []), ("y", ["b"], []), ("z", ["c"], [])],
        ["write", ("a", "A"), ("b", "B"), ("c", "FAILURE")],
        ["execute", dict(commit_on_failure=False), Exception],
        ["exec", ("x", 1), ("y", 1), ("z", 1)],
...
def test_execution_flags_2(tmp_path, commands):
    ...
    execute_mini_language(commands, tmp_path)
...
def test_execution_flags_2(tmp_path, commands):
    register_type(Path, lambda _: File(_, label=_.name, checker=ExistsChecker()))
