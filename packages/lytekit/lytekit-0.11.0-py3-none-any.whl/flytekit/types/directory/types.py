...
class FlyteDirToMultipartBlobTransformer(TypeTransformer[FlyteDirectory]):
    ...
    def assert_type(self, t: typing.Type[FlyteDirectory], v: typing.Union[FlyteDirectory, os.PathLike, str]):
        ...
        raise TypeError(
            ...
            " Use (FlyteDirectory, str, os.PathLike)"
        )
    ...
    def to_literal(
        ...
        if python_val is None:
            raise TypeTransformerFailedError("None value cannot be converted to a directory.")
        ...
        if remote_directory is None and ctx.execution_state.mode == ExecutionState.Mode.LOCAL_WORKFLOW_EXECUTION:
            should_upload = False
        ...
        if should_upload:
            ...
            put_res = ctx.file_access.put_data(source_path, remote_directory, is_multipart=True)
            if put_res is None:
                put_res = {}

            return Literal(scalar=Scalar(blob=Blob(metadata=meta, uri=remote_directory)), hash=put_res.get("cache"))
