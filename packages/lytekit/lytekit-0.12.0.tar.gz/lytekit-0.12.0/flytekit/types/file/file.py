...
class FlyteFilePathTransformer(TypeTransformer[FlyteFile]):
    ...
    def assert_type(
        ...
        if isinstance(v, os.PathLike) or isinstance(v, FlyteFile) or isinstance(v, str):
            return
        raise TypeError(
            f"No automatic conversion found from type {type(v)} to FlyteFile.Supported (os.PathLike, str, Flytefile)"
        )
    ...
    def to_literal(
        ...
        if isinstance(python_val, FlyteFile):
            ...
            if python_type is os.PathLike:
                logger.warning(
                    ...
                    " specified. Since a simpler type was specified, we'll skip uploading!"
                )
                should_upload = False
                        ...
                        try:
                            if not p.is_file():
                                raise TypeTransformerFailedError(
                                    f"Error converting {python_val} because it's not a file."
                                )
                        except:
        ...
        # Don't upload by default in local execution
        if remote_path is None and ctx.execution_state.mode == ExecutionState.Mode.LOCAL_WORKFLOW_EXECUTION:
            should_upload = False
        ...
        if should_upload:
            ...
            put_res = ctx.file_access.put_data(source_path, remote_path, is_multipart=False)
            if put_res is None:
                put_res = {}

            return Literal(scalar=Scalar(blob=Blob(metadata=meta, uri=remote_path)), hash=put_res.get("cache"))
