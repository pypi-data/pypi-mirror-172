...
# リソースIDのコピーから取得可能
# 認証ファイルがある階層を引数でセット
def gcp_secret_manager(local_secret_file_folder_path):
    # ローカルで実行
    if os.name == 'nt':
        google_seacret_path = f'{local_secret_file_folder_path}/service_account.json'
...
# GCPの認証を戻す関数
# ローカルで動かす場合､ファイルパスを設定する必要があるので､忘れないように
def generate_google_service_auth(service, local_secret_file_folder_path):
    ...
    if service == "spreadsheet":
        ...
        credentials = ServiceAccountCredentials.from_json_keyfile_name(
            gcp_secret_manager(local_secret_file_folder_path), SCOPES)
