...
class Microservice(Construct):
    ...
    def create_network_load_balancer(self, scope, id, app_name, environment, network_load_balancer_ip_1, network_load_balancer_ip_2, network_load_balancer_subnet_1, network_load_balancer_subnet_2, dns_record=None):
        ...
        if not is_production:
            ...
            route53_zone = route53.PrivateHostedZone.from_hosted_zone_attributes(self, f"PrivateHostedZone{dns_record}", hosted_zone_id=hosted_zone_id, zone_name=domain_name)
            
            route53.ARecord(self, 
                f"ALBAliasRecord{dns_record}",
                zone=route53_zone,
                target=route53.RecordTarget.from_alias(route53_targets.LoadBalancerTarget(nlb)),
