...
class GeneratorXmlFourDotZero(AbstractGeneratorXml):
    ...
    def get_complemento_pagos(self):
        try:
            ...
            totales.setAttribute('MontoTotalPagos', str(self.data.get('monto')))

            for imp_ret in self.data.get('impuestos_trasladados_p'):
                if imp_ret.get('impuesto') == '002' and imp_ret.get('tipo_factor') == 'Exento':
                    totales.setAttribute('TotalTrasladosBaseIVAExento', str(imp_ret.get('base')))

            pago = self.dom.createElement('pago20:Pago')
            pago.setAttribute('FechaPago', self.data.get('fecha_pago'))
            ...
            pago.setAttribute('TipoCambioP', self.data.get('tipo_cambio'))
            ...
            doctos = self.data.get('doctos_relacionados')
            for docto_relacionado in doctos:
                ...
                docto.setAttribute('EquivalenciaDR', docto_relacionado.get('tipo_cambio'))
                docto.setAttribute('NumParcialidad', str(docto_relacionado.get('num_parcialidad')))
            ...
            if len(self.data.get('impuestos_retenidos_p')) > 0 or len(self.data.get('impuestos_trasladados_p')):
                pago.appendChild(self.get_impuestos_pago())
            pagos.appendChild(pago)
            return pagos
        except Exception as e:
            raise Exception("Ocurrio un error. Error::: " + str(e))
    ...
    def get_impuestos_pago(self):
        try:
            impuestos_p = self.dom.createElement('pago20:ImpuestosP')
            if len(self.impuestos_retenidos_p) > 0:
                retenciones = self.dom.createElement('pago20:RetencionesP')
                total_retenciones = 0
                for retencion in self.impuestos_retenidos_p:
                    total_retenciones += float(retencion.get('importe', 0))
                    retenciones.appendChild(self.get_retenciones(retencion))
                impuestos_p.appendChild(retenciones)
            if len(self.impuestos_trasladados_p) > 0:
                traslados = self.dom.createElement('pago20:TrasladosP')
                for traslado in self.impuestos_trasladados_p:
                    traslados.appendChild(self.get_traslados_pago(traslado))
