...
class AugFactory(Factory):
    ...
    def get_aug(self, configuration):
        ...
        if self._global_rate_limiter is not None:
            ...
            limiters.append(self.create_rate_limiter(limits_spec, quota, window_size, 1))
    ...
    def create_global_rate_limiter(self, aug_id):
        ...
        try:
            global_rate_limiter = self.create_rate_limiter(config.RateLimiter.GLOBAL_RATE_LIMIT,
                                                           0, 0, config.RateLimiter.GLOBAL_RATE_LIMIT_MULTIPLIER)
    ...
    def create_rate_limiter(self, limits_spec, default_quota, default_window_size, multiplier):
        quota = default_quota
        ...
        return AugRateLimiter(quota * multiplier, window_size)
