...
class NWTask:
    ...
    def __init__(self, name: str, service: "NWService", members: Dict[str, Any]):
        ...
        self.commands: List[Dict[str, Any]] = list(
            members["commands"])
        ...
        self.not_changed_command: Optional[Dict[str, Dict[str, Any]]] = members["not_changed_command"]
        self.parameters: List[Dict[str, Any]] = list(
            members["parameters"])
        ...
        for command in self.commands:
            for command_name, parameters in command.items():
                if "working_dir" not in parameters or len(str(parameters["working_dir"])) == 0:
                    parameters["working_dir"] = self.path
                else:
                    # prepend service path
                    parameters["working_dir"] = os.path.normpath(os.path.join(self.path, str(parameters["working_dir"])
                                                                              )).replace("\\", "/").replace("//", "/")
                ...
                if "unittest_dir" in config_commands[command_name]['parameters']:
                    ...
                    if "unittest_dir" not in parameters or len(str(parameters["unittest_dir"])) < 0:
                        if len(test_path) > 0:  # prevent override
                            parameters["unittest_dir"] = test_path
                    else:
                        # prepend service path
                        parameters["unittest_dir"] = os.path.normpath(os.path.join(self.path, str(parameters["unittest_dir"])
                                                                                   )).replace("\\", "/").replace("//", "/")
    ...
    def command_to_step(self, command: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        config_commands = self.service.context.general_config['commands']
    ...
    def test_path(self) -> str:
        ...
        def find_test_path_by_id(name: str, path: Path) -> str:
            name = re.sub("[\\.\\-_]", "", name)
            test_projects = list(map(lambda x: str(x), path.rglob(
                f"*.Test.csproj")))  # case sensitive names
            service_result = list(filter(lambda a_path: re.match(
                f".*{name}\\.test.*", str(a_path), flags=re.IGNORECASE), test_projects))
            csproj_path = str(service_result.pop()).replace(
                "\\", "/") if len(service_result) else ""
            return os.path.dirname(csproj_path)

        result = find_test_path_by_id(self.id, Path(self.path))
        if not len(result) > 0:
            result = find_test_path_by_id(
    ...
    async def to_yaml_async(self) -> Dict[str, Any]:
        def job(commands: List[Union[str, Dict[str, Any]]]) -> Dict[str, Any]:
            ...
            steps.insert(0,{
                "run":{
                    "name": "Preparing ramdisk",
                    "command": "(sudo mkdir /mnt/ramdisk || echo 'Unable to create ramdisk') && (sudo mount -o size=4G -t tmpfs none /mnt/ramdisk || echo 'Unable to mount ramdisk')"
                }})
            ...
            yml_obj = {
                ...
                "working_directory": "/mnt/ramdisk", # optimization to speed up builds
                "steps": steps
            }
        ...
        empty_job = [self.command_to_step(
            name, params) for name, params in self.not_changed_command.items()] if self.not_changed_command else[
            self.command_to_step("empty_job", {
                "job_name": self.name,
                "reason": reason
            })
        ]
