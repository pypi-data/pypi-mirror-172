        ...
        for name, parameters in validate_command.items():
            ...
            for param_name in config_parameters:
                ...
                if not param_name in validate_command[name] and "default" not in config_parameters[param_name]:
                    ...
                    if "parameters" not in captured_task or any([param_name not in x for x in captured_task["parameters"]]):
                        raise SchemaError(
                ...
                Or(
                    ...
                    {  # Task converted to a Circleci job; can be referenced via "ServiceName:TaskName"
                        str: And(Use(capture_task),
                            ...
                            Optional("resource_class", default="large"): And(str, Or("small", "windows.small", "medium", "windows.medium", "medium+", "large", "windows.large", "xlarge", "windows.xlarge", "2xlarge", "windows.2xlarge", "2xlarge+", "arm.medium")),
                            Optional("working_directory", default="/mnt/ramdisk"): str,
                            ...
                            Optional("requires", default=[]): [And(str, is_requires_id)],
                            Optional("not_changed_command", default=None): Or(Optional(
                                And(str, Use(convert_string_command), is_valid_command)), And(command_schema, is_valid_command)),
                            Optional("parameters", default=[]):[{str: Or(str, bool, float, int, list)}]
                        })
                    }
                )
            }
