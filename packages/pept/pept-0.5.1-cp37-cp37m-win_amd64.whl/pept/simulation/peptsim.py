...
class Simulator:
    ...
    def __init__(
        ...
        # Beta+ endpoint/decay energy of tracer, in MeV. F-18 has a Beta+ decay
        # energy of 0.6335 MeV
        self.decay_energy = decay_energy

        # Effective atomic weight Aeff and atomic number Zeff. For water,
        # Aeff = 13, Zeff = 7.22
        self.Aeff = Aeff
        self.Zeff = Zeff
    ...
    def simulate(self):
        ...
        location_indices = np.searchsorted(
            self.trajectory[:, 0],
            self.sampling_times,
            side='left',
        )
        ...
        for i in tqdm(range(0, number_of_samples)):
            ...
            # Calculate incident/kinetic energy Ei of positron from
            # distribution approximated as gaussian
            ...
            # Find positron annihilation point, corresponding to gamma
            # radiation emission using Palmer et al., 1992 equations
            ...
            # Try at most 100 random phi and theta angles until the ray falls
            # within the PEPT screens
            ray_try = 0
            while ray_try < 100:
                ...
                if 0 < x1 < self.x_max and 0 < x2 < self.x_max:
                    if 0 < y1 < self.y_max and 0 < y2 < self.y_max:
                        pept_data.append(np.array([
                            self.sampling_times[i], x1, y1, x2, y2
                        ]))
                        break
                ray_try += 1
    ...
    def add_noise(self, noise_ratio):
        noiser = Noise(
            self.trajectory, self.x_max, self.y_max, self.separation
        )
        number_of_noise_points = int(noise_ratio * len(self.sampling_times))
        self.pept_data_with_noise = noiser.add_noise_to_pept(
            self.pept_data, number_of_noise_points
        )
    ...
    def add_spread(self, max_spread = 4, depth = 16):
        ...
        self.pept_data_with_noise = noiser.add_spread_to_pept(
    ...
    def add_noise_and_spread(self, noise_ratio, max_spread = 4, depth = 16):
        ...
        self.pept_data_with_noise = noiser.add_spread_to_pept(
            self.pept_data_with_noise, max_spread, depth
        )
    ...
    def write_noise_csv(self, fname):
        np.savetxt(fname, self.pept_data_with_noise, delimiter = '   ',
                   newline = '\n  ')
