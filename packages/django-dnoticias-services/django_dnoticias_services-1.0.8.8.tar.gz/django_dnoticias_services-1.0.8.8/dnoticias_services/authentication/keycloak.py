...
class CreateUser:
    ...
    def __call__(
        ...
        use_cached_token: Optional[bool] = False,
        **kwargs
    ) -> User:
        ...
        :param max_sessions: Max sessions allowed to this user. Default=2
        :param use_cached_token: If true, will not refresh the token until the cache expires.
        Defayult: False
        ...
        :return: Created user
        :rtype: User
        ...
        if use_cached_token:
            ...
            if not cache.get(key):
                self.base.keycloak_admin.refresh_token()
                cache.set(key, True, timeout=60*10)
...
class DeleteUserSession:
    ...
    def __call__(self, session: str) -> bool:
        ...
        return self.base.keycloak_admin.delete_user_session(session)
