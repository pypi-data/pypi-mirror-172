...
def retrieve_notebook_path(session: Session, datalab_notebook_url=None):
    if datalab_notebook_url is None:
        if _datalab.is_datalab():
            return _datalab.get_data_lab_orchestrator_url(), _datalab.get_data_lab_project_id(), \
        ...
        if not data_lab_url:
            if _datalab.is_datalab() or _datalab.is_executor():
                data_lab_url = _datalab.get_data_lab_orchestrator_url()
            else:
...
def is_referencing_this_project(datalab_notebook_url):
    if datalab_notebook_url is None:
        ...
        return datalab_notebook_url.lower().startswith(_datalab.get_data_lab_project_url().lower())
...
def parse_data_lab_url_project_id_and_path(notebook_url):
    ...
    data_lab_url = _datalab.get_data_lab_orchestrator_url() if matches.group(1) == '/data-lab' else matches.group(1)
