...
def push(data=None, *, metadata=None, replace=None, workbook=_common.DEFAULT_WORKBOOK_PATH,
         ...
         errors=None, quiet=False, status=None, session: Optional[Session] = None):
    """
    ...
    if metadata is not None and not metadata.empty:
        ...
        push_result_df = _metadata.push(session, metadata, workbook_context, datasource_output, sync_token,
                                        _metadata_push_first_pass_filter, None, push_metadata_status)
    ...
    if metadata is not None and not metadata.empty:
        ...
        push_result_df = _metadata.push(session, metadata, workbook_context, datasource_output, sync_token,
                                        _metadata_push_second_pass_filter, push_result_df, push_metadata_status)
...
def _push_condition(session: Session, condition_metadata, replace, data, status_index, status: Status,
    ...
    condition_update_input = ConditionInputV1()
    _metadata.dict_to_condition_input(condition_metadata, condition_update_input)
...
def _push_workbooks(session: Session, push_result_df, workbook_context: WorkbookContext, datasource,
    ...
    return spy.workbooks.push(workbook_context.to_push, path=workbook_context.folder_id, refresh=True,
                              datasource=datasource, lookup_df=push_result_df,
