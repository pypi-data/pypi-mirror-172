...
class GreenhouseService(object):
    ...
    def __init__(self, conf:GHConf=None):
        ...
        self.analog = GHAnalogService(pi, spi)
        self.buzzer = GHBuzzer(pi, GHgpio.BUZZER)
        self.fan = GHFan(pi, GHgpio.FAN)
        self.lamps = GHLamps(pi, GHgpio.RED_LED, GHgpio.WHITE_LED, GHgpio.RED_LED, GHgpio.DUAL_LED)
        self.servo = GHServo(pi, GHgpio.SERVO_PWM, conf.servo_cw_position, conf.servo_ccw_position)
        self.switches = GHSwitches(pi, GHgpio.SWITCH_PB, GHgpio.SWITCH_TOGGLE)
    ...
    def update_greenhouse(self, greenhouse:Greenhouse ):
    ...
    def _make_message(self, gh:Greenhouse):
        m = ""
