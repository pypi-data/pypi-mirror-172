...
@LastEditTime: 2022-10-17 14:47:19
...
class SaveActPrizeHandler(ClientBaseHandler):
    ...
    def post_async(self):
        ...
        if invoke_result_data.data["is_add"] == True:
            # 记录日志
            self.create_operation_log(operation_type=OperationType.add.value, model_name=invoke_result_data.data["new"].__str__(), old_detail=None, update_detail=invoke_result_data.data["new"], title=prize_name)
        else:
            self.create_operation_log(operation_type=OperationType.update.value, model_name=invoke_result_data.data["new"].__str__(), old_detail=invoke_result_data.data["old"], update_detail=invoke_result_data.data["new"], title=prize_name)
        ref_params = {}
...
class DeleteActPrizeHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        self.create_operation_log(operation_type=OperationType.delete.value, model_name="act_prize_tb", title=invoke_result_data.data["prize_name"])
        ref_params = {}
        invoke_result_data = self.business_process_executed(invoke_result_data, ref_params)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
...
class ReviewActPrizeHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        self.create_operation_log(operation_type=OperationType.review.value, model_name="act_prize_tb", title=invoke_result_data.data["prize_name"])
        ref_params = {}
        invoke_result_data = self.business_process_executed(invoke_result_data, ref_params)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
...
class ReleaseActPrizeHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        self.create_operation_log(operation_type=operation_type, model_name="act_prize_tb", title=invoke_result_data.data["prize_name"])
        ref_params = {}
        invoke_result_data = self.business_process_executed(invoke_result_data, ref_params)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
...
class ActPrizeExportHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        field = invoke_result_data.data["field"] if invoke_result_data.data.__contains__("field") else "*"
        menu_name = invoke_result_data.data["menu_name"] if invoke_result_data.data.__contains__("menu_name") else "奖品列表"
        ...
        self.create_operation_log(operation_type=OperationType.export.value, model_name="act_prize_tb", title=menu_name)
