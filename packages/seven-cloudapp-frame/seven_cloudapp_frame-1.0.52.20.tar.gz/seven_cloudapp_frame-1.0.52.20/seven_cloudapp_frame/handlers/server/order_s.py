...
@LastEditTime: 2022-10-17 17:21:42
...
class UpdatePrizeOrderSellerRemarkHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        title = "修改订单备注；用户昵称：" + invoke_result_data.data["user_nick"] + "，opnid：" + invoke_result_data.data["open_id"]
...
class UpdatePrizeOrderStatusHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if order_status == 1:
            ...
            title = "不予发货；"
        title = title + "用户昵称：" + invoke_result_data.data["user_nick"] + "，opnid：" + invoke_result_data.data["open_id"]
        self.create_operation_log(operation_type=OperationType.operate.value, model_name="prize_order_tb", title=title)
        ref_params = {}
...
class ImportPrizeOrderHandler(ClientBaseHandler):
    ...
    def post_async(self):
        ...
        if not invoke_result_data.data:
            invoke_result_data.data = {}
        prize_roster_sub_table = invoke_result_data.data["prize_roster_sub_table"] if invoke_result_data.data.__contains__("prize_roster_sub_table") else None
        menu_name = invoke_result_data.data["menu_name"] if invoke_result_data.data.__contains__("menu_name") else "发货列表"
        order_base_model = OrderBaseModel(context=self)
        ...
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        self.create_operation_log(operation_type=OperationType.import_data.value, model_name="prize_order_tb", title=menu_name)
        ref_params = {}
        invoke_result_data = self.business_process_executed(invoke_result_data, ref_params)
        if invoke_result_data.success ==False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
...
class PrizeOrderExportHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if page_size <= 500:
            prize_order_list_dict, prize_order_list_total = order_base_model.get_prize_order_list(app_id, act_id, user_id, user_open_id, user_nick, order_no, real_name, telephone, address, order_status, create_date_start, create_date_end, page_size, page_index, order_by, field=field, is_search_roster=True, is_cache=False, condition=condition, params=params, prize_roster_sub_table=prize_roster_sub_table)
        else:
            max_page_size = 100
            repeat_count = page_size // max_page_size  # 循环次数
            begin_page_index = page_index * repeat_count  # 开始页码
            for i in range(repeat_count):
                list_dict, list_total = order_base_model.get_prize_order_list(app_id, act_id, user_id, user_open_id, user_nick, order_no, real_name, telephone, address, order_status, create_date_start, create_date_end, max_page_size, begin_page_index + i, order_by, field=field, is_search_roster=True, is_cache=False, condition=condition, params=params, prize_roster_sub_table=prize_roster_sub_table)
                if len(list_dict) <= 0:
                    break
        ...
        self.create_operation_log(operation_type=OperationType.export.value, model_name="prize_order_tb", title=menu_name)
...
class PrizeRosterExportHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        order_by = invoke_result_data.data["order_by"] if invoke_result_data.data.__contains__("order_by") else "create_date desc"
        field = invoke_result_data.data["field"] if invoke_result_data.data.__contains__("field") else "*"
        menu_name = invoke_result_data.data["field"] if invoke_result_data.data.__contains__("menu_name") else "中奖记录列表"
        ...
        resource_path = OSS2Helper.export_excel(result_data)
        self.create_operation_log(operation_type=OperationType.export.value, model_name="prize_roster_tb", title=menu_name)
        return self.response_json_success(resource_path)
...
class TaoPayOrderExportHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        order_by = invoke_result_data.data["order_by"] if invoke_result_data.data.__contains__("order_by") else "create_date desc"
        field = invoke_result_data.data["field"] if invoke_result_data.data.__contains__("field") else "*"
        menu_name = invoke_result_data.data["field"] if invoke_result_data.data.__contains__("menu_name") else "支付订单列表"
        ...
        resource_path = OSS2Helper.export_excel(result_data)
        self.create_operation_log(operation_type=OperationType.export.value, model_name="tao_pay_order_tb", title=menu_name)
        return self.response_json_success(resource_path)
