...
@LastEditTime: 2022-10-17 17:14:19
...
class UpdateUserStatusByBlackHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code,invoke_result_data.error_message)
        title = "拉黑用户；用户昵称：" + invoke_result_data.data["user_nick"] + "，opnid：" + invoke_result_data.data["open_id"]
        self.create_operation_log(operation_type=OperationType.operate.value, model_name="user_info_tb", title=title)
        ref_params = {}
...
class AuditUserBlackHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if audit_status == 2:
            ...
            title = "拒绝解锁用户；"
        title = title + "用户昵称：" + invoke_result_data.data["user_info"]["user_nick"] + "，opnid：" + invoke_result_data.data["user_info"]["open_id"]
...
class UpdateAuditRemarkHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        title = "修改黑名单审核备注；用户昵称：" + invoke_result_data.data["user_black"]["user_nick"] + "，opnid：" + invoke_result_data.data["user_black"]["open_id"]
        self.create_operation_log(operation_type=OperationType.operate.value, model_name="user_black_tb", title=title)
        return self.response_json_success()
...
class UpdateUserAssetHandler(ClientBaseHandler):
    ...
    def post_async(self):
        ...
        if asset_type == 1:
            ...
            title = "档位配置；"
        title = title + "用户昵称：" + user_info_dict["user_nick"] + "，opnid：" + user_info_dict["open_id"]
...
class BatchUpdateUserAssetHandler(ClientBaseHandler):
    ...
    def post_async(self):
        ...
        for asset_object in asset_object_json:
            invoke_result_data = asset_base_model.update_user_asset(app_id, act_id, 0, user_id, user_info_dict["open_id"], user_info_dict["user_nick"], int(asset_object["asset_type"]), int(asset_object["asset_value"]), asset_object["asset_object_id"], 3, "", "手动配置", "手动配置",handler_name = self.__class__.__name__,request_code = self.request_code)
            if invoke_result_data.success == False:
...
class AssetLogListHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        asset_base_model = AssetBaseModel(context=self)
        page_list, total = asset_base_model.get_asset_log_list(app_id, act_id, asset_type, page_size, page_index, user_id, asset_object_id, start_date, end_date, user_nick, user_open_id, source_type, source_object_id, field, is_cache=False, operate_type=operate_type)
        ref_params = {}
