    ...
    def download(self, path: str = None) -> DownloadFileResponse:
        ...
        try:
            ...
            with open(temp_file, "wb") as f:
                with httpx.stream(method="GET", url=self.signed_url) as http_response:
        ...
        try:
            ...
            with open(temp_file, "wb") as f:
                async with client.stream(
                    method="GET", url=self.signed_url
                ) as http_response:
                    if http_response.is_error:
                        # Raise Error
...
filings_retrieval_data_provider = DataProvider(
    request=FilingsRequestFactory(),
    response=ResponseFactory(data_class=FilingsData),
