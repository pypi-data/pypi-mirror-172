...
class Basket(Serializable["Basket"]):
    ...
    def _from_dict(cls, data: dict) -> "Basket":
        ...
        return cls(
            constituents=constituents, normalize_by_weight=normalize_by_weight
        )
...
class ManualItem:
    ...
    def _to_dict(self) -> dict:
        return {"month": self.month, "year": self.year, "startDate": self.start_date}
...
class ManualRollover(Serializable["ManualRollover"]):
    ...
    @classmethod
    def _from_dict(cls, data: dict) -> "ManualRollover":
        data = [ManualItem(**convert_camel_to_snake(item)) for item in data]
        return cls(*data)
