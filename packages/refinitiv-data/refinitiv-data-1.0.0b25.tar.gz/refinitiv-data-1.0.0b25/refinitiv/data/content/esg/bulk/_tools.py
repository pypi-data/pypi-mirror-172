...
regexp_filename_obj = re.compile(
    r".*((d|D)elta|(i|I)nit).*(\d{4}-\d{2}-\d{2}).*(\.jsonl.gz|\.jsonl)$"
)
regexp_init_archives = re.compile(r".*(-(i|I)nit-).*\d{4}-\d{2}-\d{2}.*(.gz)$")
regexp_delta_files = re.compile(r".*(Jsonl-(d|D)elta-).*\d{4}-\d{2}-\d{2}.*")
regexp_init_file = re.compile(r".*(Jsonl-(i|I)nit-).*\d{4}-\d{2}-\d{2}.*")
