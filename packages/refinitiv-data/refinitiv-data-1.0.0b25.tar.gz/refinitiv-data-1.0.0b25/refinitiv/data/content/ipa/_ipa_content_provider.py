...
from ...delivery._data._data_provider_factory import (
    get_base_url,
    get_api_config,
    get_url,
)
...
class IPAContentProviderLayer(ContentProviderLayer):
    ...
    def _get_data_with_async_mode(self) -> "Response":
        ...
        while loop_retrieving:
            ...
            with self._lock:
                self._session.debug(f"Request operation :\n {url}")
                operation_response = self._provider.get_data(
                    self._session, url, method=RequestMethod.GET, **self._kwargs
                )

            status_code = operation_response.http_status.get("http_status_code")
            if status_code != 200:
