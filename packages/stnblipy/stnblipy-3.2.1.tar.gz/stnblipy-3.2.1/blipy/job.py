...
class Job():
    ...
    def importa_tabela_por_sql(self, conn_entrada, conn_saida, sql_entrada,
        ...
        cols_saida: lista das colunas que serão gravadas na tabela 
                    ...
                    transformanção a ser aplicada (função de transformação é
    ...
    def importa_planilha(self, conn_saida, 
        nome_tab_saida, cols_saida, cols_entrada=None,
        ...
        Args:
            ...
            cols_saida:     lista das colunas que serão gravadas na tabela 
                            ...
                            transformação é opcional; se não
                            informado, faz só uma cópia do dado)
            cols_entrada:   lista com o índice (zero-based) das colunas que
                            serão utilizadas da planilha. Se None, utiliza
                            todas as colunas, na ordem da planilha.  Esse
                            parâmetro é importante quando se quer que uma única
                            coluna da planilha seja fonte de entrada para mais
                            de uma coluna de saída, já que a relação entre
                            cols_saida e cols_entrada tem que ser de 1 para 1.
                            Por exemplo, se a primeira coluna da planilha for
                            fonte de entrada para a primeira e terceira colunas
                            da tabela de saída, esse parâmetro deve ser [0, 1,
                            0]
            estrategia:     estratégia de gravação que será utilizada (enum
                            TpEstrategia)
        ...
        Obs.: os argumentos de io a engine são os mesmos do método read_excel do Pandas.
        """
    ...
    def __grava_tabela(self, 
        ...
        Args:
        entrada: a fonte de entrada dos dados (tabela de banco, planilha etc.)
        conn_saida: conexão com o esqumea de saída (geralmente a produção)
        nome_tabela_saida: nome da tabela de saida
        cols_saida: lista das colunas que serão gravadas na tabela 
                    de saida, com o nome da coluna, seu tipo e a função de
