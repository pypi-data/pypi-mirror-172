...
class FinFlotransition():
    ...
    def gets_base_action(self):
        try:
            gets_action_data = Action.objects.get(description=self.action.upper(), model__description = self.type.lower() )
    ...
    def manualtransitions(self):
        try:
            ...
            wf = workflowitems.objects.update_or_create( transitionmanager= queryset , defaults= {"initial_state" : self.source, "interim_state" : self.interim ,
                    "final_state" : self.target, "model_type" : self.type.upper(), "event_user" : get_current_user() , "current_from_party " : self.from_party, "current_to_party " : self.to_party})
            wf.save()
            workevents.objects.create(workflowitems = wf , event_user=get_current_user(),  initial_state=self.source  , 
                    interim_state = self.interim , final_state=self.target ,type=self.type.upper() , record_datas = self.get_record_datas() , from_party = self.from_party , to_party = self.to_party)
        except:
            return None  
    ...
    def transition(self):
        ...
        if overall_model[0] is not None :
            def Transition_Handler():
                    ...
                    #     workevents.objects.create(workflowitems=overall_model[3], event_user=get_current_user(),  initial_state=overall_model[1].from_state.description,
                    #                               interim_state = overall_model[4][1 + overall_model[0].sub_sign], record_datas = self.get_record_datas() , final_state=overall_model[1].to_state.description, action= self.action, subaction= self.action, type= self.type.upper(), from_party = overall_model[1].from_party , to_party = overall_model[1].from_party)
                    ...
                    if len(overall_model[4]) != overall_model[0].sub_sign:
                        ...
                        try:
                            ...
                            workevents.objects.create(workflowitems=overall_model[3], event_user=get_current_user(),  initial_state=overall_model[1].from_state.description,
                                                  interim_state = overall_model[1].to_state.description,record_datas = self.get_record_datas() ,  final_state=overall_model[1].to_state.description, action= self.action, subaction= self.action, type = self.type.upper(), from_party = overall_model[1].from_party , to_party = overall_model[1].to_party , final_value = True)
                            overall_model[0].sub_sign = 0
