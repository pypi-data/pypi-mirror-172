...
class SignList(models.Model):
    sign_id = models.IntegerField(validators=[validate_postive],blank=True, null=True)
    name = models.CharField(max_length = 255 )
    sub_action_name = models.CharField(max_length = 255 )
    ...
    def save(self, *args, **kwargs):
        try:
            self.name  ,  self.sub_action_name = self.name.capitalize() , self.sub_action_name.capitalize()
            ...
            for i in range(0,sign_list_counter):
                SignList.objects.all().update(sign_id = i)
            pass
