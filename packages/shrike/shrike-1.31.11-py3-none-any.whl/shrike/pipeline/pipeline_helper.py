...
class AMLPipelineHelper:
    ...
    def _apply_windows_runsettings(
        ...
        **custom_runtime_arguments,
        ...
        if self.module_loader.is_local(module_name):
            ...
            if self.config.compute.hdi_dc_target:
    ...
    def _apply_synapse_runsettings(
        ...
        **custom_runtime_arguments,
        ...
        if self.module_loader.is_local(module_name) and self.is_eyesoff():
            log.warning(
                "The job is expected to fail, as local Synapse components are not supported in eyes-off environments"
            )

        target = (
            target if target is not None else self.config.compute.synapse_prod_target
        )
        log.info(
