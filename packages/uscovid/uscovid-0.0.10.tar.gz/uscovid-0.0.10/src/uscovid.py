...
#print(daysdate)
for i in states:
 dd.loc[dd.states==i, 'population']=round(int(str(pp.loc[pp.Name==i,'Pop. 2020'].tolist().pop()).replace(',',''))/100000,3)
print(dd)
...
for i in states:
 ...
 for j in daysdate:
  ...
  ddd.loc[ddd.date==j,'score']=round(float(d.loc[(d.date==j) & (d.state==i),'deaths']*10)/int(dd.loc[dd.states==i,'population']),2)
