...
@Repository(model = SubscriptionModel.SubscriptionModel)
class SubscriptionModelRepository:
    ...
    def findAllByQueueKey(self, queueKey) :
        modelList = self.repository.session.query(self.model).filter(self.model.queueKey == queueKey).all()
        self.repository.session.commit()
        return modelList
