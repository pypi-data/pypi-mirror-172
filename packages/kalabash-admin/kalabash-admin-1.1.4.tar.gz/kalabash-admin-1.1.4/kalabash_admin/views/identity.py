...
from kalabash.lib.listing import (
...
@user_passes_test(
    ...
    u.has_perm("kalabash_admin.add_alias")
)
def _identities(request):
    ...
    if page is None:
        ...
        context["headers"] = _render_to_string(
            request, "kalabash_admin/identity_headers.html", {})
        context["rows"] = _render_to_string(
            request, "kalabash_admin/identities_table.html", {
...
@login_required
@permission_required("kalabash_admin.add_mailbox")
def list_quotas(request):
    ...
    context = {
        "headers": _render_to_string(
            request, "kalabash_admin/quota_headers.html", {}
        )
    }
    if page is None:
        context["length"] = 0
        ...
        context["rows"] = _render_to_string(
            request, "kalabash_admin/quotas.html", {
                "mboxes": page
            }
        )
        context["pages"] = [page.number]
...
@login_required
@user_passes_test(
    lambda u: u.has_perm("kalabash_admin.add_user") or
    u.has_perm("kalabash_admin.add_alias") or
    u.has_perm("kalabash_admin.add_mailbox")
)
...
@ensure_csrf_cookie
def identities(request, tplname="kalabash_admin/identities.html"):
...
def accounts_list(request):
    accs = User.objects.filter(is_superuser=False) \
        .exclude(groups__name='StandardUsers')
