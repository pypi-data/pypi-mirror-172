...
from kalabash.lib.web_utils import (
    _render_to_string, render_to_json_response
)
from kalabash.lib.exceptions import (
    PermDeniedException
)
from kalabash.lib.listing import get_sort_order, get_listing_page
...
@login_required
def index(request):
    return HttpResponseRedirect(reverse("kalabash_admin:domain_list"))


@login_required
@user_passes_test(
    lambda u: u.has_perm("kalabash_admin.view_domains")
    or u.has_perm("kalabash_admin.view_mailboxes")
)
def _domains(request):
    ...
    if page is None:
        ...
        context["rows"] = _render_to_string(
            request, "kalabash_admin/domains_table.html", {
...
@login_required
@ensure_csrf_cookie
def domains(request, tplname="kalabash_admin/domains.html"):
    if not request.user.has_perm("kalabash_admin.view_domains"):
        if request.user.has_perm("kalabash_admin.view_mailboxes"):
            return HttpResponseRedirect(
                reverse("kalabash_admin:identity_list")
            )
...
@login_required
@permission_required("kalabash_admin.add_domain")
...
@login_required
@permission_required("kalabash_admin.view_domains")
@reversion.create_revision()
...
@login_required
@permission_required("kalabash_admin.delete_domain")
...
def domain_statistics(request):
    ...
    return render(request, "kalabash_admin/domain_statistics.html", context)
