"""Registration with Kalabash takes place here."""

from django.core.urlresolvers import reverse
from django.utils.translation import ugettext as _, ugettext_lazy

from kalabash.core.extensions import KalmonakExtension, exts_pool
from kalabash.lib import parameters, events
from kalabash.lib.email_utils import split_mailbox
from kalabash.lib.exceptions import PermDeniedException, BadRequest, Conflict
...
PERMISSIONS = {
    "DomainAdmins": [
        ...
        ["kalabash_admin", "domain", "view_domains"],
        ["kalabash_admin", "domain", "view_domain"],
        ["kalabash_admin", "mailbox", "add_mailbox"],
        ["kalabash_admin", "mailbox", "change_mailbox"],
        ["kalabash_admin", "mailbox", "delete_mailbox"],
        ["kalabash_admin", "alias", "add_alias"],
        ["kalabash_admin", "alias", "change_alias"],
        ["kalabash_admin", "alias", "delete_alias"],
        ["kalabash_admin", "mailbox", "view_mailboxes"],
        ["kalabash_admin", "alias", "view_aliases"],
        ["kalabash_admin", "domainalias", "view_domaliases"],
        ["kalabash_admin", "domainalias", "add_domainalias"],
        ["kalabash_admin", "domainalias", "change_domainalias"],
        ["kalabash_admin", "domainalias", "delete_domainalias"]
    ]
}


class AdminConsole(KalmonakExtension):
    name = "kalabash_admin"
...
def extra_roles(user, account):
    ...
    if user.has_perm("kalabash_admin.add_domain") and \
...
def extra_routes():
    ...
    return [
        url(r'^user/forward/',
            'kalabash_admin.views.user.forward',
            name="user_forward"),
    ]
...
def admin_menu(target, user):
    if target != "top_menu":
        return []
    entries = []
    if user.has_perm("kalabash_admin.view_domains"):
        entries += [
            {"name": "domains",
             "url": reverse("kalabash_admin:domain_list"),
             "label": _("Domains")}
        ]
    conditions = (
        user.has_perm("core.add_user"),
        user.has_perm("kalabash_admin.add_alias")
    )
    if any(conditions):
        entries += [
            {"name": "identities",
             "url": reverse("kalabash_admin:identity_list"),
             "label": _("Identities")},
        ]
    return entries
...
def grant_access_to_all_objects(user, role):
    ...
    from kalabash.lib.permissions import grant_access_to_objects
...
def import_account_mailbox(user, account, row):
    ...
    if account.email:
        ...
        mb.set_quota(
            quota, override_rules=user.has_perm("kalabash_admin.change_domain")
        )
...
def account_auto_created(user):
    ...
    if user.group != 'StandardUsers' and domname is None:
        return
