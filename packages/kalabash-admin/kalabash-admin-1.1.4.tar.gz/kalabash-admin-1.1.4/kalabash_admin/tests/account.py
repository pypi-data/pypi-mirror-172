...
class AccountTestCase(KalmonakTestCase):
    ...
    def test_crud(self):
        ...
        self.ajax_post(
            reverse("kalabash_admin:account_change", args=[account.id]), values
        )
    ...
    def _set_quota(self, email, value, expected_status=200):
        ...
        values = {
            "username": email, "role": "StandardUsers", "quota_act": False,
            "is_active": True, "quota": value, "email": email
        }
        self.ajax_post(
            reverse("kalabash_admin:account_change", args=[account.id]),
    ...
    def test_master_user(self):
        ...
        self.ajax_post(
            reverse("kalabash_admin:account_add"), values
        )
        ...
        self.ajax_post(
            reverse("kalabash_admin:account_add"), values, status=400
        )
...
class PermissionsTestCase(KalmonakTestCase):
    ...
    def test_domain_admins(self):
        self.ajax_post(
            reverse("kalabash_admin:account_change", args=[self.user.id]),
            self.values
        )
    ...
    def test_self_modif(self):
        ...
        self.ajax_post(
            reverse("kalabash_admin:account_change", args=[admin.id]),
            values
        )
    ...
    def test_domadmin_access(self):
        ...
        self.assertEqual(
            self.clt.login(username="admin@test.com", password="toto"),
            True)
        response = self.clt.get(reverse("kalabash_admin:domain_list"))
    ...
    def test_domainadmin_deletes_superadmin(self):
        ...
        self.clt.login(username="admin@test.com", password="toto")
        self.ajax_post(
            reverse("kalabash_admin:account_delete", args=[account.id]), {}, 403
        )
    ...
    def test_domainadmin_dlist_local_domain_not_owned(self):
        ...
        self.ajax_post(reverse("kalabash_admin:dlist_add"), values)
    ...
    def test_domainadmin_master_user(self):
        ...
        values = dict(
            ...
            role="StandardUsers", is_active=True, master_user=True,
