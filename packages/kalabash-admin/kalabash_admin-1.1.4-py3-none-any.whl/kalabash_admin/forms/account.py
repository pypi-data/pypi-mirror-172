...
from kalabash.lib.exceptions import PermDeniedException, Conflict, NotFound
...
class AccountFormGeneral(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        if self.cleaned_data["role"] != "StandardUsers":
            return self.cleaned_data["username"]
...
class AccountFormMail(forms.Form, DynamicForm):
    ...
    def create_mailbox(self, user, account):
        ...
        self.mb.set_quota(self.cleaned_data["quota"],
                          user.has_perm("kalabash_admin.add_domain"))
    ...
    def update_mailbox(self, user, account):
        ...
        elif (account.group == "StandardUsers" and
        ...
        override_rules = True \
            if not self.mb.quota or user.has_perm("kalabash_admin.add_domain") \
            else False
...
class AccountForm(TabForms):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        self.forms = [
            dict(id="general", title=_("General"),
                 formtpl="kalabash_admin/account_general_form.html",
                 cls=AccountFormGeneral,
                 new_args=[self.user], mandatory=True),
            dict(id="mail",
                 title=_("Mail"), formtpl="kalabash_admin/mailform.html",
                 cls=AccountFormMail),
            dict(
                id="perms", title=_("Permissions"),
                formtpl="kalabash_admin/permsform.html",
    ...
    def extra_context(self, context):
        ...
        context.update({
            ...
            'formid': 'accountform',
            'action': reverse("kalabash_admin:account_change",
                              args=[account.id]),
        })
    ...
    def check_perms(self, account):
        if account.is_superuser:
            return False
        return self.user.has_perm("kalabash_admin.add_domain") \
...
class AccountWizard(WizardForm):
    ...
    def extra_context(self, context):
        context.update({
            'title': _("New account"),
            'action': reverse("kalabash_admin:account_add"),
