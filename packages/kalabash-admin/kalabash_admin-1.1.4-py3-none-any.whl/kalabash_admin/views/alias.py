...
from kalabash.lib.exceptions import PermDeniedException, Conflict
...
def _new_alias(request, title, action, successmsg,
               tplname="kalabash_admin/aliasform.html"):
...
@login_required
@permission_required("kalabash_admin.add_alias")
@reversion.create_revision()
def newdlist(request):
    return _new_alias(
        request, _("New distribution list"),
        reverse("kalabash_admin:dlist_add"),
...
def newalias(request):
    return _new_alias(
        request, _("New alias"), reverse("kalabash_admin:alias_add"),
        _("Alias created")
    )
...
def newforward(request):
    return _new_alias(
        request, _("New forward"), reverse("kalabash_admin:forward_add"),
        _("Forward created")
    )


@login_required
@permission_required("kalabash_admin.change_alias")
@reversion.create_revision()
def editalias(request, alid, tplname="kalabash_admin/aliasform.html"):
    ...
    ctx = {
        'action': reverse("kalabash_admin:alias_change", args=[alias.id]),
        'formid': 'aliasform',
        'title': alias.address,
...
@login_required
@permission_required("kalabash_admin.delete_alias")
