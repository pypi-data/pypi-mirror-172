...
class Host:
    ...
    def ai_detected(self, channel: int, object_type: Optional[str] = None):
        ...
        if object_type is not None:
            if self._ai_detection_states is not None and channel in self._ai_detection_states:
    ...
    async def login(self) -> bool:
        ...
        await self.logout() # Ensure there would be no "max session" error
        ...
        if json_data is not None:
            try:
                if json_data[0]["code"] == 0:
                    ...
                    # At least I've seen RLC-510A failed with 0.07s, but OK with 0.12s. Need to gather some more logging data from users...
                    asyncio.sleep(0.5)
                    return True
            except Exception:
    ...
    async def set_email(self, channel: Optional[int], enable: bool) -> bool:
        ...
        if channel is None:
            if self._api_version_getemail == 0:
                ...
                body = [{"cmd": "SetEmail", "action": 0, "param": self._email_settings[0]}]
    ...
    async def unsubscribe_all(self):
        ...
        if self._nvr_model == "RLN8-410" or self._nvr_model == "RLN16-410":
            ...
            parameters = {"To": "http://{self._host}:{self._onvifport}/onvif/Notification?Idx=00_0"}
            parameters.update(await self.get_digest())
            xml = template.format(**parameters)
            await self.subscription_send(headers, xml)

            parameters = {"To": "http://{self._host}:{self._onvifport}/onvif/Notification?Idx=00_1"}
            parameters.update(await self.get_digest())
            xml = template.format(**parameters)
            await self.subscription_send(headers, xml)

            parameters = {"To": "http://{self._host}:{self._onvifport}/onvif/Notification?Idx=00_2"}
            parameters.update(await self.get_digest())
            xml = template.format(**parameters)
            await self.subscription_send(headers, xml)
