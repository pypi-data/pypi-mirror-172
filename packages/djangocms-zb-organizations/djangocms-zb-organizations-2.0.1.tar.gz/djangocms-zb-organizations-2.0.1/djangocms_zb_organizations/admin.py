...
# Clase perzonalida para agregar un field en el form del admin de organizaciones
class ExtraField(forms.ModelForm):
    # Fiel para subida multiple de archivos
    file_field = forms.FileField(label=_("Polygon files"), widget=forms.ClearableFileInput(attrs={'multiple': True}),
                                 required=False,
...
class OrganizationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Main Options"), {
            'fields': (
                ('name', 'initial'), ('address', 'phone'), ('email', 'country'), ('region', 'subregion'), 'logo',
                'category'),
        }),
        (_('Location'), {
    ...
    def save_model(self, request, obj, form, change):
        ...
        if request.FILES and request.FILES["file_field"]:
            ...
            if len(files) == 2:
                ...
                ext_file0 = parts_file0.pop()
                parts_file1 = files[1].name.split('.')
                ext_file1 = parts_file1.pop()
                # Se valida que los archivos tengan el mismo nombre y que sean .shp y .shx
                if (parts_file0 == parts_file1) and ((ext_file0 == "shp" and ext_file1 == "shx") or (
...
class MicroSiteAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Main Options"), {
            'fields': (
                ('organization', 'slug'), 'abstract', 'content', 'is_enabled')
        }),
    )
