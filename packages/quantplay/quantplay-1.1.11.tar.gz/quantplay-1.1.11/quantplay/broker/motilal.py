...
class Motilal(Broker):
    ...
    def __init__(self, is_uat=False):
    ...
    def initialize_expiry_fields(self):
        ...
        self.instrument_file_FO.loc[:, 'expiry'] = pd.to_datetime(self.instrument_file_FO.expirydate + 315513000, unit='s')
        ...
        self.instrument_file_FO.loc[:, "month_number"] = self.instrument_file_FO["expiry"].dt.strftime("%m").astype(float).astype(str)
        self.instrument_file_FO.loc[:, 'month_number'] = np.where(self.instrument_file_FO.month_number == 'nan',
                                                         np.nan,
                                                         self.instrument_file_FO.month_number.str.split(".").str[0]
                                                         )
    ...
    def add_quantplay_fut_tradingsymbol(self):
        seg_condition = [
            ((self.instrument_file_FO["instrumentname"] == "FUTIDX") | (self.instrument_file_FO["instrumentname"] == "FUTSTK"))
        ]
    ...
    def add_quantplay_opt_tradingsymbol(self):
        seg_condition = ((self.instrument_file_FO["instrumentname"] == "OPTIDX") | (
                    self.instrument_file_FO["instrumentname"] == "OPTSTK"))
        weekly_option_condition = (self.instrument_file_FO.expiry.dt.month == self.instrument_file_FO.next_expiry.dt.month)
        month_option_condition = (self.instrument_file_FO.expiry.dt.month != self.instrument_file_FO.next_expiry.dt.month)
        ...
        self.instrument_file_FO.loc[:, "tradingsymbol"] = np.where(
            seg_condition,
            self.instrument_file_FO.tradingsymbol + self.instrument_file_FO.strikeprice.astype(float).astype(str).str.split(".").str[0],
            self.instrument_file_FO.tradingsymbol
        )
    ...
    def generate_token(self):
        ...
        response = requests.post(self.url, headers=Motilal.headers, data=json.dumps(data))
    ...
    def send_otp(self):
        response = requests.post(self.otp_url, headers=Motilal.headers).json()
    ...
    def verify_otp(self, otp):
        ...
        response = requests.post(self.verify_otp_url, headers=Motilal.headers, data=json.dumps(data)).json()
        Constants.logger.info(response)
        return response
    ...
    def get_ltp(self, exchange=None, tradingsymbol=None):
        ...
        response = requests.post(self.ltp_utl, headers=Motilal.headers, data=json.dumps(data))
        Constants.logger.info("[GET_LTP_RESPONSE] response {}".format(response.json()))
        return response.json()["data"]["ltp"] / 100.0

    def get_orders(self):
        response = (requests.post(self.order_book_url, headers=Motilal.headers)).json()
    ...
    def modify_price(self, order_id, price):
    ...
    def modify_order(self, order):
        ...
        try:
            ...
            response = requests.post(self.modify_order_url, headers=Motilal.headers, data=json.dumps(data)).json()
            Constants.logger.info("[MODIFY_ORDER_RESPONSE] {}".format(response))
        except Exception as e:
            exception_message = "[ORDER_MODIFICATION_FAILED] for {} failed with exception {}".format(order['uniqueorderid'],
                                                                                                 e)
    ...
    def cancel_order(self, unique_order_id):
        ...
        try:
            ...
            response = requests.post(self.cancel_order_url, headers=Motilal.headers, data=json.dumps(data)).json()
    ...
    def place_order(self, tradingsymbol=None, exchange=None, quantity=None, order_type=None, transaction_type=None,
        ...
        try:
            ...
            response = requests.post(self.place_order_url, headers=Motilal.headers, data=json.dumps(data)).json()
