...
class FileToSend:
    ...
    def handle_multi_file(self, path, pdfCounter):
        self.multi = True
    ...
    def construct(self):
        if isdir(self.path):  # Handle file with annexes and detached signature
            for subfile_paraph in os.listdir(self.path):
                ...
                # traitement multi documents (si plus d'un pdf dans le répertoire)
                pdfCounter = 0
                ...
                if pdfCounter > 1:
                    print("Usage : multi documents")
                    print("Attention : Ne peut gérer que des PDF")
                    self.handle_multi_file(subfile_path, pdfCounter)
    ...
    def send_to_paraph(self):
        if self.file_to_send is not None:
            ...
            # traitement en cas de multi documents (nouveau paramètre : -b)    
            if self.multi:
                bash_command = 'java -jar %s ' \
                           ...
                           '-b "%s" ' \
                           '-e "%s" ' \
                           '-N "multi from pushDoc" ' \
                           '-v "/tmp/test2.pdf" ' \
                           '%s' % \
                           (jar_file, self.type, self.subtype, self.path, user_mail, limite_date)
                print(bash_command)
            else:
                bash_command = 'java -jar %s ' \
                            '-T "%s" ' \
