...
from ois_api_client import decode_invoice_data
...
def test_deserialize_v30(pytestconfig):
    ...
    with open(f'{dir_}/tests/dto/deserialization/v30/Belfoldi ertekesites tobb AFA tipus compressed.xml', 'r') as file:
        par = dto.InvoiceDataResult(
            invoice_data=file.read(),
            audit_data=dto.AuditData(
                ...
                index=None,
                batch_index=None,
                original_request_version=dto.OriginalRequestVersion.O_3_0
            ),
            compressed_content_indicator=True,
            electronic_invoice_hash=None
        )
        xml = decode_invoice_data(par)
