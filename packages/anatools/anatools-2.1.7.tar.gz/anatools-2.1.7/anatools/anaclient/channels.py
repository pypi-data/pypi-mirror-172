...
def create_managed_channel(self, name, organizationId=None, volumes=[], instance='p2.xlarge', timeout=120, interfaceVersion=1):
    ...
    if self.check_logout(): return
    if organizationId is None: organizationId = self.organization
    result = self.ana_api.createManagedChannel(organizationId=organizationId, name=name, volumes=volumes, instance=instance, timeout=timeout, interfaceVersion=interfaceVersion)
    self.get_channels()
    return result

def edit_managed_channel(self, channelId, name=None, volumes=None, instance=None, timeout=None, status=None, interfaceVersion=None):
    ...
    result = self.ana_api.editManagedChannel(channelId=channelId, name=name, volumes=volumes, instance=instance, timeout=timeout, status=status, interfaceVersion=interfaceVersion)
    self.get_channels()
    return result
...
USER root"""
    ...
    for filepath in files:
        if os.path.isdir(os.path.join(channeldir, filepath)):  shutil.copytree(os.path.join(channeldir, filepath), os.path.join(builddir, filepath))
        else: shutil.copyfile(os.path.join(channeldir, filepath), os.path.join(builddir, filepath))
    print('done.')
    ...
    if not __check_docker_image_size(image):
        return False
...
def deploy_managed_channel(self, channelId=None, channelfile=None, image=None):
    ...
    try: 
        ...
        if not __check_docker_image_size(channelimage):
            return False
