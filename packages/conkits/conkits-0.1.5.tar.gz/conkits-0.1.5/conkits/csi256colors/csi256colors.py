from conkits.csi import Style
from .csi256colorscodes import Codes, codes_set
...
class Csi256Colors(Codes):

    __FORE = '\033[38;5;'
    __BACK = '\033[48;5;'

    FORE = ''  # 仅供编译器查找属性，方便补全代码
    BACK = ''
    ...
    def get_fore(self, num, *kwargs):
        return self.__get_value(num, Colors256.__FORE, *kwargs)

    def get_back(self, num, *kwargs):
        return self.__get_value(num, Colors256.__BACK, *kwargs)

    def get_double(self, fore_num, back_num, *kwargs):
        return self.__get_fore_and_back_value(fore_num, back_num, *kwargs)

    def __get_value(self, num, csi_str, *kwargs):
        style_str = ''
        if num < 0 or num > 255:
            raise ValueError(ERROR_CSI_STR + f'数值{num}超出范围0 - 255' + RESET_ALL)
        for kwarg in kwargs:
            if kwarg not in STYLE_VALUE:
                raise ValueError(ERROR_CSI_STR + f'数值{kwarg}不正确，值256色的可变参数的值必须存在于Style类' + RESET_ALL)
            style_str += f';{kwarg}'
        style_str += 'm'
        return csi_str + str(num) + style_str

    def __get_fore_and_back_value(self, fore_num, back_num, *kwargs):
        style_str = ''
        if fore_num < 0 or fore_num > 255:
            raise ValueError(ERROR_CSI_STR + f'前景色数值{fore_num}超出范围0 - 255' + RESET_ALL)
        if back_num < 0 or back_num > 255:
            raise ValueError(ERROR_CSI_STR + f'背景色数值{fore_num}超出范围0 - 255' + RESET_ALL)
        for kwarg in kwargs:
            if kwarg not in STYLE_VALUE:
                raise ValueError(ERROR_CSI_STR + f'数值{kwarg}不正确，值256色的可变参数的值必须存在于Style类' + RESET_ALL)
            style_str += f';{kwarg}'
