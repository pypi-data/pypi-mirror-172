...
ANSI_CSI_RE = re.compile('\001?\033\\[((?:\\d|;)*)([a-zA-Z])\002?')  # 从colorama复制过来的


def error_str(err_info):
    return ERROR_CSI_STR + err_info + RESET_ALL


class BasicPrintTool:
    """用于提供基本的print工具函数"""
    @staticmethod
    def _scr_width_limit(scr_width):
        """限制屏幕宽度"""
        if scr_width <= 0:
    ...
    def __init__(self, screen_width=None):
        self.__screen_width = os.get_terminal_size()[0] if screen_width is None else self._scr_width_limit(screen_width)
        self.__trace_stack = []
    ...
    def _trace_cursor_movement(self, string):
        ...
        """
        csi_codes = re.findall(ANSI_CSI_RE, string)
