...
class LivePrint(CsiStringMethods):
    """用于动态打印的类"""
    def __init__(self, screen_wid=None,  print_delay=0.012, csi_code='', step=1, is_wait=False, auto_reset=False):
        ...
        self.auto_reset = False  # 每次打印完是否重置样式设置
        self.is_wait = is_wait  # 是否自动打印
        self.is_hide_cursor = False  # 是否隐藏光标
    ...
    def lprint(self, *kwargs, csi_code=None, print_delay=None, step=None, sep=' ', end='\n', is_wait=None,
               auto_reset=None, dict_info=None):
        """
        动态逐字打印字符串
        ...
        :param auto_reset: 打印完是否重置字符串中所设置的样式
        ...
        # 如果参数未赋值就给参数添加默认值
        csi_code = self.__csi_code if csi_code is None else '\033[0m' + csi_code
        ...
        auto_reset = self.auto_reset if auto_reset is None else auto_reset
        press_btn = None
        is_jump = False
        right = 0
        right_idx = 0
        ...
        while left_idx < len(sorted_info_ls):
            ...
            if not is_jump:
                ...
                if press_btn is not None:
                    is_jump = True
        # 如果最后一部分的字符串的csi代码为空，说明使用了类里面预设的，这时候重置一下样式，不让预设csi代码影响打印范围之外的显示
        if sorted_info_ls[-1]['csi'] == '':
...
class Choice(CsiStringMethods):
    def __init__(self, screen_wid=None, options=None, methods=None):
        super().__init__(screen_wid)
        """可直接通过属性设置的变量"""
        self.checked_csi_code = Colors256.BACK64 + Colors256.FORE255 + Style.BRIGHTs  # 被选中效果
        self.unchecked_csi_code = Colors256.FORE255 + Style.DIMs  # 未选中效果
        ...
        self.__lp = LivePrint(screen_wid)  # 内置一个lp类实例
    ...
    def __sort_options(self):
        ...
        for line_info in sorted_str_info:
            ...
            line_info['str_info'].append({
                'str_wid': diff, 'csi': '\033[0m', 'str': ' '*diff, 'len': diff
            })
    ...
    def _print_options(self):
        ...
        if self.is_pop:
            for option_info in self.__sorted_options_info:
                self.__lp.pop_print_line(csi_code=self.unchecked_csi_code, dict_info=option_info,
...
if __name__ == '__main__':
    ...
    text_ls = """
    '这是很长的一句话'
    '\033[30mcon\033[32;3mkits\033[0m',
    '即\033[1mconsole kits\033[0m',
    '里面包含了基本\033[34mcsi\033[0m控制代码，\033[38;5;215m256色\033[0m的csi控制代码的封装',
    '基本输入输出函数的封装还有一些能使\033[46mprint更加有趣的工具类\033[0m',
    """
    print(text_ls)
    #lp.lprint(text_ls)
    options = [
        f' 如何使用{Fore.REDs}Fore, {Fore.GREENs}Back, {Fore.YELLOWs}Style{Fore.RESETs} ',
        f' 如何使用{Fore.BLUEs}Conio{Fore.RESETs} ',
        f' 如何使用{Fore.PURPLEs}Colors256{Fore.RESETs} ',
        f' 如何使用{Fore.CYANs}LivePrint类{Fore.RESETs} ',
        f' 如何使用{Colors256.FORE215}Choice类{Colors256.RESET_ALL + Fore.BLUEs}哈哈 ',
    ]
    def op1():
        print('选择了选项1')
    ch = Choice()
    ch.set_options(options)
    ch.set_methods([op1])
    while True:
        ch.run()
        print()
