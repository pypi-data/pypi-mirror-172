...
class CsiStringMethods(BasicPrintTool):
    """提供CsiString类字符串信息整理的方法"""
    ...
    def _sort_string(self, string):
        ...
        prev_csi = ''
        while True:
            csi = re.search(FIND_CSI_PATTERN, search_str)
            if csi is None:
                info.append({'csi': prev_csi, 'str': search_str, 'len': len(search_str)})
                break
            else:
                csi_left = csi.span()[0]
                csi_right = csi.span()[1]
            if csi_left != 0:
                info.append({'csi': prev_csi, 'str': search_str[:csi_left], 'len': len(search_str[:csi_left])})
                prev_csi = '' if search_str[csi_left:csi_right] == '\033[0m' else search_str[csi_left:csi_right]
            else:
                prev_csi += '' if search_str[csi_left:csi_right] == '\033[0m' else search_str[csi_left:csi_right]
            search_str = search_str[csi_right:]
        # if info[-1]['csi'] == '\033[0m':
        #     info[-1]['csi'] = ''
        # 这时候就说明这部分csi控制代码应该用类里面预设的了
        # 为防止把'\033[0m'加到末尾csi控制代码，导致无法使用类里面预设的，所以手动把他设置为空字符
        # pprint(info)
        # print('\033[32m' + '='*80 + '\033[0m')
        return info

    def _sort_string_with_limited_wid(self, single_string, limited_wid, limited_line=None, default_csi_code='',
                                       is_complete=False):
        """
        它的参数是__sort_prt_string函数的返回结果
        格式如下，记为sorted_info1
        [
            {'csi': str_value, 'str': str_value, 'len': str_value},
            ...
        ]
        ...
        [
            ...
            'str_info': [
                            {'str_wid': int_value, 'csi': str_value, 'str': str_value, 'len': str_value},
                            ...
                        ]  （供打印函数解析的字符串信息）
            }, （这一个字典代表一行的字符串信息）
            ...
        ...
        :param default_csi_code: 当字符串某部分csi代码没有或者为\033[0m的时候，自动加上的csi代码
        ...
        for info in info_list:
            if line_count >= limited_line:
                break
            current_csi_code = default_csi_code if info['csi'] == '' else info['csi']  # 当前的csi代码
            ...
            if first_cut_str.endswith('\n'):  # 如果第一个字符串是以换行符结尾的
                ...
                sorted_info[-1]['str_info'].append(
                    {'str_wid': first_cut_str_wid, 'csi': current_csi_code,
            ...
            if len(cut_str_list) > 1:
                ...
                for idx, cut_str in enumerate(cut_str_list):
                    ...
                    if cut_str.endswith('\n'):  # 如果第一个字符串是以换行符结尾的
                        # 直接附加到尾行（除去换行）
                        sorted_info[-1]['str_info'].append(
                            {'str_wid': cut_str_wid, 'csi': current_csi_code,
        ...
        if is_complete:
            while len(sorted_info) < limited_line:
                sorted_info.append({
                    ...
                    'str_info': [{'str_wid': 0, 'csi': default_csi_code, 'str': '', 'len': 0}]})
        return sorted_info


class CsiString(CsiStringMethods):
    """
    在区分好csi作用范围和与可显示字符分离的情况下，
...
class CsiStringWidLimited(CsiStringMethods):
    """
    在区分好csi作用范围和与可显示字符分离的情况下，
    将字符串按限制宽度分为多行，
