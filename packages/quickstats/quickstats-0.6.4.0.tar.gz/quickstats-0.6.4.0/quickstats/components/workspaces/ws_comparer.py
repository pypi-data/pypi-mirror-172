...
class WSItem(GeneralEnum):
    ...
    def __init__(self, enum_id:int, item_type:WSItemType, title:str, var_repr:Optional[WSArgument],
...
class ComparisonData:
    ...
    def _get_ext_mapping_contents(self, df:"pandas.DataFrame", ext_ref:Optional[Dict]=None):
        ...
        contents_1 = []
        contents_2 = []
        for name, alt_name in mapping_1.items():
            contents_1.append(df_1.loc[name]["definition_left"])
            # left same item type -> right same item type
            if (alt_name in mapping_2):
                if (mapping_2[alt_name] == name):
                    contents_2.append(df_2.loc[alt_name]["definition_right"])
