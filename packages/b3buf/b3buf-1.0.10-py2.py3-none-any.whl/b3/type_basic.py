...
from b3.utils import VALID_INT_TYPES, VALID_STR_TYPES, IntByteAt
from b3.datatypes import U64, S64, DATATYPE_NAMES
from b3.type_varint import encode_svarint, decode_svarint
...
# In: a python datetime.date object (timezone info is ignored)

def encode_basicdate(value):
    if not isinstance(value, datetime.date):
        raise TypeError("BASICDATE only accepts datetime.date values")
    out = [int2byte(value.day), int2byte(value.month), encode_svarint(value.year)]
    return b"".join(out)


def decode_basicdate(buf, index, end):
    day, index = IntByteAt(buf, index)
    month, index = IntByteAt(buf, index)
    year, index = decode_svarint(buf, index)
    if index != end:
        raise ValueError("mismatch between tlv field size and basicdate data size")
