...
from song.util import find_spread_tightness, \
    bulk_grow_with_drifters, embed_batch_epochs, \
     sq_eucl_opt, get_closest_for_inputs, train_for_batch_batch_umap
...
class SONG(BaseEstimator):
    ...
    def __init__(self, n_components=2, n_neighbors=1,
                 ...
                 um_min_dist = 0.001, um_lr = 0.01, um_epochs = 11, vis_neis = 10):
    ...
    def fit(self, X, reduction = 'PCA', corrected_PC = np.array([])):
        ...
        for i in range(self.max_its):
            ...
            if not i % self.non_so_rate:
                ...
                for chunk in range(n_chunks):
                    ...
                    W, G, E_q = train_for_batch_batch_umap(X_chunk, pdists, i, self.max_its, lrst, self.pow_err, im_neix,
                                                         W,
                    ...
                    #         Y = UMAP(n_neighbors=im_neix * 3, n_epochs=10, random_state=self.random_seed,
                    #                  min_dist=self.min_dist,
                    #                  spread=self.spread, init=Y, metric='precomputed').fit_transform(pcvdist)
            ...
            if not i%20:
                ...
                with warnings.catch_warnings():
                    warnings.simplefilter('ignore', UserWarning)
                    Y = UMAP(n_neighbors=self.vis_neis,n_epochs=50, random_state=self.random_seed, min_dist=self.min_dist,
                             spread=self.spread, init=Y, metric='precomputed').fit_transform(pcvdist)
        if reduction == 'PCA' and X.shape[1] > 100:
            W_ = self._get_XPCA(W).astype(np.float32)
        else:
        ...
        with warnings.catch_warnings():
            warnings.simplefilter('ignore', UserWarning)
            Y = UMAP(n_neighbors=self.vis_neis, random_state=self.random_seed, min_dist=self.min_dist,
                     spread=self.spread, init=Y, metric='precomputed', learning_rate=self.reduced_lr).fit_transform(pcvdist)
