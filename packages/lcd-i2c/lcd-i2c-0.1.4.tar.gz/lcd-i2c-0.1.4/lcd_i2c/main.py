...
from .custom_chars import default_custom_chars, replazable_chars, available_chars_ROM_A00
...
class LCD_I2C():
    ...
    def load_default_custom_chars(self):
        for i in range(len(default_custom_chars.keys())):
            key = list(default_custom_chars.keys())[i]
            custom_char = default_custom_chars[key]
            self.createChar(i, custom_char, key)
    
    def createChar(self, location: int, chars: list[int], char_name: str):
        # we only have 8 locations 0-7 available to store custom chars
        self.command(LCD_SET_CGRAM_ADDR | (location & 0x7 << 3))
        for i in range(8):
            self.write(chars[i])
        self.custom_chars[i] = char_name
    ...
    def write_text(self, text: str, encoding: str = 'utf-8'):
        for char in text:
            if char in replazable_chars:
                byte = available_chars_ROM_A00.find(char)
                self.write(byte)
            elif char in self.custom_chars.values():
                for i in range(len(self.custom_chars.values())):
                    if self.custom_chars[i] == char:
                        self.write(i)
            else:
