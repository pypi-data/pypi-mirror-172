...
class Recognizer(WebSocketClient):
    ...
    def __init__(self, speech_config: SpeechConfig, audio_config: Optional[audio.AudioConfig] = None, itn: bool = True, punctuation: bool = True):
    ...
    def get_server_url(self) -> bool:
        ...
        if response.status_code == 200:
            ...
            svc_logger.error(f"Error while getting stt server url: {response.status_code} for account_id: {self.__account_id}")
            return False
    ...
    def thread_start_continuous_recognition_async(self):
        ...
        try:
            ...
            svc_logger.error(f"Exception in thread_start_continuous_recognition_async: {e}")
        finally:
    ...
    async def continuous_recognition_async(self):
        ...
        try:
            if self.__input_stream:
                ...
                sample_rate = 16000
            async with websockets.client.connect(self.__ws_uri, extra_headers={"sample_rate": sample_rate, "itn": self.itn, "punctuation": self.punctuation}) as ws:
    ...
    def opened(self):
        ...
        if self.__session_started_event.is_set:
            evt = {"session_id": self.session_id, "status": 0, "message": "recognition started"}
            evt_res = EventResults(evt)
            self.__session_started_event.callback(evt_res)
    ...
    def closed(self, code, reason):
        ...
        if self.__session_stopped_event.is_set:
            evt = {"session_id": self.session_id, "status": code, "messge": reason}
            evt_res = EventResults(evt)
            self.__session_stopped_event.callback(evt_res)
        self.close()
    ...
    def received_message(self, message: ws4py.messaging.TextMessage) -> None:
        ...
        try:
            ...
            svc_logger.error(f"Exception in receive_message: {evt}, e: {e}")
            if self.__canceled_event.is_set:
                evt_res = EventResults(evt)
                self.__canceled_event.callback(evt_res)
            self.stop_continuous_recognition_async()
...
class SpeechRecognizer(Recognizer):
    ...
    def __init__(self, speech_config: SpeechConfig, audio_config: Optional[audio.AudioConfig] = None, itn: bool = True, punctuation: bool = True):
        ...
        super().__init__(speech_config=speech_config, audio_config=audio_config, itn=itn, punctuation=punctuation)
...
class SlothWebsocketClient:
    def __init__(self, speech_config: SpeechConfig, output_stream=None, output_filename=None):
...
class SpeechSynthesizer(SlothWebsocketClient):
    ...
    def __init__(self, speech_config: SpeechConfig, audio_config: Optional[audio.AudioOutputConfig] = None):
        """Initialize speech recognizer"""
    ...
    def speak_text(self, text: str) -> Optional[SpeechSynthesisResult]:
        ...
        if self.get_server_url_token() is True:
            ...
            svc_logger.error("Can't get an available tts server, please confirm language and voice settings, then try again.")
            return None
