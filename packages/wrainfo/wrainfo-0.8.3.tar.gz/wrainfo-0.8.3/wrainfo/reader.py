...
def create_filelist(starttime, endtime, path, pattern="_000.scnx.gz"):
    ...
    pattern : str
        ...
        (netcdf file: = ".nc" , elevation angle is included in the filename)
    ...
    while date <= endtime:
        ...
        raw_path = get_furuno_path(path=path, start_time=date)
                ...
                if time >= date:
                    if time < endtime:
                        ...
                        if fname.endswith(".nc"):
                            if fname not in error_flist:
                                flist_nc.append(fname)

            if len(flist_nc) > 0:
                for file in flist_nc:
                    flist.append(file)
...
def read_single_file(file, grp="dataset1"):
    ...
    if extension == ".gz":
        ...
        tmp = xr.open_mfdataset((file,), engine="furuno", group=1,
                                concat_dim="time", combine="nested",
                                backend_kwargs=dict(reindex_angle=1.0))

    else:
        tmp = xr.open_mfdataset((file,), engine="odim", group=grp,
                                concat_dim="time", combine="nested",
                                backend_kwargs=dict(reindex_angle=1.0))
...
def get_cmap(starttime,
    ...
    for cmap in cmaps:
        ...
        # read cmap
        cmap = xr.open_dataset(selected_map, engine="h5netcdf")
