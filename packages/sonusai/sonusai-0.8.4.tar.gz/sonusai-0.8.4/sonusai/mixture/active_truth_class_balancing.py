        ...
        while len(records) < num_cpus:
            for target_index in target_indices:
                ...
                for augmentation_index in augmentation_indices:
                    records.append(MRecord(target_file_index=[target_index],
...
def _process_target(mixture: MRecord) -> MRecord:
    from sonusai.mixture.process import process_target

    return process_target(mrecord=mixture, mixdb=MP_DICT['mixdb'],
                          raw_target_audios=MP_DICT['raw_target_audio'])


def _get_class_balancing_samples(mixdb: MixtureDatabase, mixtures: MRecords) -> List[int]:
    ...
    class_count = get_class_count_from_mixtures(mixdb, mixtures)
...
def _get_augmented_targets(mixdb: MixtureDatabase) -> MRecords:
...
def _remove_unused_augmentations(mixdb: MixtureDatabase, mixtures: MRecords) -> None:
