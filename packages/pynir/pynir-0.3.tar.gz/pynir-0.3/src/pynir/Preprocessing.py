...
class cwt():
    def __init__(self, wavelet = "morl", scale = 20, isFlip = True):
        """
        Parameters
        ----------
        ...
        isFlip : TYPE, optional
            Wheather to flip data for correction of edge effect. The default is True.
    ...
    def transform(self, X):
        nrow, ncol = X.shape
        if self.isFlip:
            Xnew = np.hstack((X[::-1], X, X[::-1]))
            Xcwt = pywt.cwt(Xnew, self.scale, self.wavelet)
            Xcwt = Xcwt[0][0,:,ncol+1:2*ncol+1]
        else:
            Xcwt = pywt.cwt(X, self.scale, self.wavelet)[0][0,:,:]
        return Xcwt
