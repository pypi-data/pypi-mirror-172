# -*- coding: utf-8 -*-
from lsms.from_dta import from_dta
import re
...
def get_food_expenditures(fn,purchased,away,produced,given,itmcd='itmcd',HHID='HHID',units=None,
    ...
    df,labels = sum_food_expenditures_from_different_sources(fn,sources,itmcd=itmcd,HHID=HHID,units=units,
                                                             convert_categoricals=convert_categoricals,labels=itemlabels,
                                                             fn_type=fn_type)

    if itemlabels is not None:
        df = df.loc[:,df.columns.isin(itemlabels.values())]  # Drop items not in itemlabels
...
def sum_food_expenditures_from_different_sources(fn,sources,itmcd='itmcd',HHID='HHID',units=None,
    ...
    if fn_type == 'stata':
        if type(fn)==list:
            ...
            df=from_dta(fn, convert_categoricals=convert_categoricals)
    elif fn_type == 'csv':
        if type(fn)==list:
            files=[f for f in set(fn)]
            df=pd.read_csv(files.pop(),na_values='.')
            for f in files:
                current_df=from_dta(f)
                df=pd.merge(df,current_df)
        else:
            df=pd.read_csv(fn,na_values='.')
    elif fn_type is None:
        df = fn
...
def get_household_roster(fn,sex='sex',sex_converter=None,age='age',age_converter=None,months_spent='months_spent',HHID='HHID',months_converter=None, convert_categoricals=True,Age_ints=None,fn_type='stata'):
    ...
    for ages in Age_ints:
        ...
        valvars.append('Females %02d-%02d' % agestr)
