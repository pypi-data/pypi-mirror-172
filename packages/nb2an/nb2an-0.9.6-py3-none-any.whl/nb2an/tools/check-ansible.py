...
def main():
    ...
    im = InventoryManager(loader=dl,
                          sources=[os.path.join(args.ansible_directory,
                                                args.inventory_file)])
    vm = VariableManager(loader=dl, inventory=im)

    check_interfaces(vm, dl, im, b, args.hosts)


def check_interfaces(vm, dl, im, b, hosts=None):
    # set up ansible config grabber

    # get netbox devices
    netbox_addrs = b.get_addresses()
    ...
    for host in host_list:
        ...
        try:
            ...
            search_host = host
            if not host.endswith(".b.isi.edu"):
                search_host = search_host + ".b.isi.edu"
            ...
            if 'net_config' not in vars:
                warning(f"{host} does not have \"net_config\" in ansible")
                continue
