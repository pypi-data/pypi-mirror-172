...
class Bucket(object):
    ...
    def list_objects(self, prefix='', maxkeys=1000):
        try:
            ...
            list_objects = []
            '''
            bck = self.resource.Bucket(self.default_bucket)
           
            if prefix is None:
                # obtener todos
                for key in bck.objects.filter().all():
                    list_objects.append(key.key)
            else:
                # filtrar
                for key in bck.objects.filter(Prefix=prefix).all():
                    list_objects.append(key.key)
            return list_objects
            '''
            '''
            ...
            response = self.client.list_objects_v2(Bucket=self.default_bucket, MaxKeys=maxkeys if maxkeys<=1000 else 1000, Prefix=prefix)
            if isinstance(response,dict) and "HTTPStatusCode" in response["ResponseMetadata"] and response["ResponseMetadata"]["HTTPStatusCode"]==200:
                ...
                while response['IsTruncated'] and maxkeys>=1000:
                    ...
                    response = self.client.list_objects_v2(Bucket=self.default_bucket, MaxKeys=maxkeys if maxkeys<=1000 else 1000, Delimiter="/", ContinuationToken=continuation_key, Prefix=prefix)
                    if 'Contents' in response:
                        for key in response['Contents']:
                            list_objects.append(key['Key'])
                            if maxkeys != 1000:
    ...
    def download_object(self, prefix='', list_objects=[]):
        try:
            ...
            for file in list_objects:
                ...
                download_file_key = ''.join([prefix,file.replace(prefix,'')])
    ...
    def delete_object(self, prefix='', list_objects=[]):
        try:
            ...
            for file in list_objects:
                ...
                delete_file_key = ''.join([prefix,file.split('/')[-1:][0]])                                
    ...
    def backup_object(self, prefix=''):
        try:
            ...
            #for n in range(0,len(_list_objects)):
            #    _list_objects[n] = _list_objects[n].replace(_prefix,'')
            print(_list_objects)   
            self.download_object(prefix=_prefix,list_objects=_list_objects)
        except botocore.exceptions.ClientError as error:
            for key, value in sorted(botocore.exceptions.__dict__.items()):
                if isinstance(value, type):
                    print(key)
