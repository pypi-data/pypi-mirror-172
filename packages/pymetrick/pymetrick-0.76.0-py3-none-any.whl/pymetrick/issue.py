...
class Order(FPDF):
    ...
    def header(self):
        try:
            ...
            if self.allowed_data['debug']:
                self.allowed_data['footer_head'] = """{0:^1s}  {1:^9s} {2:^27s} {3:^8s}  {4:^9s}  {5:^5s}%  {6:^7s}  {7:^5s}%  {8:^9s}  {9:^9s}""".format(' ','Imp.Neto','Dto.%','Imp.Dto.','Base Imp.','IVA','Imp.IVA','R.EQ.','Imp.R.EQ.','Total')
            ...
            if sys.platform[:-2] == 'win':    
                ...
                try:
                    ...
                    try:
                        ...
                        win32print.WritePrinter (self.hPrinter,CODEPAGE_CHANGE + b'\x00')   # CODEPAGE CHARACTER_CODE_TABLE  PAGE 0
...
class PolyTag(FPDF):
    ...
    def config(self,*args,**kwargs):
        try:
            self.allowed_data = {'logo':'','dealer':'','sku':'','provider':list(),'EAN13':'','name':'','rate':list()}
            self.tags = list()
        ...
        try:
            ...
            for l in self.tags:  # list of list of polytags
                ...
                if os.path.isfile(l[0]['logo']):
                    self.image(l[0]['logo'], 10, nY-7, 10)
                # legalname
                self.set_font('Arial','B',8)
                self.text(33, nY-7, l[0]['dealer'])
                for m in range(len(l)):  # list of polytags
                    ...
                    self.text(80, nY, self.tag['sku'])
                    # Codebar
                    if len(self.tag['EAN13'])>0:
                        self.EAN13(33,nY-4,self.tag['EAN13'],h=4)
                    # Product name
                    self.set_font('Arial','',14)
                    self.text(10, nY+8, self.tag['name'].strip()[:60])

                    # Rate
                    self.set_font('Arial','B',16)
                    self.text(160, nY+4, ' '.join(self.tag['rate']))

                    # Provider and Item number
                    self.set_font('Arial','',14)
                    self.text(120, nY, '  '.join(self.tag['provider']))
                    nY += 15
                self.line(11,nY-2,204,nY-2)
                nY += 10
                np += 1
                '''
                if np == 5:
...
def XLSX_Report(report_path='', item_list=list(), head_list=list(), title=None, orientation='portrait', total=list(), head_trademark=[], footer=[]):
