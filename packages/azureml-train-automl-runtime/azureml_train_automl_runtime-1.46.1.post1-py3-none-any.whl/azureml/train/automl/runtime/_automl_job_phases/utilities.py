...
class PhaseUtil:
    ...
    def build_raw_data_context(
        ...
        return RawDataContext(
            ...
            data_snapshot_str=raw_experiment_data.data_snapshot_str,
            data_snapshot_str_with_quantiles=raw_experiment_data.data_snapshot_str_with_quantiles,
            output_data_snapshot_str_with_quantiles=raw_experiment_data.output_data_snapshot_str_with_quantiles)
    ...
    @staticmethod
    def log_with_memory(info: str) -> None:
        logger.info(info)
    ...
    @staticmethod
    def combine_dict(a: Dict[str, Any], b: Dict[str, Any], override: bool) -> Dict[str, Any]:
        new_dict = copy.deepcopy(a)
        for key in b.keys():
            if key not in a.keys() or override:
                new_dict[key] = b[key]
        return new_dict
