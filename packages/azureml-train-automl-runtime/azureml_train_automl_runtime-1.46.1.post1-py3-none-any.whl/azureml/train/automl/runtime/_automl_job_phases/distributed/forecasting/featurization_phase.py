...
def _fit_unique_target_grain_dropper(
        transform_grain_plugin: TransformGrainPlugin, train_X_grain: pd.DataFrame, train_y_grain: np.ndarray
) -> None:
...
def _fit_transform_data(
    ...
    return train_transformed_data, featurized_file_path
...
def _build_grain_transform_result(
    ...
    if not is_unique_target_grain:
        log_transform_results = profile_transformed_data_and_get_log_transform_results(
            featurized_file_path, expr_store_for_worker, train_transformed_data,
            transform_grain_plugin.ts_transformer.target_column_name, grain_keyvalues_and_path
        )
...
def _transform_one_grain(grain_keyvalues_and_path: _PartitionKeyValueCommonPath,
    ...
    # TODO: Enable in v1.47 https://msdata.visualstudio.com/Vienna/_workitems/edit/1993396
    ...
    _fit_unique_target_grain_dropper(transform_grain_plugin, train_X_grain, train_y_grain)
    # TODO: Enable in v1.47 https://msdata.visualstudio.com/Vienna/_workitems/edit/1993396
    ...
    if not is_unique_target:
        train_transformed_data, featurized_file_path = _fit_transform_data(
    ...
    return _build_grain_transform_result(
        transform_grain_plugin, grain_keyvalues_and_path, is_unique_target, featurized_file_path,
        expr_store_for_worker, train_transformed_data
    )


def profile_transformed_data_and_get_log_transform_results(
        featurized_file_path: str,
        expr_store_for_worker: ExperimentStore,
        train_transformed_data: pd.DataFrame,
    ...
    expr_store_for_worker.metadata.timeseries.set_featurized_data_profile_by_grain(
        grain_keyvalues_and_path.key_values, data_profile)
