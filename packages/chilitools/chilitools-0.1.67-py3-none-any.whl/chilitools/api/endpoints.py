...
class Resources:
  ...
  def getFullResourceTree(self, resourceType: str, folder: str = '') -> dict:
    items = {}
    return self._resourceTreeIter(resourceType, items, folder)
    ...
    if items is not None:
      if not isinstance(items, list):
        if items['@isFolder'] == 'true':
          itemsDict[items['@name']] = dict()
          return self._resourceTreeIter(resourceType, itemsDict[items['@name']], items['@path'])
        else:
          itemsDict[items['@name']] = items
      else:
        for i, v in enumerate(items):
          if v['@isFolder'] == 'true':
            itemsDict[v['@name']] = dict()
            return self._resourceTreeIter(resourceType, itemsDict[v['@name']], v['@path'])
          else:
            itemsDict[v['@name']] = v
    return itemsDict
