...
class HTTPStatus(IntEnum):
    ...
    MAKEUP_TYPE_MUST_PRAM_ERR = (400006, "必传的参数未传", "必须的参数（MakeupType）未传")
    ...
    INPUTS_OR_URLS_MUST_PRAM_ERR = (400006, "必传的参数未传", "必须的参数（inputs 或 input_urls）未传")
    WEBSOCKET_MUST_PRAM_ERR = (400006, "必传的参数未传", "必须的参数（Action、Signal）未传")
    DBNAME_MUST_PRAM_ERR = (400006, "必传的参数未传", "必须的参数（DbName）未传")
    ENTITY_ID_MUST_PRAM_ERR = (400006, "必传的参数未传", "必须的参数（EntityId）未传")
    LABEL_MUST_PRAM_ERR = (400006, "必传的参数未传", "必须的参数（Label）未传")
    OFFSET_MUST_PRAM_ERR = (400006, "必传的参数未传", "必须的参数（Offset）未传")
    EXTRA_DATA_MUST_PRAM_ERR = (400006, "必传的参数未传", "必须的参数（ExtraData）未传")
    LIMIT_MUST_PRAM_ERR = (400006, "必传的参数未传", "必须的参数（Limit）未传")
    FACE_ID_MUST_PRAM_ERR = (400006, "必传的参数未传", "必须的参数（FaceId）未传")

    # 400007 传递非法参数
    ILLEGAL_PRAM_ERR = (400007, "传递非法参数", "请求体字典内有除（Action、ImageData）外的参数")

    # 400008 请求体的字段类型错误
    PRAM_TYPE_ERR = (400008, "请求体的参数字段类型错误", "请求体的字段（Action、ImageData）类型错误，类型只能为字符串，不能为其他类型")
    IMAGE_AB_PRAM_TYPE_ERR = (400008, "请求体的参数字段类型错误", "请求体的字段（Action、ImageDataA、ImageDataB）类型错误，类型只能为字符串，不能为其他类型")
    TEXT_PRAM_TYPE_ERR = (400008, "请求体的参数字段类型错误", "请求体的字段（Action、TextData）类型错误，类型只能为字符串，不能为其他类型")
    AUDIO_PRAM_TYPE_ERR = (400008, "请求体的参数字段类型错误", "请求体的字段（Action、AudioData）类型错误，类型只能为字符串，不能为其他类型")
    VIDEO_PRAM_TYPE_ERR = (400008, "请求体的参数字段类型错误", "请求体的字段（Action、VideoData）类型错误，类型只能为字符串，不能为其他类型")
    ...
    LANGUAGE_STRING_TYPE_ERR = (400008, "请求体的参数字段类型错误", "Language 字段应该是 string 类型") # 语种分类
    ...
    INPUTS_LIST_TYPE_ERR = (400008, "请求体的参数字段类型错误", "inputs 字段应该是 list 类型")
    WEBSOCKET_PRAM_TYPE_ERR = (400008, "请求体的参数字段类型错误","请求体的字段（Action、Signal）类型错误，类型只能为字符串，不能为其他类型")
    DBNAME_STRING_TYPE_ERR = (400008, "请求体的参数字段类型错误","DbName 字段应该是 string 类型")
    ENTITY_ID_STRING_TYPE_ERR = (400008, "请求体的参数字段类型错误", "EntityId 字段应该是 string 类型")
    LABEL_STRING_TYPE_ERR = (400008, "请求体的参数字段类型错误", "Label 字段应该是 string 类型")
    OFFSET_INT_TYPE_ERR = (400008, "请求体的参数字段类型错误", "Offset 字段应该是 int 类型")
    EXTRA_DATA_STRING_TYPE_ERR = (400008, "请求体的参数字段类型错误", "ExtraData 字段应该是 string 类型")
    LIMIT_INT_TYPE_ERR = (400008, "请求体的参数字段类型错误", "Limit 字段应该是 int 类型")
    FACE_ID_STRING_TYPE_ERR = (400008, "请求体的参数字段类型错误", "FaceId 字段应该是 string 类型")

    # 400009 请求体的参数字段值为空
    IMAGE_DATA_EMPTY_ERR = (400009, "请求体的参数字段值为空", "ImageData 字段值为空字符")
    IMAGE_AB_DATA_EMPTY_ERR = (400009, "请求体的参数字段值为空", "ImageDataA 或 ImageDataB 字段值为空字符")
    ...
    WEBSOCKET_DATA_EMPTY_ERR = (400009, "请求体的参数字段值为空", "Signal 字段值为空字符")

    # 400010 请求体的参数字段值设置错误
    ACTION_VALUE_ERR = (400010, "请求体的参数字段值设置错误", "Action 值设置错误") # ACTION
    IMAGE_URL_VALUE_ERR = (440003, "请求体的参数字段值设置错误", "ImageURL 字段不符合规范，请参考接口文档说明")
    TEXT_URL_VALUE_ERR = (441003, "请求体的参数字段值设置错误", "TextURL 字段不符合规范，请参考接口文档说明")
    AUDIO_URL_VALUE_ERR = (442003, "请求体的参数字段值设置错误", "AudioURL 字段不符合规范，请参考接口文档说明")
    VIDEO_URL_VALUE_ERR = (443003, "请求体的参数字段值设置错误","VideoURL 字段不符合规范，请参考接口文档说明")
    INPUT_URLS_VALUE_ERR = (444003, "请求体的参数字段值设置错误","input_urls 字段不符合规范，请参考接口文档说明")
    VOICE_TYPE_VALUE_ERR = (450002, "请求体的参数字段值设置错误","VoiceType 字段不符合规范，请参考接口文档说明") # 语音合成
    PITCH_VALUE_ERR = (450004, "请求体的参数字段值设置错误", "Pitch 字段不符合规范，请参考接口文档说明") # 语音合成
    SPEED_VALUE_ERR = (450006, "请求体的参数字段值设置错误", "Speed 字段不符合规范，请参考接口文档说明") # 语音合成
    PERSON_THRESH_VALUE_ERR = (450008, "请求体的参数字段值设置错误", "PersonThresh 字段不符合规范，请参考接口文档说明") # 行人检测
    FACE_THRESH_VALUE_ERR = (450010, "请求体的参数字段值设置错误","FaceThresh 字段不符合规范，请参考接口文档说明") # 人脸检测
    FIRE_THRESH_VALUE_ERR = (450012, "请求体的参数字段值设置错误","FireThresh 字段不符合规范，请参考接口文档说明") # 烟雾明火检测
    SMOKE_THRESH_VALUE_ERR = (450014, "请求体的参数字段值设置错误", "SmokeThresh 字段不符合规范，请参考接口文档说明") # 烟雾明火检测
    CAR_THRESH_VALUE_ERR = (450016, "请求体的参数字段值设置错误","CarThresh 字段不符合规范，请参考接口文档说明") # 车辆检测
    HELMET_THRESH_VALUE_ERR = (450018, "请求体的参数字段值设置错误", "HelmetThresh 字段不符合规范，请参考接口文档说明") # 安全帽检测
    SMOKE_LEVEL_VALUE_ERR = (450020, "请求体的参数字段值设置错误","Level 字段不符合规范，请参考接口文档说明") # 吸烟检测
    SUIT_THRESH_VALUE_ERR = (450022, "请求体的参数字段值设置错误","SuitThresh 字段不符合规范，请参考接口文档说明") # 防护服检测
    IMG_CENSOR_SUBTASK_VALUE_ERR = (450024, "请求体的参数字段值设置错误", "SubTask 字段不符合规范，请参考接口文档说明") # 内容审核
    EBIKE_THRESH_VALUE_ERR = (450028, "请求体的参数字段值设置错误", "EbikeThresh 字段不符合规范，请参考接口文档说明") # 电动车检测
    SCORE_THRESH_VALUE_ERR = (450030, "请求体的参数字段值设置错误", "ScoreThresh 字段不符合规范，请参考接口文档说明") # 通用分数阈值
    LANGUAGE_VALUE_ERR = (450032, "请求体的参数字段值设置错误", "Language 字段不符合规范，请参考接口文档说明") # 语种分类
    IS_MONOLINGUAL_VALUE_ERR = (450034, "请求体的参数字段值设置错误", "IsMonolingual 字段不符合规范，请参考接口文档说明") # 语种分类
    AREA_THRESH_VALUE_ERR = (450036, "请求体的参数字段值设置错误", "AreaThresh 字段不符合规范，请参考接口文档说明") # 遮挡检测
    HAND_THRESH_VALUE_ERR = (450038, "请求体的参数字段值设置错误", "HandThresh 字段不符合规范，请参考接口文档说明") # 手势关键点检测
    MAKEUP_TYPE_VALUE_ERR = (450041, "请求体的参数字段值设置错误", "MakeupType 字段不符合规范，请参考接口文档说明") # 人脸美妆
    STRENGTH_VALUE_ERR = (450043, "请求体的参数字段值设置错误", "Strength 字段不符合规范，请参考接口文档说明") # 人脸美妆
    MASK_THRESH_VALUE_ERR = (450051, "请求体的参数字段值设置错误", "MaskThresh 字段不符合规范，请参考接口文档说明") # 口罩检测
    APPKEY_VALUE_ERR = (459003, "请求体的参数字段值设置错误", "AppKey 字段不符合规范，请参考接口文档说明")
    TOKEN_VALUE_ERR = (459006, "请求体的参数字段值设置错误", "Token 字段不符合规范，请参考接口文档说明")
    VERSION_VALUE_ERR = (459009, "请求体的参数字段值设置错误","Version 字段不符合规范，请参考接口文档说明")
    INPUTS_VALUE_ERR = (459019, "请求体的参数字段值设置错误","inputs 字段不符合规范，请参考接口文档说明")
    SIGNAL_VALUE_ERR = (460004, "请求体的参数字段值设置错误", "Signal 值设置错误")
    DBNAME_VALUE_ERR = (470003, "请求体的参数字段值设置错误", "DbName 字段不符合规范，请参考接口文档说明")
    ENTITY_ID_VALUE_ERR = (470006, "请求体的参数字段值设置错误","EntityId 字段不符合规范，请参考接口文档说明")
    LABEL_VALUE_ERR = (470009, "请求体的参数字段值设置错误", "Label 字段不符合规范，请参考接口文档说明")
    OFFSET_VALUE_ERR = (470012, "请求体的参数字段值设置错误", "Offset 字段不符合规范，请参考接口文档说明")
    EXTRA_DATA_VALUE_ERR = (470015, "请求体的参数字段值设置错误","ExtraData 字段不符合规范，请参考接口文档说明")
    LIMIT_VALUE_ERR = (470018, "请求体的参数字段值设置错误", "Limit 字段不符合规范，请参考接口文档说明")
    FACE_ID_VALUE_ERR = (470021, "请求体的参数字段值设置错误", "FaceId 字段不符合规范，请参考接口文档说明")
    ...
    # 4000014 请求时间范围不合法
    TIME_RANGE_ERR = (400013, "请求时间范围不合法", "请求时间范围不合法")


    # 4000015 文件下载错误
    IMAGE_URL_DOWNLOAD_ERR = (4000015, "文件下载错误", "无法解析图片链接，下载失败")
    TEXT_URL_DOWNLOAD_ERR = (4000015, "文件下载错误", "无法解析文本链接，下载失败")
    AUDIO_URL_DOWNLOAD_ERR = (4000015, "文件下载错误", "无法解析音频链接，下载失败")
    VIDEO_URL_DOWNLOAD_ERR = (4000015, "文件下载错误", "无法解析视频链接，下载失败")
    VINPUT_URLS_DOWNLOAD_ERR = (4000015, "文件下载错误", "无法解析链接，下载失败")

    # 4000016 必传的参数重复
    INPUTS_AND_URLS_DUPLI_PRAM_ERR = (4000016, "必传的参数重复", "必须的参数（inputs 或 input_urls）只能二选一")
    ...
    # 420003 中文占比过低
    TEXT_CHINESE_TOO_LOW_ERR = (420003, "中文占比过低", "文本中的中文（仅包含汉字，不包含任何数字、符号）占比不低于50%")
