...
from pysparkgui.helper.utils import (
    log_action,
    log_view,
...
from pysparkgui.helper.error import PysparkguiError
from pysparkgui.helper.gui_outlets import (
...
from pysparkgui.widgets import Text, Button, BrowserCheck
...
class Viewable:
    ...
    def __init__(
        ...
        if df is not None:
            if isinstance(df, pd.DataFrame):
                from pysparkgui.df_manager import (
                    DfManager,
    ...
    def get_df(self):
        ...
        :raises PysparkguiError: when the method is called but no DataframeManager exists
        """
        if self.df_manager is not None:
            return self.df_manager.get_latest_df()
        else:
            raise PysparkguiError(
...
class TabViewable(Viewable):
    ...
    def set_content(self, *embeddables, **kwargs):
        ...
        box.add_class("pysparkgui-min-height-for-tab-viewables")
    ...
    def _ipython_display_(self, *args, **kwargs):
        ...
        """
        from pysparkgui.spark_wrangler import (
            SparkWrangler,
        )  # inline import to prevent circular import
...
class Transformation(Viewable):
    ...
    https://github.com/tkrabel/pysparkgui/tree/master/plugins#pysparkguipluginstransformationplugin
    ...
    def set_content(self, *embeddables, **kwargs):
        ...
        from pysparkgui.widgets.focus_box import FocusBox
    ...
    def execute_again_as_side_effect_of_a_previous_transformation(self):
        self._execute_internal(self, execution_triggered_by_user=False)
    ...
    def _execute_internal(self, *args, execution_triggered_by_user=True, **kwargs):
        ...
        self.result = {
            ...
            # Answer: placeholders are not filled in yet so that the old and new df name can change without having to update the code
            "code": {
                "pyspark_assignment_format": self.get_code(),  # default
                "pyspark_chain_format": self.get_pyspark_chain_code(),
            },
            "type": self._get_type(),
            ...
            "preview_columns_selection": None,  # always reset the selection
            "row_preview_result": {},
            "row_count": None,
        }

        # Attention: this should only happen after the result has been successfully calculated
        self.df_manager.maybe_add_transformation(self)
        
        # Attention: needs to happen after maybe_add_transformation
        # self._log_metainfos(is_update=is_update)

        # calculation insights are disabled for now because they don't make sense with spark anyway??
        # because they are only valid if done for the whole df?
        # # calculating the insight needs to happen AFTER the transformation is added to the df_manager
        # # AND BEFORE the subsequent steps are reexecuted
        # # because during the calculation of the insight, we make use of the df_manager e.g. current_df and previous_df etc
        # self._bam_transformation_insight_outlet.children = [
        #     self.get_transformation_insight()
        # ]

        if execution_triggered_by_user:
            self.df_manager.execute_subsequent_transformations(self)
            self.df_manager.maybe_update_live_code_export_and_user_symbols()
            self.df_manager.notify_that_df_did_change()
    ...
    def _get_old_df_name(self):
        if self._is_update():
            return self.df_manager.get_previous_df_name(self)
        else:
            return self.df_manager.get_latest_df_name()
    ...
    def is_valid_transformation(self) -> bool:
        ...
        You can override this method to raise PysparkguiErrors or to stop the execution process.
    ...
    def get_df(self):
        ...
        """
        if self._is_update():
            return self.df_manager.get_previous_df(self)
        else:
            return self.df_manager.get_latest_df()
    ...
    def get_result_df(self):
        ...
        symbols[self._get_old_df_name()] = self.get_df()
    ...
    def get_code(self):
        ...
        """
        from pysparkgui.helper import DF_OLD, DF_NEW

        assignment_code = self.get_pyspark_assignment_code()
        if assignment_code is not None:
            return assignment_code

        chain_code = self.get_pyspark_chain_code()
        if chain_code is not None:
            return f"{DF_NEW} = {DF_OLD}{chain_code}"
        
        # otherwise: (assignment_code is None) and (chain_code is None)
        raise NotImplementedError

    def get_pyspark_assignment_code(self):
        """
        MIGHT BE OVERRIDEN
        Can be implemented as an alternative to get_code to be very explicit about the code style
    ...
    def get_pyspark_chain_code(self):
        """
        MIGHT BE OVERRIDEN
        Can be implemented to support code export with chain format

        :return str - code for the transformation which can be chained on to a pyspark Dataframe expression.
        Otherwise return None if the transformation does not support the chain style code format for pyspark syntax
        This might be useful when only some versions of the transformation cannot be expressed in this format.
        """
        return None
    ...
    #     :return bool if the preview columns selection in the SparkWrangler should be reset after the execution of the transformation
    #     """
    #     return True
    ...
    def _ipython_display_(self, *args, **kwargs):
        ...
        """
        from pysparkgui.spark_wrangler import SparkWrangler  # prevent circular import

        df_manager = self.df_manager
        tab_section = TabSection(df_manager)
        wrangler = SparkWrangler(df_manager=df_manager, parent_tabs=tab_section)
...
class Loader(TabViewable):
    ...
    To see an overview of the most important methods and attributes, please check
    https://github.com/tkrabel/pysparkgui/tree/master/plugins#pysparkguipluginsloaderplugin
    ...
    def _execute_internal(self, *args, **kwargs):
        ...
        # but does not show the code in Jupyter because the SparkWrangler/DfManager trigger the code export
        df = exec_code(
            code, symbols=self.symbols, result_name=df_name, manipulate_symbols=True, add_pandas_and_numpy=True
        )

        from pysparkgui.spark_df_manager import SparkDfManager  # inline to prevent circular import
        from pysparkgui.spark_wrangler import SparkWrangler  # inline to prevent circular import

        self.df_manager = SparkDfManager(
            df,
            self.symbols,
            setup_code=code,
            df_name=df_name,
        ...
        tab_section.add_tab(self, closable=False)
        tab_section.add_tab(
            SparkWrangler(df_manager=self.df_manager, parent_tabs=tab_section),
            closable=False,
        )
    ...
    def _ipython_display_(self, *args, **kwargs):
        ...
        from pysparkgui.setup.user_symbols import (
            get_user_symbols,
...
class TabSection(widgets.VBox):
    ...
    def __init__(self, df_manager):
        ...
        self.header.add_class("pysparkgui-tab-header")
    ...
    def render(self):
        ...
        if len(self.tab_windows) <= 1:
            self.header.add_class("pysparkgui-hidden")
        else:
            self.header.remove_class("pysparkgui-hidden")
    ...
    def activate_tab(self, tab_window):
        ...
        if self.active_window:
            self.active_window.tab.remove_class("pysparkgui-tab-active")
            self.active_window.outlet.add_class("pysparkgui-hidden")

        tab_window.tab.add_class("pysparkgui-tab-active")
        tab_window.outlet.remove_class("pysparkgui-hidden")
    ...
    def add_tab(self, viewable, closable=True):
        ...
        """
        from pysparkgui.widgets import Tab
