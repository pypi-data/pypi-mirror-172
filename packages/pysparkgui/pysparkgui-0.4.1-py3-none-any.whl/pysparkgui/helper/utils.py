...
from pysparkgui.setup.file_logger import file_logger

import pysparkgui.config as config
...
from pysparkgui.widgets import CopyButton, CloseButton, BackButton, Text, Button


DF_OLD = "old_df__pysparkgui_template_name"
DF_NEW = "new_df__pysparkgui_template_name"

FEEDBACK_SURVEY_LINK_HREF = (
    "https://forms.gle/hybNSh67rHTiTcGcA"
)


def css_spinner():
    return "<div class='pysparkgui-load-spinner'></div>"
...
def notification(message, type="info"):
    ...
    """
    ui = (
        '<div class="pysparkgui-notification pysparkgui-notification-%s">' "%s" "</div>"
    ) % (type, message)
...
def collapsible_notification(title, body, collapsed=True, type="info"):
    ...
    output.add_class("pysparkgui-collapsible-notification")
    output.add_class(f"pysparkgui-collapsible-notification-{type}")
...
def activate_license(email="", key="", _notification_embeddable=None):
    ...
    from pysparkgui._authorization import auth
    from pysparkgui.widgets import Text, BrowserCheck

    click_here_for_a_trial_license = """<a href="https://pysparkgui.8080labs.com/trial/" target=_blank class="pysparkgui-link">Click here</a> for a trial license."""
    header = widgets.HTML(
        f"""
        <h2>Activate pysparkgui Pro</h2>
        Thank you for using pysparkgui! It seems like you need a Pro license.<br>
    ...
    ).add_class("pysparkgui-width-15px")
    accept_eula_text_and_link = widgets.HTML(
        (
            "I read and accept the "
            '<a href="https://pysparkgui.com/eula" target=_blank class="pysparkgui-link">'
...
def log_base(
    ...
    **kwargs,
    ...
    """
    from pysparkgui._authorization import auth
    from pysparkgui import __version__ as version
    ...
    # general information about the user and the pysparkgui setup
...
def log_databricks_funnel_event(description):
    ...
    if can_log and auth.is_databricks():
        env.DBUTILS.entry_point.getLogger().logDriverEvent({'eventType': 'pysparkgui_funnel_v0', 'status': description, 'error': 'None'})
        # How to verify in Databricks: use the cluster's webterminal
        # cat /databricks/driver/logs/usage.json | grep pysparkgui_funnel_v0
...
def set_license(encrypted_string):
    ...
    reads the license file under ~/.pysparkgui/LICENSE, and activates pysparkgui in the sandbox
    by calling:

    set_license(<content of ~/.pysparkgui/LICENSE>), e.g.
