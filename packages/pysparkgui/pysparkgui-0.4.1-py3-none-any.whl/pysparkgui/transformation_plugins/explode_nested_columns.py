from pysparkgui.plugins import TransformationPlugin, DF_OLD, DF_NEW, PysparkguiError
from pysparkgui.helper import string_to_code
from pysparkgui.widgets import Singleselect
...
class ExplodeNestedColumns(TransformationPlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.column = Singleselect(
            options=list(self.df_manager.get_latest_df().columns),
    ...
    def is_valid_transformation(self):
        if self.column.value is None:
            raise PysparkguiError("It seems like you did not select a column")
        return True
