...
from pysparkgui.edaviz.base import *
from pysparkgui.edaviz.constants import *
from pysparkgui.edaviz.plots import *
...
from pysparkgui.edaviz.interactive_histogram import InteractiveHistogram
from pysparkgui.edaviz.timeseries_plot import (
    UnivariateTimeseriesPlot,
    BivariateTimeseriesPlot,
)

# _underscore_functions are not exposed when running from ... import *
from pysparkgui.edaviz.utils import (
    _get_bin_settings,
...
from pysparkgui.edaviz.data_types import (
    is_dtype_timedelta,
    is_binary,
    is_numeric,
    is_object,
...
class GlimpseTable(AsyncEmbeddable):
    ...
    def init_embeddable(
        ...
        self.add_class("pysparkgui-glimpse-table")
        output = []
        rows_dict = {}
        SAMPLE_SIZE = 10_000
...
def unsupported_dtype_distribution(
    ...
    final_output = [
        notification(
            ...
            <a href="mailto:pysparkgui-feedback+unsupported_dtype_distribution@databricks.com?subject=Support of {data_type}&body=Please add support for {data_type}. For data type {data_type}, I'd like to see [what would you like to see?]">
                send us an email
            </a>.""",
            type="error",
        )
    ...
    def init_embeddable(
        ...
        **kwargs,
        ...
        for index, column in enumerate(df_column_indices):
            ...
            ).add_class("pysparkgui-width-200px")
...
def _relation_has_no_visualization_yet(df, x, y, **kwargs):
    message = (
        ...
        "If you need this kind of visualization, please contact us via pysparkgui-feedback@databricks.com"
    )
...
def create_dt_score_focus_row(
    ...
    label = widgets.HTML(f"{score:.3f} using '{feature}'").add_class(
        "pysparkgui-width-300px"
    )

    from pysparkgui.viz import RelateColumns
...
def bivariate_plot(df, col_a=None, col_b=None, **kwargs):
    ...
    return (
        function_hint_,
        widgets.HBox(
            [widgets.HTML("Plot").add_class("pysparkgui-width-40px"), dd_col_a]
        ).add_class("pysparkgui-overflow-visible"),
        widgets.HBox(
            [widgets.HTML("and").add_class("pysparkgui-width-40px"), dd_col_b]
        ).add_class("pysparkgui-overflow-visible"),
        spacer,
        comparison_outlet,
    )
