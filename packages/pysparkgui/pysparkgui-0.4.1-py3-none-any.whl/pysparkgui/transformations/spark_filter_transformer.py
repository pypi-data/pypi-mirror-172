...
from pysparkgui.widgets import (
    Singleselect,
    Multiselect,
    Text,
    Button,
...
from pysparkgui.transformations.base_components import SelectorGroupMixin, SelectorMixin


MAX_UNIQUE_COLUMN_VALUES_FOR_DROPDOWN = 100_000

AND_CONNECTOR_STRING = "AND"
OR_CONNECTOR_STRING = "OR"

CONTAINS = "contains"


def format_column_names(formula, column_names):
    """
    This method formats column names to spark format in a formula string.
    e.g. 'a + ab' -> 'f.col("a") + f.col("ab")'
    """
    column_names = column_names.copy()  # don't sort the original list
    # start with longer strings first!
    column_names.sort(key=lambda string: len(string), reverse=True)
    # this has to happen in 2 steps via placeholders because otherwise smaller substrings will destroy longer strings
    translation_dict = {}
    counter = 0
    for column_name in column_names:
        if column_name in formula:
            placeholder = f"BAM_PLACEHOLDER_{counter}"
            formula = formula.replace(column_name, placeholder)
            translation_dict[placeholder] = column_name
            counter += 1
    for placeholder in translation_dict.keys():
        column_name = translation_dict[placeholder]
        formula = formula.replace(placeholder, f"f.col({string_to_code(column_name)})")
    return formula
...
class ConditionMixin:
    ...
    def value_is_not_empty(self, value, value_name="textfield"):
        if value == "":
            raise PysparkguiError(f"The {value_name} is empty.<br>Please insert a value")
        else:
            return True


class NoFurtherInputCondition(ConditionMixin, widgets.VBox):
...
class StringInputCondition(ConditionMixin, widgets.VBox):
    ...
    def get_code(self):
        ...
        return self.code_template % (string_to_code(self.column), string_to_code(text))
...
class FormulaCondition(ConditionMixin, widgets.VBox):
    ...
    in (age < (age + 5)) this condition will handle the (age + 5) part
    and the part before (age <) will be handled by the surrounding condition row.
    """
    ...
    def get_code(self) -> str:
        formula = format_column_names(self.formula_input.value, self.column_names)
        return self.code_template % (string_to_code(self.column), formula)
...
class ContainsFilterTextfieldCondition(ConditionMixin, widgets.VBox):
    ...
    This one is different from :class:`StringInputCondition` as it allows regular expressions.
    """
    ...
    def __init__(self, *args, value="", **kwargs):
        ...
        self.case_sensitive_checkbox.add_class("pysparkgui-checkbox")

        self.regexp_checkbox = widgets.Checkbox(
            value=False, description="regular expression"
        )
        self.regexp_checkbox.add_class("pysparkgui-checkbox")
...
STRING_DROPDOWN_OPTIONS = {
    ...
    STARTS_WITH_LABEL: {
        "embeddable_class": StringInputCondition,
        "embeddable_kwargs": {
    ...
    "is missing": {
        "embeddable_class": NoFurtherInputCondition,
        "embeddable_kwargs": {
...
SPARK_NUMERIC_COLUMN_DROPDOWN_OPTIONS = {
    "<": {
        "embeddable_class": FormulaCondition,
        "embeddable_kwargs": {
            "description_template": "%s < %s",
            "code_template": f"f.col(%s) < %s",
            # "code_template": f"`%s` < %s",
        },
    },
    "<=": {
    ...
    "= (equals)": {
        "embeddable_class": FormulaCondition,
        "embeddable_kwargs": {
            "description_template": "%s equals %s",
            "code_template": f"f.col(%s) == %s",
            # "code_template": f"`%s` = %s",
        },
    },
    "!= (does not equal)": {
        "embeddable_class": FormulaCondition,
        "embeddable_kwargs": {
            "description_template": "%s does not equal %s",
            "code_template": f"f.col(%s) != %s",
            # "code_template": f"`%s` != %s",
        },
    },
    ...
    "is missing": {
        ...
        "embeddable_kwargs": {
            "description_template": "%s is missing",
            "code_template": f"f.col(%s).isNull()",
            # "code_template": f"`%s` IS NULL",
        },
    },
    "is not missing": {
        "embeddable_class": NoFurtherInputCondition,
        "embeddable_kwargs": {
            "description_template": "%s is not missing",
            "code_template": f"f.col(%s).isNotNull()",
            # "code_template": f"`%s` IS NOT NULL",
        },
    },
}


SPARK_BOOLEAN_COLUMN_DROPDOWN_OPTIONS = {
    "is True": {
        "embeddable_class": NoFurtherInputCondition,
        "embeddable_kwargs": {
            "description_template": "%s is True",
            "code_template": f"f.col(%s).isin([True])",
            # "code_template": f"`%s` IS TRUE",
        },
    },
    "is False": {
        "embeddable_class": NoFurtherInputCondition,
        "embeddable_kwargs": {
            "description_template": "%s is False",
            "code_template": f"f.col(%s).isin([False])",
            # "code_template": f"`%s` IS FALSE",
        },
    },
    "is missing": {
        "embeddable_class": NoFurtherInputCondition,
...
SPARK_STRING_COLUMN_DROPDOWN_OPTIONS = {
    "contains": {
        "embeddable_class": StringInputCondition,
        "embeddable_kwargs": {
            "description_template": "%s contains %s",
            "code_template": f"f.col(%s).contains(%s)",
        },
    },
    "starts with": {
        "embeddable_class": StringInputCondition,
        "embeddable_kwargs": {
            "description_template": "%s starts with %s",
            "code_template": f"f.col(%s).startswith(%s)",
        },
    },
    "ends with": {
        "embeddable_class": StringInputCondition,
        "embeddable_kwargs": {
            "description_template": "%s ends with %s",
            "code_template": f"f.col(%s).endswith(%s)",
        },
    },
    "is missing": {
        "embeddable_class": NoFurtherInputCondition,
    ...
    # Attention: in spark, it's also possible to apply <, <=, >, >= to string columns
    # This condition is checked on all values in a column that can be parsed as numeric
    "<": {
        "embeddable_class": StringInputCondition,
        "embeddable_kwargs": {
            "description_template": "%s < %s",
...
class ConditionRow(SelectorMixin, widgets.VBox):
    ...
    def _update_selected_column(self, **kwargs):
        ...
        self.column_dtype = [dtype for name, dtype in self.df.dtypes if name == self.column][0]
    ...
    def _get_dropdown_options(self):
        ...
        if dtype in ["bigint", "double"]:
            return SPARK_NUMERIC_COLUMN_DROPDOWN_OPTIONS
        if dtype == "boolean":
            return SPARK_BOOLEAN_COLUMN_DROPDOWN_OPTIONS
        if dtype == "string":
            return SPARK_STRING_COLUMN_DROPDOWN_OPTIONS
        else:
    ...
    def _show_unsupported_column_type_warning(self):
        ...
        self.condition_outlet.children = [
            ...
            widgets.HTML(
                # TBD: alternatively, we could also log when a user sees this and then do it ourselves?
                f"""Sorry, <b>{self.column_dtype}</b> columns are not supported yet. Please reach out via pysparkgui-feedback@databricks.com"""
            ),
        ]
    ...
    def _get_connector_code(self):
        """In pyspark, is either "AND" or "OR"."""
        if self.has_connector:
...
class SparkFilterTransformer(Transformation):
    ...
    def __init__(self, *args, column=None, **kwargs):
        ...
        self.filter_type = Singleselect(
            options=[("Select rows", "keep"), ("Remove rows", "delete")],
            focus_after_init=True,
            set_soft_value=True,
            width="sm",
        )
    ...
    def get_description(self):
        ...
        if type_ == "delete":
            return f"<b>Remove rows</b> where {boolean_series_description}"

    def get_pyspark_chain_code(self):
        filter_expression = self.condition_section.get_code()
