...
from pysparkgui.helper import Transformation, DF_OLD, log_error, notification, VSpace
from pysparkgui.helper import string_to_code

from pysparkgui.widgets import Singleselect, Text


# DATETIME_FORMAT_HELP_TEXT = """
# <u>Format string examples:</u> <a href='https://docs.python.org/3/library/datetime.html#strftime-and-strptime-behavior' target='_blank' class='pysparkgui-link'>(see full documentation)</a><br>
# <ul>
...
class NoOptionsEmbeddable(widgets.VBox):
    """
    Basically an empty embeddable when no options are required.
    """
...
class DefaultHandler:
    ...
    # to be overriden
    code_template_with_column_name = None
    default_code_template = None  # column name and kwargs will be filled in
    ...
    def get_code(self, option_section, *args, **kwargs):
        ...
        if self.__class__.code_template_with_column_name is not None:
            return self.__class__.code_template_with_column_name % string_to_code(self.column)
        else:
...
# class SparkToInt(DefaultHandler):
#     """Handles casting data type to int."""
...
class SparkToInt(DefaultHandler):
    code_template_with_column_name = f"f.col(%s).cast('int')"


class SparkToDouble(DefaultHandler):
    code_template_with_column_name = f"f.col(%s).cast('double')"


class SparkToString(DefaultHandler):
    code_template_with_column_name = f"f.col(%s).cast('string')"


class SparkToBoolean(DefaultHandler):
    code_template_with_column_name = f"f.col(%s).cast('boolean')"


DTYPE_TRANSFORMATION = {
    "double": {"name": "Numeric - default (Double)", "handler": SparkToDouble},
    "int": {"name": "Numeric (Integer)", "handler": SparkToInt},
    "string": {"name": "Text (String)", "handler": SparkToString},
    "boolean": {"name": "True/False (Boolean)", "handler": SparkToBoolean},
...
class DtypeSelector(widgets.VBox):
    ...
    def __init__(self, transformation, df, column, target_dtype, focus_after_init=True):
        ...
        self.has_na = False
        self.old_column_dtype = [dtype for column, dtype in df.dtypes if column == self.column][0]
...
class SparkDtypeTransformer(Transformation):
    ...
    def _update_old_dtype_output(self):
        old_dtype = [dtype for column, dtype in self.get_df().dtypes if column == self.column][0]
    ...
    def get_pyspark_chain_code(self):
        new_column_name = self.new_column_name_input.value
        return f".withColumn({string_to_code(new_column_name)}, {self.dtype_selector.get_code()})"
        # e.g. df2 = df2.withColumn("n2", f.col("n").cast('double')))
...
"""
Children of SparkDtypeTransformer which are shown as transformations from the search bar in the wrangler e.g. "To numeric"
"""


# class SparkToDoubleTransformer(SparkDtypeTransformer):
#     target_dtype = "double"
#     modal_title = "To Numeric (Double)"


# class SparkToIntTransformer(SparkDtypeTransformer):
#     target_dtype = "int"
#     modal_title = "To Numeric (Integer)"


# class SparkToStringTransformer(SparkDtypeTransformer):
#     target_dtype = "string"
#     modal_title = "To Text (String)"


# class SparkToBooleanTransformer(SparkDtypeTransformer):
#     target_dtype = "boolean"
#     modal_title = "To True/False (Boolean)"



# class ToIntegerTransformer(SparkDtypeTransformer):
#     target_dtype = "int64"
#     modal_title = "To Integer"


# class ToInteger32Transformer(SparkDtypeTransformer):
#     target_dtype = "int32"
#     modal_title = "To Integer (32-bit)"


# class ToInteger16Transformer(SparkDtypeTransformer):
#     target_dtype = "int16"
#     modal_title = "To Integer (16-bit)"


# class ToInteger8Transformer(SparkDtypeTransformer):
#     target_dtype = "int8"
#     modal_title = "To Integer (8-bit)"


# class ToUnsignedIntegerTransformer(SparkDtypeTransformer):
#     target_dtype = "uint64"
#     modal_title = "To Unsigned Integer"


# class ToUnsignedInteger32Transformer(SparkDtypeTransformer):
#     target_dtype = "uint32"
#     modal_title = "To Unsigned Integer (32-bit)"


# class ToUnsignedInteger16Transformer(SparkDtypeTransformer):
#     target_dtype = "uint16"
#     modal_title = "To Unsigned Integer (16-bit)"


# class ToUnsignedInteger8Transformer(SparkDtypeTransformer):
#     target_dtype = "uint8"
#     modal_title = "To Unsigned Integer (8-bit)"


# class ToFloatTransformer(SparkDtypeTransformer):
#     target_dtype = "float64"
#     modal_title = "To Float"


# class ToFloat32Transformer(SparkDtypeTransformer):
#     target_dtype = "float32"
#     modal_title = "To Float (32-bit)"


# class ToFloat16Transformer(SparkDtypeTransformer):
#     target_dtype = "float16"
#     modal_title = "To Float (16-bit)"


# class ToBoolTransformer(SparkDtypeTransformer):
#     target_dtype = "bool"
#     modal_title = "To Boolean"


# class ToCategoryTransformer(SparkDtypeTransformer):
#     target_dtype = "category"
#     modal_title = "To Category"


# class ToStringTransformer(SparkDtypeTransformer):
#     target_dtype = "string"
#     modal_title = "To Text/String"


# class ToObjectTransformer(SparkDtypeTransformer):
#     target_dtype = "object"
#     modal_title = "To Object"


# class ToDatetimeTransformer(SparkDtypeTransformer):
#     target_dtype = "datetime64[ns]"
#     modal_title = "To Datetime"


# class ToTimedeltaTransformer(SparkDtypeTransformer):
