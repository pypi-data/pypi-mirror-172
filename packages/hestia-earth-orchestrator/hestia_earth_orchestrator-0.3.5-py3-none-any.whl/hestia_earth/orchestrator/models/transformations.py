...
def _apply_transformation_share(previous: dict, current: dict):
    share = current.get('previousTransformationShare')
    ...
    def replace_field(input: dict, field: str):
        ...
        should_replace = len(product_value) > 0
    ...
    return {**current, 'inputs': list(map(replace_value, current.get('inputs', [])))} if share else current
