...
class HkShSz(object):
    @classmethod
    def hkshsz(cls, hkshsz_data, indicators, name, width='1600px', height = '600px'):
        hkshsz_data = pd.concat([hkshsz_data,indicators[['close_price']]], axis=1)
        hkshsz_data['hkshsz_value'] = hkshsz_data['close_price'] * hkshsz_data['party_vol']
        ...
        bar_plot = cls.hkshsz_bar(hkshsz_data[['vol_rate']], trade_date_list, name)
        line_plot = cls.hkshsz_line(hkshsz_data[['shhk_money_flowin']], trade_date_list, name)
        grid = Grid()
        grid.add(bar_plot,
                grid_opts=opts.GridOpts(
                     pos_left="5%", pos_right="4%", height="45%"
                 ))
    
        grid.add(
            line_plot,
            grid_opts=opts.GridOpts(
                pos_left="5%", pos_right="4%", pos_top="55%", height="40%"
            ))
    ...
    def hkshsz_line(cls, shhk_money_data, trade_date_list, name):
        ...
        result_format = [result  for result in shhk_money_data['shhk_money_flowin'].values.tolist()]
    ...
    def hkshsz_bar(cls, vol_data, trade_date_list, name):
        ...
        bar_plot.add_yaxis('净买入变化率(%)', vol_drop, 
                           label_opts=opts.LabelOpts(is_show=False))
