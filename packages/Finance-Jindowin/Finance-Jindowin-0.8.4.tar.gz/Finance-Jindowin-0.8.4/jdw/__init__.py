...
import requests, warnings, os, requests

version_url = 'https://ultronsandbox.oss-cn-hangzhou.aliyuncs.com/version/jdw.json'
...
try:
    ...
    warnings.warn("pip install --upgrade  Jindowin")
#try:
...
def check_version():
    ...
    if not remote_version or remote_version <= __version__:
        return
    print(
        'JinDowin 版本由%s升级到%s,升级内容%s,执行 pip install --uprgrade Finance-Jindowin'
