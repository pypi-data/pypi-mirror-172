...
from azureml.automl.dnn.nlp.common._diagnostics.nlp_error_definitions import SuspectedVnetIssue
...
class TestResourcePathResolver:
    ...
    def test_rpr_raises_on_cdn_failure(self):
        with patch("azureml.automl.dnn.nlp.common._resource_path_resolver"
                   ".AutoMLPretrainedDNNProvider.get_model_dirname") as mock_fetch_model:
            mock_fetch_model.side_effect = ClientException()
            with pytest.raises(ClientException) as exc:
                rpr = ResourcePathResolver()
                rpr.model_path
            assert exc.value.error_code == SuspectedVnetIssue.__name__
