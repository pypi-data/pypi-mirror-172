...
## MAIN CODE
def link_company_names(df, min_similarity_score=.80):
    ...
    grouped_df = df.value_counts().rename_axis('first_plaintiff').reset_index(name='counts')
...
def _link_company_names_levenshtein(names, min_similarity_score):
    ...
    for name in names:
        ...
        matches = process.extract(name, names, processor=None, score_cutoff=min_similarity_score)
...
def _link_company_names_tfidf_cos(names,min_similarity_score):
    ...
    matches = _conduct_cos_sim(tf_idf_matrix, tf_idf_matrix.transpose(), MAX_NAMES_TO_LINK, min_similarity_score)
    ...
    for name in tqdm(names):
        ...
        temp_dict = dict(zip(temp_df.right_side, [linked_name] * len(temp_df.right_side)))
