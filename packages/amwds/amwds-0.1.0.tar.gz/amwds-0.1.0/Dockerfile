...
# and `pip install amwds` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "amwds", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n amwds -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "amwds", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "amwds", "YOUR_CONSOLE_SCRIPT"]
