...
class Diary(DiaryBase):
    ...
    def _get_diary(self, date: datetime, span: int):
        ...
        self.session.headers["Host"] = self.base_address
    ...
    def get_period_marks(self, index: int, period: int):
        """
        ...
        r = self._get(f"https://schools.{self.base_address}/marks.aspx?"
                      ...
                      # index of a year
                      # each year has it own index (e.g. 2019-0 2020-1 2021-2 and so on)
                      # we can get data for any year by passing this index here
                      f"index={index}&"
                      "tab=period&"
                      f"period={period}&"
