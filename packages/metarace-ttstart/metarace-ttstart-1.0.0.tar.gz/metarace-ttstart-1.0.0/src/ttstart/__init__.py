...
_NOHDR = [u'Start', u'start', u'time', u'Time', u'']
...
def _fit_cent(cr, pr, h, w, msg, charHeight, fontName):
    """Squeeze-fit msg centered at h in a box of width w."""
    if msg is not None:
        cr.save()
        ...
        if tw > w:
            cr.scale(float(w) / float(tw), 1.0)
            tw = w
        cr.move_to(0.5 * (w - tw), h)
...
def _text_cent(cr, pr, h, w, msg, charHeight, fontName):
    """Position text centered at h,w."""
    if msg is not None:
        cr.save()
        l = Pango.Layout.new(pr)
        l.set_alignment(Pango.Alignment.CENTER)
...
class ttstart(Gtk.Window):
    ...
    def __init__(self):
        ...
        self._area.set_size_request(512, 300)
        self.add(self._area)
        self._area.show()
    ...
    def _do_drawing(self, cr, pr):
        ...
        if self._tod is not None:
            ...
            _text_cent(cr, pr, oh, 0.5 * self._width, self._tod.meridiem(), ch,
                       _TIMEFONT)
        ...
        if self._countdown is not None:
            ...
            _text_cent(cr, pr, oh, 0.5 * self._width, ctx, ch, _COUNTERFONT)

        # rider name txt
        if self._riderstr is not None:
            oh = 0.78 * self._height
            ch = 0.10 * self._height
            _fit_cent(cr, pr, oh, self._width, self._riderstr, ch,
                      _COUNTERFONT)
    ...
    def _loadconfig(self):
        ...
        self._player.set_property('uri', u'file://' + audioFile)
        ...
        try:
            ...
            if postype:
                ...
                if err > self._syncthresh:
                    _log.error('Audio desync ~%0.3fs', err)
                    self._player.set_state(Gst.State.NULL)
                else:
                    pass
    ...
    def _timeout(self, data=None):
        ...
        while self._nc < tt:  # ensure interval is positive
            ...
            self._nc += tod.ONE  # 0.01 allows for processing delay
