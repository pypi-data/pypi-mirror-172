...
__all__ = ["LOCALTERRA_MNEMONICS", "LocalBostrom", "AsyncLocalBostrom"]
...
class AsyncLocalBostrom(AsyncLCDClient):
    """An :class:`AsyncLCDClient` that comes preconfigured with the default settings for
    connecting to a LocalBostrom node.
    """

    wallets: Dict[str, AsyncWallet]
    """Ready-to use :class:`Wallet` objects with LocalBostrom default accounts."""
...
class LocalBostrom(LCDClient):
    ...
    >>> bostrom.wallets['test1'].key.acc_address
