...
def body_post(fixtures: Dict[str | CollectionKey, List[DesktopObjectDict]], coll: str | CollectionKey,
              default_db_manager_cls: Type[DatabaseManager]) \
...
def body_put(fixtures: Dict[str | CollectionKey, List[DesktopObjectDict]], coll: str | CollectionKey,
             default_db_manager_cls: Type[DatabaseManager]) \
             -> Dict[str, Dict[str, List[Dict[str, Any]]]]:
    """
    Create a Collection+JSON template from a data test fixture.
...
def expected_one_wstl(fixtures: Dict[str | CollectionKey, List[DesktopObjectDict]],
...
def expected_one(fixtures: Dict[str | CollectionKey, List[DesktopObjectDict]], coll: str | CollectionKey,
                 wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                 default_db_manager_cls: Type[DatabaseManager],
...
def expected_opener_body(fixtures: Dict[str | CollectionKey, List[DesktopObjectDict]], coll: str | CollectionKey,
                         wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                         default_db_manager_cls: Type[DatabaseManager],
...
def expected_one_duplicate_form(fixtures: Dict[str | CollectionKey, List[DesktopObjectDict]],
                                coll: str | CollectionKey,
                                wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                                default_db_manager_cls: Type[DatabaseManager],
...
def expected_all_wstl(fixtures: Dict[str | CollectionKey, List[DesktopObjectDict]],
                      coll: str | CollectionKey,
                      wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                      default_db_manager_cls: Type[DatabaseManager],
    ...
    return [{
        'wstl': {
            'data': _wstl_data_transform(query_fixture_collection(fixtures, coll, default_db_manager=default_db_manager_cls)),
...
def expected_all(fixtures: Dict[str | CollectionKey, List[DesktopObjectDict]], coll: str | CollectionKey,
                 wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                 default_db_manager_cls: Type[DatabaseManager],
...
def expected_values(fixtures: Dict[str | CollectionKey, List[DesktopObjectDict]],
                    coll: str | CollectionKey,
                    wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                    duplicate_action_name: str,
                    ...
                    default_db_manager_cls: Type[DatabaseManager] = DatabaseManager) -> Dict[str, Any]:
    """
    ...
    for input in action['inputs']:
        ...
        if 'add' in targets:
            ...
            if isinstance(val, list):
                if wstl.has_section(input):
                    if val:
                        for i_, v in enumerate(val):
                            yield _new_template_data_item(input, v.get(nm)) | {
                                'index': i_,
                                'section': section
                ...
                yield _new_template_data_item(input, val.get(nm)) | {
                    'section': section
                } | _section_prompt_key_value(input)
            else:
                yield _new_template_data_item(input, val)
        elif not section:
            yield _new_template_data_item(input, None)
        else:
            yield _new_template_data_item(input, None) | {'section': section, 'index': -1} | _section_prompt_key_value(input)
...
def _expected_one_form(fixtures: Dict[str | CollectionKey, List[DesktopObjectDict]],
                       coll: str | CollectionKey,
                       wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                       default_db_manager_cls: Type[DatabaseManager],
                       action_name: str,
    ...
    return [{
        'collection': {
            ...
            'template': {
                ...
                'data': [d for d in _template_data_generator(action, obj)]}
        }}]
