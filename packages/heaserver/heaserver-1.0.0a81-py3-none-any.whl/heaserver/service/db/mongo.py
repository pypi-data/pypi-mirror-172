...
class Mongo(Database):
    ...
    async def put_content(self, request: web.Request, collection: str, sub: Optional[str] = None) -> bool:
        ...
        if obj is None or not has_permissions(desktop_obj, sub, PermissionGroup.PUTTER_PERMS.perms):
            return False
        try:
            fs = self.__new_gridfs_bucket(collection)
...
class MongoManager(DatabaseManager):
    """
    Database manager for MongoDB. This class is not designed to be subclassed.
    """
