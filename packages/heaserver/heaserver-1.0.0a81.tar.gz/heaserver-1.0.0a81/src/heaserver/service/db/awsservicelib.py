...
async def get_object_content(request: Request) -> web.StreamResponse:
    ...
    try:
        ...
        headers = ((hdrs.CONTENT_DISPOSITION, "attachment; filename=%s" % (key.split("/")[-1],)),)

        response_ = web.StreamResponse(status=200, reason='OK', headers=headers)
