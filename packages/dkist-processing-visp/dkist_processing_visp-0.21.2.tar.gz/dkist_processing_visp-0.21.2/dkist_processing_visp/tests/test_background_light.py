...
def background_testing_parameters(background_full_beam_shape) -> VispTestingParameters:
    ...
    return VispTestingParameters(
        visp_background_num_spatial_bins=1,
        visp_background_wavelength_subsample_factor=20,
        visp_background_num_fit_iterations=10,
...
@pytest.fixture(scope="session")
def dummy_all_spectra_array() -> np.ndarray:
    return np.random.random((12, 100, 2560))
...
def test_background_light_calibration_task(
    ...
    with open(quality_record[0], "r") as f:
        ...
        assert data["total_frames"] == total_polcal_files
        assert data["frames_not_used"] == num_dark_polcal_files
...
def test_looping_spatial_resample(
    background_light_calibration_task_non_polarimetric_dataset, dummy_all_spectra_array
):
    """
    ...
    spat_block = dummy_all_spectra_array.shape[-1] // task.parameters.background_num_spatial_bins
    expected = skime.block_reduce(
        dummy_all_spectra_array, block_size=(1, 1, spat_block), func=np.median
    )
    observed = task.resample_spatial_dimension(
        dummy_all_spectra_array, task.parameters.background_num_spatial_bins
    )
...
def test_looping_spatial_resample_fail_on_bad_bin_param(
    ...
    bad_bin_number = dummy_all_spectra_array.shape[-1] / 2 + 1

    with pytest.raises(ValueError):
        observed = task.resample_spatial_dimension(dummy_all_spectra_array, bad_bin_number)
