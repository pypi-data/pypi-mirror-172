...
class BackgroundLightCalibration(
    ...
    def run(self) -> None:
        ...
        for beam in range(1, self.constants.num_beams + 1):
            ...
            logging.info(f"Dark-subtracting raw polcals for {beam = }")
            all_data = self.reduce_and_collect_polcals(beam=beam)

            num_bins = self.parameters.background_num_spatial_bins
            num_wave, num_slit_pos = all_data.shape[-2:]
            logging.info(f"Using median filtering to resample spatial dimension to {num_bins} bins")
            resampled_data = self.resample_spatial_dimension(all_data, num_bins)
            ...
            full_background_light = self.upsample_background_light(
                small_background_light, full_shape=(num_wave, num_slit_pos)
            )

            self.intermediate_frame_helpers_write_arrays(
                arrays=full_background_light, beam=beam, task="BACKGROUND"
            )
            num_used_polcal_files = all_data.shape[0]
        ...
        with self.apm_processing_step("Computing and logging quality metrics"):
            num_raw_polcal: int = self.scratch.count_all(
                tags=[
                    VispTag.input(),
                    VispTag.frame(),
                    VispTag.task("POLCAL"),
            ...
            self.quality_store_task_type_counts(
                ...
                total_frames=num_raw_polcal,
                frames_not_used=num_raw_polcal - num_used_polcal_files,
            )
    ...
    def reduce_and_collect_polcals(self, beam: int) -> np.ndarray:
        """
        Remove dark signal from all polcal frames and collect them in a big array stack.
        ...
        for modstate in range(1, self.constants.num_modstates + 1):
            for cs_step in range(self.constants.num_cs_steps):
                ...
                array_list.append(dark_subtracted_array)
    ...
    def compute_background_light(self, input_data: np.ndarray) -> np.ndarray:
        ...
        ----------
        input_data
            An array with shape ((N - D) * M, x, y') (see `reduce_and_collect_polcals`) representing the stack of all
            polcal arrays.
    ...
    def upsample_background_light(self, small_data: np.ndarray, full_shape: tuple) -> np.ndarray:
    ...
    def resample_spatial_dimension(data: np.ndarray, num_spatial_bins: int) -> np.ndarray:
        ...
        num_spec, num_wave, num_spat_pos = data.shape
        ...
        reshaped = data.reshape(
            (num_spec, num_wave, num_spatial_bins, num_spat_pos // num_spatial_bins)
        )
