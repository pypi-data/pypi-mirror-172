...
def calibration_collection_with_geo_shifts() -> Tuple[CalibrationCollection, np.ndarray]:
    shifts = np.random.random((2, 10, 2)) * 100.0 - 50
    geo_shifts = {str(b + 1): {f"m{m + 1}": shifts[b, m, :] for m in range(10)} for b in range(2)}
    return (
...
def test_science_calibration_task(science_calibration_task, mocker):
    ...
    for file in files:
        hdul = fits.open(file)
        assert hdul[0].data.shape == expected_final_shape
        ...
        assert hdul[0].header["VSPNMAPS"] == 2
        assert hdul[0].header["VSPMAP"] == map_scan

        # Check that WCS keys were updated
        if offset[0] > 0:
            assert hdul[0].header["CRPIX2"] == og_header["CRPIX2"] - np.ceil(offset[0])
        if offset[1] > 0:
            assert hdul[0].header["CRPIX1"] == og_header["CRPIX1"] - np.ceil(offset[1])
...
def test_beam_overlap_slice(calibration_collection_with_geo_shifts):
    """
    Given: A CalibrationCollection object with populated state_offsets
    When: Computing the overlapping beam slices
    Then: The correct values are returned
    """
    calibrations, shifts = calibration_collection_with_geo_shifts
    flat_shifts = shifts.reshape((shifts.shape[0] * shifts.shape[1], shifts.shape[2]))
    x_max = flat_shifts[np.argmax(np.abs(flat_shifts[:, 0])), 0]
    y_max = flat_shifts[np.argmax(np.abs(flat_shifts[:, 1])), 1]
    x_slice, y_slice = calibrations.beams_overlap_slice

    assert x_slice.step is None
    assert y_slice.step is None
    if x_max < 0:
        assert x_slice.start is None
        assert x_slice.stop == np.floor(x_max)
    else:
        assert x_slice.start == np.ceil(x_max)
        assert x_slice.stop is None

    if y_max < 0:
        assert y_slice.start is None
        assert y_slice.stop == np.floor(y_max)
    else:
        assert y_slice.start == np.ceil(y_max)
        assert y_slice.stop is None
