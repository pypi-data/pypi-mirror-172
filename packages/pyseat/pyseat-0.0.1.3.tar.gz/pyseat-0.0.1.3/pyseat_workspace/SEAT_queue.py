...
class Node():
    ...
    def setBins(self):
        ...
        if self.leaf:
            vs += self.children
            self.sum_b_log_b = None  # np.sum(d_log_d[self.vs])
        else:
            for c in self.children:
                if isinstance(c, Node):
                    vs += c.setBins()
...
class SETree():
    ...
    def __init__(self, X, aff_m, min_k=2, max_k=10,
                 max_g_ratio=0,
        ...
        self.node_num = aff_m.shape[0]
        if self.max_k > self.node_num:
            self.max_k = self.node_num - 1

        self.ks = range(self.min_k, self.max_k+1)

        if strategy == 'top_down':
            self.node_id = 2*self.node_num - 3
        else:
            self.node_id = -2
        self.node_list = {}
    ...
    def graph_stats_init(self, aff_m):
        ...
        d = None  # np.sum(M, 1) - M.diagonal()
        if np.any(d == 0):
            M += 1e-3
            np.fill_diagonal(M, 0)
            d = None  # np.sum(M, 1) - M.diagonal()
        # print("d=", d)
        log_d = None  # np.log2(d)
        d_log_d = None  # np.multiply(d, log_d)
        # print("sum(d)=", sum(d))
        ...
        if self.objective == 'modularity':
            ...
            if self.max_g_ratio:
                ...
                for child in children:
                    g = max(child.g - self.node_num*(self.node_num-len(child.vs))*self.max_g_ratio, 0)
    ...
    def dividing_tree(self, node, nodes_to_divide, Z):
        ...
        if delta > 0 and self.node_num != len(node.vs):  # not split
            node.leaf = True
        ...
        Z[node.id-(self.node_num)] = [children[0].id, children[1].id, children[0].height, len(node.vs), node.id]
    ...
    def _merge_phase(self, G, root, singletons, leafs, Z, z_i, delta_m, heap, dist,
        ...
        while singletons:
            ...
            max_n1 = singletons.get()
            max_n1 = self.node_list[max_n1]

            max_delta = -10000
            max_n2 = -1
            for x in G.neighbors(max_n1.id):
                i += 1
                node2 = self.node_list[x]
                if self.objective == 'structure_entropy':
                    delta = se.get_delta_se_plus(M, sparse_m, vG, d, root, max_n1, node2)
                    # delta_se = se.get_delta_se(M, sparse_m, vG, root, node, new_node)
                else:
                    delta = se.get_delta_nm(M, sparse_m, vG, root, max_n1, node2)
            ...
            # for x in singletons:
            for x in list(chain(G.neighbors(max_n1.id), G.neighbors(max_n2.id))):
                i += 1
    ...
    def _binary_merge(self, G, root, leafs, Z, z_i, delta_m, only_positive_delta=True, by='heap'):
        ...
        while z_i < self.node_num - 1:
            ...
            xs = list(chain(G.neighbors(max_n1.id), G.neighbors(max_n2.id)))
    ...
    def cut_tree(self, Z, n_clusters):
        ...
        if self.strategy == 'bottom_up':
            ...
            root = self.node_list[2*self.node_num - 2]
        self.root = root
    ...
    def _cut_tree_dp(self, root):
        ...
        for k in self.ks:
            if k == 1:
                ks_clusters.append([0]*self.node_num)
                continue
            clusters = []
            # print('k', k, '----')
            ...
            # print(len(clusters), self.node_num)
            if len(clusters) != self.node_num:  # happens in bottom up node if k larger than number of leafs
                ks_clusters.append([0]*self.node_num)
                continue
        ...
        for n in nodes:
            ...
            for k in self.ks:
                ...
                if self.objective == 'structure_entropy':
                    ...
                    if self.max_g_ratio:
                        g = max(node.g - self.node_num*(self.node_num-len(node.vs))*self.max_g_ratio, 0)
    ...
    def _cut_tree_dp_recursive(self, parent, node, k, vG, cost_m, cutoff_m, nodes):
        ...
        for i in range(1, k):
            ...
            if cost < min_cost:
                min_cost = cost
                min_i = i
        # print('k', k, 'min_cost', min_cost, node, node.children[0], node.children[1], min_i, k)
        cost_m[node.id, k] = min_cost
        cutoff_m[node.id, k] = min_i
        return min_cost
...
class SEAT(AgglomerativeClustering):
    ...
    def fit(self, X, y=None):
        ...
        # build the tree
        se_tree = SETree(X, self.affinity_m, self.min_k, self.max_k,
                         max_g_ratio=self.max_g_ratio,
        ...
        self.node_num = se_tree.node_num
