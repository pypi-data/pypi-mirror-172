...
class StreamDataSource(BaseDataSource):
    ...
    def _get_stream_preview_dataframe(
        ...
        if apply_translator or isinstance(self.ds_spec.stream_source, specs.SparkStreamSourceSpec):
            return data_source_helper.get_ds_dataframe(
                spark, self.ds_spec, consume_streaming_data_source=True, stream_option_overrides=option_overrides
            )
        else:
            return data_source_helper.get_non_dsf_raw_stream_dataframe(
                spark, self.ds_spec.stream_source, option_overrides
            )
...
def get_data_source(
...
) -> Union[BatchDataSource, StreamDataSource, unified_data_source.BatchSource, unified_data_source.StreamSource]:
    """
    ...
    assert ds_proto.fco_metadata.framework_version == FrameworkVersion.FWV5.value

    if conf.get_bool("UNIFIED_TECTON_OBJECTS_ENABLED"):
        if ds_proto.HasField("stream_data_source"):
            return unified_data_source.StreamSource._create_from_data_proto(ds_proto)
        else:
            return unified_data_source.BatchSource._create_from_data_proto(ds_proto)
    else:
        if ds_proto.HasField("stream_data_source"):
