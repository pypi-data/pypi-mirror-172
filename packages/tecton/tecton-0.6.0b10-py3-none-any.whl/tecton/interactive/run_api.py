...
def _build_run_batch_querytree(
    ...
    if fd.is_temporal:
        ...
        return RenameColsNode(qt, drop=[ANCHOR_TIME]).as_ref()
        ...
        if aggregation_level == AGGREGATION_LEVEL_PARTIAL:
            ...
            return RenameColsNode(qt, mapping={ANCHOR_TIME: fd.trailing_time_window_aggregation.time_key}).as_ref()
...
def _legacy_run_batch(
    ...
    for key in mock_inputs.keys():
        ...
        spark_schema = SparkSchemaWrapper.from_proto(ds.batch_source.spark_schema).unwrap()
