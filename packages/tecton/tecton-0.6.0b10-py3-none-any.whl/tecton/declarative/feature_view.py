...
def data_source_to_pipeline_node(source: PushSource):
    return PipelineNode(data_source_node=DataSourceNode(virtual_data_source_id=source._args.virtual_data_source_id))
    ...
    **Do not instantiate this class directly.** Use :class:`tecton.declarative.on_demand_feature_view` instead.
    ...
    def __init__(
        ...
        **Do not directly use this constructor.** Internal constructor for OnDemandFeatureView.
        ...
        pipeline_root = transformation_to_pipeline_node(
            pipeline_function=pipeline_function, params_to_sources=params_to_sources
        )

        args = prepare_common_fv_args(basic_info=basic_info, entities=[], pipeline_root=pipeline_root)
        args.feature_view_type = FeatureViewType.FEATURE_VIEW_TYPE_ON_DEMAND

        transformation_type_checker(basic_info.name, pipeline_root, "pipeline", ["pipeline", "pandas", "python"])
...
class MaterializedFeatureView(FeatureDefinition):
    ...
    **Do not instantiate this class directly.** Use a decorator-based constructor instead:
    ...
    def __init__(
        ...
        sources: Sequence[Union[BaseDataSource, FilteredSource, PushSource]],
        ...
        **Do not directly use this constructor.** Internal constructor for materialized FeatureViews.
        ...
        if data_source_type == DataSourceType.PUSH:
            push_source = sources[0]
            pipeline_root = data_source_to_pipeline_node(push_source)
        else:
        ...
        args = prepare_common_fv_args(basic_info=basic_info, entities=entities, pipeline_root=pipeline_root)
        args.feature_view_type = feature_view_type
        ...
        if aggregations:
            assert (
                aggregation_interval or stream_processing_mode == StreamProcessingMode.CONTINUOUS
            ...
            if stream_processing_mode == StreamProcessingMode.CONTINUOUS:
                args.materialized_feature_view_args.aggregation_interval.FromTimedelta(datetime.timedelta(seconds=0))
            if stream_processing_mode == StreamProcessingMode.TIME_INTERVAL:
                args.materialized_feature_view_args.aggregation_interval.FromTimedelta(aggregation_interval)

            args.materialized_feature_view_args.stream_processing_mode = stream_processing_mode.value
            ...
            assert (
                stream_processing_mode is None
            ), "`stream_processing_mode` can only be specified when using `aggregations`"
    ...
    def _prepare_common_materialization_args(
        ...
        if batch_compute:
            ...
            args.batch_compute.CopyFrom(
                feature_view_pb2.ClusterConfig(
...
def stream_feature_view(
    *,
    ...
    def decorator(user_function):
        ...
        _stream_processing_mode = stream_processing_mode or StreamProcessingMode.TIME_INTERVAL if aggregations else None
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            data_source_type=DataSourceType.PUSH,
            user_function=None,
