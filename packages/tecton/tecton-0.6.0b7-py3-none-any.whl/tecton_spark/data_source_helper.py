...
def get_table_dataframe(
    ...
    if data_source.post_processor:
        translator_fn = function_deserialization.from_proto(data_source.post_processor)
        df = translator_fn(df)
...
def get_non_dsf_raw_stream_dataframe(
    ...
    if isinstance(stream_data_source, specs.KinesisSourceSpec):
        ...
        df = create_kinesis_stream_reader(
            spark,
            stream_data_source.stream_name,
            stream_data_source.region,
        ...
        df = create_kafka_stream_reader(
            spark,
            stream_data_source.bootstrap_servers,
            stream_data_source.topics,
            stream_data_source.options,
            option_overrides,
            ssl_keystore_location=stream_data_source.ssl_keystore_location,
            ssl_keystore_password_secret_id=stream_data_source.ssl_keystore_password_secret_id,
            ssl_truststore_location=stream_data_source.ssl_truststore_location,
            ssl_truststore_password_secret_id=stream_data_source.ssl_truststore_password_secret_id,
            security_protocol=stream_data_source.security_protocol,
        )
    else:
...
def get_stream_dataframe(
    ...
    translator_fn = function_deserialization.from_proto(stream_data_source.post_processor)
...
def get_ds_dataframe(
    ...
    if consume_streaming_data_source:
        ...
        if isinstance(data_source.stream_source, specs.SparkStreamSourceSpec):
            df = _get_data_source_function_stream_dataframe(spark, data_source.stream_source)
        else:
...
def get_data_source_function_batch_dataframe(
    ...
    data_source_func = function_deserialization.from_proto(data_source.function)
...
def _get_data_source_function_stream_dataframe(spark: SparkSession, data_source: specs.SparkStreamSourceSpec):
    data_source_func = function_deserialization.from_proto(data_source.function)
    return data_source_func(spark)
