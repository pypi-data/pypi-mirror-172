...
class OWGradientDescent(OWWidget):
    name = "梯度下降(Gradient Descent)"
    description = "Demonstration of gradient descent " \
                  "in logistic or linear regression"
    keywords = ["gradient descent", "optimization", "gradient", 'tiduxiajiang']
    ...
    class Outputs:
        ...
        coefficients = Output("系数(Coefficients)", Table, replaces=['Coefficients'])
    ...
    def __init__(self):
        ...
        self.step_button = gui.button(
            hbox, self, callback=self.step, label="步进", default=True)
        self.step_back_button = gui.button(
            hbox, self, callback=self.step_back, label="后退")
        self.restart_button = gui.button(
            widget=self.step_box, master=self,
            callback=self.restart, label="重启")
