...
class OWPolynomialRegression(OWBaseLearner):
    ...
    description = "单变量多项式回归."
    keywords = ["polynomial regression", "regression",
                "regression visualization", "polynomial features", "duoxiangshihuigui"]
    icon = "icons/UnivariateRegression.svg"
    priority = 500
    category = "教育(Educational)"

    class Inputs(OWBaseLearner.Inputs):
        learner = Input("学习器(Learner)", Learner, replaces=['Learner'])

    class Outputs(OWBaseLearner.Outputs):
        coefficients = Output("系数(Coefficients)", Table, default=True, replaces=['Coefficients'])
    ...
    learner_name = settings.Setting("多项式回归")
    ...
    def __init__(self):
        ...
        self.scatterplot_item1 = None
        self.scatterplot_item2 = None
        self.plot_item = None
    ...
    def clear_plot(self):
        ...
        if self.scatterplot_item1 is not None:
            self.scatterplot_item1.setParentItem(None)
            self.plotview.removeItem(self.scatterplot_item1)
            self.scatterplot_item1 = None

        if self.scatterplot_item2 is not None:
            self.scatterplot_item2.setParentItem(None)
            self.plotview.removeItem(self.scatterplot_item2)
            self.scatterplot_item2 = None
    ...
    def plot_scatter_points(self, x_data, y_data):
        test_len = max(round(len(x_data)* 0.2), 1)
        train_len = len(x_data) - test_len
        if self.scatterplot_item1:
            self.plotview.removeItem(self.scatterplot_item1)
        if self.scatterplot_item2:
            self.plotview.removeItem(self.scatterplot_item2)
        self.scatterplot_item1 = pg.ScatterPlotItem(
            x=x_data[:train_len], y=y_data[:train_len],
            symbol="o", size=10, pen=pg.mkPen(0.2), brush=pg.mkBrush(0.7),
            antialias=True)
        self.scatterplot_item2 = pg.ScatterPlotItem(
            x=x_data[train_len:], y=y_data[train_len:],
            symbol="+", size=10, pen=pg.mkPen(0.2), brush=pg.mkBrush(0.7),
            antialias=True)
        self.scatterplot_item1.opts["useCache"] = False
        self.scatterplot_item2.opts["useCache"] = False
        self.plotview.addItem(self.scatterplot_item1)
        self.plotview.addItem(self.scatterplot_item2)
        self.plotview.replot()

    def set_range(self, x_data, y_data, x_line, y_line):
        min_x_data, max_x_data = np.nanmin(x_data), np.nanmax(x_data)
        min_y_data, max_y_data = np.nanmin(y_data), np.nanmax(y_data)

        min_x_line, max_x_line = np.nanmin(x_line), np.nanmax(x_line)
        min_y_line, max_y_line = np.nanmin(y_line), np.nanmax(y_line)

        min_x = np.min((min_x_line, min_x_data))
        max_x = np.max((max_x_line, max_x_data))
        min_y = np.min((min_y_line, min_y_data))
        max_y = np.max((max_y_line, max_y_data))
    ...
    def apply(self):
        ...
        if self.data is not None:
            ...
            test_len = max(round(len(data_table) * 0.2), 1)
            predictor = learner(poly_preprocessor(preprocessed_table[:-test_len]))
            ...
            predicted = validation(
                data_table[:-test_len], [learner], preprocessor=PreprocessorList(preprocessors)
            )
