...
class OWPolynomialClassification(OWBaseLearner):
    name = "多项式分类(Polynomial Classification)"
    ...
    keywords = ["polynomial classification", "classification", "class",
                "classification visualization", "polynomial features","duoxiagnshifenlei"]
    ...
    def add_main_layout(self):
        box = gui.widgetBox(self.controlArea, "变量")
        gui.comboBox(
        ...
        gui.checkBox(
            box, self.graph, 'show_legend',"显示图例",
            callback=self.graph.update_legend_visibility)
        gui.checkBox(
            box, self, 'contours_enabled', label="显示等高线",
    ...
    def _contour_pen(value, hovered):
        return pg.mkPen(0.2,
            ...
            style=Qt.SolidLine if hovered or value == 0.5 else Qt.DashDotLine)
            # Alternative:
            # Qt.SolidLine
