...
def find_config_post_training_errors(
    *, cinnaroll_model: model.CinnarollModel, config: rollout_config.RolloutConfig
) -> List[Exception]:
    ...
    try:
        ...
        errors.append(e)
    errors += find_metrics_post_training_errors(config.metrics)
    return errors


# metrics declaration should look like this:
# metrics = {
...
def find_metrics_post_training_errors(
    ...
    if isinstance(metrics, dict):
        ...
        if not all([isinstance(x, (str, int, float)) for x in metrics.values()]):
            errors.append(
                MetricsError(
                    "All values in the metrics dictionary must be strings, integers or floats."
                )
            )

        if any([x is None or x == "" for x in metrics.values()]):
            errors.append(
                MetricsError(
