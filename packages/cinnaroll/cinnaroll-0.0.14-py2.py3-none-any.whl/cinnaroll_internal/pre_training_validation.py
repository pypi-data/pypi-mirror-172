...
def find_project_id_error(project_id: str) -> Optional[Exception]:
    ...
    # }
    # try:
    #     response = requests.get(url=constants.BACKEND_BASE_URL + f"/projects/{project_id}",
    #                             headers=headers)
...
def find_config_none_value_errors(config: rollout_config.RolloutConfig) -> List[Exception]:
    ...
    for name, value in config_parameters.items():
        if value is None:
            errors.append(ConfigParameterUndefinedError(f"Required config parameter {name} is undefined "
                                                        f"(its value is None)."))
...
def find_unknown_model_framework_error(model_object: Any) -> Optional[Exception]:
    if model.infer_framework(model_object) == constants.UNKNOWN_FRAMEWORK:
        return UnknownFrameworkError(f"Unknown machine learning framework used. Currently only "
...
def find_config_pre_training_errors(config: rollout_config.RolloutConfig) -> List[Exception]:
    ...
    errors += io_formats.find_disallowed_io_format_errors(config.infer_func_input_format,
                                                          config.infer_func_output_format)
    utils.append_if_not_none(errors, io_formats.find_infer_func_input_format_mismatch_error(
