...
def test_get_api_key_from_toml_file_content(input_file_content: str, expected_output: str) -> None:
    with mock.patch('builtins.open', mock.mock_open(read_data=input_file_content)):
        actual_output = environment_check.get_api_key_from_toml_file_content(input_file_content)
...
def test_find_disallowed_write_error_when_cwd_is_not_writable(mock_access: MagicMock) -> None:
...
@mock.patch("os.access")
def test_find_disallowed_write_error_when_cwd_is_writable(mock_access: MagicMock) -> None:
