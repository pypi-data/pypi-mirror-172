...
class WrongAPIKeyError(Exception):
    def __init__(self, message: str = "API Key is wrong. Has it been pasted correctly?") -> None:
...
def find_disallowed_write_error() -> Optional[Exception]:
    if not os.access(WORKING_DIRECTORY, os.W_OK):
        return DisallowedWriteError(f"Can’t save files in current working directory ({WORKING_DIRECTORY}). "
...
def find_api_key_error() -> Optional[Exception]:
    try:
        _ = get_api_key()
    except CinnarollAPIKeyMissingError as e:
        return e

    return None  # delete line on uncommenting stuff below
    # headers = {
