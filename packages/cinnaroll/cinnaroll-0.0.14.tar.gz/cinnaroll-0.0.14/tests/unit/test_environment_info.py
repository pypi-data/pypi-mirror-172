...
@pytest.mark.parametrize("framework, requirements, expected_framework_version", test_get_framework_version_cases)
def test_get_framework_version_when_framework_is_known(framework: str, requirements: List[str], expected_framework_version: str) -> None:
...
def test_get_framework_version_when_framework_is_unknown() -> None:
    with pytest.raises(environment_info.UnknownFrameworkError):
        environment_info._get_framework_version("definitely not a known framework", PYTORCH_REQUIREMENTS)
