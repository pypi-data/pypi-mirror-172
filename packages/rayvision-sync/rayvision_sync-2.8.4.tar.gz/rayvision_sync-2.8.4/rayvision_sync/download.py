...
class RayvisionDownload(object):
    ...
    def _run_download(self, task_id_list, local_path, max_speed=None,
        ...
        for output_file_name in output_file_names:
            ...
            if engine_type == "aspera":
            ...
            elif engine_type == "raysyncweb":
                tranfer_code = self.raysync_engine.start_transfer(**{
                    "server_ip": server_ip if server_ip else self.trans.transport_info['raysyncweb']['server_ip'],
                    "server_port": server_port if server_port else self.trans.transport_info['raysyncweb']['server_port'],
                    "local_path": local_path,
                    "server_path": output_file_name,
                    "storage_id": self.trans.output_bid,
                    "task_type" : "download",
                ...
                msg = "{} is not a supported transport engine, " \
                      "currently only support 'aspera' and 'raysyncweb'".format(engine_type)
