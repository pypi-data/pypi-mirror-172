...
from kalabash.lib.tests import KalmonakAPITestCase, KalmonakTestCase
    ...
    def enable_cdav_sync(self):
        ...
        with httmock.HTTMock(mocks.options_mock, mocks.mkcol_mock):
            response = self.client.post(
                url, {"kalabash_contacts-enable_carddav_sync": True,
                      "kalabash_contacts-sync_frequency": 300},
            )
...
class ViewsTestCase(TestDataMixin, KalmonakTestCase):
    ...
    # def test_index(self):
    #     """Test index view."""
    #     url = reverse("kalabash_contacts:index")
    #     with self.assertRaises(IOError):
    #         response = self.client.get(url)
        # self.assertContains(response, '<div id="app">')


class AddressBookViewSetTestCase(TestDataMixin, KalmonakAPITestCase):
...
class CategoryViewSetTestCase(TestDataMixin, KalmonakAPITestCase):
...
class ContactViewSetTestCase(TestDataMixin, KalmonakAPITestCase):
...
class EmailAddressViewSetTestCase(TestDataMixin, KalmonakAPITestCase):
