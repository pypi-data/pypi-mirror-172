...
from .._shared.utils import (_supported_float_type,
                             _to_ndimage_mode, _validate_interpolation_order,
...
def downscale_local_mean(image, factors, cval=0, clip=True):
    ...
    """
    integer_image = image.dtype.kind in 'iu'
    func_kwargs = {}
    if integer_image:
        func_kwargs = dict(dtype=_supported_float_type(image.dtype))
    out = block_reduce(image, factors, cp.mean, cval, func_kwargs=func_kwargs)
    if integer_image:
