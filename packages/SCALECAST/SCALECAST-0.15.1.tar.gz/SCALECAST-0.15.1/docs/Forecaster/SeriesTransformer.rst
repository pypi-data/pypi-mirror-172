...
When using `DiffRevert()`, fitted values and confidence intervals can look very distorted, so there is an option to not revert fitted values for that reason. The `Forecaster.diff()` function is still the easiest way to take first and second differences, but the `DiffTransform()` and `DiffRevert()` functions allow for seasonal differences, as well as for more than second differences. The two methods of taking differences should not be combined. Either use `Forecaster.diff()` or `SeriesTransformer.DiffTransform()`. Do not use both.
