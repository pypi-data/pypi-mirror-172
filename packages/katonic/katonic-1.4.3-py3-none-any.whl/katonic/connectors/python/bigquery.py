...
class BigQueryConnector:
    ...
    def get_data(self) -> None:
        ...
        try:
            ...
            print("Connection to bigquery stablished Successfully.")
            ...
            try:
                fname = f"biquery_{self._file_name or self._query_job.destination.project}_{self._query_job.created.strftime('%Y_%m_%d_%H_%M_%S')}.csv"
                self._dstn_path = f"{self._dst_path}/{fname}"
                self._query_job.to_dataframe().to_csv(self._dstn_path, index=False)
                print(
