...
class SAPHANAConnector:
    ...
    def get_data(self) -> None:
        ...
        """
        fname = f"sap_hana_{self._schema}_{self._file_name or self._table_name}_{strftime('%Y_%m_%d_%H_%M_%S', gmtime())}.csv"
        ...
        print("Connection to SAP HANA stablished Successfully.")

        if not self._custom_query:
            self._custom_query = f"SELECT * FROM {self._schema}.{self._table_name}"

        try:
            # _data = _conn.table(table=self._table_name).collect()
            _data = dataframe.DataFrame(_conn, self._custom_query).collect()
