...
class TestRates:
    ...
    def test_get_rates(self, mock_logger):
        result = get_rates(log=mock_logger, platform='binance-spot', count=20, symbol='btcusdt', timeframe='1m')
        from binanceSpotEasyT.rates import Rates as Binance
        assert isinstance(result, Binance)

        result = get_rates(log=mock_logger, platform='metatrader5', count=20, symbol='eurusd', timeframe=1)
        ...
        with pytest.raises(NoPlatformFound):
            get_rates(log=mock_logger, platform='wrong platform', count=20, symbol='eurusd', timeframe=1)