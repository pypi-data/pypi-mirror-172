...
class TestTrade:
    ...
    def test_get_trade(self, mock_logger, mock_symbol_info, mock_position_get):
        ...
        result = get_trade(log=mock_logger, platform='binance-spot', symbol='btcusdt', lot=0.01, stop_loss=0.00,
                           take_profit=0.00)
        from binanceSpotEasyT.trade import Trade as Binance
        assert isinstance(result, Binance)

        result = get_trade(log=mock_logger, platform='metatrader5', symbol='eurusd', lot=0.01, stop_loss=0.00,
                           take_profit=0.00)
        ...
        with pytest.raises(NoPlatformFound):
            get_trade(log=mock_logger, platform='wrong platform', symbol='eurusd', lot=0.01, stop_loss=0.00,
                      take_profit=0.00)
