...
class TaskExecutor:
    ...
    def _execute(self, variables=None, from_loop=False):
        ...
        projections = list(self._task.register.items()) if self._task.register else []
        register = self._task.default_register
        implicit_register = 'ansible_loop_result' if from_loop else 'ansible_result'
        ...
        for attempt in range(1, retries + 1):
            ...
            vars_copy[implicit_register] = result
            if register:
        ...
        variables[implicit_register] = result
        if register:
