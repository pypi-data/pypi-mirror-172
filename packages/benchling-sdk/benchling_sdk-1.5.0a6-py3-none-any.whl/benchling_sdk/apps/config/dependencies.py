...
from typing import cast, Dict, Generic, List, Optional, Tuple, Type, TypeVar, Union
...
from benchling_sdk.models import Field
...
class ConfiguredFieldDecorator(ABC):
    """
    Configured Field Decorator.

    A decorator containing a reference to a field.
    """

    _field: Field
    _scalar_definition: Optional[ScalarDefinition]

    def __init__(self, field: Field, scalar_definition: Optional[ScalarDefinition]):
        """
        Init ConfiguredFieldDecorator.

        ConfiguredFieldDecorator is abstract. This constructor is expected to be inherited by implementing classes.
        """
        self._field = field
        self._scalar_definition = scalar_definition
    ...
    def field(self) -> Field:
        ...
        Provides access to the underlying Field model.
        """
        return self._field


class RequiredField(ConfiguredFieldDecorator, Generic[ScalarType]):
    """
    Required Field.
    ...
    def value(self) -> ScalarType:
        ...
        """
        field_value = str(self.field.value)
        if self._scalar_definition:
            required_value = self._scalar_definition.from_str(value=field_value)
            assert required_value is not None
            return required_value
        raise MissingScalarDefinitionError(
            f"No definition registered for scalar config of type {self.field.type}"
        )
    ...
    def display_value(self) -> str:
        ...
        Returns the field's display value.
        """
        return cast(str, self._field.display_value)


class OptionalField(ConfiguredFieldDecorator, Generic[ScalarType]):
    """
    Optional Field.
    ...
    def value(self) -> Optional[ScalarType]:
        ...
        if self.field.value:
            field_value = str(self.field.value)
            if self._scalar_definition:
                return self._scalar_definition.from_str(value=field_value)
            raise MissingScalarDefinitionError(
                f"No definition registered for scalar config of type {self.field.type}"
            )
        return None
    ...
    def display_value(self) -> Optional[str]:
        ...
        Returns the field's display value, if present.
        """
        return self._field.display_value


class RequiredMultiValueField(ConfiguredFieldDecorator, Generic[ScalarType]):
    """
    ...
    def value(self) -> List[ScalarType]:
        ...
        """
        if self._scalar_definition:
            assert self.field.value is not None
            # Casts and None checks to appease type checking
            typed_values: List[ScalarType] = [
                cast(ScalarType, self._scalar_definition.from_str(value=str(field_value)))
                for field_value in cast(List[str], self.field.value)
...
class OptionalMultiValueField(ConfiguredFieldDecorator, Generic[ScalarType]):
    """
