...
use super::{
    ...
    visit_circuit, Add, ArrayConstant, HashBytes, IrreducibleNode, Leaf, ModuleNodeSpec,
...
#[pymethods]
impl Schedule {
    pub fn replace_tensors(&self, map: HashMap<usize, Tensor>) -> PyResult<Self> {
        ...
        for (k, v) in map {
            ...
            result
                .constants
                .insert(k, ArrayConstant::new(v, None).into());
        }
        Ok(result)
    }
    ...
    pub fn map_tensors_py(&self, f: PyObject) -> PyResult<Self> {
        ...
        result.constants = result
            ...
            .map(|(key, node)| match node {
                IrreducibleNode::ArrayConstant(ac) => (
                    key,
                    ArrayConstant::new(pycall!(f, (key, ac.value.clone())), ac.name_cloned())
                        .into(),
                ),
                _ => (key, node.clone()),
            })
    ...
    pub fn get_stats(&self) -> ScheduleStats {
        ...
        let mut biggest: HashMap<usize, CircuitRc> = HashMap::new();
        let mut current: HashMap<usize, CircuitRc> = HashMap::new();
...
pub fn order_to_schedule(
    ...
    let mut seen_dependencies: HashSet<usize> = HashSet::new();
...
pub fn circuits_to_dag(circuits: &[CircuitRc]) -> Dag {
    ...
    for circuit in circuits {
        visit_circuit(circuit.clone(), &mut |c: CircuitRc| {
...
pub fn circuit_to_schedule(
    ...
    let (mut out, kept_keys) = order_to_schedule(
        ...
        &to_node
            .iter()
            .filter_map(|x| Option::<IrreducibleNode>::from((***x.1).clone()))
            .collect(),
        &to_node
            .iter()
            ...
            Instruction::Compute(outermost_k, v) => match &***v {
                Circuit::ModuleNode(mn) => {
                    ...
                    if inner_schedule.instructions.is_empty() {
                        ...
                        let mut inner_to_outer_key: HashMap<usize, usize> = HashMap::from([(
