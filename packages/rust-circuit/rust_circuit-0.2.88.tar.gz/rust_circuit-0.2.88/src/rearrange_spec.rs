...
impl RearrangeSpec {
    ...
    #[staticmethod]
    pub fn prepend_batch_shape(new_shape: Shape, old_rank: usize) -> Self {
        RearrangeSpec {
            input_ints: (new_shape.len()..new_shape.len() + old_rank)
                .map(|i| sv![i as u8])
                .collect(),
            output_ints: (0..new_shape.len() + old_rank)
                .map(|i| sv![i as u8])
                .collect(),
            int_sizes: new_shape
                .iter()
    ...
    #[staticmethod]
    pub fn new_permute(permutation: Vec<usize>) -> RearrangeSpec {
        RearrangeSpec::new(
            (0..permutation.len()).map(|i| sv![i as u8]).collect(),
            permutation.iter().map(|i| sv![*i as u8]).collect(),
    ...
    pub fn to_einops_string(&self, size_on_letter: bool) -> String {
        let ints_in_output = self.ints_in_out();
        let ints_needing_sizes = self.ints_needing_sizes();
                ...
                let one_axis_letters = one_axis_ints
                    ...
                    .map(|i| {
                        if ints_in_output.contains(i) || self.int_sizes[*i as usize].is_none() {
                            if size_on_letter
                                && ints_needing_sizes.contains(i)
                                && self.int_sizes[*i as usize].is_some()
                            {
                                format!(
                                    "{}:{}",
                                    ALPHABET[*i as usize].to_owned(),
                                    Option::<usize>::from(self.int_sizes[*i as usize]).unwrap()
                                )
                            } else {
                                ALPHABET[*i as usize].to_owned()
                            }
    ...
    #[staticmethod]
    pub fn from_einops_string(string: &str) -> Option<Self> {
        static RE_TOP_LEVEL: Lazy<Regex> = Lazy::new(|| {
            Regex::new(r"(\d+)|(\w[\w\d]*(?::\d+)?)|\(\s*((?:(\w[\w\d]*(?::\d+)?|\d+)\s*)*)\)")
                .unwrap()
        });

        if let [input, output] = string.split("->").map(str::trim).collect::<Vec<_>>()[..] {
            let mut sizes: OpShape = sv![];
            let mut string_to_int: HashMap<String, u8> = HashMap::new();
            let mut f = |s: &str| -> Option<RInts> {
                ...
                let mut handle_string_nosize = |s: String, sizes: &mut OpShape| {
                    if let Some(old) = string_to_int.get(&s) {
                        *old
                        ...
                        string_to_int.insert(s.to_owned(), (sizes.len() - 1) as u8);
                        (sizes.len() - 1) as u8
                    }
                };
                let mut handle_string = |string: String, sizes: &mut OpShape| -> Option<u8> {
                    if let [string, num] = string.split(':').collect::<Vec<_>>()[..] {
                        ...
                        Some(handle_string_nosize(string, sizes))
                    }
                };
                RE_TOP_LEVEL
                    .captures_iter(s)
                    .map(|tl| {
                        if let Some(c) = tl.get(1) {
                            let num = c.as_str().parse::<usize>().ok()?;
                            Some(sv![handle_int(num, &mut sizes)])
                        } else if let Some(c) = tl.get(2) {
                            Some(sv![handle_string(c.as_str().to_owned(), &mut sizes)?])
                            ...
                            c.as_str()
                                ...
                                .map(|s| {
                                    s.parse::<usize>()
                                        .map(|z| Some(handle_int(z, &mut sizes)))
                                        .unwrap_or_else(|_e| {
                                            handle_string(s.to_owned(), &mut sizes)
                                        })
                                })
                                .collect::<Option<_>>()
                        } else {
            ...
            return Some(RearrangeSpec {
                input_ints: f(input)?,
                output_ints: f(output)?,
                int_sizes: sizes,
            });
        }
        None
    ...
    pub fn add_batch_dims(&self, batch_rank: usize) -> Self {
        let to_prepend =
            || (self.int_sizes.len()..self.int_sizes.len() + batch_rank).map(|x| sv![x as u8]);
        Self {
            input_ints: to_prepend()
                .chain(self.input_ints.iter().cloned())
                .collect(),
            output_ints: to_prepend()
    ...
    pub fn ints_needing_sizes(&self) -> HashSet<EinInt> {
        self.input_ints
            .iter()
            .filter(|x| x.len() > 1)
            .flatten()
    ...
    pub fn to_einops_string_and_letter_sizes(&self) -> (String, Vec<(String, u64)>) {
        (self.to_einops_string(false), self.letter_sizes())
    }
    ...
    pub fn conform_to_input_shape(
        ...
        for (ints, l) in zip(&self.input_ints, shape) {
            ...
            let non_wildcard_size: u64 = ints
                ...
                .product();
            // dbg!(&int_sizes,&non_wildcard_size,&ints,&l);
            if non_wildcard_size != *l as u64 {
                if none_indices.len() > 1 {
                    return Err(TensorInvariantError::RearrangeTooManyWildcardSizes {
                        spec: self.clone(),
                    });
                } else if coerce && ints.len() == 1 {
                    int_sizes[ints[0] as usize] = OpSize(*l as u64);
                } else if *l as u64 % non_wildcard_size != 0 {
                    return Err(TensorInvariantError::RearrangeInputNotConformable {
                        spec: self.clone(),
                        shape: shape.clone(),
                    });
                    ...
                    int_sizes[ints[none_indices[0]] as usize] =
                        OpSize(*l as u64 / non_wildcard_size);
                } else {
                    return Err(TensorInvariantError::RearrangeInputNotConformable {
                        spec: self.clone(),
                        shape: shape.clone(),
