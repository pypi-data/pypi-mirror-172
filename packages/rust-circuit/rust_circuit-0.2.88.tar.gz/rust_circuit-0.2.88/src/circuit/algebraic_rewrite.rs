...
use super::{
    circuit_optimizer::OptimizationContext, computational_node::EinsumArgs, prelude::*,
    py_circuit_items::PY_CIRCUIT_ITEMS, Add, Concat, Einsum, GeneralFunction, HashBytes, Index,
    Rearrange, ScalarConstant, Scatter, Symbol,
};
use crate::{
    filter_by_variant,
...
pub fn add_collapse_scalar_inputs(add: &Add) -> Option<Add> {
    ...
    let (scalar_inputs, non_scalar_inputs): (Vec<ScalarConstant>, Vec<Arc<Circuit>>) =
...
pub fn einsum_concat_to_add(einsum: &Einsum) -> Option<Add> {
    ...
    let mut nodes_by_einsum_dim = HashMap::new();
    ...
    let inner_einsum: Vec<CircuitRc> = concat_node
        ...
        .collect();
    Some(Add::try_new(inner_einsum, None).unwrap())
}
...
pub fn push_down_index(node: &Index) -> Option<CircuitRc> {
    match &**node.node {
        ...
        Circuit::Concat(inner) => {
            ...
            let sections = &inner.get_sections();
            let old_index_on_concat_axis = node.index.0[inner.axis].clone();
            let concat_axis_indices_maybe =
                old_index_on_concat_axis.split_sections_for_concat(sections);
                        ...
                        zip(inner.nodes.iter(), concat_indices)
                            .filter_map(|(child, idx)| {
                                idx.map(|idx| {
                                    ...
                                    Index::nrc(child.clone(), index, node.name_cloned())
                                })
                            })
                            .collect(),
                        new_axis,
...
pub fn add_fuse_scalar_multiples(add: &Add) -> Option<Add> {
    ...
    let scalars_and_children: Vec<(Einsum, f64, Option<String>)> =
        add.children()
            .map(|c| match &**c {
                Circuit::Einsum(e) => {
                    ...
                    .filter(|args| {
                        ...
                        if let Some(x) = child.as_scalar_constant() && x.info().shape.len() == 0  {
                            scalar *= x.value;
                            reduced_any += 1;
                            if x.name().is_some() {
                    ...
                    (
                        Einsum::try_new(no_scalar_children, e.out_axes.clone(), e.name_cloned())
                            .unwrap(),
                        scalar,
                        new_name,
                    )
                }
                Circuit::ScalarConstant(s) => {
    ...
    let mut hm: HashMap<Einsum, (f64, Option<String>)> = HashMap::new();
    ...
    let nodes: Vec<CircuitRc> = hm
        ...
        .map(|(einsum, (scalar, scalar_name))| {
            if scalar == 1.0 {
                ...
                v.push((ScalarConstant::new(scalar, sv![], scalar_name).rc(), sv![]));
                Einsum::try_new(v, out_axes, name).unwrap().rc()
            }
        })
        .collect();

    Some(Add::try_new(nodes, add.name_cloned()).unwrap())
}
