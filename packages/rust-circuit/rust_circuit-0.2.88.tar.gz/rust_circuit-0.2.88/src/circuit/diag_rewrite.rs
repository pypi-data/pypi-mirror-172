...
use crate::{hashmaps::AHashSet as HashSet, smallvec::Sv};
use std::iter::zip;
...
use crate::{
    ...
    unwrap,
    util::{is_unique, EinInt},
};

use super::{
    algebraic_rewrite::add_make_broadcasts_explicit, prelude::*, Concat, Einsum, EinsumAxes, Index,
    Rearrange,
};
...
pub fn diags_intersection(diags: &Vec<EinsumAxes>) -> EinsumAxes {
    ...
    let transpose: Vec<EinsumAxes> = (0..diags[0].len())
        .map(|i| diags.iter().map(|d| d[i]).collect::<Sv<_>>())
        .collect();
