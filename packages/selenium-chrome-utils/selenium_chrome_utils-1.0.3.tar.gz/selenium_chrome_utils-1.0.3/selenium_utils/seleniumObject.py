...
class SeleniumUtils(BrowserUtils):
    ...
    def __debugOpen(self, port=92222, chromePath=None):
        ...
        subprocess.Popen(
            '"' + chromePath + '"' + ' --remote-debugging-port=%s --user-data-dir="C:\selenum\AutomationProfile"' % port,
            shell=True)
        return port
    ...
    #
    def find_by_xpath(self, xpath, count=3, log_print=True):
        while count > 0:
            count -= 1
            try:
                ...
                if log_print:
                    log.error("find_by_xpath() 找不到此obj:%s" % xpath)
        return None

    def finds_by_xpath(self, xpath, count=3, log_print=True):
        while count > 0:
            count -= 1
            try:
                ...
                if log_print:
                    log.error("finds_by_xpath() 找不到此obj:%s" % xpath)
        return []

    def find_by_css(self, css, count=3, log_print=True):
        while count > 0:
            count -= 1
            try:
                ...
                if log_print:
                    log.error("find_by_css() 找不到此obj:%s" % css)
        return None

    def finds_by_css(self, css, count=3, log_print=True):
        while count > 0:
            count -= 1
            try:
                return self.driver.find_elements(By.CLASS_NAME, css)
            except:
                if log_print:
                    log.error("finds_by_css() 找不到此obj:%s" % css)
        return []

    def find_by_id(self, id, count=3, log_print=True):
        while count > 0:
            count -= 1
            try:
                ...
                if log_print:
                    log.error("find_by_id() 找不到此obj:%s" % id)
        return None

    def finds_by_id(self, id, count=3, log_print=True):
        while count > 0:
            count -= 1
            try:
                ...
                if log_print:
                    log.error("finds_by_id() 找不到此obj:%s" % id)
        return []

    def find_by_object(self, obj: By, value, count=3, log_print=True):
        while count > 0:
            count -= 1
            try:
                return self.driver.find_element(obj, value)
            except:
                if log_print:
                    log.error("find_by_object() 找不到此obj:%s" % value)
        return None

    def finds_by_object(self, obj: By, value, count=3):
        while count > 0:
            count -= 1
            try:
                return self.driver.find_elements(obj, value)
            except:
                log.error("finds_by_object() 找不到此obj:%s" % value)
        return []
