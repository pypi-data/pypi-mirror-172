...
Meta Jobs as data jobs which trigger one or more other jobs, wait for their completion, and then trigger another set of jobs until either the entire job pipeline has succeeded.
...
For example the following example dependency graph can be implemented with below code.

When defining a job to be run following attributs are supported:
...
* **fail_meta_job_on_error**: optional, should the meta job fail on error. If set to True if the orchestrated job fails, the meta job will continue.
