...
class RootWindow(tk.Tk, mixin.ContainerWidget):
    ...
    def __init__(self, *args, theme: str = None, imgfolder: typing.Optional[Path] = None, **kwargs):
        ...
        self.after_idle(self.setup_styleID)
        if __debug__:
            import traceback
    ...
    def setup_styleID(self) -> None:
        ...
        self_sids = self.styleIDs or {}
        cfg_sids = set(self_sids.keys())
        assert widget_sids <= cfg_sids, f'Missing configuration for "{" ".join(widget_sids - cfg_sids)}"'
        logger.debug('Set styleID Settings: CFG#%d Widgets#%d', len(self_sids), len(widget_sids))
    ...
    def register_styleID(self, argument: typing.Optional[str], thisClass: str) -> typing.Optional[str]:
        ...
        Args:
            argument: The styleID argument. This is defined by the final GUI
                designer. Optional.
        ...
        if argument is None:
            ...
            assert '.' not in argument, f'Invalid StyleID: "{argument}'
            styleID = f'{argument}.{thisClass}'
            self._styleIDs.add(styleID)
        return styleID
...
class Entry(ttk.Entry, mixin.SingleWidget):
    ...
    Args:
        ...
        justify: Justify entry text. Defaults to `model.Justification.NoJustify`, the upstream default.
...
class FrameRadio(ttk.LabelFrame, mixin.ContainerWidget):
    ...
    def genChanged_rstate(self, rvalue: str) -> typing.Callable[[var.Variable, str], None]:
        def onChanged_rstate(rstate, etype):
            assert etype == 'write'
