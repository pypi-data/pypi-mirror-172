...
class PermissionsTestCase(lib_tests.KalmonakTestCase):
...
class ResourceTestCase(lib_tests.KalmonakTestCase):
    ...
    def _create_account(
            self, username, role="StandardUsers", status=200, **kwargs):
        values = {
...
class ResellerTestCase(ResourceTestCase):
    ...
    def test_change_role(self):
        ...
        # to the reseller.
        values = {
            "username": user.username, "role": "StandardUsers",
            "quota_act": True,
            "is_active": user.is_active, "email": user.email,
            "language": "en",
        }
