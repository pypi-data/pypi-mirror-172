...
class MXTestCase(KalmonakTestCase):
    ...
    def test_management_command(
        ...
        management.call_command("kalmonak", "check_mx", "--no-dnsbl", "--ttl=0")
        self.assertTrue(
        ...
        id_ = qs[0].id
        management.call_command("kalmonak", "check_mx", "--no-dnsbl", "--ttl=7200")
        qs = models.MXRecord.objects.filter(domain=self.domain)
        ...
        # ttl
        management.call_command("kalmonak", "check_mx", "--no-dnsbl")
        qs = models.MXRecord.objects.filter(domain=self.domain)
    ...
    def test_single_domain_update(
        ...
        management.call_command(
            "kalmonak", "check_mx", "--domain", self.domain.name)
        self.assertTrue(
            models.MXRecord.objects.filter(domain=self.domain).exists())
        self.assertFalse(
        ...
        management.call_command(
            "kalmonak", "check_mx", "--domain", str(self.bad_domain.pk))
        self.assertFalse(
            models.MXRecord.objects.filter(domain=self.bad_domain).exists())
        self.assertEqual(len(mail.outbox), 1)

        management.call_command(
            "kalmonak", "check_mx", "--domain", "toto.com")
    ...
    def test_invalid_mx(
        ...
        management.call_command(
            "kalmonak", "check_mx", "--domain", domain.name, "--no-dnsbl")
...
class DNSBLTestCase(KalmonakTestCase):
    """TestCase for DNSBL related features."""
    ...
    def test_management_command(
        ...
        management.call_command("kalmonak", "check_mx")
        self.assertTrue(
    ...
    def test_notifications(
        ...
        mock_g_gethostbyname.return_value = "1.2.3.4"
        management.call_command(
            "kalmonak", "check_mx", "--email", "user@example.test")
...
class DNSChecksTestCase(KalmonakTestCase):
