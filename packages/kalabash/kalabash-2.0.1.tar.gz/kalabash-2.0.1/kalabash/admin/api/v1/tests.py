...
from kalabash.lib.tests import KalmonakAPITestCase


class DomainAPITestCase(KalmonakAPITestCase):
    """Check API."""
    ...
    def test_get_domains(self):
        ...
        url = reverse(
            "v1:domain-detail", args=[domain["pk"]])
        response = self.client.get(url)
        data = response.json()
        self.assertEqual(response.status_code, 200)
...
class DomainAliasAPITestCase(KalmonakAPITestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual(len(response.data), 2)
        url = reverse(
            "v1:domain_alias-detail", args=[response.data[0]["pk"]])
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
...
class AccountAPITestCase(KalmonakAPITestCase):
    """Check Account API."""
    ...
    def test_create_domainadmin_account(self):
        ...
        response = self.client.get(
            reverse("v1:account-detail", args=[admin.pk]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_change_password(self):
        ...
        account = core_models.User.objects.get(username="user@test.com")
        url = reverse(
            "v1:account-password", args=[account.pk])
        response = self.client.put(
...
class AliasAPITestCase(KalmonakAPITestCase):
    """Check Alias API."""
...
class SenderAddressAPITestCase(KalmonakAPITestCase):
