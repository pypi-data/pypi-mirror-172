...
class AuthenticationTestCase(KalmonakTestCase):
...
class ManagementCommandsTestCase(TestCase):
    ...
    def test_change_default_admin(self):
        ...
        management.call_command(
            "load_initial_data", "--admin-username", "kalmonakadmin")
        self.assertTrue(
            self.client.login(username="kalmonakadmin", password="password"))
    ...
    def test_clean_inactive_accounts(self):
        ...
        account = factories.UserFactory(
            username="user1@domain.test", groups=("StandardUsers", ),
...
class ProfileTestCase(KalmonakTestCase):
    ...
    def test_update_password_url(self):
        ...
        non_local_user = factories.UserFactory(
            username="user@external.com", groups=("StandardUsers",),
            is_local=False
        )
...
class APIAccessFormTestCase(KalmonakTestCase):
...
class APICommunicationTestCase(KalmonakTestCase):
    """Check communication with the API."""

    def test_management_command(self):
        """Check command."""
        with httmock.HTTMock(
                mocks.kalmonak_api_instance_search,
                mocks.kalmonak_api_instance_create,
                mocks.kalmonak_api_instance_update,
                mocks.kalmonak_api_versions):
        ...
        with httmock.HTTMock(
                ...
                mocks.kalmonak_api_instance_create,
                mocks.kalmonak_api_instance_update,
                mocks.kalmonak_api_versions_no_update):
            management.call_command("communicate_with_public_api")
        self.assertEqual(len(mail.outbox), 1)
