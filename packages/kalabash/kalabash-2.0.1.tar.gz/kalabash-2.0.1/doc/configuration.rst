...
Kalabash provides online panels to modify internal parameters. There
are two available levels:

* Application level: global parameters, define how the application
  behaves. Available at *Kalabash > Parameters*
...
|                    |                     |                               |                    |
+--------------------+---------------------+-------------------------------+--------------------+
|Send statistics     |General              |Send statistics to Kalabash     |yes                 |
...
.. warning::
    ...
    If Dovecot is not running on the same host than Kalabash, you will have
...
Kalabash uses a specific directory to upload files (ie. when the
webmail is in use) or to create ones (ex: graphical statistics). This
directory is named ``media`` and is located inside kalabash's
installation directory (called ``kalabash_site`` in this
documentation).

To work properly, the system user which runs kalabash (``www-data``,
...
``KALABASH_CUSTOM_LOGO`` variable. This variable must contain the
...
.. note::
  ...
  This section is only relevant when Kalabash handles mailboxes
...
runs Kalabash to execute commands as the user who owns mailboxes.

To do so, edit the :file:`/etc/sudoers` file and add the following inside::

  <user_that_runs_kalabash> ALL=(<mailboxes owner>) NOPASSWD: ALL
...
Kalabash is available in many languages.
...
Kalabash uses `Django's session framework
...
file to make Kalabash behave as expected::
...
The default configuration file provided by the ``kalabash-admin.py``
...
To trace login attempts to the web interface, Kalabash uses python
...
Kalabash supports external LDAP authentication using the following extra components:
...
like this::
    ...
    AUTHENTICATION_BACKENDS = (
      ...
      # 'kalabash.lib.authbackends.LDAPSecondaryBackend',  # Useful for a fallback mechanism
      'django.contrib.auth.backends.ModelBackend',
    )

Finally, go to *Kalabash > Parameters > General* and set *Authentication
type* to LDAP.

From there, new parameters will appear to let you configure the way
Kalabash should connect to your LDAP server. They are described just below:
...
your LDAP directory will be able to connect to Kalabash, the associated
domain and mailboxes will be automatically created if needed.

The first time a user connects to Kalabash, a local account is created
if the LDAP username is a valid email address. By default, this
account belongs to the *StandardUsers* group and it has a mailbox.
...
.. note::
   ...
   Kalabash doesn't provide any synchronization mechanism once a user
   is registered into the database. Any modification done from the
   directory to a user account will not be reported to Kalabash (an
   email address change for example). Currently, the only solution is
   to manually delete the Kalabash record, it will be recreated on the
   next user login.
...
Kalabash can synchronize accounts with an LDAP directory (tested with
OpenLDAP) but this feature is not enabled by default. To activate it,
add ``kalabash.ldapsync`` to ``KALABASH_APPS`` in the
...
Kalabash logs administrator specific actions into the database. A
...
To use it, you can setup a cron job to run every night::
  ...
  0 0 * * * <kalabash_site>/manage.py cleanlogs
  #
  # Or like this if you use a virtual environment:
  # 0 0 * * * <virtualenv path/bin/python> <kalabash_site>/manage.py cleanlogs
...
For example, you could setup a cron job to run this script every night::
  ...
  #
  # Or like this if you use a virtual environment:
  # 0 0 * * * <virtualenv path/bin/python> <kalabash_site>/manage.py clearsessions
...
night::
  ...
  0 0 * * * <kalabash_site>/manage.py clean_inactive_accounts
  #
  # Or like this if you use a virtual environment:
  # 0 0 * * * <virtualenv path/bin/python> <kalabash_site>/manage.py clean_inactive_accounts
