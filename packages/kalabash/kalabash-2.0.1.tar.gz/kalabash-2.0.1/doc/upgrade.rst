...
procedure is almost the same than the one used for Kalabash.
...
The `migration will probably fail <https://github.com/amonak/kalabash/issues/2508>`_ because of the
...
.. sourcecode:: bash
   ...
   postgres=# \c kalabash
   You are now connected to database "kalabash" as user "postgres".
   kalabash-# \d+ dkim
                                         View "public.dkim"
...
.. sourcecode:: bash
   ...
   kalabash=# CREATE OR REPLACE VIEW dkim AS
   kalabash-#  SELECT admin_domain.id,
   kalabash-#     admin_domain.name AS domain_name,
   kalabash-#     admin_domain.dkim_private_key_path AS private_key_path,
   kalabash-#     admin_domain.dkim_key_selector AS selector
   kalabash-#    FROM admin_domain
   kalabash-#   WHERE admin_domain.enable_dkim;
   CREATE VIEW
   kalabash=# grant select on dkim to opendkim;
   GRANT
...
``/srv/kalabash/instance/frontend``:

.. sourcecode:: bash

   mkdir /srv/kalabash/instance/frontend
   cp -r /srv/kalabash/env/lib/pythonX.X/site-packages/kalabash/frontend_dist/* /srv/kalabash/instance/frontend

Then, edit the :file:`/srv/kalabash/instance/frontend/config.json`
...
.. sourcecode:: nginx
   ...
   location ^~ /new-admin {
       alias  /srv/kalabash/instance/frontend/;
       index  index.html;
...
Kalabash now supports Two-Factor authentication using TOTP
applications.
...
Add ``'kalabash.policyd'`` to ``KALABASH_APPS``:
...
The ``kalabash-stats`` plugin has been merged into the core.

Add ``'kalabash.maillog'`` to ``KALABASH_APPS``:
...
And remove any reference to ``kalabash_stats`` in this same variable.
...
Add ``'kalabash.dnstools'`` to ``KALABASH_APPS``:
...
kalabash-postfix-autoreply 1.5.0
===============================
...
.. warning::
   ...
   .. sourcecode:: python
      ...
      from kalabash_amavis import settings as kalabash_amavis_settings
      kalabash_amavis_settings.apply(globals())

+------------------------------+------------------------------+
|Name                          |Version                       |
+==============================+==============================+
|kalabash-amavis                |1.2.0                         |
+------------------------------+------------------------------+
|kalabash-contacts              |0.5.0                         |
+------------------------------+------------------------------+
|kalabash-dmarc                 |1.1.0                         |
+------------------------------+------------------------------+
|kalabash-imap-migration        |1.2.0                         |
+------------------------------+------------------------------+
|kalabash-pdfcredentials        |1.3.0                         |
+------------------------------+------------------------------+
|kalabash-postfix-autoreply     |1.4.0                         |
+------------------------------+------------------------------+
|kalabash-radicale              |1.2.0                         |
+------------------------------+------------------------------+
|kalabash-sievefilters          |1.4.0                         |
+------------------------------+------------------------------+
|kalabash-stats                 |1.4.0                         |
+------------------------------+------------------------------+
|kalabash-webmail               |1.4.0                         |
+------------------------------+------------------------------+

Edit the :file:`settings.py` file and apply the following modifications.

Add ``'kalabash.transport'`` to ``KALABASH_APPS``:
...
.. sourcecode:: bash
   ...
   > rm -f <path>/kalabash-postfix-maps.chk
...
Add the following cron job in order to generate DKIM keys::
  ...
  *       *       *       *       *       root    $PYTHON $INSTANCE/manage.py kalmonak manage_dkim_keys
...
Kalabash now relies on `Django's builtin password validation system
...
.. sourcecode:: python
   ...
   urlpatterns = [
       url(r'', include('kalabash.urls')),
   ]

Edit the :file:`settings.py` and apply the following changes:

* Add ``'kalabash.parameters'`` to ``KALABASH_APPS``:
...
* Add ``'kalabash.core.middleware.LocalConfigMiddleware'`` to ``MIDDLEWARE_CLASSES``:
...
* Kalabash used to provide a custom authentication backend
  (``kalabash.lib.authbackends.SimpleBackend``) but it has been
...
|Name                          |Version                       |
+==============================+==============================+
|kalabash-amavis                |1.1.0                         |
+------------------------------+------------------------------+
|kalabash-dmarc                 |1.0.0                         |
+------------------------------+------------------------------+
|kalabash-imap-migration        |1.1.0                         |
+------------------------------+------------------------------+
|kalabash-pdfcredentials        |1.1.0                         |
+------------------------------+------------------------------+
|kalabash-postfix-autoreply     |1.2.0                         |
+------------------------------+------------------------------+
|kalabash-radicale              |1.1.0                         |
+------------------------------+------------------------------+
|kalabash-sievefilters          |1.1.0                         |
+------------------------------+------------------------------+
|kalabash-stats                 |1.1.0                         |
+------------------------------+------------------------------+
|kalabash-webmail               |1.1.0                         |
+------------------------------+------------------------------+
...
.. sourcecode:: bash
   ...
   $ pip install kalabash-amavis==1.1.0
   $ pip install kalabash-dmarc==1.0.0
   $ pip install kalabash-imap-migration==1.1.0
   $ pip install kalabash-pdfcredentials==1.1.0
   $ pip install kalabash-postfix-autoreply==1.2.0
   $ pip install kalabash-radicale==1.1.0
   $ pip install kalabash-sievefilters==1.1.0
   $ pip install kalabash-stats==1.1.0
   $ pip install kalabash-webmail==1.1.0
...
available updates directly in your Kalabash console.
...
|Name                          |Version                       |
+==============================+==============================+
|kalabash-amavis                |1.0.10                        |
+------------------------------+------------------------------+
|kalabash-postfix-autoreply     |1.1.7                         |
+------------------------------+------------------------------+
|kalabash-radicale              |1.0.5                         |
+------------------------------+------------------------------+
|kalabash-stats                 |1.0.9                         |
+------------------------------+------------------------------+
...
.. sourcecode:: bash
  ...
  $ pip install kalabash-amavis==1.0.10
  $ pip install kalabash-postfix-autoreply==1.1.7
  $ pip install kalabash-radicale==1.0.5
  $ pip install kalabash-stats==1.0.9
...
.. warning::
   ...
   In particular: `kalabash-amavisd <https://github.com/amonak/kalabash-amavis/commit/35df4e48b124e56df930cda8c013af0c1fcaabf3>`_, `kalabash-stats <https://github.com/amonak/kalabash-stats/commit/aa4a39ce65eb306ad6dec30a54eb58945b120274>`_, `kalabash-postfix-autoreply <https://github.com/amonak/kalabash-postfix-autoreply/commit/20f98c8d1c0c0dbd420f47aefcbb0290022414a4>`_ are concerned.

An interesting feature brougth by this version is the capability to
make different checks about MX records. For example, Kalabash can
...
following line to your crontab::
  ...
  */30 * * * * <optional_virtualenv_path/>python <kalabash_instance_dir>/manage.py kalmonak check_mx

The communication with Kalabash public API has been reworked. Instead
of sending direct synchronous queries (for example to check new
versions), a cron job has been added. To activate it, add the
following line to your crontab::

  0 * * * * <optional_virtualenv_path/>python <kalabash_instance_dir>/manage.py communicate_with_public_api
...
the ``REST_FRAMEWORK`` variable::
  ...
  SWAGGER_SETTINGS = {
      ...
      "exclude_namespaces": [],
      "info": {
          "contact": "contact@kalabash.com",
          "description": ("Kalabash API, requires a valid token."),
          "title": "Kalabash API",
      }
  }
...
.. warning::
   ...
   Please make sure to use Kalabash 1.3.5 with an up-to-date database
...
reason for that is that using Kalabash without them doesn't make sense.
...
|Old name                  |New name            |
+==========================+====================+
|kalabash_admin             |kalabash.admin       |
+--------------------------+--------------------+
|kalabash_admin_limits      |kalabash.limits      |
+--------------------------+--------------------+
|kalabash_admin_relaydomains|kalabash.relaydomains|
+--------------------------+--------------------+
...
#. Uninstall old extensions::
   ...
   $ pip uninstall kalabash-admin kalabash-admin-limits kalabash-admin-relaydomains

#. Install all extension updates using pip (check the *Kalabash > Information* page)
...
To enhance security, Kalabash now checks the `strength of user
...
Kalabash now uses the *atomic requests* mode to preserve database
...
bridge to the future of Kalabash. All extensions now have their own git
...
+========================================+==========================+==========================+
|kalabash.extensions.admin                |kalabash-admin             |kalabash_admin             |
+----------------------------------------+--------------------------+--------------------------+
|kalabash.extensions.limits               |kalabash-admin-limits      |kalabash_admin_limits      |
+----------------------------------------+--------------------------+--------------------------+
|kalabash.extensions.postfix_autoreply    |kalabash-postfix-autoreply |kalabash_postfix_autoreply |
+----------------------------------------+--------------------------+--------------------------+
|kalabash.extensions.postfix_relay_domains|kalabash-admin-relaydomains|kalabash_admin_relaydomains|
+----------------------------------------+--------------------------+--------------------------+
|kalabash.extensions.radicale             |kalabash-radicale          |kalabash_radicale          |
+----------------------------------------+--------------------------+--------------------------+
|kalabash.extensions.sievefilters         |kalabash-sievefilters      |kalabash_sievefilters      |
+----------------------------------------+--------------------------+--------------------------+
|kalabash.extensions.stats                |kalabash-stats             |kalabash_stats             |
+----------------------------------------+--------------------------+--------------------------+
|kalabash.extensions.webmail              |kalabash-webmail           |kalabash_webmail           |
+----------------------------------------+--------------------------+--------------------------+
...
#. Rename old extension names inside ``KALABASH_APPS`` (look at the third column in the table above)

#. Remove ``kalabash.lib.middleware.ExtControlMiddleware`` from ``MIDDLEWARE_CLASSES``

#. Change ``DATABASE_ROUTERS`` to::

    DATABASE_ROUTERS = ["kalabash_amavis.dbrouter.AmavisRouter"]
...
#. The cleanup job has been renamed in Django, so you have to modify your crontab entry::
     ...
     - 0 0 * * * <kalabash_site>/manage.py cleanup
     + 0 0 * * * <kalabash_site>/manage.py clearsessions

1.2.0
=====

A new notification service let administrators know about new Kalabash
...
and to define the new ``KALABASH_API_URL`` variable::

  KALABASH_API_URL = 'http://api.alphamonak.com/1/'
...
new path to the ``STATICFILES_DIRS``::
  ...
  STATICFILES_DIRS = (
    ...
    "<path/to/kalabash/install/dir>/bower_components",
  )
...
#. Add ``'kalabash.extensions.radicale'`` to the ``KALABASH_APPS`` variable
...
.. warning::
    ...
    ``<path to your site's dir>`` to ``<kalabash's root url>`` (it's probably
