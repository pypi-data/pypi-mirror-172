...
title_todo = _('{number} TODO flags')
...
class LogModel(Model):
	...
	status.arguments = ''
	status.description = _('Show untracked, unstaged and staged changes if existing')
	status.settings = [
	...
	stashes.arguments = ''
	stashes.description = _('Show stashed changes if existing')
	stashes.settings = [
	...
	def todo(self, args):
		...
		root = get_git_root()
		self.cmd = self.replace_in_cmd(DetailsModel.cmd_grep_todo, settings.WC_GIT_ROOT, root)
		try:
			lines = super().get_lines()
		except CommandError as e:
			if e.returncode == 1:
			...
			ln = formatter.format(title_todo, number=len(lines))
			ln = self.format_log_entry(settings.COLOR_LOG_TODO, ln)
			return [(ln, ID_TODO, TYPE_TODO)]

	todo.arguments = ''
	todo.description = _('Show TODO flags')
	todo.settings = [
...
class DetailsModel(Model):
	...
	cmd_todo = ["$todo"]
	cmd_grep_todo = ["git", "grep", "--no-index", "--exclude-standard", "-n", "-I", "-E", '(#|%|"|//)TODO\>|\<TODO[:]', '--', settings.WC_GIT_ROOT]
	cmd_pattern_stashed = [
	...
	def todo(self, args):
		...
		try:
			...
			if e.returncode == 1:
				# no todo flags found
				return [[(settings.COLOR_TITLE, title_todo.format(number=0))]]
			return self.format_command_error(e, setting='details-view.cmd.todo.grep')
		else:
			out = []
			n = 0
			for ln in lines:
				...
				try:
					...
					markup = formatter.format(self.pattern_todo, filename=filename, linenumber=linenumber, todoflag=todoflag)
					markup = utils.colored_str_to_markup(markup, self.app.define_color)
					out.append((markup, TYPE_TODO, FileInfo(absfilename), LineInfo(linenumber)))
					n += 1
			out.insert(0, [(settings.COLOR_TITLE, title_todo.format(number=n))])
			out.insert(1, '')
			return out

	todo.arguments = ''
	todo.description = _('Show a list of the TODO flags in the code')
	todo.settings = [
		'details-view.cmd.todo.grep',
