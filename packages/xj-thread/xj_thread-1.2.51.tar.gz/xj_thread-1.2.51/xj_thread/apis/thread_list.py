...
class ThreadListAPIView(APIView):
    ...
    def get(self, request, category_value=None, user_info=None, *args, **kwargs):
        ...
        filter_fields = request.GET.get('filter_fields', None)
        ...
        # 获取列表数据
        thread_serv, error_text = ThreadListService.list(params, page=page, size=size, filter_fields=filter_fields, )
        if error_text:
            return util_response(err=1003, msg=error_text)
        # print("> thread_serv:", thread_serv)
        # 按权限自动过滤数据
        thread_list = PermissionService.auto_filter_by_permission(
        ...
        thread_serv['size'] = len(thread_list)
        thread_serv['list'] = thread_list
