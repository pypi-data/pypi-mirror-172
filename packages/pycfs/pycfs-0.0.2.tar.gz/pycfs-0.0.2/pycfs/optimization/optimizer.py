...
class SimOptimizer(Problem):
    ...
    def __init__(self):
        ...
        # ugly and dirty but makes the whole setup work 
        # very nicely.
        from sim_setup import get_setup, objective

        self.setup = get_setup()

        # Define objective function as only function of x 
        self.objective_fun = lambda x: objective(self.setup["simulation"], x)

        super().__init__(n_var=self.setup["n_vars"], n_obj=self.setup["n_obj"], 
                         n_constr=0, xl=self.setup["ranges"][:,0], 
    ...
    def dump_result(self):
        ...
        data = {"F": F, "X": X, "n_evals": n_evals, 
                "hist_F": hist_F, "hist_cv": hist_cv, 
                "hist_cv_avg": hist_cv_avg, "config": self.config}
