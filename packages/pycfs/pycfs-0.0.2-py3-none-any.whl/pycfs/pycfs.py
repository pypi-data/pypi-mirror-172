...
class PyCfs:
    def __init__(
        ...
        OpenCFS and Trelis/CoreformCubit python interfacing package. The main goal 
        of this module is to make an easy to use interface which provides a class 
        that handles the automatic simulation setup from given CFS and Trelis 
        parameters and the result storage.

        Args:
            project_name (str): Name of the simulation project (needs to be the same 
                                as the .xml file name)

            cfs_install_dir (str): Install path of CFS.

            init_params (np.ndarray): Array containing the initial set of parameters. 
                                      First trelis parameters and then cfs (xml) ones.
    ...
    def _init_functions(self) -> None:
        ...
        """        
        self._forward = self._forward_parallel if self.parallelize else self._forward_serial
        self.get_results = self._get_results_txt if self.result_type == "txt" else self._get_results_hdf
        self._clean_sim_files_if_on = self._clean_sim_files if self.clean_finish else self.dummy_fun
        self._save_hdf_results_if_on = self._save_hdf_results if self.save_hdf_results else self.dummy_fun
        self._allocate_result_arrays = self._allocate_result_arrays_txt if self.result_type == "txt" else self.dummy_fun
        self._set_results = self._set_results_txt if self.result_type == "txt" else self._set_results_hdf
        self._init_results = self._init_results_hdf if self.result_type == "hdf5" else self._allocate_result_arrays
        self.print_init_info = self._print_hdf_info if self.result_type == "hdf5" else self._print_txt_info
        self.res_manip_fun = self._default_external_result_manip if self.res_manip_fun is None else self.res_manip_fun
    ...
    def _init_file_names(self) -> None:
        ...
        self.sim_files = [self.cfs_file, self.jou_file, self.mat_file, f"{self.sim_file}.info.xml", f"{self.project_name}.cdb"]
    ...
    def _init_param_setup(self) -> None:
        ...
        if self.additional_param_fun != None:
    ...
    def _set_hdf_names(self) -> None:
        ...
        """        
        self.tracked_results, self.result_regions = h5t.get_result_names_list(self.hdf_file_path)
    ...
    def _run_pipeline(self, ind: int=None) -> None:
        ...
        if self.parallelize or np.any(self.params_changed):
            cfs_comm = self._make_cfs_command(ind) if self.parallelize else self.cfs_comm
            self._run(cfs_comm)
    ...
    def _make_cfs_command(self, ind: int=None) -> CommandStr:
        ...
        return self.cfs_install_dir + "cfs" + cfs_options + "-p" + subsim_name + ".xml" + subsim_name
    ...
    def _update_params(self, params: np.ndarray) -> None:
        ...
        if ~np.all(self.cfs_params == params[0:1, 0 : self.n_cfs_params]):
    ...
    def _update_cfs_xml(self, ind:int=None) -> None:
        ...
        """
        self._update_file(
            self.cfs_file_init, self.cfs_file, self.cfs_params, self.cfs_params_names, ind
        )
    ...
    def _update_mat_xml(self, ind:int=None) -> None:
        ...
        """
        self._update_file(
            self.mat_file_init, self.mat_file, self.mat_params, self.mat_params_names, ind
        )
    ...
    def read_cfs_results(self):
        ...
        for key in list(regions.keys()):
            ...
            elems_nodes[key] = elems_nodes[key].reshape(elems_reg[key].shape[0], n_elems, 3)
    ...
    def get_region_names(self, region_res: dict, res_type: str, regions: List[str]) -> List[str]:
        regions_out = []
    ...
    def plot_scalar(self, res_type, regions, zero_dim=2):
        ...
        for region in regions:
            ...
            values = np.concatenate((values, np.sqrt(np.sum(region_res[res_type][region]**2, axis=1))))
    ...
    def plot_quiver(self, res_type, regions, Nth: int = 1):
        ...
        for region in regions:
            ...
            E = np.concatenate((E, np.sqrt(np.sum(region_res[res_type][region]**2, axis=1)[pruner])))
        ...
        q = ax.quiver(coord[:,0], coord[:,1], coord[:,2], Ev[:,0], Ev[:,1], Ev[:,2], cmap='jet', length=0.001, normalize=True)
        q.set_array(E)
