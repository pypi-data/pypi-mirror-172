...
def get_result_names_list(hdf5_file: str):
  ...
  for mstep in msteps:
    subres_list = list(hdf5_file["Results"]["Mesh"][mstep]["ResultDescription"].keys())

    for result in subres_list:
      bytes_list = list(hdf5_file["Results"]["Mesh"][mstep]["ResultDescription"][result]["EntityNames"])
  ...
  for multistep in msteps:
    ...
    for ind, name in enumerate(ms):
      ...
      for region in regions[region_ind]:
        regions_res[region] = get_result(hdf5_file,name,region,step,multistep)
