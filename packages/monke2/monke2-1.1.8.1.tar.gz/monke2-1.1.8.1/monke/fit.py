...
class linear_fit():
   ...
   # Style des Diagramms
    ...
    def plot(self,title='title',xlabel='x_Achse',ylabel='y_achse',err=True,legend=True, infos = False, save = False,name='name.png'):
        ...
        ax.plot(x_axis, self.__f(x_axis),color=self.colors[1],label=self.legend_label[0])
        ...
        if(err==True):
            ax.plot(x_axis, self.__f(x_axis)+self._s(x_axis),'--',color=self.colors[2],zorder=1,alpha=0.8,lw=1.3)
            ax.plot(x_axis, self.__f(x_axis)-self._s(x_axis),'--',color=self.colors[2],zorder=1,alpha=0.8,lw=1.3)
        ...
        try:
            if self.custom_multi_array !=0:
                for i in range(np.shape(self.custom_plot_x)[1]):
                    ax.plot(self.custom_plot_x[:,i],self.custom_plot_y[:,i],label=self.custom_label[i],color = self.custom_colors[i])
            else:
                ax.plot(self.custom_plot_x,self.custom_plot_y,label=self.custom_label,color = self.custom_colors)
        finally:
