...
!==============================================================================
  ...
  type :: optionsType
    ...
    character(len=3)     :: algorithm     = 'KRR'
      ...
      character(len=11)  :: kernel        = 'Gaussian'
    ...
    ! Gradients
!    integer              :: gradForAtom   = 0                         ! Atom for which gradient should be calculated
!    integer              :: gradForCoord  = 1                         ! Coordinate of atom for which gradient should be calculated
    ...
    logical              :: calcGradXYZCompnts = .false.              ! Calculate gradient components in XYZ using ML models for each component
...
subroutine writeOptions(unit)
  ...
  if (trim(option%MLmodelFileIn) /= '') write(unit,'(a)') '      MLmodelFileIn:       ' // trim(option%MLmodelFileIn)
  ...
  if (trim(option%MLmodelFileOut) /= '')write(unit,'(a)') '      MLmodelFileOut:      ' // trim(option%MLmodelFileOut)
...
!  if (option%gradForAtom > 0) then
 !   write(unit,'(a,i0,a,i0)')           '    Calculate gradient component for atom ', option%gradForAtom, ' and coordinate ', option%gradForCoord
!  end if
...
!  write(unit,'(a)') "   gradForAtom=I         calculate gradient for atom I"
!  write(unit,'(a)') "     0 (no gradient will be calculated) [default]"
!  write(unit,'(a)') "     <user-defined>"
!  write(unit,'(a)') "   gradForCoord=I        calculate gradient for coordinate I of above atom"
!  write(unit,'(a)') "     1 (X-coordinate) [default]"
!  write(unit,'(a)') "     2 (Y-coordinate)"
!  write(unit,'(a)') "     3 (Z-coordinate)"
  write(unit,'(a)') ""
  ...
  write(unit,'(a)') "                          only works for molDescrType=RE molDescrType=sorted"    
...
subroutine getOptions()
  ...
  character(len=256) :: userOption      ! User-defined options as provided and with all upper-case letters
...
subroutine parseOption(userOption, errorMessage)
  ...
  ! Gradient components
!  elseif (strUpper(userOption(1:12))=='GRADFORATOM=') then
!    read(userOption(13:),*) option%gradForAtom
!  elseif (strUpper(userOption(1:13))=='GRADFORCOORD=') then
!    read(userOption(14:),*) option%gradForCoord
...
subroutine checkOptions(errorMessage)
  ...
  if (trim(option%sampling) == 'user-defined') then
    ...
    if (option%CVopt .and. (trim(option%iTrainIn)/='')) then
...
!      (option%d      == 'OPT')) then
  ...
  if ( .not. ( &
       ...
       (option%minimizeError(1:4) == 'RMSE') ) ) then
  ...
  if (option%permInvKernel) then
    if (.not. (trim(option%Nperm) /= '' .and. option%readX) &
	   .and. .not. ((trim(option%permInvGroups) /= '' .or. trim(option%permInvNuclei) /= '') &
  ...
  .not. (trim(option%permInvGroups) /= '' .or. trim(option%permInvNuclei) /= ''))  then
