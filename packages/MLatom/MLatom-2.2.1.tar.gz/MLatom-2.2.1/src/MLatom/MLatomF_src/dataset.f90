...
!==============================================================================
  ...
  elseif (trim(option%sampling) == 'user-defined') then
    if (option%createMLmodel .and. option%CVopt) then
      do i = 1, Ntrain
        indicesTrain(i) = i
      end do
    else
...
!==============================================================================
  ...
  if (Nsubtrain + Nvalidate > Ntrain) &
    call stopMLatomF('Number of entries in the sub-training and validation sets is larger than the training set')
  if (Nsubtrain + Nvalidate /= Ntrain) then
    write(6,'(a,I0,a,I0)') ' <!> Use only ', Nsubtrain + Nvalidate, ' data entries out of ', Ntrain
    write(6,'(a)') ''
  end if
