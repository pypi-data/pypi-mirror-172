...
module A_KRR
  use A_KRR_kernel,  only : NtrTot, NtrVal, itrval
  use dataset,       only : arrayOfArrays, Yprior, Nprops, NgradXYZmax!, Yest, NitemsTot
...
subroutine init_KRR()
!==============================================================================
  use A_KRR_kernel,  only : getSigma, getPeriod, getSigmaP, getC, getD, getNN
  use optionsModule, only : option
...
!==============================================================================
  ...
  use dataset,       only : Yest, YgradXYZest
  ...
  elseif (option%lambda == 'OPT') then
    ...
    elseif (.not. option%CVopt) then
      if ((trim(option%kernel) == 'Gaussian')    .or. &
        ...
        if (.not. option%onTheFly) call calc_Kvalidate(Nsubtrain, indicesSubtrain, &
...
!==============================================================================
  ...
  use dataset,       only : Y, Yest, Ntrain, indicesTrain
  ...
  call calcAlpha(NtrTot, Nprops, K(1)%twoDrArr, Ytrain)
  deallocate(Ytrain)
...
subroutine getYtrain(NtrainPoints,indicesTrainPoints,Nprops)
  use dataset, only : Y, Ygrad, YgradXYZ, XvecSize
  implicit none
  ! Arguments
...
!==============================================================================
subroutine calcAlpha(NtrainPoints, Nprops, Kmatrix, Yref)
!==============================================================================
...
!==============================================================================
  ...
  integer,          intent(in)  :: NtrainPoints                           ! Number  of the training points
  integer,          intent(in)  :: Nprops                                 ! Number  of the properties
  real(kind=rprec), intent(in)  :: Kmatrix(1:NtrainPoints,1:NtrainPoints) ! Indices of the training points
  real(kind=rprec), intent(inout) :: Yref(1:NtrainPoints,1:Nprops)        ! Reference values
  ! Variables
  ...
  allocate(alpha(1:NtrainPoints,Nprops),stat=Error)
  if(Error/=0)call stopMLatomF('Unable to allocate alpha')
  allocate(InvMat(1:NtrainPoints,1:NtrainPoints),stat=Error)
  if(Error/=0)call stopMLatomF('Unable to allocate inverse matrix')

  ! Add identity matrix multiplied by lambda to kernel matrix
  InvMat(1:NtrainPoints,1:NtrainPoints) = Kmatrix(1:NtrainPoints,1:NtrainPoints) ! To keep kernel matrix unmodified
  do jj=1,NtrainPoints
  ...
  if (option%invMatrix) then
    call invertMatrix(InvMat,NtrainPoints)
    alpha(:,1) = matmul(InvMat,Yref(:,1))
  else
    call solveSysLinEqs(NtrainPoints, Nprops, InvMat, Yref, alpha)
  end if
...
!==============================================================================
  ...
  if (option%calcGradXYZCompnts) then
    YgradXYZest = 0.0_rprec
    !$OMP PARALLEL DO PRIVATE(i) SHARED(YgradXYZest,indicesForEst) SCHEDULE(STATIC)
    do i=1, NestIndices
      YgradXYZest(1:3,1:NgradXYZmax,indicesForEst(i)) = YgradXYZCompntsEst_KRR(indicesForEst(i), alpha)
    end do
    !$OMP END PARALLEL DO
  end if
    ...
    if (option%CVopt) then
      ...
      if (option%onTheFly) then
        ...
        do mm = 1, size(indicesCVoptSplitsPt)
          ! Train, i.e. calculate regression coefficients alpha
          call getYtrain(CVoptFolds(mm)%Nsubtrain, CVoptFolds(mm)%indicesSubtrain, Nprops)
          call calcAlpha(NtrTot, Nprops, K(mm)%twoDrArr, Ytrain)
          ! Predict values using ML model
