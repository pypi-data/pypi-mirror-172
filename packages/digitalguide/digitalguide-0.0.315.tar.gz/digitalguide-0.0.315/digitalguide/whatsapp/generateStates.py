...
class CommandHandler:
    ...
    def check_update(self, update: WhatsAppUpdate):
        if update.get_message()._type == "text" and update.get_message_text().strip("/").lower() == self.command.lower():
            return True
...
class RegexFilter:
    ...
    def __call__(self, update: WhatsAppUpdate) -> bool:
        if update.get_message()._type == "text":
...
class TextFilter:
    def __call__(self, update: WhatsAppUpdate) -> bool:
        return update.get_message()._type == "text"
